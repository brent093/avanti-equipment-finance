...
`;
const FONT_URL = "https://fonts.googleapis.com/css2?family=Outfit:wght@300;400;500;600;700;800;900&family=JetBrains+Mono:wght@400;500;600&display=swap";
...
export default function App() {
  // Inject fonts and CSS on mount (SSR-safe)
  useEffect(() => {
    if (typeof document !== "undefined") {
      ...
      if (!document.getElementById("av-crm-css")) {
        const css = document.createElement("style");
        css.id = "av-crm-css";
        css.textContent = CRM_CSS;
  ...
  // Persistence
  useEffect(() => { if(typeof window==="undefined")return; try{const r=localStorage.getItem("av5_deals");if(r){const p=JSON.parse(r);if(p.length)setDeals(p)}}catch(e){}try{const r=localStorage.getItem("av5_dlrs");if(r){const p=JSON.parse(r);if(p.length)setDealers(p)}}catch(e){}try{const r=localStorage.getItem("av5_lndr");if(r){const p=JSON.parse(r);if(p.length)setLenders(p)}}catch(e){}try{const r=localStorage.getItem("av5_acct");if(r){const p=JSON.parse(r);if(p.length)setAccounts(p)}}catch(e){}try{const r=localStorage.getItem("av5_notes");if(r)setDnotes(JSON.parse(r))}catch(e){} }, []);
