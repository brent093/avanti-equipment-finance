"use client";
import { useState, useEffect, useCallback, useRef, createContext, useContext, useMemo } from "react";
import _ from "lodash";
import * as DB from './supabase-data';

// ════════════════════════════════════════════════════════════════
// AVANTI EQUIPMENT FINANCE — UNIFIED WEB APPLICATION
// CRM + Admin + Dealer Portal + Customer Portal
// ════════════════════════════════════════════════════════════════

const fontLink = document.createElement("link");
fontLink.href = "https://fonts.googleapis.com/css2?family=Outfit:wght@300;400;500;600;700;800;900&family=JetBrains+Mono:wght@400;500;600&display=swap";
fontLink.rel = "stylesheet";
document.head.appendChild(fontLink);

const css = document.createElement("style");
css.textContent = `
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ::-webkit-scrollbar{width:5px;height:5px}
  ::-webkit-scrollbar-track{background:transparent}
  ::-webkit-scrollbar-thumb{background:#CDD1D6;border-radius:10px}
  ::selection{background:#4F8EAE44}
  input:focus,select:focus,textarea:focus{border-color:#4F8EAE !important;outline:none}
  @keyframes fadeIn{from{opacity:0;transform:translateY(6px)}to{opacity:1;transform:translateY(0)}}
  @keyframes slideIn{from{transform:translateX(100%)}to{transform:translateX(0)}}
  @keyframes progFill{from{width:0}}
  @keyframes scaleIn{from{opacity:0;transform:scale(.96)}to{opacity:1;transform:scale(1)}}
  .av-row:hover{border-color:#CDD1D6 !important;background:#EDF0F3 !important}
  .av-card:hover{border-color:#CDD1D6 !important;transform:translateY(-1px)}
  .av-btn:hover{filter:brightness(1.15)}
  .av-ghost:hover{border-color:#4F8EAE !important;color:#4F8EAE !important}
  .av-tab:hover{color:#bbb !important}
  .av-kcard:hover{border-color:#CDD1D6 !important;background:#1E1E22 !important}
  .av-row,.av-card,.av-btn,.av-ghost,.av-tab,.av-kcard{transition:all .2s ease}
  @media(max-width:768px){
    .av-nav-tabs{display:none !important}
    .av-nav-tabs.open{display:flex !important;flex-direction:column;position:absolute;top:100%;left:0;right:0;background:#F8F9FA;border-bottom:1px solid #E2E5E9;z-index:60;padding:8px 0}
    .av-nav-tabs.open button{padding:14px 24px !important;border-bottom:none !important;border-left:3px solid transparent !important;width:100%}
    .av-nav-tabs.open button[data-active="true"]{border-left-color:#4F8EAE !important}
    .av-hamburger{display:flex !important}
    .av-crm-tabs{overflow-x:auto;-webkit-overflow-scrolling:touch}
    .av-crm-tabs::-webkit-scrollbar{display:none}
    .av-dash-grid{grid-template-columns:1fr !important}
    .av-stats-row{flex-direction:column !important}
    .av-stat-card{min-width:unset !important}
    .av-kpi-row{flex-wrap:wrap !important}
    .av-kpi-row>div{min-width:calc(50% - 4px) !important;flex:unset !important}
  }
  @media(min-width:769px){
    .av-hamburger{display:none !important}
  }
`;
document.head.appendChild(css);

// ── CONFIG ────────────────────────────────────────────────────

// Brand colors from Avanti logo
const BRAND = {
  steelBlue: "#4F8EAE",    // Primary accent — logo's steel blue
  darkBlue: "#3A6F8F",     // Darker steel blue
  lightBlue: "#6BA3BF",    // Lighter variant
  black: "#0D0D0D",        // Logo black
  charcoal: "#1C1C1C",     // Near black
  white: "#F0F0F0",        // Clean white
  green: "#4CAF50",
  red: "#B0413E",
  slate: "#5A6872",
};

const STAGES = [
  { id:"new_lead", label:"New Lead", color:BRAND.steelBlue, order:0 },
  { id:"contacted", label:"Contacted", color:"#6BA3BF", order:1 },
  { id:"app_submitted", label:"Submitted / In Review", color:"#3A6F8F", order:2 },
  { id:"in_review", label:"More Info Needed", color:"#E89300", order:3 },
  { id:"approved", label:"Approved", color:BRAND.green, order:4 },
  { id:"docs_out", label:"Docs Out", color:"#2E9A8F", order:5 },
  { id:"docs_signed", label:"Docs Signed", color:"#25876E", order:6 },
  { id:"funded", label:"Funded", color:"#2E7D32", order:7 },
  { id:"declined", label:"Declined", color:BRAND.red, order:8 },
  { id:"dead", label:"Dead", color:"#999", order:9 },
];

const CUST_STAGES = {
  new_lead:{label:"Application Received",desc:"We've received your application and it's in our queue.",progress:10},
  contacted:{label:"Initial Review",desc:"Our team is reviewing your information.",progress:20},
  app_submitted:{label:"Submitted / In Review",desc:"Your application has been submitted and is currently under review.",progress:40},
  in_review:{label:"Additional Information Needed",desc:"The reviewing institution has requested additional documentation. Please check the items needed below and provide them as soon as possible to avoid delays.",progress:55},
  approved:{label:"Approved!",desc:"Congratulations! Your financing has been approved. You may accept the approval below or your account manager will be in touch with next steps.",progress:75},
  docs_out:{label:"Documents Sent",desc:"Your financing documents have been sent for signature.",progress:85},
  docs_signed:{label:"Documents Signed",desc:"Your signed documents have been received and are being processed for funding.",progress:93},
  funded:{label:"Funded",desc:"Your deal has been funded! The vendor will be notified to proceed.",progress:100},
  declined:{label:"Unable to Approve",desc:"We were unable to secure approval at this time. Your account manager will discuss alternatives.",progress:0},
  dead:{label:"Inactive",desc:"This application is currently inactive. Contact your account manager to restart.",progress:0},
};

const EQ_TYPES = ["Tow Truck","Flatbed","Excavator","Loader","Crane","Dump Truck","Semi Truck","Trailer","Skid Steer","Bulldozer","Forklift","Box Truck","Other"];
const SOURCES = ["Jotform Application","Dealer Referral","Website","Phone Call","Email","Social Media","Repeat Customer","Other"];
const REQ_DOCS = [
  {id:"credit_app",label:"Credit Application",req:true,stage:"always"},
  {id:"equipment_quote",label:"Equipment Quote / Invoice",req:true,stage:"always"},
  {id:"sos",label:"Secretary of State / Articles",req:false,stage:"always"},
  {id:"drivers_license",label:"Driver's License",req:false,stage:"always"},
  {id:"bank_statements",label:"Bank Statements (3 mo)",req:false,stage:"always"},
  {id:"tax_returns",label:"Tax Returns (2 yr)",req:false,stage:"always"},
  {id:"pnl",label:"Profit & Loss Statement",req:false,stage:"always"},
  {id:"business_license",label:"Business License",req:false,stage:"always"},
  {id:"insurance_cert",label:"Certificate of Insurance",req:false,stage:"always"},
  {id:"voided_check",label:"Voided Check",req:false,stage:"always"},
];

// Auto-added when deal moves to docs_out (after approval accepted)
const DOCS_OUT_ITEMS = [
  {text:"Driver's License Copy (front & back)",category:"customer"},
  {text:"Voided Check (matching business bank account)",category:"customer"},
];

// Auto-added when deal moves to funded (dealer must provide)
const FUNDING_ITEMS_AUTO = [
  {text:"Final Invoice (must list make, model, VIN, mileage & customer info)",category:"dealer"},
  {text:"Title Copy (front & back)",category:"dealer"},
];

// Dealer required docs after approval
const DEALER_REQ_DOCS = [
  {docId:"dealer_final_invoice",label:"Final Invoice",icon:"📄"},
  {docId:"dealer_title_front",label:"Title Copy (Front)",icon:"📋"},
  {docId:"dealer_title_back",label:"Title Copy (Back)",icon:"📋"},
];

const COMMON_STIPS = [
  "Bank Statements (3 months)",
  "Bank Statements (6 months)",
  "Tax Returns (2 years)",
  "Profit & Loss Statement",
  "Balance Sheet",
  "Equipment Invoice / Quote",
  "Debt Schedule",
  "Business Plan",
  "Personal Financial Statement",
  "Proof of Insurance",
  "Voided Check",
  "Articles of Incorporation",
  "EIN Verification Letter",
  "Driver's License (front & back)",
  "Accounts Receivable Aging",
  "Accounts Payable Aging",
];

const LENDERS_INIT = [
  {id:"l1",name:"Channel Partners Capital",code:"CPC",contact:"Jennifer Walsh",email:"submissions@channelpartnerscap.com",phone:"(214) 555-3200",taxRule:"include",lienholder:"Channel Partners Capital, LLC",address:"5005 LBJ Freeway Ste 1250, Dallas TX 75244",insListYMM:true,insListVin:true,insPropertyCov:"Full financed amount",insMaxDeductible:"$2,500",insCompCollision:true,insACV:true,insAddlNotes:"Loss payee must match lienholder name exactly"},
  {id:"l2",name:"Navitas Lease Finance",code:"NAV",contact:"Mark Tremblay",email:"deals@navitaslease.com",phone:"(770) 555-1800",taxRule:"exclude",lienholder:"Navitas Lease Finance Receivables, LLC",address:"125 TownPark Dr Ste 300, Kennesaw GA 30144",insListYMM:true,insListVin:true,insPropertyCov:"Full replacement value",insMaxDeductible:"$5,000",insCompCollision:true,insACV:true,insAddlNotes:""},
  {id:"l3",name:"Beacon Funding",code:"BCN",contact:"Lisa Park",email:"newdeals@beaconfunding.com",phone:"(312) 555-4100",taxRule:"customer_choice",lienholder:"Beacon Funding Corporation",address:"500 N Dearborn St Ste 700, Chicago IL 60654",insListYMM:true,insListVin:false,insPropertyCov:"",insMaxDeductible:"$2,500",insCompCollision:true,insACV:false,insAddlNotes:""},
  {id:"l4",name:"Amur Equipment Finance",code:"AMR",contact:"Steve Nowak",email:"underwriting@amurfinance.com",phone:"(484) 555-2200",taxRule:"include",lienholder:"Amur Equipment Finance, Inc.",address:"1055 Westlakes Dr Ste 300, Berwyn PA 19312",insListYMM:true,insListVin:true,insPropertyCov:"Financed amount + 10%",insMaxDeductible:"$1,000",insCompCollision:true,insACV:true,insAddlNotes:"Binder must be received before funding"},
  {id:"l5",name:"Crossroads Equipment Lease",code:"CRE",taxRule:"exclude",lienholder:"Crossroads Equipment Lease & Finance, LLC",address:"7322 SW Freeway Ste 1400, Houston TX 77074"},
];

const AM = { name:"Brent Christiansen", title:"Senior Account Manager", phone:"(603) 459-9731", email:"Brent@AvantiEquipFi.com" };

const AVANTI_LOGO = "data:image/jpeg;base64,";

// ── DEMO DATA ─────────────────────────────────────────────────

const DEMO_ACCOUNTS = [
  {id:"acc1",biz:"Torres Towing LLC",contact:"Miguel Torres",email:"mtorres@torrestowing.com",custPassword:"Torres2026!",phone:"(555) 234-1122",address:"45 Main St, Nashua, NH 03060",industry:"Towing & Recovery",dateStarted:"2018",notes:"Strong credit, loyal customer. 8 years in business. Fleet of 6 trucks.",createdAt:"2026-02-18T10:30:00Z",vault:[{id:"v1",name:"Torres_DL_Front_Back.jpg",at:"2026-02-18T10:31:00Z",by:"admin",fromDeal:"d1"},{id:"v2",name:"Torres_CreditApp_2026.pdf",at:"2026-02-19T09:00:00Z",by:"admin",fromDeal:"d1"},{id:"v3",name:"Torres_Bank_Statements_6mo.pdf",at:"2026-02-19T08:00:00Z",by:"admin",fromDeal:"d1"}],activities:[{id:"aa1",type:"call_cust",who:"customer",detail:"Initial call — great rapport, expanding fleet",at:"2026-02-18T15:00:00Z"},{id:"aa2",type:"email_cust",who:"customer",detail:"Sent approval details and next steps",at:"2026-02-22T14:30:00Z"}]},
  {id:"acc2",biz:"Granite State Excavation",contact:"Dan Murphy",email:"dan@graniteexcavation.com",custPassword:"Granite2026!",phone:"(603) 555-8877",address:"120 Industrial Way, Concord, NH 03301",industry:"Excavation",dateStarted:"2015",notes:"Spring project deadline. May need 2nd piece of equipment.",createdAt:"2026-02-20T08:15:00Z",activities:[]},
  {id:"acc3",biz:"Apex Hauling Inc",contact:"Sarah Chen",email:"schen@apexhauling.com",custPassword:"Apex2026!",phone:"(555) 901-3344",address:"",industry:"Hauling",dateStarted:"",notes:"Website inquiry — not yet qualified.",createdAt:"2026-02-23T01:00:00Z",activities:[]},
  {id:"acc4",biz:"JR Builders",contact:"James Richardson",email:"jr@jrbuilders.net",custPassword:"JRBuild2026!",phone:"(555) 776-2100",address:"88 Elm St, Manchester, NH 03101",industry:"Construction",dateStarted:"2012",notes:"Sent credit app link, waiting on docs.",createdAt:"2026-02-21T15:45:00Z",activities:[]},
  {id:"acc5",biz:"Coastal Recovery Services",contact:"Mike Devlin",email:"mdevlin@coastalrecovery.com",custPassword:"Coastal2026!",phone:"(207) 555-6600",address:"310 Harbor Rd, Portland, ME 04101",industry:"Towing & Recovery",dateStarted:"2019",notes:"Good relationship. Looking to add a 2nd truck next year.",createdAt:"2026-02-10T12:00:00Z",activities:[{id:"aa3",type:"call_cust",who:"customer",detail:"Confirmed equipment specs with Mike",at:"2026-02-10T14:00:00Z"},{id:"aa4",type:"call_cust",who:"customer",detail:"Discussed next truck purchase for Q4",at:"2026-02-22T11:00:00Z"}]},
  {id:"acc6",biz:"Summit Earthworks",contact:"Tom Healey",email:"tom@summitearthworks.com",custPassword:"Summit2026!",phone:"(802) 555-9900",address:"5 Quarry Rd, Burlington, VT 05401",industry:"Earthwork & Grading",dateStarted:"2010",notes:"Funded deal. Has 3 more pieces to finance this year.",createdAt:"2026-01-15T09:00:00Z",activities:[{id:"aa5",type:"call_cust",who:"customer",detail:"Discussed upcoming equipment needs for 2026 season",at:"2026-02-20T10:00:00Z"}]},
];

const DEMO_DEALERS = [
  {id:"dlr1",name:"Northeast Truck Sales",contact:"Rick Brennan",email:"rick@netruck.com",phone:"(603) 555-2200",region:"New England",specs:["Tow Truck","Flatbed","Semi Truck"],code:"NTS2026",password:"NETruck2026!",active:true,referralPts:3,salesRep:"Tommy Gallagher",salesRepPhone:"(603) 555-2210",salesRepEmail:"tommy@netruck.com"},
  {id:"dlr2",name:"Milton CAT",contact:"Angela Ruiz",email:"aruiz@miltoncat.com",phone:"(508) 555-4400",region:"Northeast",specs:["Excavator","Loader","Bulldozer"],code:"MCAT2026",password:"MiltonCAT2026!",active:true,referralPts:2,salesRep:"Jake Moreno",salesRepPhone:"(508) 555-4411",salesRepEmail:"jmoreno@miltoncat.com"},
  {id:"dlr3",name:"Atlantic Truck Center",contact:"Dave Kohler",email:"dkohler@atlantictruck.com",phone:"(207) 555-8800",region:"New England",specs:["Tow Truck","Flatbed","Semi Truck"],code:"ATC2026",password:"Atlantic2026!",active:true,referralPts:0,salesRep:"",salesRepPhone:"",salesRepEmail:""},
  {id:"dlr4",name:"Bigge Crane & Rigging",contact:"Pat Simmons",email:"psimmons@bigge.com",phone:"(510) 555-1100",region:"West Coast",specs:["Crane"],code:"BCR2026",password:"BiggeCrane2026!",active:true,referralPts:1.5,salesRep:"Dennis Park",salesRepPhone:"(510) 555-1108",salesRepEmail:"dpark@bigge.com"},
];

const DEMO_DEALS = [
  {id:"d1",accountId:"acc1",biz:"Torres Towing LLC",contact:"Miguel Torres",email:"mtorres@torrestowing.com",custPassword:"Torres2026!",phone:"(555) 234-1122",eqType:"Tow Truck",eqDetail:"2025 Kenworth T370 Jerr-Dan",eqYear:"2025",eqMake:"Kenworth",eqModel:"T370 Jerr-Dan",eqVin:"1NKZL70X5RJ412885",eqMileage:"0",amount:185000,stage:"approved",source:"Jotform Application",notes:"Strong credit, 8 years in business.",createdAt:"2026-02-18T10:30:00Z",updatedAt:"2026-02-22T14:00:00Z",dealerId:"dlr1",dealerName:"Northeast Truck Sales",code:"TOR-7721",lenderId:"l1",terms:{amt:185000,pmt:4082.62,months:60,rate:"5.2%",buyRate:5.2,points:10,feeAmt:18500,dlrPts:3,dlrFee:5550,financedAmt:209050},docs:[{docId:"credit_app",name:"Torres_CreditApp.pdf",at:"2026-02-18T10:30:00Z",by:"customer"},{docId:"drivers_license",name:"Torres_DL.jpg",at:"2026-02-18T10:31:00Z",by:"customer"},{docId:"bank_statements",name:"Torres_Bank_3mo.pdf",at:"2026-02-19T08:00:00Z",by:"customer"},{docId:"equipment_quote",name:"KW_T370_Quote.pdf",at:"2026-02-18T15:00:00Z",by:"dealer"}],accepted:null,taxChoice:null,approvalLetter:{name:"CPC_Approval_Torres_Towing.pdf",at:"2026-02-22T14:00:00Z",by:"admin"},emailConfSent:"2026-02-18T10:30:00Z",emailCodeSent:"2026-02-18T15:30:00Z",activities:[{id:"a1",type:"call_cust",who:"customer",detail:"Verified business info and equipment needs",at:"2026-02-18T15:00:00Z"},{id:"a2",type:"email_dlr",who:"dealer",detail:"Sent Rick the app status update",at:"2026-02-19T09:15:00Z"},{id:"a3",type:"call_lndr",who:"lender",detail:"Followed up with Channel Partners on approval timeline",at:"2026-02-20T14:00:00Z"},{id:"a4",type:"email_cust",who:"customer",detail:"Sent approval details and next steps",at:"2026-02-22T14:30:00Z"}],history:[{s:"new_lead",d:"2026-02-18T10:30:00Z",n:"Application via Jotform"},{s:"contacted",d:"2026-02-18T15:00:00Z",n:"Called Miguel, verified info"},{s:"app_submitted",d:"2026-02-19T09:00:00Z",n:"Sent to 3 lenders"},{s:"in_review",d:"2026-02-20T11:00:00Z",n:"Under review with Channel Partners"},{s:"approved",d:"2026-02-22T14:00:00Z",n:"Approved — 5.2% / 60mo"}]},
  {id:"d2",accountId:"acc2",biz:"Granite State Excavation",contact:"Dan Murphy",email:"dan@graniteexcavation.com",custPassword:"Granite2026!",phone:"(603) 555-8877",eqType:"Excavator",eqDetail:"CAT 320 Next Gen",amount:275000,stage:"in_review",source:"Dealer Referral",notes:"Needs fast turnaround for spring project.",createdAt:"2026-02-20T08:15:00Z",updatedAt:"2026-02-22T11:00:00Z",dealerId:"dlr2",dealerName:"Milton CAT",code:"GRA-4455",lenderId:null,terms:null,docs:[{docId:"credit_app",name:"GraniteState_App.pdf",at:"2026-02-20T08:15:00Z",by:"customer"},{docId:"equipment_quote",name:"CAT320_Quote.pdf",at:"2026-02-20T08:16:00Z",by:"dealer"}],accepted:null,taxChoice:null,history:[{s:"new_lead",d:"2026-02-20T08:15:00Z",n:"Dealer referral from Milton CAT"},{s:"contacted",d:"2026-02-20T10:00:00Z",n:"Spoke with Dan"},{s:"app_submitted",d:"2026-02-21T09:00:00Z",n:"Submitted to lenders"},{s:"in_review",d:"2026-02-22T11:00:00Z",n:"Pending with two lenders"}],stips:[{id:"st1",text:"Bank Statements (6 months)",done:false,addedAt:"2026-02-22T12:00:00Z"},{id:"st2",text:"Tax Returns (2 years)",done:false,addedAt:"2026-02-22T12:00:00Z"},{id:"st3",text:"Profit & Loss Statement",done:true,addedAt:"2026-02-22T12:00:00Z"}]},
  {id:"d3",accountId:"acc3",biz:"Apex Hauling Inc",contact:"Sarah Chen",email:"schen@apexhauling.com",custPassword:"Apex2026!",phone:"(555) 901-3344",eqType:"Semi Truck",eqDetail:"Peterbilt 579 Day Cab",amount:165000,stage:"new_lead",source:"Website",notes:"Website inquiry.",createdAt:"2026-02-23T01:00:00Z",updatedAt:"2026-02-23T01:00:00Z",dealerId:null,dealerName:"",code:"APX-9102",lenderId:null,terms:null,docs:[],accepted:null,taxChoice:null,history:[{s:"new_lead",d:"2026-02-23T01:00:00Z",n:"Website inquiry"}]},
  {id:"d4",accountId:"acc4",biz:"JR Builders",contact:"James Richardson",email:"jr@jrbuilders.net",custPassword:"JRBuild2026!",phone:"(555) 776-2100",eqType:"Loader",eqDetail:"John Deere 644 X-Tier",amount:310000,stage:"contacted",source:"Phone Call",notes:"Sent credit app link.",createdAt:"2026-02-21T15:45:00Z",updatedAt:"2026-02-22T09:30:00Z",dealerId:null,dealerName:"",code:"JRB-6630",lenderId:null,terms:null,docs:[],accepted:null,taxChoice:null,history:[{s:"new_lead",d:"2026-02-21T15:45:00Z",n:"Inbound call"},{s:"contacted",d:"2026-02-22T09:30:00Z",n:"Sent credit app link"}]},
  {id:"d5",accountId:"acc5",biz:"Coastal Recovery Services",contact:"Mike Devlin",email:"mdevlin@coastalrecovery.com",custPassword:"Coastal2026!",phone:"(207) 555-6600",eqType:"Tow Truck",eqDetail:"2026 Ford F-650 Century",eqYear:"2026",eqMake:"Ford",eqModel:"F-650 Century",eqVin:"1FDNF6FC0RDA09344",eqMileage:"12",amount:142000,stage:"docs_out",source:"Dealer Referral",notes:"Approved with Navitas. Docs sent.",createdAt:"2026-02-10T12:00:00Z",updatedAt:"2026-02-22T16:30:00Z",dealerId:"dlr3",dealerName:"Atlantic Truck Center",code:"CST-3318",lenderId:"l2",terms:{amt:142000,pmt:2975.52,months:60,rate:"6.1%",buyRate:6.1,points:8,feeAmt:11360,dlrPts:0,dlrFee:0,financedAmt:153360},docs:[{docId:"credit_app",name:"Coastal_App.pdf",at:"2026-02-10T12:00:00Z",by:"customer"},{docId:"drivers_license",name:"Devlin_DL.jpg",at:"2026-02-10T12:01:00Z",by:"customer"},{docId:"bank_statements",name:"Coastal_Bank.pdf",at:"2026-02-11T09:00:00Z",by:"customer"},{docId:"equipment_quote",name:"F650_Quote.pdf",at:"2026-02-10T14:00:00Z",by:"dealer"}],accepted:true,taxChoice:false,emailConfSent:"2026-02-10T12:00:00Z",emailCodeSent:"2026-02-10T14:30:00Z",activities:[{id:"a5",type:"call_cust",who:"customer",detail:"Confirmed equipment specs with Mike",at:"2026-02-10T14:00:00Z"},{id:"a6",type:"email_lndr",who:"lender",detail:"Submitted to Navitas and CPC",at:"2026-02-11T09:00:00Z"},{id:"a7",type:"call_dlr",who:"dealer",detail:"Checked with Dave on title status",at:"2026-02-22T10:00:00Z"}],history:[{s:"new_lead",d:"2026-02-10T12:00:00Z",n:"Dealer referral"},{s:"contacted",d:"2026-02-10T14:00:00Z",n:"Verified"},{s:"app_submitted",d:"2026-02-11T09:00:00Z",n:"Submitted to Navitas & CPC"},{s:"in_review",d:"2026-02-14T11:00:00Z",n:"Under review"},{s:"approved",d:"2026-02-19T10:00:00Z",n:"Approved — Navitas 6.1%/60mo"},{s:"docs_out",d:"2026-02-22T16:30:00Z",n:"Docs sent for signing"}],fundingItems:[{id:"fi1",text:"Final Invoice",file:"F650_Final_Invoice.pdf",done:true,addedAt:"2026-02-19T12:00:00Z",resolvedAt:"2026-02-21T09:00:00Z"},{id:"fi2",text:"Title Copy",file:null,done:false,addedAt:"2026-02-19T12:00:00Z"}]},
  {id:"d6",accountId:"acc6",biz:"Summit Earthworks",contact:"Tom Healey",email:"tom@summitearthworks.com",custPassword:"Summit2026!",phone:"(802) 555-9900",eqType:"Bulldozer",eqDetail:"CAT D6T LGP",amount:395000,stage:"funded",source:"Dealer Referral",notes:"Funded and complete.",createdAt:"2026-01-15T09:00:00Z",updatedAt:"2026-02-15T11:00:00Z",dealerId:"dlr2",dealerName:"Milton CAT",code:"SMT-7701",lenderId:"l4",terms:{amt:395000,pmt:7002.32,months:72,rate:"5.8%",buyRate:5.8,points:5,feeAmt:19750,dlrPts:2,dlrFee:7900,financedAmt:422650},docs:[{docId:"credit_app",name:"Summit_App.pdf",at:"2026-01-15T09:00:00Z",by:"customer"},{docId:"equipment_quote",name:"D6T_Quote.pdf",at:"2026-01-15T09:01:00Z",by:"dealer"}],accepted:true,taxChoice:true,history:[{s:"new_lead",d:"2026-01-15T09:00:00Z",n:"Dealer referral"},{s:"contacted",d:"2026-01-15T11:00:00Z",n:"Verified"},{s:"app_submitted",d:"2026-01-16T09:00:00Z",n:"Submitted"},{s:"in_review",d:"2026-01-20T10:00:00Z",n:"Under review"},{s:"approved",d:"2026-01-28T14:00:00Z",n:"Approved with Amur"},{s:"docs_out",d:"2026-02-03T09:00:00Z",n:"Docs sent"},{s:"docs_signed",d:"2026-02-10T14:00:00Z",n:"Docs received signed"},{s:"funded",d:"2026-02-15T11:00:00Z",n:"Funded!"}]},
];

// ── UTILS ─────────────────────────────────────────────────────

const fmt = v => v ? new Intl.NumberFormat("en-US",{style:"currency",currency:"USD",maximumFractionDigits:0}).format(v) : "$0";
const fDate = d => d ? new Date(d).toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"}) : "";
const fDT = d => d ? new Date(d).toLocaleDateString("en-US",{month:"short",day:"numeric",hour:"numeric",minute:"2-digit"}) : "";
function relT(d){if(!d)return"";const m=Math.floor((Date.now()-new Date(d).getTime())/60000);if(m<60)return`${m}m ago`;const h=Math.floor(m/60);if(h<24)return`${h}h ago`;const dy=Math.floor(h/24);if(dy<7)return`${dy}d ago`;return fDate(d)}
const gId = () => "d"+Date.now().toString(36)+Math.random().toString(36).slice(2,6);
const gCode = n => {const p=(n||"AVT").replace(/[^A-Z]/gi,"").slice(0,3).toUpperCase();return`${p}-${Math.floor(1000+Math.random()*9000)}`};
const gPassword = () => {const chars="ABCDEFGHJKLMNPQRSTUVWXYZabcdefghjkmnpqrstuvwxyz23456789";let pw="";for(let i=0;i<10;i++)pw+=chars[Math.floor(Math.random()*chars.length)];return pw+"!"};

// ── ICONS ─────────────────────────────────────────────────────

const I={
  Pipe:()=><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><rect x="3" y="3" width="5" height="18" rx="1"/><rect x="10" y="8" width="5" height="13" rx="1"/><rect x="17" y="5" width="5" height="16" rx="1"/></svg>,
  List:()=><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/><circle cx="4" cy="6" r="1" fill="currentColor"/><circle cx="4" cy="12" r="1" fill="currentColor"/><circle cx="4" cy="18" r="1" fill="currentColor"/></svg>,
  Plus:()=><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>,
  Search:()=><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>,
  X:()=><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>,
  User:()=><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>,
  Truck:()=><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="1" y="3" width="15" height="13"/><polygon points="16 8 20 8 23 11 23 16 16 16 16 8"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg>,
  Edit:()=><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>,
  Trash:()=><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>,
  Phone:()=><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/></svg>,
  Mail:()=><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>,
  Bldg:()=><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><rect x="4" y="2" width="16" height="20" rx="2"/><line x1="9" y1="6" x2="9" y2="6.01"/><line x1="15" y1="6" x2="15" y2="6.01"/><line x1="9" y1="10" x2="9" y2="10.01"/><line x1="15" y1="10" x2="15" y2="10.01"/><line x1="9" y1="14" x2="9" y2="14.01"/><line x1="15" y1="14" x2="15" y2="14.01"/><line x1="9" y1="18" x2="15" y2="18"/></svg>,
  Eye:()=><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>,
  EyeOff:()=><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"/><line x1="1" y1="1" x2="23" y2="23"/></svg>,
  Check:()=><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>,
  Send:()=><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>,
  Copy:()=><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>,
  Back:()=><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="19" y1="12" x2="5" y2="12"/><polyline points="12 19 5 12 12 5"/></svg>,
  Archive:()=><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="21 8 21 21 3 21 3 8"/><rect x="1" y="3" width="22" height="5"/><line x1="10" y1="12" x2="14" y2="12"/></svg>,
  Refresh:()=><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"/></svg>,
  Shield:()=><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>,
  Doc:()=><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>,
  Upload:()=><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>,
  Download:()=><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>,
  Dollar:()=><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>,
  Gear:()=><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 1 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 1 1-2.83-2.83l.06-.06A1.65 1.65 0 0 0 4.68 15a1.65 1.65 0 0 0-1.51-1H3a2 2 0 1 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 1 1 2.83-2.83l.06.06A1.65 1.65 0 0 0 9 4.68a1.65 1.65 0 0 0 1-1.51V3a2 2 0 1 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 1 1 2.83 2.83l-.06.06A1.65 1.65 0 0 0 19.4 9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 1 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg>,
  CheckCirc:()=><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>,
  Star:()=><svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" strokeWidth="1"><polygon points="12,2 15.09,8.26 22,9.27 17,14.14 18.18,21.02 12,17.77 5.82,21.02 7,14.14 2,9.27 8.91,8.26"/></svg>,
  Logout:()=><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>,
  Alert:()=><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>,
  Clip:()=><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21.44 11.05l-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48"/></svg>,
  Menu:()=><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><line x1="3" y1="6" x2="21" y2="6"/><line x1="3" y1="12" x2="21" y2="12"/><line x1="3" y1="18" x2="21" y2="18"/></svg>,
  FileText:()=><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><line x1="10" y1="9" x2="8" y2="9"/></svg>,
};

// ── DATA CONTEXT ──────────────────────────────────────────────

const Ctx = createContext(null);

// ════════════════════════════════════════════════════════════════
// MAIN APP
// ════════════════════════════════════════════════════════════════

export default function App() {
  const [deals, setDeals] = useState([]);
  const [dealers, setDealers] = useState([]);
  const [lenders, setLenders] = useState(LENDERS_INIT);
  const [accounts, setAccounts] = useState([]);
  const [dnotes, setDnotes] = useState({});
  const [dealerNotes, setDealerNotes] = useState([]);
  const [reminders, setReminders] = useState([]);
  const [portal, setPortal] = useState("crm");
  const [crmAuth, setCrmAuth] = useState(false);
  const [crmUser, setCrmUser] = useState(null);
  const [crmProfile, setCrmProfile] = useState(null);
  const [dlrAuth, setDlrAuth] = useState(null);
  const [custAuth, setCustAuth] = useState(null);
  const [toast, setToast] = useState(null);
  const [mobileMenu, setMobileMenu] = useState(false);
  const [dataLoaded, setDataLoaded] = useState(false);

  // Supabase Auth listener + data loading
  useEffect(()=>{
    const {data:{subscription}} = DB.onAuthChange(async(event,session)=>{
      if(session?.user){
        const profile = await DB.getProfile();
        setCrmUser(session.user);
        setCrmProfile(profile);
        if(profile && ['admin','manager','readonly'].includes(profile.role)){
          setCrmAuth(true);
          // Load all data from Supabase
          const [d,dl,acc,ln,dn,dlrN,rem] = await Promise.all([
            DB.fetchDeals(), DB.fetchDealers(), DB.fetchAccounts(),
            DB.fetchLenders(), DB.fetchDealNotes(), DB.fetchDealerNotes(), DB.fetchReminders()
          ]);
          if(d.length||!dataLoaded) setDeals(d);
          if(dl.length) setDealers(dl);
          if(acc.length) setAccounts(acc);
          if(ln.length) setLenders(ln);
          setDnotes(dn);
          setDealerNotes(dlrN);
          setReminders(rem);
          setDataLoaded(true);
        } else if(profile?.role==='dealer'){
          setDlrAuth(profile.dealer_id);
          setPortal("dealer");
        } else if(profile?.role==='customer'){
          setCustAuth(profile.account_id);
          setPortal("customer");
        }
      } else {
        setCrmAuth(false);setCrmUser(null);setCrmProfile(null);
      }
    });
    // Check existing session on mount
    (async()=>{
      const session = await DB.getSession();
      if(session?.user){
        const profile = await DB.getProfile();
        setCrmUser(session.user);
        setCrmProfile(profile);
        if(profile && ['admin','manager','readonly'].includes(profile.role)){
          setCrmAuth(true);
          const [d,dl,acc,ln,dn,dlrN,rem] = await Promise.all([
            DB.fetchDeals(), DB.fetchDealers(), DB.fetchAccounts(),
            DB.fetchLenders(), DB.fetchDealNotes(), DB.fetchDealerNotes(), DB.fetchReminders()
          ]);
          if(d.length) setDeals(d);
          if(dl.length) setDealers(dl);
          if(acc.length) setAccounts(acc);
          if(ln.length) setLenders(ln);
          setDnotes(dn);
          setDealerNotes(dlrN);
          setReminders(rem);
          setDataLoaded(true);
        }
      }
    })();
    return()=>subscription.unsubscribe();
  },[]);

  // Sync deals to Supabase on changes (debounced)
  const syncDealsRef = useRef(null);
  useEffect(()=>{
    if(!dataLoaded||!crmAuth) return;
    if(syncDealsRef.current) clearTimeout(syncDealsRef.current);
    syncDealsRef.current = setTimeout(()=>{
      deals.forEach(d=>DB.saveDealToDb(d));
    },2000);
  },[deals]);

  // Sync dealers to Supabase on changes
  const syncDlrRef = useRef(null);
  useEffect(()=>{
    if(!dataLoaded||!crmAuth) return;
    if(syncDlrRef.current) clearTimeout(syncDlrRef.current);
    syncDlrRef.current = setTimeout(()=>{
      dealers.forEach(d=>DB.saveDealer(d));
    },2000);
  },[dealers]);

  // Sync accounts to Supabase on changes
  const syncAccRef = useRef(null);
  useEffect(()=>{
    if(!dataLoaded||!crmAuth) return;
    if(syncAccRef.current) clearTimeout(syncAccRef.current);
    syncAccRef.current = setTimeout(()=>{
      accounts.forEach(a=>DB.saveAccount(a));
    },2000);
  },[accounts]);

  const notify = useCallback((msg,t="ok")=>{setToast({msg,t});setTimeout(()=>setToast(null),2500)},[]);

  const saveDeal = useCallback(deal=>{
    if(deal.id && deals.find(d=>d.id===deal.id)){
      setDeals(p=>p.map(d=>d.id===deal.id?{...deal,updatedAt:new Date().toISOString()}:d));
    } else {
      // Auto-create account if no accountId
      let accId=deal.accountId;
      if(!accId&&deal.biz){
        const existing=accounts.find(a=>a.biz?.toLowerCase()===deal.biz?.toLowerCase());
        if(existing){accId=existing.id}
        else{accId="acc"+Date.now().toString(36);setAccounts(p=>[...p,{id:accId,biz:deal.biz,contact:deal.contact||"",email:deal.email||"",phone:deal.phone||"",address:"",industry:"",dateStarted:"",notes:"",activities:[],createdAt:new Date().toISOString()}])}
      }
      setDeals(p=>[{...deal,id:deal.id||gId(),accountId:accId,code:deal.code||gCode(deal.biz),custPassword:deal.custPassword||gPassword(),docs:deal.docs||[],stips:deal.stips||[],fundingItems:deal.fundingItems||[],terms:deal.terms||null,accepted:deal.accepted??null,taxChoice:deal.taxChoice??null,showTaxChoice:deal.showTaxChoice??false,lenderId:deal.lenderId||null,history:deal.history||[{s:deal.stage||"new_lead",d:new Date().toISOString(),n:"Deal created"}],createdAt:deal.createdAt||new Date().toISOString(),updatedAt:new Date().toISOString()},...p]);
    }
  },[deals,accounts]);

  const moveDeal = useCallback((id,stg,note="")=>{
    setDeals(p=>p.map(d=>{
      if(d.id!==id) return d;
      let updated = {...d,stage:stg,updatedAt:new Date().toISOString(),history:[...(d.history||[]),{s:stg,d:new Date().toISOString(),n:note||`Moved to ${STAGES.find(s=>s.id===stg)?.label}`}]};
      // Store formal disposition for dead/declined deals (audit trail)
      if(stg==="dead"||stg==="declined"){
        const prevStage=(d.history||[]).slice().reverse().find(h=>h.s!=="dead"&&h.s!=="declined");
        updated.disposition={reason:note||"No reason provided",stage:stg,date:new Date().toISOString(),previousStage:prevStage?.s||d.stage,lender:d.lenderId||null,dealerName:d.dealerName||null};
      }
      // Clear disposition if reactivated
      if(stg!=="dead"&&stg!=="declined"&&d.disposition){
        updated.disposition=null;
      }
      // Auto-add docs_out items (customer needs to provide DL + voided check)
      if(stg==="docs_out"){
        const existing = (updated.stips||[]).map(s=>s.text);
        const toAdd = DOCS_OUT_ITEMS.filter(i=>!existing.includes(i.text));
        if(toAdd.length) updated.stips = [...(updated.stips||[]),...toAdd.map(i=>({id:gId(),text:i.text,done:false,file:null,resolvedBy:null,addedAt:new Date().toISOString(),visibility:"all",category:i.category,auto:true}))];
      }
      // Auto-add funding items (dealer needs to provide invoice + title)
      if(stg==="funded"||stg==="docs_out"||stg==="docs_signed"){
        const existingFI = (updated.fundingItems||[]).map(f=>f.text);
        const toAddFI = FUNDING_ITEMS_AUTO.filter(i=>!existingFI.includes(i.text));
        if(toAddFI.length) updated.fundingItems = [...(updated.fundingItems||[]),...toAddFI.map(i=>({id:gId(),text:i.text,file:null,done:false,addedAt:new Date().toISOString(),category:i.category,auto:true}))];
      }
      return updated;
    }));
    notify(`Moved to ${STAGES.find(s=>s.id===stg)?.label}`);
  },[notify]);

  const delDeal = useCallback(id=>{setDeals(p=>p.filter(d=>d.id!==id));notify("Deal deleted","info")},[notify]);
  const addDoc = useCallback((id,doc)=>{
    setDeals(p=>{
      const updated = p.map(d=>{
        if(d.id!==id) return d;
        const newDeal = {...d,docs:[...(d.docs||[]),doc],updatedAt:new Date().toISOString()};
        // Auto-save to account vault
        if(d.accountId){
          setAccounts(accs=>accs.map(a=>{
            if(a.id!==d.accountId) return a;
            const existing=(a.vault||[]).map(v=>v.name);
            if(existing.includes(doc.name)) return a;
            return {...a,vault:[...(a.vault||[]),{id:gId(),name:doc.name,at:doc.at||new Date().toISOString(),by:doc.by||"auto",fromDeal:d.id}]};
          }));
        }
        return newDeal;
      });
      return updated;
    });
  },[]);
  const rmDoc = useCallback((id,docId)=>setDeals(p=>p.map(d=>d.id===id?{...d,docs:(d.docs||[]).filter(x=>x.docId!==docId),updatedAt:new Date().toISOString()}:d)),[]);
  const toggleDocReq = useCallback((dealId,docId)=>setDeals(p=>p.map(d=>{
    if(d.id!==dealId) return d;
    const reqs = {...(d.docReqs||{})};
    const globalReq = REQ_DOCS.find(x=>x.id===docId)?.req??false;
    const current = reqs[docId]!==undefined ? reqs[docId] : globalReq;
    reqs[docId] = !current;
    return {...d,docReqs:reqs,updatedAt:new Date().toISOString()};
  })),[]);
  const addStip = useCallback((dealId,item,vis="all")=>setDeals(p=>p.map(d=>d.id===dealId?{...d,stips:[...(d.stips||[]),{id:gId(),text:item,done:false,file:null,resolvedBy:null,addedAt:new Date().toISOString(),visibility:vis}],updatedAt:new Date().toISOString()}:d)),[]);
  const removeStip = useCallback((dealId,stipId)=>setDeals(p=>p.map(d=>d.id===dealId?{...d,stips:(d.stips||[]).filter(s=>s.id!==stipId),updatedAt:new Date().toISOString()}:d)),[]);
  const toggleStip = useCallback((dealId,stipId)=>setDeals(p=>p.map(d=>d.id===dealId?{...d,stips:(d.stips||[]).map(s=>s.id===stipId?{...s,done:!s.done,file:!s.done?s.file:null,resolvedBy:!s.done?s.resolvedBy:null}:s),updatedAt:new Date().toISOString()}:d)),[]);
  const resolveStip = useCallback((dealId,stipId,fileName,by)=>setDeals(p=>p.map(d=>{
    if(d.id!==dealId) return d;
    const updated = {...d,stips:(d.stips||[]).map(s=>s.id===stipId?{...s,done:true,file:fileName,resolvedBy:by,resolvedAt:new Date().toISOString()}:s),updatedAt:new Date().toISOString()};
    if(d.accountId&&fileName){setAccounts(accs=>accs.map(a=>{if(a.id!==d.accountId)return a;const ex=(a.vault||[]).map(v=>v.name);if(ex.includes(fileName))return a;return{...a,vault:[...(a.vault||[]),{id:gId(),name:fileName,at:new Date().toISOString(),by:by||"auto",fromDeal:d.id}]}}));}
    return updated;
  })),[]);
  // Dealer funding items — required before docs go out
  const addFundingItem = useCallback((dealId,item)=>setDeals(p=>p.map(d=>d.id===dealId?{...d,fundingItems:[...(d.fundingItems||[]),{id:gId(),text:item,file:null,done:false,addedAt:new Date().toISOString()}],updatedAt:new Date().toISOString()}:d)),[]);
  const resolveFundingItem = useCallback((dealId,itemId,fileName)=>setDeals(p=>p.map(d=>{
    if(d.id!==dealId) return d;
    const updated = {...d,fundingItems:(d.fundingItems||[]).map(f=>f.id===itemId?{...f,done:true,file:fileName,resolvedAt:new Date().toISOString()}:f),updatedAt:new Date().toISOString()};
    if(d.accountId&&fileName){setAccounts(accs=>accs.map(a=>{if(a.id!==d.accountId)return a;const ex=(a.vault||[]).map(v=>v.name);if(ex.includes(fileName))return a;return{...a,vault:[...(a.vault||[]),{id:gId(),name:fileName,at:new Date().toISOString(),by:"dealer",fromDeal:d.id}]}}));}
    return updated;
  })),[]);
  const removeFundingItem = useCallback((dealId,itemId)=>setDeals(p=>p.map(d=>d.id===dealId?{...d,fundingItems:(d.fundingItems||[]).filter(f=>f.id!==itemId),updatedAt:new Date().toISOString()}:d)),[]);
  const acceptDeal = useCallback((id,tax)=>{setDeals(p=>p.map(d=>d.id===id?{...d,accepted:true,taxChoice:tax,updatedAt:new Date().toISOString()}:d));notify("Approval accepted!")},[notify]);
  const sendConfEmail = useCallback((id)=>setDeals(p=>p.map(d=>d.id===id?{...d,emailConfSent:new Date().toISOString(),updatedAt:new Date().toISOString()}:d)),[]);
  const sendCodeEmail = useCallback((id)=>setDeals(p=>p.map(d=>d.id===id?{...d,emailCodeSent:new Date().toISOString(),updatedAt:new Date().toISOString()}:d)),[]);
  const addNote = useCallback((id,txt,who="Admin")=>setDnotes(p=>({...p,[id]:[...(p[id]||[]),{id:gId(),txt,who,at:new Date().toISOString()}]})),[]);
  const addDealerNote = useCallback((dealId,txt,dealerName,dealBiz)=>setDealerNotes(p=>[{id:gId(),dealId,txt,dealer:dealerName,biz:dealBiz,at:new Date().toISOString(),read:false},...p]),[]);
  const dismissDealerNote = useCallback((noteId)=>setDealerNotes(p=>p.map(n=>n.id===noteId?{...n,read:true}:n)),[]);
  const dismissAllDealerNotes = useCallback(()=>setDealerNotes(p=>p.map(n=>({...n,read:true}))),[]);
  const addActivity = useCallback((id,type,who,detail)=>setDeals(p=>p.map(d=>d.id===id?{...d,activities:[...(d.activities||[]),{id:gId(),type,who,detail:detail||"",at:new Date().toISOString()}],updatedAt:new Date().toISOString()}:d)),[]);
  const addDlr = useCallback(d=>{setDealers(p=>[...p,{...d,id:"dlr"+Date.now().toString(36),active:true}]);notify("Dealer added")},[notify]);
  const updDlr = useCallback(d=>{setDealers(p=>p.map(x=>x.id===d.id?d:x));notify("Dealer updated")},[notify]);

  // Account functions
  const saveAccount = useCallback(acc=>{
    if(acc.id && accounts.find(a=>a.id===acc.id)){
      setAccounts(p=>p.map(a=>a.id===acc.id?{...acc}:a));
    } else {
      setAccounts(p=>[...p,{...acc,id:acc.id||"acc"+Date.now().toString(36),activities:acc.activities||[],createdAt:new Date().toISOString()}]);
    }
  },[accounts]);
  const addAcctActivity = useCallback((accId,type,who,detail)=>setAccounts(p=>p.map(a=>a.id===accId?{...a,activities:[...(a.activities||[]),{id:gId(),type,who,detail:detail||"",at:new Date().toISOString()}]}:a)),[]);

  // Reminders
  const addReminder = useCallback((reminder)=>setReminders(p=>[{...reminder,id:gId(),createdAt:new Date().toISOString(),fired:false},...p]),[]);
  const removeReminder = useCallback((id)=>setReminders(p=>p.filter(r=>r.id!==id)),[]);
  const dismissReminder = useCallback((id)=>setReminders(p=>p.map(r=>r.id===id?{...r,fired:true,dismissed:true}:r)),[]);
  const snoozeReminder = useCallback((id,mins)=>setReminders(p=>p.map(r=>r.id===id?{...r,fired:false,dueAt:new Date(Date.now()+mins*60000).toISOString()}:r)),[]);

  // Reminder timer - checks every 30s for due reminders
  const [activeReminder,setActiveReminder]=useState(null);
  useEffect(()=>{
    const check=()=>{
      const now=Date.now();
      const due=reminders.find(r=>!r.fired&&!r.dismissed&&new Date(r.dueAt).getTime()<=now);
      if(due){
        setActiveReminder(due);
        setReminders(p=>p.map(r=>r.id===due.id?{...r,fired:true}:r));
      }
    };
    check();
    const iv=setInterval(check,30000);
    return()=>clearInterval(iv);
  },[reminders]);
  const loginCrm = useCallback(async(email,pw)=>{
    const {data,error} = await DB.signIn(email,pw);
    if(error){return {ok:false,error:error.message,mfa:false}}
    // Check if MFA is required
    if(data?.session===null && data?.user===null){
      // MFA challenge needed
      return {ok:false,error:null,mfa:true,factors:data}
    }
    return {ok:true,error:null,mfa:false};
  },[]);
  const logoutCrm = useCallback(async()=>{
    await DB.signOut();
    setCrmAuth(false);setCrmUser(null);setCrmProfile(null);
    setDeals([]);setDealers([]);setAccounts([]);setDnotes({});setDealerNotes([]);setReminders([]);
    setDataLoaded(false);
  },[]);
  const loginDlr = useCallback(async(email,pw)=>{
    const {data,error} = await DB.signIn(email,pw);
    if(error) return false;
    const profile = await DB.getProfile();
    if(profile?.role==='dealer'){setDlrAuth(profile.dealer_id);return true}
    await DB.signOut();return false;
  },[]);
  const loginCust = useCallback(async(email,pw)=>{
    const {data,error} = await DB.signIn(email,pw);
    if(error) return false;
    const profile = await DB.getProfile();
    if(profile?.role==='customer'){setCustAuth(profile.account_id);return true}
    await DB.signOut();return false;
  },[]);

  const ctx = useMemo(()=>({deals,dealers,lenders,accounts,dnotes,dealerNotes,reminders,crmUser,crmProfile,saveDeal,moveDeal,delDeal,addDoc,rmDoc,toggleDocReq,addStip,removeStip,toggleStip,resolveStip,addFundingItem,resolveFundingItem,removeFundingItem,acceptDeal,sendConfEmail,sendCodeEmail,addNote,addDealerNote,dismissDealerNote,dismissAllDealerNotes,addActivity,addDlr,updDlr,saveAccount,addAcctActivity,addReminder,removeReminder,dismissReminder,snoozeReminder,notify,setLenders,loginCrm,logoutCrm}),[deals,dealers,lenders,accounts,dnotes,dealerNotes,reminders,crmUser,crmProfile,saveDeal,moveDeal,delDeal,addDoc,rmDoc,toggleDocReq,addStip,removeStip,toggleStip,resolveStip,addFundingItem,resolveFundingItem,removeFundingItem,acceptDeal,sendConfEmail,sendCodeEmail,addNote,addDealerNote,dismissDealerNote,dismissAllDealerNotes,addActivity,addDlr,updDlr,saveAccount,addAcctActivity,addReminder,removeReminder,dismissReminder,snoozeReminder,notify,loginCrm,logoutCrm]);

  return(
    <Ctx.Provider value={ctx}>
      <div style={S.app}>
        <nav style={{...S.nav,position:"relative"}}><div style={S.navIn}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <button className="av-hamburger" onClick={()=>setMobileMenu(!mobileMenu)} style={{background:"none",border:"none",color:"#1a1a2e",cursor:"pointer",padding:6,display:"none"}}><I.Menu/></button>
            <div style={S.logo}><img src={AVANTI_LOGO} alt="Avanti Equipment Finance" style={{height:34,objectFit:"contain",borderRadius:6,padding:"2px 8px",background:"#f0f0f0"}}/></div>
          </div>
          <div className={`av-nav-tabs${mobileMenu?" open":""}`} style={S.navTabs}>
            {[{id:"crm",lb:"CRM",ic:<I.Shield/>,auth:true},{id:"apply",lb:"Apply",ic:<I.FileText/>},{id:"dealer",lb:"Dealer Portal",ic:<I.Bldg/>},{id:"customer",lb:"Customer Portal",ic:<I.Eye/>}].map(p=>
              <button key={p.id} data-active={portal===p.id} className="av-tab" onClick={()=>{setPortal(p.id);setMobileMenu(false);if(p.id!=="dealer")setDlrAuth(null);if(p.id!=="customer")setCustAuth(null)}} style={{...S.navTab,...(portal===p.id?S.navTabA:{})}}>{p.ic}<span>{p.lb}</span></button>
            )}
          </div>
          <div style={S.navR}>
            {portal==="crm"&&crmAuth&&<><span style={S.navUser}><I.User/> {crmProfile?.full_name||"Admin"}</span><button className="av-ghost" style={{...S.ghost,marginLeft:6}} onClick={()=>{logoutCrm();setPortal("apply")}}><I.Logout/> Log Out</button></>}
            {portal==="dealer"&&dlrAuth&&<button className="av-ghost" style={S.ghost} onClick={async()=>{await DB.signOut();setDlrAuth(null)}}><I.Logout/> Log Out</button>}
            {portal==="customer"&&custAuth&&<button className="av-ghost" style={S.ghost} onClick={async()=>{await DB.signOut();setCustAuth(null)}}><I.Logout/> Log Out</button>}
          </div>
        </div></nav>
        <main style={S.main} onClick={()=>mobileMenu&&setMobileMenu(false)}>
          {portal==="crm"&&(crmAuth?<CRM/>:<Login type="crm" onGo={loginCrm}/>)}
          {portal==="apply"&&<ApplyPage/>}
          {portal==="dealer"&&(dlrAuth?<DealerDash did={dlrAuth} out={()=>setDlrAuth(null)}/>:<Login type="dealer" onGo={loginDlr}/>)}
          {portal==="customer"&&(custAuth?<CustDash did={custAuth} out={()=>setCustAuth(null)}/>:<Login type="customer" onGo={loginCust}/>)}
        </main>
        {toast&&<div style={{...S.toast,background:toast.t==="ok"?"#2E7D32":toast.t==="error"?"#B0413E":"#4F8EAE",animation:"fadeIn .3s"}}>{toast.msg}</div>}
        {activeReminder&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.3)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={()=>setActiveReminder(null)}>
          <div style={{background:"#fff",borderRadius:12,padding:24,width:380,maxWidth:"90vw",boxShadow:"0 12px 40px rgba(0,0,0,.2)",border:"1px solid #E2E5E9"}} onClick={e=>e.stopPropagation()}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
              <span style={{fontSize:24}}>🔔</span>
              <div style={{fontSize:16,fontWeight:700,color:"#1a1a2e"}}>Reminder</div>
            </div>
            <div style={{fontSize:14,color:"#333",marginBottom:6,fontWeight:600}}>{activeReminder.text}</div>
            {activeReminder.dealId&&<div style={{fontSize:11,color:"#4F8EAE",marginBottom:8}}>Deal: {deals.find(d=>d.id===activeReminder.dealId)?.biz||"Unknown"}</div>}
            <div style={{fontSize:10,color:"#999",marginBottom:16}}>Due: {new Date(activeReminder.dueAt).toLocaleString()}</div>
            <div style={{display:"flex",gap:8}}>
              <button className="av-btn" style={{...S.prim,flex:1,justifyContent:"center"}} onClick={()=>{dismissReminder(activeReminder.id);setActiveReminder(null)}}>Dismiss</button>
              <button className="av-ghost" style={{...S.ghost,flex:1,justifyContent:"center"}} onClick={()=>{snoozeReminder(activeReminder.id,15);setActiveReminder(null);notify("Snoozed 15 min")}}>Snooze 15m</button>
              <button className="av-ghost" style={{...S.ghost,flex:1,justifyContent:"center"}} onClick={()=>{snoozeReminder(activeReminder.id,60);setActiveReminder(null);notify("Snoozed 1 hour")}}>Snooze 1h</button>
            </div>
          </div>
        </div>}
      </div>
    </Ctx.Provider>
  );
}

// ════════════════════════════════════════════════════════════════
// CRM
// ════════════════════════════════════════════════════════════════

function CRM() {
  const {deals,dealers,accounts,moveDeal,delDeal,saveDeal,dealerNotes,notify} = useContext(Ctx);
  const [view,setView]=useState("dashboard");
  const unreadDlr=(dealerNotes||[]).filter(n=>!n.read).length;
  const [sel,setSel]=useState(null);
  const [form,setForm]=useState(false);
  const [editD,setEditD]=useState(null);
  const [q,setQ]=useState("");
  const [dragId,setDragId]=useState(null);
  const [dragO,setDragO]=useState(null);
  const [dlrFilter,setDlrFilter]=useState(null); // null = all dealers, string = specific dealer id
  // PDF import state
  const [pdfDrag,setPdfDrag]=useState(false);
  const [pdfParsed,setPdfParsed]=useState(null);
  const [pdfParsing,setPdfParsing]=useState(false);
  const fileRef=useRef(null);

  const filt = useMemo(()=>{if(!q)return deals;const lq=q.toLowerCase();return deals.filter(d=>d.biz?.toLowerCase().includes(lq)||d.contact?.toLowerCase().includes(lq)||d.eqType?.toLowerCase().includes(lq)||d.dealerName?.toLowerCase().includes(lq)||d.code?.toLowerCase().includes(lq)||d.email?.toLowerCase().includes(lq)||d.phone?.replace(/\D/g,"").includes(lq.replace(/\D/g,"")))},[deals,q]);

  // Universal search results
  const uniResults = useMemo(()=>{
    if(!q||q.length<2)return{accounts:[],deals:[],dealers:[]};
    const lq=q.toLowerCase();
    const qDigits=q.replace(/\D/g,"");
    const matchAcct=accounts.filter(a=>a.biz?.toLowerCase().includes(lq)||a.contact?.toLowerCase().includes(lq)||a.email?.toLowerCase().includes(lq)||(a.phone&&qDigits.length>=3&&a.phone.replace(/\D/g,"").includes(qDigits)));
    const matchDeal=deals.filter(d=>d.biz?.toLowerCase().includes(lq)||d.contact?.toLowerCase().includes(lq)||d.email?.toLowerCase().includes(lq)||d.code?.toLowerCase().includes(lq)||(d.phone&&qDigits.length>=3&&d.phone.replace(/\D/g,"").includes(qDigits)));
    const matchDlr=dealers.filter(d=>d.name?.toLowerCase().includes(lq)||d.contact?.toLowerCase().includes(lq)||d.email?.toLowerCase().includes(lq)||d.code?.toLowerCase().includes(lq)||(d.phone&&qDigits.length>=3&&d.phone.replace(/\D/g,"").includes(qDigits))||(d.salesRepEmail&&d.salesRepEmail.toLowerCase().includes(lq))||(d.salesRepPhone&&qDigits.length>=3&&d.salesRepPhone.replace(/\D/g,"").includes(qDigits)));
    return{accounts:matchAcct.slice(0,5),deals:matchDeal.slice(0,5),dealers:matchDlr.slice(0,3)};
  },[q,accounts,deals,dealers]);
  const uniHasResults=uniResults.accounts.length+uniResults.deals.length+uniResults.dealers.length>0;
  const [srchFocus,setSrchFocus]=useState(false);
  const srchRef=useRef(null);
  const stats = useMemo(()=>({act:deals.filter(d=>!["funded","declined","dead"].includes(d.stage)).length,pipe:deals.filter(d=>!["funded","declined","dead"].includes(d.stage)).reduce((s,d)=>s+(d.amount||0),0),fun:deals.filter(d=>d.stage==="funded").reduce((s,d)=>s+(d.amount||0),0),tot:deals.length,dead:deals.filter(d=>d.stage==="dead"||d.stage==="declined").length,won:deals.filter(d=>d.stage==="funded").length}),[deals]);

  useEffect(()=>{if(sel){const u=deals.find(d=>d.id===sel.id);if(u)setSel(u);else setSel(null)}},[deals]);

  // ── PDF PARSER ──
  const parsePdf=async(file)=>{
    setPdfParsing(true);
    try{
      const buf=await file.arrayBuffer();
      const pdfjsLib=await import("https://cdnjs.cloudflare.com/ajax/libs/pdf.js/4.0.379/pdf.min.mjs");
      pdfjsLib.GlobalWorkerOptions.workerSrc="https://cdnjs.cloudflare.com/ajax/libs/pdf.js/4.0.379/pdf.worker.min.mjs";
      const pdf=await pdfjsLib.getDocument({data:buf}).promise;
      let fields={};
      const allItems=[];
      for(let i=1;i<=pdf.numPages;i++){
        const page=await pdf.getPage(i);
        const annots=await page.getAnnotations();
        annots.forEach(a=>{
          const name=a.fieldName||a.alternativeText||"";
          const val=a.fieldValue||a.buttonValue||"";
          if(name&&val)fields[name]=typeof val==="string"?val.trim():String(val);
        });
        const txt=await page.getTextContent();
        txt.items.forEach(t=>{
          if(t.str&&t.str.trim()){
            allItems.push({str:t.str.trim(),x:Math.round(t.transform[4]),y:Math.round(t.transform[5]),page:i,font:t.fontName||""});
          }
        });
      }
      console.log("PDF Form Fields:",Object.keys(fields).length,fields);
      console.log("All Text Items:",allItems.length);
      
      // STRATEGY 1: Form field annotations (fillable PDFs)
      if(Object.keys(fields).length>5){
        const g=(keys)=>{for(const k of keys){const kl=k.toLowerCase();for(const[fn,fv] of Object.entries(fields)){const fnl=fn.toLowerCase().replace(/^q\d+_/,"");if((fnl.includes(kl)||fn.toLowerCase().includes(kl))&&fv)return fv}}return ""};
        setPdfParsed({
          bizName:g(["business","company","legal name","bizName"]),dba:g(["dba"]),
          bizAddress:g(["business address","bizAddress","address","street"]),bizCity:g(["city","bizCity"]),bizState:g(["state","bizState"]),bizZip:g(["zip","bizZip"]),
          bizPhone:g(["business phone","bizPhone","phone"]),bizEmail:g(["email","bizEmail"]),
          fedId:g(["ein","tax id","federal","fedId"]),legalStructure:g(["structure","entity","legalStructure"]),
          employees:g(["employees"]),dateStarted:g(["date started","dateStarted"]),bizType:g(["business type","bizType"]),
          o1Name:g(["owner","guarantor","o1Name","applicant"]),o1Title:g(["title","o1Title"]),o1Pct:g(["ownership","percent","o1Pct"])||"100",
          o1Address:g(["home address","o1Address","residential"]),o1City:g(["o1City"]),o1State:g(["o1State"]),o1Zip:g(["o1Zip"]),
          o1Cell:g(["cell","mobile","o1Cell"]),o1Ssn:g(["ssn","social","o1Ssn"]),o1Dob:g(["dob","birth","o1Dob"]),
          o2Name:g(["owner 2","o2Name","co-owner"]),o2Ssn:g(["o2Ssn"]),o2Dob:g(["o2Dob"]),o2Cell:g(["o2Cell"]),
          o2Address:g(["o2Address"]),o2City:g(["o2City"]),o2State:g(["o2State"]),o2Zip:g(["o2Zip"]),o2Pct:g(["o2Pct"]),o2Title:g(["o2Title"]),
          bankName:g(["bank","bankName"]),bankAcct:g(["account","bankAcct"]),bankPhone:g(["bank phone","bankPhone"]),bankContact:g(["bank contact","bankContact"]),
          trade1:g(["trade ref","reference 1","trade1"]),trade1Ph:g(["trade1Ph"]),trade2:g(["reference 2","trade2"]),trade2Ph:g(["trade2Ph"]),
          vendorName:g(["vendor","dealer","vendorName"]),vendorPhone:g(["vendor phone","vendorPhone"]),salesPerson:g(["salesperson","salesPerson"]),
          eqType:g(["equipment type","eqType"]),eqDesc:g(["equipment desc","eqDesc"]),eqYear:g(["year","eqYear"]),
          eqMake:g(["make","eqMake"]),eqModel:g(["model","eqModel"]),eqVin:g(["vin","serial","eqVin"]),
          eqMileage:g(["mileage","hours"]),newUsed:g(["new","used","condition"])||"New",price:g(["price","amount","cost"]),termReq:g(["term","months","termReq"]),
        });setPdfParsing(false);return;
      }
      
      // STRATEGY 2: Jotform flattened PDF - separate template text from filled values
      // Jotform renders filled values in different fonts than the template labels
      // Count fonts to find template vs value fonts
      const fontMap={};
      allItems.forEach(it=>{if(!fontMap[it.font])fontMap[it.font]=[];fontMap[it.font].push(it)});
      console.log("Fonts:",Object.entries(fontMap).map(([f,items])=>f+":"+items.length));
      
      // Template labels are ALL CAPS or known boilerplate
      const SKIP=/^(LEGAL BUSINESS NAME|DBA|BUSINESS TYPE|PHYSICAL BUSINESS ADDRESS|CITY,? STATE,? ZIP|PHONE|FAX|EMAIL|FED\.?\s*ID|LEGAL STRUCTURE|CORP|LLC|SOLE PROP|# OF EMPLOYEES|DATE BUSINESS STARTED|DATE YOU BECAME|OWNER [12] NAME|HOME ADDRESS|HOME PHONE|CELL|SOCIAL SECURITY|DATE OF\s*BIRTH|% OF OWNERSHIP|BUSINESS BANK\s*NAME|ACCOUNT\s*#|TRADE REFERENCE|VENDOR NAME|SALES PERSON|EQUIPMENT DESCRIPTION|NEW OR USED|PRICE|TERM REQUESTED|CONTACT|Business Information|Ownership Information|Business Bank\/Trade|Vendor Information|Office:|Finance Application|Delivery of|I hereby|institutions|LLC or its|provide credit|renewal or|BY:|TITLE:|DATE:|Can we communicate|Document ID|Signed at|Jotform|FINCAP|FINANCIAL CAPITAL|www\.|nh\.bdc|888-419|617-843|□|Yes|No|Upon completion|facsimile|certify|authorize|reporting|designee|applicant.*owner|guarantor.*authorize|account history|employment history|similar information|obtaining|update|information it may|or fax to)$/i;
      
      // Filter: keep only non-template, non-boilerplate text items
      const vals=allItems.filter(it=>{
        const s=it.str;
        if(s.length<2&&!/\d/.test(s))return false;
        if(SKIP.test(s))return false;
        // Also skip partial template matches
        if(/^(LEGAL|PHYSICAL|BUSINESS|OWNER|HOME|SOCIAL|TRADE|VENDOR|EQUIPMENT|ACCOUNT|SALES|NEW OR|TERM|PRICE|CONTACT|DATE |# OF|% OF|FED|CELL|FAX|PHONE|EMAIL|DBA|CITY|STATE|ZIP|CORP|BY:)/i.test(s)&&s===s.toUpperCase())return false;
        if(/Office:|Finance Application|Delivery of|I hereby|institutions|provide credit|renewal|designee|facsimile|certify|authorize|reporting|account history|information it may|employment/i.test(s))return false;
        if(/Signed at|Document ID|Jotform|FINCAP|FINANCIAL CAPITAL|www\.|nh\.bdc|888-419-2528|617-843-5612/i.test(s))return false;
        if(/^_{3,}$/.test(s))return false;
        return true;
      });
      
      console.log("Value items:",vals.map(v=>v.str));
      
      // Sort by Y desc (top of page first in pdf.js coords) then X asc
      const sorted=_.orderBy(vals,["y","x"],["desc","asc"]);
      
      // Group into lines by Y proximity
      const lines=[];
      let curLine=[sorted[0]];
      for(let i=1;i<sorted.length;i++){
        if(Math.abs(sorted[i].y-sorted[i-1].y)>4){
          lines.push(curLine.sort((a,b)=>a.x-b.x));
          curLine=[sorted[i]];
        }else{
          curLine.push(sorted[i]);
        }
      }
      if(curLine.length)lines.push(curLine.sort((a,b)=>a.x-b.x));
      
      const lts=lines.map(l=>l.map(i=>i.str).join(" "));
      console.log("Value lines:",lts);
      
      // Now parse by content pattern recognition
      const parsed={bizName:"",dba:"",bizAddress:"",bizCity:"",bizState:"",bizZip:"",bizPhone:"",bizEmail:"",fedId:"",
        legalStructure:"",employees:"",dateStarted:"",bizType:"",o1Name:"",o1Title:"",o1Pct:"100",o1Address:"",o1City:"",
        o1State:"",o1Zip:"",o1Cell:"",o1Ssn:"",o1Dob:"",o2Name:"",o2Ssn:"",o2Dob:"",o2Cell:"",o2Address:"",o2City:"",
        o2State:"",o2Zip:"",o2Pct:"",o2Title:"",bankName:"",bankAcct:"",bankPhone:"",bankContact:"",trade1:"",trade1Ph:"",
        trade2:"",trade2Ph:"",vendorName:"",vendorPhone:"",salesPerson:"",eqType:"",eqDesc:"",eqYear:"",eqMake:"",
        eqModel:"",eqVin:"",eqMileage:"",newUsed:"New",price:"",termReq:""};
      
      // Track which lines we've consumed
      const used=new Set();
      
      // Pass 1: Extract by strong patterns
      lts.forEach((l,i)=>{
        // Email
        if(/@/.test(l)&&!parsed.bizEmail&&!/nh\.bdc|fincap/i.test(l)){
          const em=l.match(/([\w.+-]+@[\w-]+\.[\w.]+)/);
          if(em){parsed.bizEmail=em[1];used.add(i);return}
        }
        // SSN+ownership line: "253477441 50 284869048 50"
        const ssn2=l.match(/(\d{9})\s+(\d{1,3})\s+(\d{9})\s+(\d{1,3})/);
        if(ssn2){parsed.o1Ssn=ssn2[1];parsed.o1Pct=ssn2[2];parsed.o2Ssn=ssn2[3];parsed.o2Pct=ssn2[4];used.add(i);return}
        // Single SSN+ownership: "253477441 50"
        const ssn1=l.match(/^(\d{9})\s+(\d{1,3})$/);
        if(ssn1&&!parsed.o1Ssn){parsed.o1Ssn=ssn1[1];parsed.o1Pct=ssn1[2];used.add(i);return}
        // Two addresses: "150 Vandy cir sparta GA 38583 150 Vandy cir sparta GA 38583"
        const addr2=l.match(/^(.+?\b[A-Z]{2}\s+\d{5})\s+(.+?\b[A-Z]{2}\s+\d{5})$/);
        if(addr2&&parsed.bizAddress){
          const p1=addr2[1].match(/^(.+?)\s+([A-Za-z]+)\s+([A-Z]{2})\s+(\d{5})$/);
          const p2=addr2[2].match(/^(.+?)\s+([A-Za-z]+)\s+([A-Z]{2})\s+(\d{5})$/);
          if(p1){parsed.o1Address=p1[1];parsed.o1City=p1[2];parsed.o1State=p1[3];parsed.o1Zip=p1[4]}
          if(p2){parsed.o2Address=p2[1];parsed.o2City=p2[2];parsed.o2State=p2[3];parsed.o2Zip=p2[4]}
          used.add(i);return;
        }
        // Single address
        const addr1=l.match(/^(.+?)\s+([A-Za-z]+)\s+([A-Z]{2})\s+(\d{5})$/);
        if(addr1){
          if(!parsed.bizAddress){parsed.bizAddress=addr1[1];parsed.bizCity=addr1[2];parsed.bizState=addr1[3];parsed.bizZip=addr1[4]}
          else if(!parsed.o1Address){parsed.o1Address=addr1[1];parsed.o1City=addr1[2];parsed.o1State=addr1[3];parsed.o1Zip=addr1[4]}
          used.add(i);return;
        }
        // Two phones: "(706) 871-0854 (706) 871-0854"
        const ph2=l.match(/(\(?\d{3}\)?[\s.\-]?\d{3}[\s.\-]?\d{4})\s+(\(?\d{3}\)?[\s.\-]?\d{3}[\s.\-]?\d{4})/);
        if(ph2&&parsed.bizPhone){parsed.o1Cell=ph2[1];parsed.o2Cell=ph2[2];used.add(i);return}
        // Single phone
        if(/^\(?\d{3}\)?[\s.\-]?\d{3}[\s.\-]?\d{4}$/.test(l.trim())){
          if(!parsed.bizPhone){parsed.bizPhone=l.trim()}
          else if(!parsed.o1Cell){parsed.o1Cell=l.trim()}
          used.add(i);return;
        }
        // EIN: 9 digits alone
        if(/^\d{9}$/.test(l.trim())&&!parsed.fedId){parsed.fedId=l.trim();used.add(i);return}
        // EIN with dash: "93-4808029"
        if(/^\d{2}-\d{7}$/.test(l.trim())&&!parsed.fedId){parsed.fedId=l.trim();used.add(i);return}
        // Date + structure: "01-24-2024 Inc"
        const ds=l.match(/^(\d{1,2}[-\/]\d{1,2}[-\/]\d{2,4})\s+(.+)$/);
        if(ds&&!/202[5-9]/.test(ds[1])){parsed.dateStarted=ds[1];parsed.legalStructure=ds[2].trim();used.add(i);return}
        // Just a date (not signature date)
        if(/^\d{1,2}[-\/]\d{1,2}[-\/]\d{4}$/.test(l.trim())&&!/02-24-2026/.test(l)){
          if(!parsed.dateStarted){parsed.dateStarted=l.trim();used.add(i);return}
        }
        // Title + signature date: "Ceo 02-24-2026" or "Vp 02-24-2026"
        const tsd=l.match(/^([A-Za-z]{2,20})\s+(\d{1,2}[-\/]\d{1,2}[-\/]\d{4})$/);
        if(tsd){
          if(!parsed.o1Title){parsed.o1Title=tsd[1]}
          else if(!parsed.o2Title){parsed.o2Title=tsd[1]}
          used.add(i);return;
        }
      });
      
      // Pass 2: Remaining lines are likely names, DBA, biz name, structure
      const remaining=lts.filter((_,i)=>!used.has(i)).filter(l=>l.length>1);
      console.log("Remaining after pattern pass:",remaining);
      
      remaining.forEach(l=>{
        const t=l.trim();
        if(!t||t.length<2)return;
        // Legal structure alone
        if(!parsed.legalStructure&&/^(Inc|LLC|Corp|Sole Prop|Partnership|S.Corp|C.Corp|Sole Proprietorship|Corporation|Limited Liability)\.?$/i.test(t)){
          parsed.legalStructure=t;return;
        }
        // Number of employees
        if(!parsed.employees&&/^\d{1,4}$/.test(t)&&parseInt(t)<5000){
          parsed.employees=t;return;
        }
        // Business name / DBA (first text-only remaining line)
        if(!parsed.bizName&&/^[A-Za-z]/.test(t)&&!/[@]/.test(t)){
          // Check if it's "DBA FullBusinessName" pattern (short word + longer name)
          const parts=t.match(/^(\S{2,15})\s+(.{4,})$/);
          if(parts&&parts[1].length<parts[2].length){
            parsed.dba=parts[1];
            parsed.bizName=parts[2];
          }else{
            parsed.bizName=t;
          }
          return;
        }
        // Owner names line: "Mickey Koelling Kimberly combs"
        if(!parsed.o1Name&&/^[A-Za-z]/.test(t)&&!/[@]/.test(t)&&t.length>3){
          const words=t.split(/\s+/);
          if(words.length>=4){
            // Four+ words: split into two names (2+2 or proportional)
            const mid=Math.floor(words.length/2);
            parsed.o1Name=words.slice(0,mid).join(" ");
            parsed.o2Name=words.slice(mid).join(" ");
          }else if(words.length>=2){
            parsed.o1Name=t;
          }
          return;
        }
        // If we have o1Name but not o2Name
        if(parsed.o1Name&&!parsed.o2Name&&/^[A-Za-z]/.test(t)&&!/[@]/.test(t)&&t.split(/\s+/).length>=2){
          parsed.o2Name=t;return;
        }
      });
      
      console.log("Final parsed:",parsed);
      setPdfParsed(parsed);
    }catch(e){console.error("PDF Parse Error:",e);notify("Could not parse PDF — try a different file");}
    setPdfParsing(false);
  };
  const handlePdfDrop=(e)=>{e.preventDefault();setPdfDrag(false);const f=e.dataTransfer?.files?.[0]||e.target?.files?.[0];if(f&&f.type==="application/pdf")parsePdf(f);else notify("Please drop a PDF file");};
  const createFromPdf=()=>{if(!pdfParsed)return;const p=pdfParsed;const code=p.bizName.replace(/[^A-Z]/gi,"").slice(0,3).toUpperCase()+"-"+Math.floor(1000+Math.random()*9000);
    saveDeal({biz:p.bizName,contact:p.o1Name,email:p.bizEmail,phone:p.bizPhone||p.o1Cell,eqType:p.eqType,eqDetail:[p.eqYear,p.eqMake,p.eqModel].filter(Boolean).join(" ")||p.eqDesc,eqYear:p.eqYear,eqMake:p.eqMake,eqModel:p.eqModel,eqVin:p.eqVin,eqMileage:p.eqMileage,amount:parseFloat((p.price||"").replace(/[^0-9.]/g,""))||0,stage:"new_lead",source:"PDF Import",code,notes:`Imported from PDF. Structure: ${p.legalStructure||"N/A"}. EIN: ${p.fedId||"N/A"}.`,
      appData:{bizName:p.bizName,dba:p.dba,bizType:p.bizType,bizAddress:p.bizAddress,bizCity:p.bizCity,bizState:p.bizState,bizZip:p.bizZip,bizPhone:p.bizPhone,bizEmail:p.bizEmail,fedId:p.fedId,legalStructure:p.legalStructure,employees:p.employees,dateStarted:p.dateStarted,
      o1Name:p.o1Name,o1Title:p.o1Title,o1Pct:p.o1Pct,o1Address:p.o1Address,o1City:p.o1City,o1State:p.o1State,o1Zip:p.o1Zip,o1Cell:p.o1Cell,o1Ssn:p.o1Ssn,o1Dob:p.o1Dob,
      o2Name:p.o2Name,o2Title:p.o2Title,o2Pct:p.o2Pct,o2Address:p.o2Address,o2City:p.o2City,o2State:p.o2State,o2Zip:p.o2Zip,o2Cell:p.o2Cell,o2Ssn:p.o2Ssn,o2Dob:p.o2Dob,hasOwner2:!!p.o2Name,
      bankName:p.bankName,bankAcct:p.bankAcct,bankPhone:p.bankPhone,bankContact:p.bankContact,trade1:p.trade1,trade1Ph:p.trade1Ph,trade2:p.trade2,trade2Ph:p.trade2Ph,
      vendorName:p.vendorName,vendorPhone:p.vendorPhone,salesPerson:p.salesPerson,
      eqDesc:p.eqDesc,newUsed:p.newUsed,price:p.price,termReq:p.termReq}
    });
    notify("Deal created from PDF — "+p.bizName);setPdfParsed(null);
  };

  return(
    <div style={S.wrap} onDragOver={e=>{e.preventDefault();if(e.dataTransfer.types.includes("Files"))setPdfDrag(true)}} onDragLeave={e=>{if(!e.currentTarget.contains(e.relatedTarget))setPdfDrag(false)}} onDrop={handlePdfDrop}>
      {/* PDF Drop Overlay */}
      {pdfDrag&&<div style={{position:"fixed",inset:0,background:"#000000dd",zIndex:100,display:"flex",alignItems:"center",justifyContent:"center",pointerEvents:"none"}}>
        <div style={{border:"3px dashed #4F8EAE",borderRadius:16,padding:"60px 80px",textAlign:"center"}}>
          <I.Upload/><div style={{fontSize:18,fontWeight:700,color:"#4F8EAE",marginTop:12}}>Drop PDF Application</div>
          <div style={{fontSize:12,color:"#999",marginTop:4}}>Auto-creates a new deal from the application</div>
        </div>
      </div>}
      {/* PDF Parsing Spinner */}
      {pdfParsing&&<div style={{position:"fixed",inset:0,background:"#00000066",zIndex:100,display:"flex",alignItems:"center",justifyContent:"center"}}>
        <div style={{textAlign:"center",color:"#4F8EAE"}}><div style={{fontSize:16,fontWeight:600,animation:"pulse 1s infinite"}}>Parsing PDF...</div><div style={{fontSize:11,color:"#999",marginTop:6}}>Extracting application fields</div></div>
      </div>}
      {/* PDF Preview Modal */}
      {pdfParsed&&<div style={{position:"fixed",inset:0,background:"#000000dd",zIndex:100,display:"flex",alignItems:"center",justifyContent:"center",padding:20}} onClick={()=>setPdfParsed(null)}>
        <div style={{background:"#111114",border:"1px solid #D8DCE0",borderRadius:12,maxWidth:560,width:"100%",maxHeight:"80vh",overflow:"hidden",display:"flex",flexDirection:"column"}} onClick={e=>e.stopPropagation()}>
          <div style={{padding:"16px 20px",borderBottom:"1px solid #D8DCE0",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div><div style={{fontSize:15,fontWeight:700,color:"#1a1a2e"}}>PDF Import Preview</div><div style={{fontSize:11,color:"#999",marginTop:2}}>Review parsed data before creating the deal</div></div>
            <button style={S.closeBtn} onClick={()=>setPdfParsed(null)}><I.X/></button>
          </div>
          <div style={{padding:"16px 20px",overflowY:"auto",flex:1}}>
            {[{t:"Business",items:[["Business Name",pdfParsed.bizName],["DBA",pdfParsed.dba],["Address",pdfParsed.bizAddress],["City/State/ZIP",[pdfParsed.bizCity,pdfParsed.bizState,pdfParsed.bizZip].filter(Boolean).join(", ")],["Phone",pdfParsed.bizPhone],["Email",pdfParsed.bizEmail],["EIN",pdfParsed.fedId],["Structure",pdfParsed.legalStructure],["Industry",pdfParsed.bizType],["Started",pdfParsed.dateStarted]]},
              {t:"Owner / Guarantor",items:[["Name",pdfParsed.o1Name],["Title",pdfParsed.o1Title],["Ownership",pdfParsed.o1Pct?pdfParsed.o1Pct+"%":""],["Cell",pdfParsed.o1Cell],["SSN",pdfParsed.o1Ssn?"•••-••-"+pdfParsed.o1Ssn.slice(-4):""],["DOB",pdfParsed.o1Dob],["Address",pdfParsed.o1Address]]},
              {t:"Bank & References",items:[["Bank",pdfParsed.bankName],["Account",pdfParsed.bankAcct?"••••"+pdfParsed.bankAcct.slice(-4):""],["Trade Ref 1",pdfParsed.trade1],["Trade Ref 2",pdfParsed.trade2]]},
              {t:"Equipment",items:[["Type",pdfParsed.eqType],["Year/Make/Model",[pdfParsed.eqYear,pdfParsed.eqMake,pdfParsed.eqModel].filter(Boolean).join(" ")],["VIN",pdfParsed.eqVin],["Price",pdfParsed.price],["Condition",pdfParsed.newUsed],["Term",pdfParsed.termReq],["Vendor",pdfParsed.vendorName]]}
            ].map(sec=><div key={sec.t} style={{marginBottom:12,background:"#FFFFFF",border:"1px solid #E2E5E9",borderRadius:8,overflow:"hidden"}}>
              <div style={{padding:"6px 12px",borderBottom:"1px solid #E2E5E9",fontSize:10,fontWeight:700,color:"#4F8EAE",textTransform:"uppercase",letterSpacing:.8}}>{sec.t}</div>
              <div style={{padding:"8px 12px",display:"grid",gridTemplateColumns:"1fr 1fr",gap:"3px 16px"}}>
                {sec.items.filter(([,v])=>v).map(([l,v])=><div key={l} style={{display:"flex",gap:6,fontSize:12,padding:"2px 0"}}><span style={{color:"#999",minWidth:90}}>{l}:</span><span style={{color:"#666",fontWeight:500}}>{v}</span></div>)}
                {sec.items.filter(([,v])=>v).length===0&&<div style={{fontSize:11,color:"#666",fontStyle:"italic"}}>No data found</div>}
              </div>
            </div>)}
          </div>
          <div style={{padding:"14px 20px",borderTop:"1px solid #D8DCE0",display:"flex",gap:8,justifyContent:"flex-end"}}>
            <button className="av-ghost" style={S.ghost} onClick={()=>setPdfParsed(null)}>Cancel</button>
            <button className="av-btn" style={{...S.prim,padding:"10px 24px",fontSize:13}} onClick={createFromPdf}><I.Plus/> Create Deal</button>
          </div>
        </div>
      </div>}
      <div style={S.hdr}>
        <div style={{...S.srchW,position:"relative"}} ref={srchRef}><I.Search/><input type="text" placeholder="Search by name, email, phone, code..." value={q} onChange={e=>setQ(e.target.value)} onFocus={()=>setSrchFocus(true)} onBlur={()=>setTimeout(()=>setSrchFocus(false),200)} style={S.srchIn}/>{q&&<button style={S.tinyBtn} onClick={()=>setQ("")}><I.X/></button>}
          {srchFocus&&q.length>=2&&uniHasResults&&<div style={{position:"absolute",top:"100%",left:0,right:0,marginTop:4,background:"#FFFFFF",border:"1px solid #CDD1D6",borderRadius:8,boxShadow:"0 12px 40px #00000022",zIndex:999,maxHeight:380,overflowY:"auto",padding:4}}>
            {uniResults.accounts.length>0&&<><div style={{fontSize:9,fontWeight:700,color:"#4F8EAE",textTransform:"uppercase",letterSpacing:.8,padding:"6px 10px 3px"}}>Accounts</div>
              {uniResults.accounts.map(a=><div key={a.id} style={{padding:"7px 10px",borderRadius:5,cursor:"pointer",display:"flex",alignItems:"center",gap:8,transition:"background .1s"}} className="av-card" onMouseDown={()=>{setView("accounts");setQ("");setTimeout(()=>{const el=document.querySelector(`[data-acct-id="${a.id}"]`);el?.click()},50)}}><div style={{width:28,height:28,borderRadius:"50%",background:"#4F8EAE15",display:"flex",alignItems:"center",justifyContent:"center",color:"#4F8EAE",flexShrink:0,fontSize:11}}><I.User/></div><div style={{flex:1,minWidth:0}}><div style={{fontSize:12,fontWeight:600,color:"#1a1a2e",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{a.biz}</div><div style={{fontSize:10,color:"#999",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{a.contact}{a.email?" · "+a.email:""}{a.phone?" · "+a.phone:""}</div></div></div>)}
            </>}
            {uniResults.deals.length>0&&<><div style={{fontSize:9,fontWeight:700,color:"#E89300",textTransform:"uppercase",letterSpacing:.8,padding:"6px 10px 3px",borderTop:uniResults.accounts.length?"1px solid #D8DCE0":"none"}}>Deals</div>
              {uniResults.deals.map(d=>{const stg=STAGES.find(s=>s.id===d.stage);return<div key={d.id} style={{padding:"7px 10px",borderRadius:5,cursor:"pointer",display:"flex",alignItems:"center",gap:8,transition:"background .1s"}} className="av-card" onMouseDown={()=>{setSel(d);setQ("")}}><div style={{width:28,height:28,borderRadius:"50%",background:(stg?.color||"#333")+"18",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><span style={{fontSize:12}}>{stg?.ic||"📋"}</span></div><div style={{flex:1,minWidth:0}}><div style={{fontSize:12,fontWeight:600,color:"#1a1a2e",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{d.biz} <span style={{fontSize:9,color:stg?.color||"#666",fontWeight:700}}>{stg?.label||d.stage}</span></div><div style={{fontSize:10,color:"#999",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{d.contact}{d.code?" · "+d.code:""}{d.email?" · "+d.email:""}{d.phone?" · "+d.phone:""}</div></div></div>})}
            </>}
            {uniResults.dealers.length>0&&<><div style={{fontSize:9,fontWeight:700,color:"#4CAF50",textTransform:"uppercase",letterSpacing:.8,padding:"6px 10px 3px",borderTop:(uniResults.accounts.length||uniResults.deals.length)?"1px solid #D8DCE0":"none"}}>Dealers</div>
              {uniResults.dealers.map(d=><div key={d.id} style={{padding:"7px 10px",borderRadius:5,cursor:"pointer",display:"flex",alignItems:"center",gap:8,transition:"background .1s"}} className="av-card" onMouseDown={()=>{setView("dealers");setQ("")}}><div style={{width:28,height:28,borderRadius:"50%",background:"#4CAF5015",display:"flex",alignItems:"center",justifyContent:"center",color:"#4CAF50",flexShrink:0,fontSize:11}}><I.Bldg/></div><div style={{flex:1,minWidth:0}}><div style={{fontSize:12,fontWeight:600,color:"#1a1a2e",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{d.name} <span style={{fontSize:9,color:"#999"}}>{d.code}</span></div><div style={{fontSize:10,color:"#999",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{d.contact}{d.email?" · "+d.email:""}{d.phone?" · "+d.phone:""}</div></div></div>)}
            </>}
          </div>}
        </div>
        <div style={{display:"flex",gap:6}}>
          <input type="file" accept=".pdf" ref={fileRef} style={{display:"none"}} onChange={handlePdfDrop}/>
          <button className="av-ghost" style={{...S.ghost,padding:"8px 12px",fontSize:11}} onClick={()=>fileRef.current?.click()}><I.Upload/> Import PDF</button>
          <button className="av-btn" style={S.prim} onClick={()=>{setEditD(null);setForm(true)}}><I.Plus/> New Deal</button>
        </div>
      </div>

      <div className="av-stats-row" style={S.statsG}>{[{l:"Active",v:stats.act,c:"#1a1a2e"},{l:"Pipeline",v:fmt(stats.pipe),c:"#4F8EAE"},{l:"Funded",v:fmt(stats.fun),c:"#4CAF50"},{l:"Won",v:stats.won,c:"#2E7D32"},{l:"Total",v:stats.tot,c:"#1a1a2e"},{l:"Dead",v:stats.dead,c:"#999"}].map((s,i)=><div key={i} style={S.statC}><div style={{...S.statV,color:s.c}}>{s.v}</div><div style={S.statL}>{s.l}</div></div>)}</div>
      <div className="av-crm-tabs" style={S.tabs}>{[{id:"dashboard",ic:<I.Star/>,l:"Dashboard"},{id:"pipeline",ic:<I.Pipe/>,l:unreadDlr>0?`Pipeline (${unreadDlr} 🔔)`:"Pipeline"},{id:"list",ic:<I.List/>,l:"All Deals"},{id:"won",ic:<I.CheckCirc/>,l:`Won (${stats.won})`},{id:"accounts",ic:<I.User/>,l:"Accounts"},{id:"dead",ic:<I.Archive/>,l:`Dead/Declined (${stats.dead})`},{id:"compliance",ic:<I.Shield/>,l:"Compliance"},{id:"dealers",ic:<I.Bldg/>,l:"Dealers"},{id:"lenders",ic:<I.Dollar/>,l:"Lenders"},{id:"settings",ic:<I.Gear/>,l:"Settings"}].map(t=><button key={t.id} className="av-tab" style={{...S.tab,...(view===t.id?S.tabA:{})}} onClick={()=>setView(t.id)}>{t.ic} {t.l}</button>)}</div>
      <div style={{marginTop:14,animation:"fadeIn .3s"}}>
        {view==="dashboard"&&<Dashboard onSelDeal={d=>setSel(d)}/>}
        {view==="pipeline"&&<>{(()=>{
          const activeDlrs=dealers.filter(dlr=>filt.some(d=>d.dealerId===dlr.id&&!["dead","declined"].includes(d.stage)));
          const directCount=filt.filter(d=>!d.dealerId&&!["dead","declined"].includes(d.stage)).length;
          const pipDeals=dlrFilter?filt.filter(d=>dlrFilter==="direct"?!d.dealerId:d.dealerId===dlrFilter):filt;
          return(<>
            <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:10,flexWrap:"wrap"}}>
              <span style={{fontSize:10,fontWeight:700,color:"#999",textTransform:"uppercase",letterSpacing:.8}}>Filter by Dealer:</span>
              <button onClick={()=>setDlrFilter(null)} style={{padding:"4px 10px",fontSize:10,fontWeight:600,borderRadius:5,cursor:"pointer",fontFamily:"inherit",border:`1px solid ${!dlrFilter?"#4F8EAE":"#CDD1D6"}`,background:!dlrFilter?"#4F8EAE18":"transparent",color:!dlrFilter?"#4F8EAE":"#666",transition:"all .15s"}}>All Dealers</button>
              {directCount>0&&<button onClick={()=>setDlrFilter(dlrFilter==="direct"?null:"direct")} style={{padding:"4px 10px",fontSize:10,fontWeight:600,borderRadius:5,cursor:"pointer",fontFamily:"inherit",border:`1px solid ${dlrFilter==="direct"?"#E89300":"#CDD1D6"}`,background:dlrFilter==="direct"?"#E8930018":"transparent",color:dlrFilter==="direct"?"#E89300":"#666",transition:"all .15s"}}>Direct ({directCount})</button>}
              {activeDlrs.map(dlr=>{const cnt=filt.filter(d=>d.dealerId===dlr.id&&!["dead","declined"].includes(d.stage)).length;return(
                <button key={dlr.id} onClick={()=>setDlrFilter(dlrFilter===dlr.id?null:dlr.id)} style={{padding:"4px 10px",fontSize:10,fontWeight:600,borderRadius:5,cursor:"pointer",fontFamily:"inherit",border:`1px solid ${dlrFilter===dlr.id?"#4CAF50":"#CDD1D6"}`,background:dlrFilter===dlr.id?"#4CAF5018":"transparent",color:dlrFilter===dlr.id?"#4CAF50":"#666",transition:"all .15s"}}>{dlr.name} ({cnt})</button>
              )})}
            </div>
            {dlrFilter&&<div style={{fontSize:11,color:"#999",marginBottom:8,display:"flex",alignItems:"center",gap:6}}>
              Showing: <strong style={{color:"#1a1a2e"}}>{dlrFilter==="direct"?"Direct (No Dealer)":dealers.find(x=>x.id===dlrFilter)?.name||"Unknown"}</strong>
              <button onClick={()=>setDlrFilter(null)} style={{fontSize:9,padding:"2px 6px",borderRadius:4,background:"#ffffff08",border:"1px solid #CDD1D6",color:"#999",cursor:"pointer",fontFamily:"inherit"}}>✕ Clear</button>
            </div>}
            <Pipeline deals={pipDeals} onSel={setSel} dragId={dragId} setDragId={setDragId} dragO={dragO} setDragO={setDragO} moveDeal={moveDeal}/>
          </>);
        })()}<NotificationFeed onSel={setSel}/></>}
        {view==="list"&&<ListV deals={filt.filter(d=>d.stage!=="dead")} onSel={setSel} onEdit={d=>{setEditD(d);setForm(true)}}/>}
        {view==="won"&&<WonV deals={filt.filter(d=>d.stage==="funded")} onSel={setSel}/>}
        {view==="accounts"&&<AccountsMgmt onSel={setSel}/>}
        {view==="dead"&&<DeadV deals={filt.filter(d=>d.stage==="dead"||d.stage==="declined")} onSel={setSel} onReact={id=>moveDeal(id,"new_lead","Reactivated")}/>}
        {view==="compliance"&&<ComplianceDash onSel={setSel}/>}
        {view==="dealers"&&<DlrMgmt/>}
        {view==="lenders"&&<LndrMgmt/>}
        {view==="settings"&&<Settings/>}
      </div>
      {sel&&<Drawer deal={sel} onClose={()=>setSel(null)} onEdit={d=>{setEditD(d);setForm(true);setSel(null)}} onDel={id=>{delDeal(id);setSel(null)}} onMove={moveDeal}/>}
      {form&&<DealForm deal={editD} onSave={d=>{saveDeal(d);setForm(false);setEditD(null)}} onClose={()=>{setForm(false);setEditD(null)}}/>}
    </div>
  );
}

// ── Dashboard ─────────────────────────────────────────────────

function Dashboard({onSelDeal}) {
  const {deals,dealers,dealerNotes} = useContext(Ctx);
  const now=new Date();
  
  // Date range filter state
  const [dateRange,setDateRange]=useState("all"); // all, mtd, ytd, custom
  const [customFrom,setCustomFrom]=useState("");
  
  // Compute date filter boundaries
  const filterFrom = useMemo(()=>{
    if(dateRange==="all") return null;
    if(dateRange==="mtd") return new Date(now.getFullYear(),now.getMonth(),1);
    if(dateRange==="ytd") return new Date(now.getFullYear(),0,1);
    if(dateRange==="custom"&&customFrom) return new Date(customFrom);
    return null;
  },[dateRange,customFrom]);
  
  // Filter deals by date (using createdAt)
  const filtered = useMemo(()=>{
    if(!filterFrom) return deals;
    return deals.filter(d=>{
      const dt=new Date(d.createdAt||d.updatedAt||0);
      return dt>=filterFrom;
    });
  },[deals,filterFrom]);

  const activeDeals=filtered.filter(d=>!["funded","declined","dead"].includes(d.stage));
  const fundedDeals=filtered.filter(d=>d.stage==="funded");
  const declinedDeals=filtered.filter(d=>d.stage==="declined");
  const deadDeals=filtered.filter(d=>d.stage==="dead");
  const closedDeals=[...fundedDeals,...declinedDeals,...deadDeals];
  const winRate=closedDeals.length?(fundedDeals.length/closedDeals.length*100).toFixed(0):0;
  const totalPipeline=activeDeals.reduce((s,d)=>s+(d.amount||0),0);
  const totalFunded=fundedDeals.reduce((s,d)=>s+(d.amount||0),0);
  const totalFees=fundedDeals.reduce((s,d)=>s+(d.terms?.feeAmt||0)+(d.terms?.dlrFee||0),0);
  const avgDealSize=fundedDeals.length?(totalFunded/fundedDeals.length):0;
  const yourFees=fundedDeals.reduce((s,d)=>s+(d.terms?.feeAmt||0),0);
  const dlrFees=fundedDeals.reduce((s,d)=>s+(d.terms?.dlrFee||0),0);

  // Deals by stage for funnel
  const stageCounts=STAGES.filter(s=>!["declined","dead"].includes(s.id)).map(s=>({...s,count:filtered.filter(d=>d.stage===s.id).length,total:filtered.filter(d=>d.stage===s.id).reduce((sum,d)=>sum+(d.amount||0),0)}));

  // Recent activity (last 10 across all deals)
  const allActivities=filtered.flatMap(d=>(d.activities||[]).map(a=>({...a,dealBiz:d.biz,dealId:d.id}))).sort((a,b)=>new Date(b.at)-new Date(a.at)).slice(0,10);

  // Aging: deals stuck in same stage > 3 days
  const agingDeals=activeDeals.filter(d=>{
    const lastMove=(d.history||[]).slice(-1)[0];
    if(!lastMove)return false;
    const days=(now-new Date(lastMove.d))/(1000*60*60*24);
    return days>3;
  }).map(d=>{
    const lastMove=(d.history||[]).slice(-1)[0];
    const days=Math.floor((now-new Date(lastMove.d))/(1000*60*60*24));
    return {...d,daysInStage:days};
  }).sort((a,b)=>b.daysInStage-a.daysInStage).slice(0,5);

  // Pending stips count
  const pendingStips=activeDeals.reduce((s,d)=>s+(d.stips||[]).filter(st=>!st.done).length,0);
  // Pending funding items count
  const pendingFI=activeDeals.reduce((s,d)=>s+(d.fundingItems||[]).filter(f=>!f.done).length,0);
  // Unread dealer messages
  const unreadDlrMsgs=(dealerNotes||[]).filter(n=>!n.read).length;

  // Top dealers by deal count
  const dlrStats=dealers.map(dl=>({...dl,dealCount:filtered.filter(d=>d.dealerId===dl.id).length,funded:filtered.filter(d=>d.dealerId===dl.id&&d.stage==="funded").length,volume:filtered.filter(d=>d.dealerId===dl.id&&d.stage==="funded").reduce((s,d)=>s+(d.amount||0),0)})).filter(dl=>dl.dealCount>0).sort((a,b)=>b.dealCount-a.dealCount);

  const DB=({l,v,c,sub})=><div style={{background:"#FFFFFF",borderRadius:8,padding:"14px 16px",border:"1px solid #D8DCE0",flex:1,minWidth:140}}>
    <div style={{fontSize:22,fontWeight:800,color:c||"#fff",fontVariantNumeric:"tabular-nums"}}>{v}</div>
    <div style={{fontSize:10,color:"#999",textTransform:"uppercase",letterSpacing:.8,marginTop:3}}>{l}</div>
    {sub&&<div style={{fontSize:10,color:"#aaa",marginTop:2}}>{sub}</div>}
  </div>;

  const actIcons={call_cust:"📞",email_cust:"📧",call_dlr:"📞",email_dlr:"📧",call_lndr:"📞",email_lndr:"📧",other:"📎"};

  return(<div>
    {/* Date Range Filter */}
    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:14,flexWrap:"wrap"}}>
      <span style={{fontSize:11,fontWeight:600,color:"#999",textTransform:"uppercase",letterSpacing:.8}}>Showing:</span>
      {[{id:"all",label:"All Time"},{id:"mtd",label:"Month to Date"},{id:"ytd",label:"Year to Date"},{id:"custom",label:"Custom Date"}].map(opt=>(
        <button key={opt.id} onClick={()=>{setDateRange(opt.id);if(opt.id!=="custom")setCustomFrom("")}} style={{
          padding:"5px 12px",fontSize:11,fontWeight:600,borderRadius:6,cursor:"pointer",fontFamily:"inherit",
          border:`1px solid ${dateRange===opt.id?"#4F8EAE":"#CDD1D6"}`,
          background:dateRange===opt.id?"#4F8EAE12":"#fff",
          color:dateRange===opt.id?"#4F8EAE":"#888",transition:"all .15s"
        }}>{opt.label}</button>
      ))}
      {dateRange==="custom"&&<input type="date" value={customFrom} onChange={e=>setCustomFrom(e.target.value)} style={{
        padding:"5px 10px",fontSize:11,border:"1px solid #CDD1D6",borderRadius:6,background:"#fff",color:"#1a1a2e",fontFamily:"inherit",outline:"none"
      }}/>}
      {dateRange!=="all"&&<span style={{fontSize:10,color:"#4F8EAE",fontWeight:600}}>
        {filterFrom?`From ${filterFrom.toLocaleDateString()}`:""} · {filtered.length} deal{filtered.length!==1?"s":""}
      </span>}
    </div>

    {/* KPI Cards */}
    <div className="av-kpi-row" style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:16}}>
      <DB l="Active Deals" v={activeDeals.length} c="#1a1a2e"/>
      <DB l="Pipeline Value" v={fmt(totalPipeline)} c="#4F8EAE"/>
      <DB l="Funded Volume" v={fmt(totalFunded)} c="#4CAF50"/>
      <DB l="Your Fees Earned" v={fmt(yourFees)} c="#E89300"/>
      <DB l="Win Rate" v={winRate+"%"} c={winRate>=50?"#4CAF50":"#E89300"} sub={`${fundedDeals.length} won / ${closedDeals.length} closed`}/>
      <DB l="Avg Deal Size" v={fmt(avgDealSize)} c="#4F8EAE"/>
    </div>

    {/* Reminders Section */}
    <ReminderSection deals={filtered} onSelDeal={onSelDeal}/>

    <div className="av-dash-grid" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
      {/* Deal Funnel */}
      <div style={{background:"#F5F6F8",border:"1px solid #E2E5E9",borderRadius:9,padding:"16px 18px"}}>
        <div style={{fontSize:12,fontWeight:700,color:"#1a1a2e",marginBottom:12}}>Deal Pipeline</div>
        {stageCounts.map(s=>{const pct=filtered.length?Math.max(s.count/Math.max(...stageCounts.map(x=>x.count))*100,4):0;return(
          <div key={s.id} style={{display:"flex",alignItems:"center",gap:10,marginBottom:6}}>
            <div style={{width:80,fontSize:10,color:"#999",textAlign:"right",flexShrink:0}}>{s.label}</div>
            <div style={{flex:1,height:22,background:"#E2E5E9",borderRadius:4,overflow:"hidden",position:"relative"}}>
              <div style={{height:"100%",width:pct+"%",background:s.color,borderRadius:4,transition:"width .5s",opacity:.8}}/>
              <span style={{position:"absolute",left:6,top:3,fontSize:10,fontWeight:700,color:"#1a1a2e"}}>{s.count}</span>
            </div>
            <div style={{width:80,fontSize:10,color:"#999",textAlign:"right"}}>{fmt(s.total)}</div>
          </div>
        )})}
      </div>

      {/* Alerts & Aging */}
      <div style={{background:"#F5F6F8",border:"1px solid #E2E5E9",borderRadius:9,padding:"16px 18px"}}>
        <div style={{fontSize:12,fontWeight:700,color:"#1a1a2e",marginBottom:12}}>Attention Needed</div>
        {pendingStips>0&&<div style={{display:"flex",alignItems:"center",gap:8,padding:"8px 10px",background:"#E8930008",border:"1px solid #E8930018",borderRadius:6,marginBottom:6}}>
          <span style={{color:"#E89300",fontSize:16}}>⚠</span>
          <div><div style={{fontSize:12,color:"#E89300",fontWeight:600}}>{pendingStips} pending stip{pendingStips!==1?"s":""}</div><div style={{fontSize:10,color:"#999"}}>Items needed from customers</div></div>
        </div>}
        {unreadDlrMsgs>0&&<div style={{display:"flex",alignItems:"center",gap:8,padding:"8px 10px",background:"#E8930008",border:"1px solid #E8930018",borderRadius:6,marginBottom:6}}>
          <span style={{color:"#E89300",fontSize:16}}>🔔</span>
          <div><div style={{fontSize:12,color:"#E89300",fontWeight:600}}>{unreadDlrMsgs} unread dealer message{unreadDlrMsgs!==1?"s":""}</div><div style={{fontSize:10,color:"#999"}}>Check Pipeline tab for details</div></div>
        </div>}
        {pendingFI>0&&<div style={{display:"flex",alignItems:"center",gap:8,padding:"8px 10px",background:"#4F8EAE08",border:"1px solid #4F8EAE18",borderRadius:6,marginBottom:6}}>
          <span style={{color:"#4F8EAE",fontSize:16}}>📋</span>
          <div><div style={{fontSize:12,color:"#4F8EAE",fontWeight:600}}>{pendingFI} funding item{pendingFI!==1?"s":""} outstanding</div><div style={{fontSize:10,color:"#999"}}>Waiting on dealers</div></div>
        </div>}
        {agingDeals.length>0&&<div style={{marginTop:6}}>
          <div style={{fontSize:10,fontWeight:600,color:"#999",textTransform:"uppercase",letterSpacing:.8,marginBottom:6}}>Aging Deals</div>
          {agingDeals.map(d=>{const stg=STAGES.find(s=>s.id===d.stage);return(
            <div key={d.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 8px",borderLeft:`2px solid ${stg?.color||"#555"}`,marginBottom:4,background:"#FFFFFF",borderRadius:"0 5px 5px 0"}}>
              <div><div style={{fontSize:11,color:"#666",fontWeight:600}}>{d.biz}</div><div style={{fontSize:9,color:"#999"}}>{stg?.label}</div></div>
              <div style={{fontSize:10,color:d.daysInStage>7?"#B0413E":"#E89300",fontWeight:700}}>{d.daysInStage}d</div>
            </div>
          )})}
        </div>}
        {pendingStips===0&&pendingFI===0&&unreadDlrMsgs===0&&agingDeals.length===0&&<div style={{padding:20,textAlign:"center",color:"#666",fontSize:11}}>All clear — no items need attention</div>}
      </div>

      {/* Recent Activity */}
      <div style={{background:"#F5F6F8",border:"1px solid #E2E5E9",borderRadius:9,padding:"16px 18px"}}>
        <div style={{fontSize:12,fontWeight:700,color:"#1a1a2e",marginBottom:12}}>Recent Activity</div>
        {allActivities.length?allActivities.map(a=>(
          <div key={a.id} style={{display:"flex",gap:8,padding:"5px 0",borderBottom:"1px solid #E2E5E9"}}>
            <span style={{fontSize:13,flexShrink:0}}>{actIcons[a.type]||"📎"}</span>
            <div style={{flex:1}}><div style={{fontSize:11,color:"#666"}}>{a.detail||a.type.replace(/_/g," ")}</div><div style={{fontSize:9,color:"#999"}}>{a.dealBiz} · {fDT(a.at)}</div></div>
          </div>
        )):<div style={{padding:20,textAlign:"center",color:"#666",fontSize:11}}>No recent activity</div>}
      </div>

      {/* Top Dealers */}
      <div style={{background:"#F5F6F8",border:"1px solid #E2E5E9",borderRadius:9,padding:"16px 18px"}}>
        <div style={{fontSize:12,fontWeight:700,color:"#1a1a2e",marginBottom:12}}>Dealer Performance</div>
        {dlrStats.length?dlrStats.map(dl=>(
          <div key={dl.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 10px",borderBottom:"1px solid #E2E5E9"}}>
            <div><div style={{fontSize:12,color:"#666",fontWeight:600}}>{dl.name}</div><div style={{fontSize:10,color:"#999"}}>{dl.contact}</div></div>
            <div style={{textAlign:"right"}}><div style={{fontSize:11,color:"#6BA3BF",fontWeight:700}}>{dl.dealCount} deal{dl.dealCount!==1?"s":""}</div><div style={{fontSize:9,color:"#4CAF50"}}>{dl.funded} funded · {fmt(dl.volume)}</div></div>
          </div>
        )):<div style={{padding:20,textAlign:"center",color:"#666",fontSize:11}}>No dealer activity yet</div>}
      </div>
    </div>

    {/* Fee Breakdown */}
    {totalFees>0&&<div style={{marginTop:14,background:"#F5F6F8",border:"1px solid #E2E5E9",borderRadius:9,padding:"16px 18px"}}>
      <div style={{fontSize:12,fontWeight:700,color:"#1a1a2e",marginBottom:12}}>Fee Income Summary</div>
      <div style={{display:"flex",gap:20}}>
        <div><div style={{fontSize:20,fontWeight:800,color:"#E89300"}}>{fmt(yourFees)}</div><div style={{fontSize:10,color:"#999",textTransform:"uppercase",letterSpacing:.8,marginTop:2}}>Your Points Income</div></div>
        {dlrFees>0&&<div><div style={{fontSize:20,fontWeight:800,color:"#4F8EAE"}}>{fmt(dlrFees)}</div><div style={{fontSize:10,color:"#999",textTransform:"uppercase",letterSpacing:.8,marginTop:2}}>Dealer Referral Fees</div></div>}
        <div><div style={{fontSize:20,fontWeight:800,color:"#4CAF50"}}>{fmt(totalFees)}</div><div style={{fontSize:10,color:"#999",textTransform:"uppercase",letterSpacing:.8,marginTop:2}}>Total Fees on Funded</div></div>
      </div>
    </div>}
  </div>);
}

function ReminderSection({deals,onSelDeal}) {
  const {reminders,addReminder,removeReminder,dismissReminder,notify}=useContext(Ctx);
  const [showForm,setShowForm]=useState(false);
  const [rText,setRText]=useState("");
  const [rDate,setRDate]=useState("");
  const [rTime,setRTime]=useState("");
  const [rDealId,setRDealId]=useState("");
  
  const now=Date.now();
  const upcoming=reminders.filter(r=>!r.dismissed&&new Date(r.dueAt).getTime()>now).sort((a,b)=>new Date(a.dueAt)-new Date(b.dueAt));
  const overdue=reminders.filter(r=>!r.dismissed&&new Date(r.dueAt).getTime()<=now).sort((a,b)=>new Date(b.dueAt)-new Date(a.dueAt));
  const completed=reminders.filter(r=>r.dismissed).sort((a,b)=>new Date(b.dueAt)-new Date(a.dueAt)).slice(0,5);
  
  const handleAdd=()=>{
    if(!rText.trim()){notify("Enter reminder text","error");return}
    if(!rDate||!rTime){notify("Pick a date and time","error");return}
    addReminder({text:rText.trim(),dueAt:new Date(`${rDate}T${rTime}`).toISOString(),dealId:rDealId||null,dealBiz:rDealId?deals.find(d=>d.id===rDealId)?.biz||"":""});
    setRText("");setRDate("");setRTime("");setRDealId("");setShowForm(false);
    notify("Reminder set");
  };

  const RRow=({r,isOverdue})=>{
    const deal=r.dealId?deals.find(d=>d.id===r.dealId):null;
    const due=new Date(r.dueAt);
    return(
      <div style={{display:"flex",alignItems:"center",gap:10,padding:"8px 10px",background:isOverdue?"#B0413E06":"#fff",border:`1px solid ${isOverdue?"#B0413E22":"#E2E5E9"}`,borderRadius:6,marginBottom:4}}>
        <span style={{fontSize:14}}>{isOverdue?"🔴":r.dismissed?"✅":"🔔"}</span>
        <div style={{flex:1}}>
          <div style={{fontSize:12,color:"#1a1a2e",fontWeight:600}}>{r.text}</div>
          <div style={{display:"flex",gap:8,alignItems:"center",marginTop:2}}>
            <span style={{fontSize:10,color:isOverdue?"#B0413E":"#999"}}>{due.toLocaleDateString()} {due.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}</span>
            {deal&&<span style={{fontSize:10,color:"#4F8EAE",cursor:"pointer",fontWeight:600,textDecoration:"underline"}} onClick={()=>onSelDeal&&onSelDeal(deal)}>🔗 {deal.biz}</span>}
          </div>
        </div>
        {!r.dismissed&&<>
          <button onClick={()=>dismissReminder(r.id)} style={{background:"none",border:"none",color:"#4CAF50",cursor:"pointer",fontSize:11,fontWeight:600,fontFamily:"inherit",padding:"3px 6px"}}>Done</button>
          <button onClick={()=>removeReminder(r.id)} style={{background:"none",border:"none",color:"#B0413E",cursor:"pointer",fontSize:11,fontFamily:"inherit",padding:"3px 6px"}}>✕</button>
        </>}
      </div>
    );
  };

  return(
    <div style={{marginTop:14,background:"#F5F6F8",border:"1px solid #E2E5E9",borderRadius:9,padding:"16px 18px"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          <span style={{fontSize:14}}>🔔</span>
          <span style={{fontSize:12,fontWeight:700,color:"#1a1a2e"}}>Reminders</span>
          {overdue.length>0&&<span style={{fontSize:9,padding:"2px 7px",borderRadius:8,background:"#B0413E",color:"#fff",fontWeight:700}}>{overdue.length} overdue</span>}
          {upcoming.length>0&&<span style={{fontSize:9,padding:"2px 7px",borderRadius:8,background:"#4F8EAE12",color:"#4F8EAE",border:"1px solid #4F8EAE33",fontWeight:600}}>{upcoming.length} upcoming</span>}
        </div>
        <button className="av-btn" style={{...S.prim,padding:"5px 12px",fontSize:11}} onClick={()=>setShowForm(!showForm)}>{showForm?"Cancel":"+ New Reminder"}</button>
      </div>
      
      {showForm&&<div style={{background:"#fff",border:"1px solid #E2E5E9",borderRadius:8,padding:14,marginBottom:12}}>
        <input style={{...S.inp,marginBottom:8}} placeholder="What do you need to remember?" value={rText} onChange={e=>setRText(e.target.value)} autoFocus/>
        <div style={{display:"flex",gap:8,marginBottom:8,flexWrap:"wrap"}}>
          <div style={{flex:1,minWidth:120}}>
            <label style={S.fLabel}>Date</label>
            <input type="date" style={S.inp} value={rDate} onChange={e=>setRDate(e.target.value)}/>
          </div>
          <div style={{flex:1,minWidth:100}}>
            <label style={S.fLabel}>Time</label>
            <input type="time" style={S.inp} value={rTime} onChange={e=>setRTime(e.target.value)}/>
          </div>
          <div style={{flex:1,minWidth:150}}>
            <label style={S.fLabel}>Link to Deal (optional)</label>
            <select style={S.fSel} value={rDealId} onChange={e=>setRDealId(e.target.value)}>
              <option value="">None</option>
              {deals.filter(d=>!["dead","declined"].includes(d.stage)).map(d=><option key={d.id} value={d.id}>{d.biz} — {d.eqType}</option>)}
            </select>
          </div>
        </div>
        <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:10}}>
          {[{l:"In 1 hour",m:60},{l:"Tomorrow 9am",fn:()=>{const t=new Date();t.setDate(t.getDate()+1);t.setHours(9,0,0,0);setRDate(t.toISOString().slice(0,10));setRTime("09:00")}},{l:"In 3 days",fn:()=>{const t=new Date();t.setDate(t.getDate()+3);t.setHours(9,0,0,0);setRDate(t.toISOString().slice(0,10));setRTime("09:00")}},{l:"Next Monday 9am",fn:()=>{const t=new Date();t.setDate(t.getDate()+(8-t.getDay())%7||7);t.setHours(9,0,0,0);setRDate(t.toISOString().slice(0,10));setRTime("09:00")}}].map(q=>(
            <button key={q.l} onClick={()=>{if(q.m){const t=new Date(Date.now()+q.m*60000);setRDate(t.toISOString().slice(0,10));setRTime(t.toTimeString().slice(0,5))}else q.fn()}} style={{padding:"3px 8px",fontSize:10,borderRadius:4,border:"1px solid #CDD1D6",background:"#fff",color:"#666",cursor:"pointer",fontFamily:"inherit"}}>{q.l}</button>
          ))}
        </div>
        <button className="av-btn" style={{...S.prim,width:"100%",justifyContent:"center"}} onClick={handleAdd}>Set Reminder</button>
      </div>}
      
      {overdue.length>0&&<div style={{marginBottom:8}}>
        <div style={{fontSize:10,fontWeight:600,color:"#B0413E",textTransform:"uppercase",letterSpacing:.8,marginBottom:4}}>Overdue</div>
        {overdue.map(r=><RRow key={r.id} r={r} isOverdue/>)}
      </div>}
      
      {upcoming.length>0&&<div style={{marginBottom:8}}>
        <div style={{fontSize:10,fontWeight:600,color:"#999",textTransform:"uppercase",letterSpacing:.8,marginBottom:4}}>Upcoming</div>
        {upcoming.map(r=><RRow key={r.id} r={r}/>)}
      </div>}
      
      {completed.length>0&&<div>
        <div style={{fontSize:10,fontWeight:600,color:"#999",textTransform:"uppercase",letterSpacing:.8,marginBottom:4}}>Completed</div>
        {completed.map(r=><RRow key={r.id} r={r}/>)}
      </div>}
      
      {reminders.length===0&&<div style={{padding:16,textAlign:"center",color:"#999",fontSize:11}}>No reminders set. Click "+ New Reminder" to get started.</div>}
    </div>
  );
}

// ── Dealer Notification Feed ──────────────────────────────────

function NotificationFeed({onSel}) {
  const {dealerNotes,dismissDealerNote,dismissAllDealerNotes,deals}=useContext(Ctx);
  
  // Build unified notification list from multiple sources
  const notifications = useMemo(()=>{
    const items = [];
    
    // Dealer messages
    (dealerNotes||[]).forEach(n=>{
      items.push({id:n.id,type:"dealer_note",dealId:n.dealId,text:n.txt,who:n.dealer,biz:n.biz,at:n.at,read:n.read,icon:"💬",color:"#E89300",label:"Dealer Message"});
    });
    
    // Scan all deals for docs uploaded by dealer or customer
    deals.forEach(d=>{
      (d.docs||[]).forEach(doc=>{
        if(doc.by==="dealer"){
          items.push({id:`doc-${d.id}-${doc.docId}`,type:"dealer_doc",dealId:d.id,text:`Uploaded: ${doc.name}`,who:d.dealerName||"Dealer",biz:d.biz,at:doc.at,read:doc.notifRead||false,icon:"📎",color:"#4F8EAE",label:"Dealer Upload"});
        }
        if(doc.by==="customer"){
          items.push({id:`doc-${d.id}-${doc.docId}`,type:"cust_doc",dealId:d.id,text:`Uploaded: ${doc.name}`,who:d.contact||"Customer",biz:d.biz,at:doc.at,read:doc.notifRead||false,icon:"📄",color:"#4CAF50",label:"Customer Upload"});
        }
      });
      // Stips resolved by dealer or customer
      (d.stips||[]).forEach(s=>{
        if(s.done&&s.file&&(s.resolvedBy==="dealer"||s.resolvedBy==="customer")){
          items.push({id:`stip-${d.id}-${s.id}`,type:"stip_resolve",dealId:d.id,text:`Attached: ${s.file}`,who:s.resolvedBy==="dealer"?(d.dealerName||"Dealer"):(d.contact||"Customer"),biz:d.biz,at:s.resolvedAt,read:s.notifRead||false,icon:"✅",color:s.resolvedBy==="dealer"?"#4F8EAE":"#4CAF50",label:s.resolvedBy==="dealer"?"Dealer Stip":"Customer Stip"});
        }
      });
      // Funding items resolved (always by dealer)
      (d.fundingItems||[]).forEach(f=>{
        if(f.done&&f.file){
          items.push({id:`fi-${d.id}-${f.id}`,type:"fi_resolve",dealId:d.id,text:`Attached: ${f.file}`,who:d.dealerName||"Dealer",biz:d.biz,at:f.resolvedAt,read:f.notifRead||false,icon:"📦",color:"#4F8EAE",label:"Funding Item"});
        }
      });
    });
    
    return _.orderBy(items,["at"],["desc"]).slice(0,30);
  },[dealerNotes,deals]);
  
  if(!notifications.length) return null;
  const unread=notifications.filter(n=>!n.read).length;
  
  return(<div style={{marginTop:16,background:"#F5F6F8",border:"1px solid #E2E5E9",borderRadius:9,padding:"14px 18px"}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
      <div style={{display:"flex",alignItems:"center",gap:8}}>
        <span style={{fontSize:13,fontWeight:700,color:"#1a1a2e"}}>🔔 Notifications</span>
        {unread>0&&<span style={{fontSize:10,padding:"2px 8px",borderRadius:10,background:"#E89300",color:"#fff",fontWeight:700,animation:"fadeIn .3s"}}>{unread} new</span>}
      </div>
      {unread>0&&<button className="av-ghost" style={{...S.ghost,padding:"4px 10px",fontSize:10}} onClick={dismissAllDealerNotes}>Mark all read</button>}
    </div>
    <div style={{display:"flex",flexDirection:"column",gap:4,maxHeight:340,overflowY:"auto"}}>
      {notifications.map(n=>{
        const d=deals.find(x=>x.id===n.dealId);
        const isUnread=!n.read;
        return(<div key={n.id} style={{display:"flex",gap:10,padding:"8px 10px",background:isUnread?n.color+"06":"transparent",border:`1px solid ${isUnread?n.color+"22":"#E2E5E9"}`,borderRadius:6,transition:"all .2s ease",cursor:"pointer"}} onClick={()=>{if(d)onSel(d);if(n.type==="dealer_note")dismissDealerNote(n.id)}}>
          <div style={{width:32,height:32,borderRadius:"50%",background:isUnread?n.color+"12":"#E2E5E9",border:`1px solid ${isUnread?n.color+"33":"#222"}`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:14}}>{n.icon}</div>
          <div style={{flex:1,minWidth:0}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div style={{display:"flex",alignItems:"center",gap:5}}>
                <span style={{fontSize:11,fontWeight:700,color:isUnread?n.color:"#999"}}>{n.who}</span>
                <span style={{fontSize:8,padding:"1px 5px",borderRadius:4,background:n.color+"12",color:n.color,fontWeight:600}}>{n.label}</span>
              </div>
              <span style={{fontSize:9,color:"#aaa"}}>{relT(n.at)}</span>
            </div>
            <div style={{fontSize:12,color:isUnread?"#ccc":"#777",lineHeight:1.4,marginTop:2}}>{n.text}</div>
            <div style={{fontSize:10,color:"#999",marginTop:2}}>{n.biz}</div>
          </div>
          {isUnread&&<div style={{width:8,height:8,borderRadius:"50%",background:n.color,flexShrink:0,marginTop:4}}/>}
        </div>);
      })}
    </div>
  </div>);
}

// ── Pipeline Card ─────────────────────────────────────────────

function KCard({d,stg,dragId,setDragId,setDragO,onTouchStart,touchDragging,onSel}){
  const {dnotes,addNote,addDoc,moveDeal:ctxMoveDeal,notify}=useContext(Ctx);
  const [hov,setHov]=useState(false);
  const [quickNote,setQuickNote]=useState("");
  const [popTab,setPopTab]=useState("notes");
  const [fileDrag,setFileDrag]=useState(false);
  const hoverTimer=useRef(null);
  const cardRef=useRef(null);
  const notes=dnotes[d.id]||[];
  const docs=d.docs||[];

  // Right-click context menu
  const [ctxMenu,setCtxMenu]=useState(null); // {x,y}
  const [lostModal,setLostModal]=useState(false);
  const [lostReason,setLostReason]=useState("");
  const [lostNote,setLostNote]=useState("");
  const LOST_REASONS=["Customer unresponsive","Customer went with another lender","Customer went with another broker","Deal economics didn't work","Customer no longer needs equipment","Bad contact info","Customer not qualified"];

  const handleContextMenu=(e)=>{
    e.preventDefault();e.stopPropagation();
    setCtxMenu({x:e.clientX,y:e.clientY});
    setHov(false);
  };
  useEffect(()=>{
    if(!ctxMenu)return;
    const close=()=>setCtxMenu(null);
    window.addEventListener("click",close);window.addEventListener("scroll",close);
    return()=>{window.removeEventListener("click",close);window.removeEventListener("scroll",close)};
  },[ctxMenu]);

  const [flipLeft,setFlipLeft]=useState(false);
  const leaveTimer=useRef(null);
  const enterHover=()=>{
    clearTimeout(leaveTimer.current);
    hoverTimer.current=setTimeout(()=>{
      if(cardRef.current){const rect=cardRef.current.getBoundingClientRect();setFlipLeft(rect.right+270>window.innerWidth)}
      setHov(true);
    },350);
  };
  const leaveHover=()=>{
    clearTimeout(hoverTimer.current);
    leaveTimer.current=setTimeout(()=>setHov(false),250);
  };
  const enterPopup=()=>{clearTimeout(leaveTimer.current);clearTimeout(hoverTimer.current)};
  const leavePopup=()=>{if(!fileDrag)leaveTimer.current=setTimeout(()=>setHov(false),250)};
  const submitNote=(e)=>{
    e.stopPropagation();
    if(!quickNote.trim())return;
    addNote(d.id,quickNote.trim());
    setQuickNote("");
  };
  const handleFileDrop=(e)=>{
    e.preventDefault();e.stopPropagation();setFileDrag(false);
    const files=e.dataTransfer?.files;
    if(!files||!files.length)return;
    Array.from(files).forEach(f=>{
      addDoc(d.id,{docId:"upload_"+Date.now()+"_"+Math.random().toString(36).slice(2,6),name:f.name,at:new Date().toISOString(),by:"admin"});
    });
  };
  const handleFileSelect=(e)=>{
    const files=e.target?.files;
    if(!files||!files.length)return;
    Array.from(files).forEach(f=>{
      addDoc(d.id,{docId:"upload_"+Date.now()+"_"+Math.random().toString(36).slice(2,6),name:f.name,at:new Date().toISOString(),by:"admin"});
    });
    e.target.value="";
  };
  useEffect(()=>()=>{clearTimeout(hoverTimer.current);clearTimeout(leaveTimer.current)},[]);

  return(<div ref={cardRef} style={{position:"relative"}}
    onMouseEnter={enterHover} onMouseLeave={leaveHover}>
    <div draggable className="av-kcard"
      onDragStart={e=>{setDragId(d.id);e.dataTransfer.effectAllowed="move";setHov(false)}}
      onDragEnd={()=>{setDragId(null);setDragO(null)}}
      onTouchStart={e=>{onTouchStart(e,d.id,d.biz);setHov(false)}}
      onClick={()=>{if(!touchDragging)onSel(d)}}
      onContextMenu={handleContextMenu}
      style={{...S.kCard,borderLeftColor:stg.color,...(dragId===d.id?{opacity:.3}:{}),touchAction:"none",userSelect:"none"}}>
      <div style={S.kName}>{d.biz}</div>
      <div style={S.kMeta}><I.User/> {d.contact}</div>
      <div style={S.kMeta}><I.Truck/> {d.eqType}</div>
      <div style={S.kBot}><span style={S.kAmt}>{fmt(d.amount)}</span><span style={S.kTime}>{relT(d.updatedAt)}</span></div>
      {d.dealerName&&<div style={S.kDlr}>{d.dealerName}</div>}
      {docs.length>0&&<div style={{fontSize:9,color:"#4F8EAE66",marginTop:2}}><I.Doc/> {docs.length} doc{docs.length!==1?"s":""}</div>}
      {(d.stips||[]).filter(s=>!s.done).length>0&&<div style={S.kStip}><I.Alert/> {(d.stips||[]).filter(s=>!s.done).length} items needed</div>}
      {notes.length>0&&<div style={{fontSize:9,color:"#E8930088",marginTop:2,display:"flex",alignItems:"center",gap:3}}><I.Edit/> {notes.length} note{notes.length!==1?"s":""}</div>}
    </div>
    {hov&&<div style={{position:"absolute",...(flipLeft?{right:"100%",marginRight:6}:{left:"100%",marginLeft:6}),top:0,width:280,background:"#E2E5E9",border:"1px solid #CDD1D6",borderRadius:8,padding:0,zIndex:80,boxShadow:"0 8px 24px rgba(0,0,0,.5)",animation:"fadeIn .15s"}} onClick={e=>e.stopPropagation()} onMouseEnter={enterPopup} onMouseLeave={leavePopup}>
      {(d.contact||d.phone||d.email)&&<div style={{padding:"8px 12px",borderBottom:"1px solid #222"}}>
        {d.contact&&<div style={{fontSize:12,fontWeight:600,color:"#1a1a2e",marginBottom:3}}>{d.contact}</div>}
        {d.phone&&<a href={`tel:${d.phone.replace(/\D/g,"")}`} style={{display:"flex",alignItems:"center",gap:5,fontSize:11,color:"#6BA3BF",marginBottom:2,cursor:"pointer",textDecoration:"none"}} onClick={e=>e.stopPropagation()}><I.Phone/> {d.phone}</a>}
        {d.email&&<a href={`mailto:${d.email}`} style={{display:"flex",alignItems:"center",gap:5,fontSize:11,color:"#6BA3BF",cursor:"pointer",textDecoration:"none"}} onClick={e=>e.stopPropagation()}><I.Mail/> {d.email}</a>}
      </div>}
      {/* Tabs */}
      <div style={{display:"flex",borderBottom:"1px solid #222"}}>
        {[{id:"notes",label:`Notes (${notes.length})`,ic:"📝"},{id:"files",label:`Files (${docs.length})`,ic:"📎"}].map(t=>
          <button key={t.id} onClick={e=>{e.stopPropagation();setPopTab(t.id)}} style={{flex:1,padding:"7px 0",fontSize:10,fontWeight:700,color:popTab===t.id?"#4F8EAE":"#555",background:popTab===t.id?"#4F8EAE08":"transparent",border:"none",borderBottom:popTab===t.id?"2px solid #4F8EAE":"2px solid transparent",cursor:"pointer",fontFamily:"inherit",letterSpacing:.5,textTransform:"uppercase"}}>{t.ic} {t.label}</button>
        )}
      </div>
      {/* Notes Tab */}
      {popTab==="notes"&&<>
        <div style={{maxHeight:160,overflowY:"auto",padding:"6px 12px"}}>
          {notes.length===0&&<div style={{fontSize:10,color:"#aaa",padding:"8px 0",textAlign:"center",fontStyle:"italic"}}>No notes yet</div>}
          {notes.map(n=><div key={n.id} style={{padding:"5px 0",borderBottom:"1px solid #D8DCE0"}}>
            <div style={{fontSize:11,color:"#666",lineHeight:1.4}}>{n.txt}</div>
            <div style={{fontSize:8,color:"#999",marginTop:2}}>{n.who} · {fDT(n.at)}</div>
          </div>)}
        </div>
        <div style={{padding:"6px 10px",borderTop:"1px solid #222",display:"flex",gap:4}}>
          <input value={quickNote} onChange={e=>setQuickNote(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")submitNote(e)}} onClick={e=>e.stopPropagation()} placeholder="Quick note..." style={{flex:1,background:"#111",border:"1px solid #CDD1D6",borderRadius:5,padding:"5px 8px",fontSize:11,color:"#1a1a2e",fontFamily:"inherit",outline:"none"}}/>
          <button onClick={submitNote} style={{background:"#4F8EAE",border:"none",borderRadius:5,padding:"5px 8px",cursor:"pointer",color:"#fff",fontSize:10,fontWeight:600,fontFamily:"inherit",whiteSpace:"nowrap"}}>Add</button>
        </div>
      </>}
      {/* Files Tab */}
      {popTab==="files"&&<>
        <div style={{maxHeight:140,overflowY:"auto",padding:"6px 12px"}}>
          {docs.length===0&&<div style={{fontSize:10,color:"#aaa",padding:"8px 0",textAlign:"center",fontStyle:"italic"}}>No files attached</div>}
          {docs.map((doc,i)=><div key={i} style={{display:"flex",alignItems:"center",gap:6,padding:"4px 0",borderBottom:"1px solid #D8DCE0"}}>
            <span style={{color:"#4CAF50",flexShrink:0}}><I.Doc/></span>
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontSize:10,color:"#666",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{doc.name}</div>
              <div style={{fontSize:8,color:"#999"}}>{doc.by} · {fDT(doc.at)}</div>
            </div>
          </div>)}
        </div>
        {/* Drop zone */}
        <div
          onDragOver={e=>{e.preventDefault();e.stopPropagation();setFileDrag(true)}}
          onDragEnter={e=>{e.preventDefault();e.stopPropagation();setFileDrag(true)}}
          onDragLeave={e=>{e.preventDefault();e.stopPropagation();setFileDrag(false)}}
          onDrop={handleFileDrop}
          onClick={e=>e.stopPropagation()}
          style={{margin:"6px 10px 8px",padding:fileDrag?"14px 10px":"10px",border:`2px dashed ${fileDrag?"#4F8EAE":"#CDD1D6"}`,borderRadius:6,background:fileDrag?"#4F8EAE08":"transparent",textAlign:"center",transition:"all .15s",cursor:"pointer"}}
        >
          <div style={{fontSize:fileDrag?14:18,marginBottom:2}}>{fileDrag?"📥":"📂"}</div>
          <div style={{fontSize:10,color:fileDrag?"#4F8EAE":"#555",fontWeight:600}}>{fileDrag?"Drop files here":"Drag & drop files here"}</div>
          <label style={{fontSize:9,color:"#4F8EAE",cursor:"pointer",marginTop:3,display:"inline-block"}}>
            or click to browse
            <input type="file" multiple style={{display:"none"}} onChange={handleFileSelect}/>
          </label>
        </div>
      </>}
    </div>}
    {/* Right-click context menu */}
    {ctxMenu&&<div style={{position:"fixed",left:ctxMenu.x,top:ctxMenu.y,zIndex:9999,background:"#FFFFFF",border:"1px solid #E2E5E9",borderRadius:8,boxShadow:"0 8px 32px rgba(0,0,0,.15)",padding:4,minWidth:180,animation:"fadeIn .1s"}} onClick={e=>e.stopPropagation()}>
      <button onClick={e=>{e.stopPropagation();setCtxMenu(null);setLostModal(true);setLostReason("");setLostNote("")}} style={{display:"flex",alignItems:"center",gap:8,width:"100%",padding:"9px 12px",background:"none",border:"none",borderRadius:5,cursor:"pointer",fontFamily:"inherit",fontSize:12,color:"#F44336",fontWeight:600,textAlign:"left",transition:"background .1s"}} onMouseEnter={e=>e.currentTarget.style.background="#F4433612"} onMouseLeave={e=>e.currentTarget.style.background="none"}>
        <span style={{fontSize:14}}>✕</span> Mark as Lost
      </button>
      <div style={{height:1,background:"#CDD1D6",margin:"2px 6px"}}/>
      <button onClick={e=>{e.stopPropagation();setCtxMenu(null);onSel(d)}} style={{display:"flex",alignItems:"center",gap:8,width:"100%",padding:"9px 12px",background:"none",border:"none",borderRadius:5,cursor:"pointer",fontFamily:"inherit",fontSize:12,color:"#666",fontWeight:500,textAlign:"left",transition:"background .1s"}} onMouseEnter={e=>e.currentTarget.style.background="#ffffff08"} onMouseLeave={e=>e.currentTarget.style.background="none"}>
        <span style={{fontSize:14}}><I.Eye/></span> Open Deal
      </button>
    </div>}
    {/* Lost deal modal */}
    {lostModal&&<div style={{position:"fixed",inset:0,background:"#00000066",display:"flex",alignItems:"center",justifyContent:"center",zIndex:9999}} onClick={()=>setLostModal(false)}>
      <div style={{background:"#FFFFFF",border:"1px solid #CDD1D6",borderRadius:10,padding:22,width:380,maxWidth:"90vw"}} onClick={e=>e.stopPropagation()}>
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
          <span style={{fontSize:18}}>✕</span>
          <span style={{fontSize:16,fontWeight:700,color:"#F44336"}}>Mark Deal as Lost</span>
        </div>
        <div style={{fontSize:12,color:"#999",marginBottom:14}}>{d.biz} — {d.eqType} — {fmt(d.amount)}</div>
        <div style={{fontSize:11,fontWeight:600,color:"#999",marginBottom:6}}>Why was this deal lost?</div>
        <div style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:12}}>
          {LOST_REASONS.map(r=>(
            <button key={r} onClick={()=>setLostReason(r)} style={{
              padding:"5px 10px",fontSize:10,borderRadius:5,cursor:"pointer",fontFamily:"inherit",
              border:`1px solid ${lostReason===r?"#F44336":"#CDD1D6"}`,
              background:lostReason===r?"#F4433618":"transparent",
              color:lostReason===r?"#F44336":"#777",transition:"all .15s",fontWeight:600
            }}>{r}</button>
          ))}
        </div>
        <div style={{fontSize:11,fontWeight:600,color:"#999",marginBottom:4}}>Notes</div>
        <textarea style={{...S.inp,resize:"vertical",minHeight:60,fontSize:12,width:"100%",marginBottom:14}} placeholder="Any additional details about why this deal was lost..." value={lostNote} onChange={e=>setLostNote(e.target.value)}/>
        <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
          <button className="av-ghost" style={S.ghost} onClick={()=>setLostModal(false)}>Cancel</button>
          <button className="av-btn" style={{...S.danger,padding:"9px 18px",fontSize:12}} onClick={()=>{
            const reason=lostReason+(lostNote.trim()?" — "+lostNote.trim():"");
            if(!reason.trim()){notify("Please select or type a reason","error");return}
            ctxMoveDeal(d.id,"dead",reason.trim());
            setLostModal(false);
            notify(`${d.biz} marked as lost`,"info");
          }}>Mark as Lost</button>
        </div>
      </div>
    </div>}
  </div>);
}

// ── Pipeline ──────────────────────────────────────────────────

function Pipeline({deals,onSel,dragId,setDragId,dragO,setDragO,moveDeal}) {
  const active = STAGES.filter(s=>!["declined","dead"].includes(s.id));
  const colRefs=useRef({});
  const ghostRef=useRef(null);
  const touchDragId=useRef(null);
  const [touchDragging,setTouchDragging]=useState(false);
  const touchStartPos=useRef(null);
  const longPressTimer=useRef(null);

  // Find which column a point is over
  const colAtPoint=(x,y)=>{
    for(const [stgId,el] of Object.entries(colRefs.current)){
      if(!el)continue;
      const r=el.getBoundingClientRect();
      if(x>=r.left&&x<=r.right&&y>=r.top&&y<=r.bottom)return stgId;
    }
    return null;
  };

  // Create/move ghost element
  const moveGhost=(x,y,text)=>{
    if(!ghostRef.current){
      const g=document.createElement("div");
      g.style.cssText="position:fixed;z-index:9999;padding:8px 14px;background:#1E1E22;border:1px solid #4F8EAE;border-radius:8px;color:#fff;font-size:12px;font-weight:600;font-family:Outfit,sans-serif;pointer-events:none;box-shadow:0 8px 24px #000a;opacity:.92;transform:translate(-50%,-120%);white-space:nowrap;max-width:200px;overflow:hidden;text-overflow:ellipsis";
      g.textContent=text;
      document.body.appendChild(g);
      ghostRef.current=g;
    }
    ghostRef.current.style.left=x+"px";
    ghostRef.current.style.top=y+"px";
  };
  const removeGhost=()=>{if(ghostRef.current){ghostRef.current.remove();ghostRef.current=null}};

  const onTouchStart=(e,dealId,bizName)=>{
    touchStartPos.current={x:e.touches[0].clientX,y:e.touches[0].clientY};
    longPressTimer.current=setTimeout(()=>{
      touchDragId.current=dealId;
      setDragId(dealId);
      setTouchDragging(true);
      moveGhost(e.touches[0].clientX,e.touches[0].clientY,bizName);
      // Vibrate if supported
      if(navigator.vibrate)navigator.vibrate(30);
    },300);
  };

  const onTouchMove=(e)=>{
    // Cancel long press if moved too far before triggering
    if(longPressTimer.current&&touchStartPos.current){
      const dx=Math.abs(e.touches[0].clientX-touchStartPos.current.x);
      const dy=Math.abs(e.touches[0].clientY-touchStartPos.current.y);
      if(dx>10||dy>10){clearTimeout(longPressTimer.current);longPressTimer.current=null}
    }
    if(!touchDragId.current)return;
    e.preventDefault();
    const x=e.touches[0].clientX,y=e.touches[0].clientY;
    moveGhost(x,y);
    const over=colAtPoint(x,y);
    setDragO(over);
  };

  const onTouchEnd=(e)=>{
    clearTimeout(longPressTimer.current);
    longPressTimer.current=null;
    if(!touchDragId.current){touchStartPos.current=null;return}
    const id=touchDragId.current;
    touchDragId.current=null;
    removeGhost();
    setTouchDragging(false);
    if(dragO&&id){
      moveDeal(id,dragO);
    }
    setDragId(null);
    setDragO(null);
    touchStartPos.current=null;
  };

  // Cleanup on unmount
  useEffect(()=>()=>{removeGhost();clearTimeout(longPressTimer.current)},[]);

  return(<div style={S.kanban} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd}>{active.map(stg=>{
    const now=Date.now();
    const sd=deals.filter(d=>{
      if(d.stage!==stg.id)return false;
      // Funded deals only show for 7 days after funding
      if(stg.id==="funded"){
        const fundedEntry=(d.history||[]).slice().reverse().find(h=>h.s==="funded");
        if(fundedEntry){const days=(now-new Date(fundedEntry.d).getTime())/(1000*60*60*24);if(days>7)return false}
      }
      return true;
    }),tot=sd.reduce((s,d)=>s+(d.amount||0),0),isO=dragO===stg.id;
    return(<div key={stg.id} ref={el=>colRefs.current[stg.id]=el} style={{...S.kCol,...(isO?{borderColor:stg.color,boxShadow:`0 0 0 2px ${stg.color}40`,transition:"border-color .15s, box-shadow .15s"}:{transition:"border-color .15s, box-shadow .15s"})}} onDragOver={e=>{e.preventDefault();setDragO(stg.id)}} onDragLeave={()=>setDragO(null)} onDrop={e=>{e.preventDefault();if(dragId)moveDeal(dragId,stg.id);setDragId(null);setDragO(null)}}>
      <div style={S.kHead}><div style={S.kTitle}><span style={{...S.dot,background:stg.color}}/><span>{stg.label}</span><span style={S.kCnt}>{sd.length}</span></div><div style={S.kTot}>{fmt(tot)}</div></div>
      <div style={S.kCards}>{sd.map(d=><KCard key={d.id} d={d} stg={stg} dragId={dragId} setDragId={setDragId} setDragO={setDragO} onTouchStart={onTouchStart} touchDragging={touchDragging} onSel={onSel}/>)}{sd.length===0&&<div style={S.kEmpty}>No deals</div>}</div>
    </div>)})}</div>);
}

// ── List View ─────────────────────────────────────────────────

function ListV({deals,onSel,onEdit}) {
  const sorted = _.orderBy(deals,["updatedAt"],["desc"]);
  return(<div style={{overflowX:"auto"}}><table style={S.table}><thead><tr>{["Business","Contact","Equipment","Amount","Stage","Code","Dealer","Docs","Items","Updated",""].map((h,i)=><th key={i} style={S.th}>{h}</th>)}</tr></thead><tbody>
    {sorted.map(d=>{const stg=STAGES.find(s=>s.id===d.stage);const pendStips=(d.stips||[]).filter(s=>!s.done).length;return(
      <tr key={d.id} className="av-row" style={S.tr} onClick={()=>onSel(d)}>
        <td style={{...S.td,fontWeight:600,color:"#1a1a2e"}}>{d.biz}</td>
        <td style={S.td}>{d.contact}</td><td style={S.td}>{d.eqType}</td>
        <td style={{...S.td,fontWeight:600,fontVariantNumeric:"tabular-nums"}}>{fmt(d.amount)}</td>
        <td style={S.td}><span style={{...S.badge,background:stg?.color+"18",color:stg?.color,borderColor:stg?.color+"33"}}>{stg?.label}</span></td>
        <td style={{...S.td,fontFamily:"'JetBrains Mono',monospace",color:"#4F8EAE",fontSize:11}}>{d.code}</td>
        <td style={{...S.td,color:"#999"}}>{d.dealerName||"—"}</td>
        <td style={{...S.td,color:"#999"}}>{d.docs?.length||0}</td>
        <td style={S.td}>{pendStips>0?<span style={{fontSize:10,color:"#E89300",fontWeight:600}}>{pendStips} needed</span>:<span style={{color:"#666"}}>—</span>}</td>
        <td style={{...S.td,color:"#999",fontSize:11}}>{relT(d.updatedAt)}</td>
        <td style={S.td}><button style={S.tinyBtn} onClick={e=>{e.stopPropagation();onEdit(d)}}><I.Edit/></button></td>
      </tr>)})}
    {sorted.length===0&&<tr><td colSpan={11} style={{...S.td,textAlign:"center",color:"#aaa",padding:40}}>No deals found</td></tr>}
  </tbody></table></div>);
}

// ── Compliance Dashboard ──────────────────────────────────────

function ComplianceDash({onSel}) {
  const {deals,lenders}=useContext(Ctx);
  const now=new Date();

  const declinedDeals=deals.filter(d=>d.stage==="declined");
  const missingAdverseAction=declinedDeals.filter(d=>!d.adverseAction);

  const closedDeals=deals.filter(d=>d.stage==="dead"||d.stage==="declined");
  const missingDisposition=closedDeals.filter(d=>{
    const entry=(d.history||[]).slice().reverse().find(h=>h.s==="dead"||h.s==="declined");
    return !d.disposition&&(!entry?.n||entry.n.startsWith("Moved to"));
  });

  const activeDeals=deals.filter(d=>!["dead","declined","funded"].includes(d.stage)&&d.stage!=="new_lead");
  const missingDocs=activeDeals.filter(d=>{const uploaded=(d.docs||[]).map(x=>x.docId);return!uploaded.includes("credit_app")});

  const staleDeals=deals.filter(d=>!["dead","declined","funded"].includes(d.stage)).filter(d=>{const last=d.updatedAt||d.createdAt;return last&&(now-new Date(last))/(1000*60*60*24)>14});

  const totalIssues=missingAdverseAction.length+missingDisposition.length+missingDocs.length;
  const score=deals.length?Math.max(0,Math.round(100-((totalIssues/Math.max(deals.length,1))*100))):100;

  const FlagCard=({title,count,color,icon,items,desc})=>(
    <div style={{background:"#F5F6F8",border:`1px solid ${count>0?color+"44":"#E2E5E9"}`,borderRadius:9,padding:"14px 16px",borderLeft:`3px solid ${count>0?color:"#666"}`}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <span style={{fontSize:16}}>{icon}</span>
          <div style={{fontSize:12,fontWeight:700,color:count>0?color:"#999"}}>{title}</div>
        </div>
        <span style={{fontSize:18,fontWeight:800,color:count>0?color:"#666"}}>{count}</span>
      </div>
      <div style={{fontSize:10,color:"#999",marginBottom:8}}>{desc}</div>
      {items.length>0&&<div style={{maxHeight:140,overflowY:"auto"}}>
        {items.slice(0,5).map(d=>{const daysSince=d._daysSince||"";return(
          <div key={d.id} className="av-row" style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"5px 8px",background:"#FFFFFF",borderRadius:4,marginBottom:3,cursor:"pointer"}} onClick={()=>onSel(d)}>
            <div><div style={{fontSize:11,color:"#666",fontWeight:600}}>{d.biz}</div><div style={{fontSize:9,color:"#999"}}>{d.contact} · {d.code}</div></div>
            {daysSince&&<span style={{fontSize:10,color:color,fontWeight:600}}>{daysSince}</span>}
          </div>
        )})}
        {items.length>5&&<div style={{fontSize:10,color:"#999",padding:"4px 8px"}}>+{items.length-5} more</div>}
      </div>}
      {count===0&&<div style={{fontSize:11,color:"#4CAF50",fontWeight:600}}>✓ All clear</div>}
    </div>
  );

  return(<div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
      <div style={{fontSize:15,fontWeight:700,color:"#1a1a2e"}}>Compliance Dashboard</div>
      <div style={{display:"flex",alignItems:"center",gap:12}}>
        <div style={{textAlign:"right"}}>
          <div style={{fontSize:24,fontWeight:800,color:score>=90?"#4CAF50":score>=70?"#E89300":"#F44336"}}>{score}%</div>
          <div style={{fontSize:9,color:"#999",textTransform:"uppercase",letterSpacing:.8}}>Compliance Score</div>
        </div>
        <div style={{width:48,height:48,borderRadius:"50%",border:`3px solid ${score>=90?"#4CAF50":score>=70?"#E89300":"#F44336"}`,display:"flex",alignItems:"center",justifyContent:"center"}}>
          <span style={{fontSize:18}}>{score>=90?"✓":score>=70?"⚠":"✕"}</span>
        </div>
      </div>
    </div>

    {totalIssues===0&&<div style={{padding:"20px",background:"#F5F6F8",border:"1px solid #4CAF5044",borderRadius:9,textAlign:"center",marginBottom:16}}>
      <span style={{fontSize:28}}>✓</span>
      <div style={{fontSize:14,fontWeight:700,color:"#4CAF50",marginTop:4}}>All Compliance Checks Passed</div>
      <div style={{fontSize:11,color:"#999",marginTop:4}}>No outstanding adverse action notices or missing dispositions.</div>
    </div>}

    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
      <FlagCard title="Adverse Action Notices" count={missingAdverseAction.length} color="#F44336" icon="⚖️"
        desc="ECOA requires written notice within 30 days of denial with specific reasons."
        items={missingAdverseAction.map(d=>{const h=(d.history||[]).slice().reverse().find(h=>h.s==="declined");const days=h?Math.floor((now-new Date(h.d))/(1000*60*60*24)):0;return{...d,_daysSince:`${days}d ago`}})}/>
      <FlagCard title="Missing Dispositions" count={missingDisposition.length} color="#B0413E" icon="📋"
        desc="Dead/declined deals should have a documented reason for audit trail."
        items={missingDisposition}/>
      <FlagCard title="Missing Credit Applications" count={missingDocs.length} color="#4F8EAE" icon="📄"
        desc="Active deals past new lead stage should have a credit application on file."
        items={missingDocs}/>
    </div>

    {staleDeals.length>0&&<div style={{marginTop:14,background:"#F5F6F8",border:"1px solid #E8930044",borderRadius:9,padding:"14px 16px"}}>
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
        <span style={{fontSize:16}}>⏰</span>
        <div style={{fontSize:12,fontWeight:700,color:"#E89300"}}>Stale Deals — No Activity 14+ Days ({staleDeals.length})</div>
      </div>
      <div style={{fontSize:10,color:"#999",marginBottom:8}}>Auditors may flag deals with prolonged inactivity. Either update or close these.</div>
      <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
        {staleDeals.map(d=>{const days=Math.floor((now-new Date(d.updatedAt||d.createdAt))/(1000*60*60*24));return(
          <div key={d.id} className="av-row" style={{padding:"5px 10px",background:"#FFFFFF",borderRadius:5,cursor:"pointer",display:"flex",alignItems:"center",gap:6}} onClick={()=>onSel(d)}>
            <span style={{fontSize:11,color:"#666",fontWeight:600}}>{d.biz}</span>
            <span style={{fontSize:9,color:"#E89300",fontWeight:700}}>{days}d</span>
          </div>
        )})}
      </div>
    </div>}

    <div style={{marginTop:14,display:"flex",gap:12,flexWrap:"wrap"}}>
      {[{l:"Adverse Notices Sent",v:`${declinedDeals.filter(d=>d.adverseAction).length}/${declinedDeals.length}`,c:"#4CAF50"},
        {l:"Dispositions Documented",v:`${closedDeals.filter(d=>d.disposition).length}/${closedDeals.length}`,c:"#6BA3BF"},
        {l:"Overall Score",v:score+"%",c:score>=90?"#4CAF50":score>=70?"#E89300":"#F44336"}
      ].map((s,i)=><div key={i} style={{background:"#F5F6F8",border:"1px solid #E2E5E9",borderRadius:8,padding:"12px 16px",flex:1,minWidth:140}}>
        <div style={{fontSize:20,fontWeight:800,color:s.c}}>{s.v}</div>
        <div style={{fontSize:9,color:"#999",textTransform:"uppercase",letterSpacing:.8}}>{s.l}</div>
      </div>)}
    </div>
  </div>);
}

// ── Dead Deals ────────────────────────────────────────────────

function DeadV({deals,onSel,onReact}) {
  const {lenders}=useContext(Ctx);
  const sorted = _.orderBy(deals,["updatedAt"],["desc"]);
  const deadCount=deals.filter(d=>d.stage==="dead").length;
  const declinedCount=deals.filter(d=>d.stage==="declined").length;
  const [filter,setFilter]=useState("all"); // all, dead, declined

  const filtered=filter==="all"?sorted:sorted.filter(d=>d.stage===filter);

  const exportCSV=()=>{
    const rows=[["Deal Code","Business Name","Contact","Email","Phone","Equipment","Amount","Dealer","Lender","Status","Disposition Reason","Previous Stage","Date Closed","Date Created","Full History"]];
    sorted.forEach(d=>{
      const disp=d.disposition||{};
      const deadEntry=[...(d.history||[])].reverse().find(h=>h.s==="dead"||h.s==="declined");
      const reason=disp.reason||deadEntry?.n||"No reason recorded";
      const prevStage=disp.previousStage||(d.history||[]).slice().reverse().find(h=>h.s!=="dead"&&h.s!=="declined")?.s||"";
      const lndr=lenders.find(l=>l.id===d.lenderId);
      const histStr=(d.history||[]).map(h=>`${STAGES.find(s=>s.id===h.s)?.label||h.s}: ${h.n} (${new Date(h.d).toLocaleDateString()})`).join(" → ");
      rows.push([
        d.code||"",d.biz||"",d.contact||"",d.email||"",d.phone||"",
        [d.eqYear,d.eqMake,d.eqModel].filter(Boolean).join(" ")||d.eqType||d.eqDetail||"",
        d.amount||0,d.dealerName||"Direct",lndr?.name||"",
        d.stage==="declined"?"Declined":"Dead",
        reason,STAGES.find(s=>s.id===prevStage)?.label||prevStage||"",
        disp.date?new Date(disp.date).toLocaleDateString():deadEntry?.d?new Date(deadEntry.d).toLocaleDateString():"",
        d.createdAt?new Date(d.createdAt).toLocaleDateString():"",
        histStr
      ]);
    });
    const csv=rows.map(r=>r.map(c=>`"${String(c).replace(/"/g,'""')}"`).join(",")).join("\n");
    const blob=new Blob([csv],{type:"text/csv"});
    const url=URL.createObjectURL(blob);
    const a=document.createElement("a");a.href=url;a.download=`Avanti_Dispositions_${new Date().toISOString().slice(0,10)}.csv`;
    document.body.appendChild(a);a.click();
    setTimeout(()=>{document.body.removeChild(a);URL.revokeObjectURL(url)},100);
  };

  if(!sorted.length)return(<div style={S.empty}><I.Archive/><div style={{marginTop:8,fontSize:13,fontWeight:600,color:"#999"}}>No dead or declined deals</div><div style={{fontSize:11,color:"#aaa",marginTop:4}}>Deals marked as dead or declined will appear here with their disposition reasons.</div></div>);
  return(<div>
    <div style={{display:"flex",gap:10,alignItems:"center",marginBottom:12,flexWrap:"wrap"}}>
      <div style={{display:"flex",gap:4}}>
        <button onClick={()=>setFilter("all")} style={{padding:"5px 12px",fontSize:10,fontWeight:600,borderRadius:5,cursor:"pointer",fontFamily:"inherit",border:`1px solid ${filter==="all"?"#666":"#CDD1D6"}`,background:filter==="all"?"#66666618":"transparent",color:filter==="all"?"#ccc":"#666",transition:"all .15s"}}>All ({sorted.length})</button>
        <button onClick={()=>setFilter("dead")} style={{padding:"5px 12px",fontSize:10,fontWeight:600,borderRadius:5,cursor:"pointer",fontFamily:"inherit",border:`1px solid ${filter==="dead"?"#666":"#CDD1D6"}`,background:filter==="dead"?"#66666618":"transparent",color:filter==="dead"?"#ccc":"#666",transition:"all .15s"}}>Dead ({deadCount})</button>
        <button onClick={()=>setFilter("declined")} style={{padding:"5px 12px",fontSize:10,fontWeight:600,borderRadius:5,cursor:"pointer",fontFamily:"inherit",border:`1px solid ${filter==="declined"?"#B0413E":"#CDD1D6"}`,background:filter==="declined"?"#B0413E18":"transparent",color:filter==="declined"?"#B0413E":"#666",transition:"all .15s"}}>Declined ({declinedCount})</button>
      </div>
      <div style={{flex:1}}/>
      <button className="av-btn" style={{...S.prim,padding:"6px 14px",fontSize:11,gap:4}} onClick={exportCSV}><I.Download/> Export CSV</button>
    </div>
    {filtered.map(d=>{
      const disp=d.disposition||{};
      const deadEntry=[...(d.history||[])].reverse().find(h=>h.s==="dead"||h.s==="declined");
      const reason=disp.reason||deadEntry?.n||"No reason recorded";
      const prevStage=(d.history||[]).slice().reverse().find(h=>h.s!=="dead"&&h.s!=="declined");
      const isDeclined=d.stage==="declined";
      return(
      <div key={d.id} className="av-row" style={{...S.deadRow,borderLeft:`3px solid ${isDeclined?"#B0413E":"#555"}`}}>
        <div style={{flex:1,cursor:"pointer"}} onClick={()=>onSel(d)}>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <span style={{fontSize:13,fontWeight:600,color:"#999"}}>{d.biz}</span>
            <span style={{fontSize:9,padding:"2px 7px",borderRadius:8,background:isDeclined?"#B0413E18":"#55555518",color:isDeclined?"#B0413E":"#666",border:`1px solid ${isDeclined?"#B0413E33":"#55555533"}`,fontWeight:700}}>{isDeclined?"DECLINED":"DEAD"}</span>
            {prevStage&&<span style={S.deadWas}>was: {STAGES.find(s=>s.id===prevStage.s)?.label}</span>}
          </div>
          <div style={{fontSize:12,color:"#999",marginTop:3}}>{d.contact} · {d.eqType} · {fmt(d.amount)}{d.dealerName?` · ${d.dealerName}`:""}</div>
          <div style={{fontSize:11,color:isDeclined?"#B0413E":"#E89300",marginTop:4,fontStyle:"italic"}}>
            "{reason}"
          </div>
          <div style={{fontSize:10,color:"#aaa",marginTop:2}}>
            {deadEntry?.d?relT(deadEntry.d):""}
          </div>
        </div>
        <button className="av-btn" style={S.reactBtn} onClick={()=>onReact(d.id)}><I.Refresh/> Reactivate</button>
      </div>)
    })}
  </div>);
}

// ── Won View ──────────────────────────────────────────────────

function WonV({deals,onSel}) {
  const sorted = _.orderBy(deals,["updatedAt"],["desc"]);
  const totalVol=sorted.reduce((s,d)=>s+(d.amount||0),0);
  const totalFees=sorted.reduce((s,d)=>s+(d.terms?.feeAmt||0),0);
  if(!sorted.length)return(<div style={S.empty}><I.CheckCirc/><div style={{marginTop:8,fontSize:13,fontWeight:600,color:"#999"}}>No funded deals yet</div><div style={{fontSize:11,color:"#aaa",marginTop:4}}>Deals that are funded will appear here as won.</div></div>);
  return(<div>
    <div style={{display:"flex",gap:16,marginBottom:14,padding:"12px 16px",background:"#F5F6F8",borderRadius:8,border:"1px solid #E2E5E9"}}>
      <div><div style={{fontSize:18,fontWeight:800,color:"#4CAF50"}}>{sorted.length}</div><div style={{fontSize:9,color:"#999",textTransform:"uppercase",letterSpacing:.8}}>Deals Won</div></div>
      <div><div style={{fontSize:18,fontWeight:800,color:"#6BA3BF"}}>{fmt(totalVol)}</div><div style={{fontSize:9,color:"#999",textTransform:"uppercase",letterSpacing:.8}}>Total Volume</div></div>
      <div><div style={{fontSize:18,fontWeight:800,color:"#E89300"}}>{fmt(totalFees)}</div><div style={{fontSize:9,color:"#999",textTransform:"uppercase",letterSpacing:.8}}>Your Fees</div></div>
    </div>
    {sorted.map(d=>{const fundedEntry=(d.history||[]).slice().reverse().find(h=>h.s==="funded");return(
      <div key={d.id} className="av-row" style={{display:"flex",alignItems:"center",gap:12,background:"#F5F6F8",border:"1px solid #E2E5E9",borderRadius:7,padding:"12px 16px",borderLeft:"3px solid #2E7D32",marginBottom:6,cursor:"pointer",transition:"all .15s"}} onClick={()=>onSel(d)}>
        <div style={{flex:1}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <span style={{fontSize:13,fontWeight:600,color:"#1a1a2e"}}>{d.biz}</span>
            <span style={{fontSize:9,padding:"2px 7px",borderRadius:8,background:"#2E7D3218",color:"#4CAF50",border:"1px solid #2E7D3233",fontWeight:700}}>WON</span>
          </div>
          <div style={{fontSize:12,color:"#888",marginTop:3}}>{d.contact} · {d.eqType} · {fmt(d.amount)}</div>
          <div style={{display:"flex",gap:12,marginTop:4}}>
            {d.terms&&<span style={{fontSize:10,color:"#E89300"}}>{d.terms.points}pts · {fmt(d.terms.feeAmt||0)} fee</span>}
            {d.dealerName&&<span style={{fontSize:10,color:"#4F8EAE"}}>{d.dealerName}</span>}
            {fundedEntry&&<span style={{fontSize:10,color:"#999"}}>Funded {relT(fundedEntry.d)}</span>}
          </div>
        </div>
      </div>
    )})}
  </div>);
}

// ── Drawer ─────────────────────────────────────────────────────

function Drawer({deal,onClose,onEdit,onDel,onMove}) {
  const {deals,dnotes,addNote,addActivity,lenders,addDoc,rmDoc,toggleDocReq,sendConfEmail,sendCodeEmail,notify,accounts,saveAccount} = useContext(Ctx);
  const [mt,setMt]=useState(null);
  const [mn,setMn]=useState("");
  const [nn,setNn]=useState("");
  const [cp,setCp]=useState(false);
  const [closeModal,setCloseModal]=useState(null); // {stage:"dead"|"declined"}
  const [closeReason,setCloseReason]=useState("");
  const d = deals.find(x=>x.id===deal.id)||deal;
  const stg = STAGES.find(s=>s.id===d.stage);
  const lndr = lenders.find(l=>l.id===d.lenderId);
  const notes = dnotes[d.id]||[];
  const copyC = ()=>{navigator.clipboard.writeText(d.code);setCp(true);setTimeout(()=>setCp(false),2000)};

  return(
    <div style={S.overlay} onClick={onClose}><div style={S.drawer} onClick={e=>e.stopPropagation()}>
      <div style={S.drHead}>
        <div><div style={{fontSize:17,fontWeight:700,color:"#1a1a2e",marginBottom:5}}>{d.biz}</div>
          <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
            <span style={{...S.badge,background:stg?.color+"18",color:stg?.color,borderColor:stg?.color+"33"}}>{stg?.label}</span>
            <button onClick={copyC} style={S.codeChip}><I.Copy/><span style={{fontFamily:"'JetBrains Mono',monospace",letterSpacing:1}}>{d.code}</span>{cp&&<span style={{color:"#4CAF50",fontSize:10}}>✓</span>}</button>
          </div></div>
        <button style={S.closeBtn} onClick={onClose}><I.X/></button>
      </div>
      <div style={S.drBody}>
        {/* Stage mover */}
        <Sec t="Move to Stage"><div style={{display:"flex",gap:3,flexWrap:"wrap"}}>{STAGES.map(s=>{const cur=d.stage===s.id;return(
          <button key={s.id} onClick={()=>{if(cur)return;if(s.id==="dead"||s.id==="declined"){setCloseModal({stage:s.id});setCloseReason("")}else if(mt===s.id){onMove(d.id,s.id,mn);setMt(null);setMn("")}else{setMt(s.id);setMn("")}}} style={{padding:"4px 7px",fontSize:9,fontWeight:600,textTransform:"uppercase",letterSpacing:.4,border:cur?`2px solid ${s.color}`:"1px solid #D8DCE0",borderRadius:4,cursor:cur?"default":"pointer",background:cur?s.color+"22":"#FFFFFF",color:cur?s.color:"#999",fontFamily:"inherit",transition:"all .15s"}}>{s.label}</button>)})}</div>
          {mt&&<div style={{marginTop:8,display:"flex",gap:6}}><input style={{...S.inp,flex:1,fontSize:12}} placeholder="Note..." value={mn} onChange={e=>setMn(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"){onMove(d.id,mt,mn);setMt(null);setMn("")}}} autoFocus/><button className="av-btn" style={{...S.prim,padding:"6px 14px",fontSize:11}} onClick={()=>{onMove(d.id,mt,mn);setMt(null);setMn("")}}>Move</button></div>}
        </Sec>
        <Sec t="Contact"><div style={S.field}><I.User/> <strong>{d.contact}</strong></div>{d.email&&<a href={`mailto:${d.email}`} style={{...S.field,color:"#6BA3BF",textDecoration:"none",cursor:"pointer"}}><I.Mail/> {d.email}</a>}{d.phone&&<a href={`tel:${d.phone.replace(/\D/g,"")}`} style={{...S.field,color:"#6BA3BF",textDecoration:"none",cursor:"pointer"}}><I.Phone/> {d.phone}</a>}</Sec>
        <Sec t="Deal Details"><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          <FB l="Equipment Type" v={d.eqType}/><FB l="Amount" v={fmt(d.amount)} vs={{color:"#6BA3BF",fontSize:17}}/><FB l="Source" v={d.source||"—"}/>{d.dealerName&&<FB l="Dealer" v={d.dealerName}/>}{lndr&&<FB l="Lender" v={lndr.name} sub={lndr.code}/>}
        </div></Sec>
        <Sec t="Equipment Info"><div style={{background:"#FFFFFF",border:"1px solid #E2E5E9",borderRadius:7,padding:"12px 14px"}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10}}>
            <FB l="Year" v={d.eqYear||"—"}/><FB l="Make" v={d.eqMake||"—"}/><FB l="Model" v={d.eqModel||"—"}/>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginTop:10}}>
            <FB l="VIN" v={d.eqVin||"—"} vs={{fontFamily:"'JetBrains Mono',monospace",fontSize:11,letterSpacing:.5}}/><FB l="Mileage" v={d.eqMileage?Number(d.eqMileage).toLocaleString()+" mi":"—"}/>
          </div>
          {d.eqDetail&&<div style={{marginTop:8,fontSize:11,color:"#999"}}>Notes: {d.eqDetail}</div>}
        </div></Sec>
        <GenerateCreditApp deal={d}/>
        {d.appData?.signature&&<Sec t="Electronic Signature">
          <div style={{background:"#FFFFFF",border:"1px solid #E2E5E9",borderRadius:7,padding:"12px 14px"}}>
            <div style={{background:"#FFFFFF",borderRadius:6,padding:4,marginBottom:8}}><img src={d.appData.signature} alt="Signature" style={{width:"100%",height:60,objectFit:"contain"}}/></div>
            <div style={{fontSize:11,color:"#4CAF50",fontWeight:600}}>✓ Electronically signed by {d.appData.signedBy||d.contact}</div>
            <div style={{fontSize:10,color:"#999",marginTop:2}}>{d.appData.signedAt?new Date(d.appData.signedAt).toLocaleString("en-US",{weekday:"long",year:"numeric",month:"long",day:"numeric",hour:"numeric",minute:"2-digit",timeZoneName:"short"}):"—"}</div>
            <div style={{fontSize:9,color:"#666",marginTop:4,fontStyle:"italic"}}>Valid under ESIGN Act & UETA</div>
          </div>
        </Sec>}
        {d.terms&&<Sec t="Approval Terms"><div style={S.approvalBox}><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><FB l="Approved Amount" v={fmt(d.terms.amt)} vs={{color:"#4CAF50"}}/><FB l="Monthly Payment" v={fmt(d.terms.pmt)}/><FB l="Term" v={`${d.terms.months}mo`}/><FB l="Rate" v={d.terms.rate}/></div>
          {d.terms.buyRate!=null&&d.terms.points!=null&&<div style={{marginTop:10,padding:"8px 10px",background:"#E8930008",border:"1px solid #E8930018",borderRadius:6}}>
            <div style={{fontSize:8,fontWeight:700,color:"#E89300",textTransform:"uppercase",letterSpacing:1,marginBottom:6}}>Internal — Admin Only</div>
            <div style={{display:"grid",gridTemplateColumns:d.terms.dlrPts>0?"1fr 1fr 1fr 1fr":"1fr 1fr 1fr",gap:8}}>
              <FB l="Your Points" v={d.terms.points+"pts"} vs={{color:"#E89300"}}/>
              <FB l="Your Fee" v={"$"+(d.terms.feeAmt||(d.terms.amt*(d.terms.points/100))||0).toLocaleString(undefined,{maximumFractionDigits:0})} vs={{color:"#4CAF50",fontWeight:700}}/>
              {d.terms.dlrPts>0&&<FB l="Dealer Pts" v={d.terms.dlrPts+"pts"} vs={{color:"#4F8EAE"}}/>}
              <FB l="Actual Financed" v={"$"+(d.terms.financedAmt||0).toLocaleString(undefined,{maximumFractionDigits:0})} vs={{color:"#E89300"}}/>
            </div>
            {d.terms.dlrPts>0&&<div style={{marginTop:6,fontSize:10,color:"#aaa"}}>Dealer referral: {d.terms.dlrPts}pts = ${(d.terms.dlrFee||(d.terms.amt*(d.terms.dlrPts/100))||0).toLocaleString(undefined,{maximumFractionDigits:0})} · Total points on deal: {d.terms.points+d.terms.dlrPts}</div>}
          </div>}
          {d.accepted!==null&&<div style={{marginTop:8,padding:"5px 10px",background:d.accepted?"#4CAF5012":"#B0413E12",borderRadius:5,fontSize:11,color:d.accepted?"#4CAF50":"#B0413E"}}>{d.accepted?"✓ Accepted":"✗ Not accepted"}{d.taxChoice!==null&&` · Tax: ${d.taxChoice?"Financed":"Separate"}`}</div>}
        </div></Sec>}

        {/* Adverse Action Notice - for declined deals */}
        {d.stage==="declined"&&<Sec t={<span style={{display:"flex",alignItems:"center",gap:6}}>Adverse Action Notice <span style={{fontSize:9,padding:"1px 6px",borderRadius:8,background:d.adverseAction?"#4CAF5018":"#B0413E18",color:d.adverseAction?"#4CAF50":"#B0413E",border:`1px solid ${d.adverseAction?"#4CAF5033":"#B0413E33"}`,fontWeight:600}}>{d.adverseAction?"SENT":"REQUIRED"}</span></span>}>
          <div style={{background:"#FFFFFF",border:"1px solid #E2E5E9",borderRadius:7,padding:"12px 14px"}}>
            <div style={{fontSize:11,color:"#999",marginBottom:8}}>ECOA/Reg B requires notification to the applicant within 30 days of adverse action with specific reasons for denial.</div>
            {d.adverseAction?<div>
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                <span style={{color:"#4CAF50"}}><I.CheckCirc/></span>
                <div>
                  <div style={{fontSize:12,color:"#666",fontWeight:600}}>Notice sent {fDT(d.adverseAction.date)}</div>
                  <div style={{fontSize:10,color:"#999",marginTop:1}}>Method: {d.adverseAction.method} · Reason: {d.adverseAction.reason||"See disposition"}</div>
                </div>
              </div>
              <button style={{...S.tinyBtn,color:"#B0413E",marginTop:6,fontSize:10}} onClick={()=>{const up={...d,adverseAction:null,updatedAt:new Date().toISOString()};saveDeal(up);notify("Adverse action cleared")}}>Clear</button>
            </div>:<div>
              <div style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:8}}>
                {["Email","Mail","Phone","In-person"].map(m=>(
                  <button key={m} className="av-btn" style={{...S.prim,padding:"6px 12px",fontSize:10}} onClick={()=>{
                    const reason=d.disposition?.reason||((d.history||[]).slice().reverse().find(h=>h.s==="declined")?.n)||"";
                    const up={...d,adverseAction:{date:new Date().toISOString(),method:m,reason:reason,by:"admin"},updatedAt:new Date().toISOString()};
                    saveDeal(up);notify("Adverse action notice recorded — "+m);
                  }}><I.CheckCirc/> Sent via {m}</button>
                ))}
              </div>
              {(()=>{const decDate=(d.history||[]).slice().reverse().find(h=>h.s==="declined")?.d;if(!decDate)return null;const daysSince=Math.floor((Date.now()-new Date(decDate).getTime())/(1000*60*60*24));return daysSince>20?<div style={{fontSize:11,color:"#F44336",fontWeight:600}}>⚠ {daysSince} days since decline — notice required within 30 days!</div>:daysSince>14?<div style={{fontSize:11,color:"#E89300"}}>⚠ {daysSince} days since decline — deadline approaching</div>:<div style={{fontSize:11,color:"#999"}}>{30-daysSince} days remaining to send notice</div>})()}
            </div>}
          </div>
        </Sec>}

        {/* Audit Log */}
        <Sec t={<span style={{display:"flex",alignItems:"center",gap:6}}>Audit Log <span style={{fontSize:9,padding:"1px 6px",borderRadius:8,background:"#4F8EAE18",color:"#4F8EAE",border:"1px solid #4F8EAE33",fontWeight:600}}>{((d.history||[]).length+(d.activities||[]).length+(d.docs||[]).length)} events</span></span>}>
          <div style={{background:"#FFFFFF",border:"1px solid #E2E5E9",borderRadius:7,padding:"10px 12px",maxHeight:220,overflowY:"auto"}}>
            {(()=>{
              const events=[];
              (d.history||[]).forEach(h=>events.push({t:"stage",at:h.d,desc:`Stage → ${STAGES.find(s=>s.id===h.s)?.label||h.s}${h.n?" — "+h.n:""}`,icon:"🔄",color:"#6BA3BF"}));
              (d.activities||[]).forEach(a=>events.push({t:"activity",at:a.at,desc:a.detail||a.type.replace(/_/g," "),icon:a.type?.includes("call")?"📞":"📧",color:"#E89300"}));
              (d.docs||[]).forEach(doc=>events.push({t:"doc",at:doc.at,desc:`Document: ${doc.name} (${doc.by})`,icon:"📄",color:"#4CAF50"}));
              if(d.adverseAction)events.push({t:"compliance",at:d.adverseAction.date,desc:`Adverse action notice sent (${d.adverseAction.method})`,icon:"⚖️",color:"#B0413E"});
              if(d.disposition)events.push({t:"disposition",at:d.disposition.date,desc:`Disposition: ${d.disposition.reason}`,icon:"✕",color:"#B0413E"});
              if(d.accepted!==null){const accEntry=(d.history||[]).find(h=>h.s==="approved");if(accEntry)events.push({t:"compliance",at:accEntry.d,desc:`Customer ${d.accepted?"accepted":"has not accepted"} terms`,icon:"✓",color:d.accepted?"#4CAF50":"#666"})}
              events.sort((a,b)=>new Date(b.at)-new Date(a.at));
              return events.length?events.map((e,i)=>(
                <div key={i} style={{display:"flex",gap:8,padding:"5px 0",borderBottom:i<events.length-1?"1px solid #E2E5E9":"none"}}>
                  <span style={{fontSize:12,flexShrink:0,marginTop:1}}>{e.icon}</span>
                  <div style={{flex:1}}><div style={{fontSize:11,color:"#666"}}>{e.desc}</div><div style={{fontSize:9,color:"#999"}}>{fDT(e.at)}</div></div>
                </div>
              )):<div style={{padding:10,textAlign:"center",color:"#aaa",fontSize:11}}>No events recorded</div>
            })()}
          </div>
        </Sec>
        {d.terms&&["approved","docs_out","docs_signed","funded"].includes(d.stage)&&<Sec t="Approval Letter">
          <div style={{background:"#FFFFFF",border:"1px solid #E2E5E9",borderRadius:7,padding:"12px 14px"}}>
            {d.approvalLetter?<div>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                <div style={{display:"flex",alignItems:"center",gap:8}}>
                  <span style={{color:"#4CAF50"}}><I.CheckCirc/></span>
                  <div><div style={{fontSize:12,color:"#666",fontWeight:600}}>{d.approvalLetter.name}</div>
                  <div style={{fontSize:10,color:"#999",marginTop:1}}>Uploaded {fDT(d.approvalLetter.at)}</div></div>
                </div>
                <button style={{...S.tinyBtn,color:"#B0413E"}} onClick={()=>{const up={...d,approvalLetter:null,updatedAt:new Date().toISOString()};saveDeal(up);notify("Approval letter removed")}}><I.Trash/></button>
              </div>
            </div>:<div>
              <div style={{fontSize:11,color:"#999",marginBottom:8}}>Attach the lender approval letter for this deal.</div>
              <button className="av-ghost" style={{...S.ghost,width:"100%",justifyContent:"center",padding:"8px",fontSize:11,gap:6,border:"1px dashed #4F8EAE44",color:"#4F8EAE"}} onClick={()=>{
                const nm=prompt("Approval letter filename:","Approval_Letter_"+d.biz?.replace(/\s/g,"_")+".pdf");
                if(nm){const up={...d,approvalLetter:{name:nm,at:new Date().toISOString(),by:"admin"},updatedAt:new Date().toISOString()};saveDeal(up);notify("Approval letter attached")}
              }}><I.Upload/> Attach Approval Letter</button>
            </div>}
            {/* Generate Approval Letter PDF */}
            <button className="av-btn" style={{...S.prim,width:"100%",justifyContent:"center",padding:"8px 14px",fontSize:11,marginTop:8}} onClick={async()=>{
              try{
                const jsPDF=await new Promise((resolve,reject)=>{
                  if(window.jspdf)return resolve(window.jspdf.jsPDF);
                  const s=document.createElement("script");
                  s.src="https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.2/jspdf.umd.min.js";
                  s.onload=()=>window.jspdf?resolve(window.jspdf.jsPDF):reject(new Error("jsPDF not found"));
                  s.onerror=()=>reject(new Error("Failed to load jsPDF"));
                  document.head.appendChild(s);
                });
                const pdf=new jsPDF({unit:"pt",format:"letter"});
                const mx=50,pw=612;let y=60;
                const t=d.terms;const ln=lndr;const eqLine=[d.eqYear,d.eqMake,d.eqModel].filter(Boolean).join(" ")||d.eqDetail||d.eqType;

                // Header
                pdf.setTextColor(79,142,174);pdf.setFontSize(22);pdf.setFont("helvetica","bold");
                pdf.text("AVANTI EQUIPMENT FINANCE",mx,y);y+=18;
                pdf.setFontSize(9);pdf.setFont("helvetica","normal");pdf.setTextColor(120,120,120);
                pdf.text("Equipment Financing Solutions · (603) 459-9731 · Brent@AvantiEquipFi.com",mx,y);
                y+=8;pdf.setDrawColor(79,142,174);pdf.setLineWidth(2);pdf.line(mx,y,pw-mx,y);y+=30;

                // Date & address
                pdf.setTextColor(60,60,60);pdf.setFontSize(10);pdf.setFont("helvetica","normal");
                pdf.text(new Date().toLocaleDateString("en-US",{month:"long",day:"numeric",year:"numeric"}),mx,y);y+=22;
                pdf.setFont("helvetica","bold");pdf.setFontSize(11);pdf.setTextColor(30,30,30);
                pdf.text(d.contact||d.biz,mx,y);y+=14;
                pdf.setFont("helvetica","normal");pdf.setFontSize(10);pdf.setTextColor(80,80,80);
                pdf.text(d.biz,mx,y);y+=28;

                // Subject
                pdf.setFont("helvetica","bold");pdf.setFontSize(11);pdf.setTextColor(30,30,30);
                pdf.text("RE: Equipment Financing Approval",mx,y);y+=22;

                // Body
                pdf.setFont("helvetica","normal");pdf.setFontSize(10);pdf.setTextColor(60,60,60);
                const bodyLines=[
                  `Dear ${d.contact||"Valued Customer"},`,
                  "",
                  `Congratulations! We are pleased to inform you that your application for equipment financing has been approved.`,
                  "",
                  "Below are your approved financing terms:",
                ];
                bodyLines.forEach(l=>{pdf.text(l,mx,y);y+=14});y+=6;

                // Terms box
                pdf.setFillColor(245,247,250);pdf.roundedRect(mx,y,pw-mx*2,t.months?130:100,6,6,"F");
                pdf.setDrawColor(79,142,174);pdf.setLineWidth(0.5);pdf.roundedRect(mx,y,pw-mx*2,t.months?130:100,6,6,"S");
                y+=18;const bx=mx+20;
                const tRow=(label,val)=>{pdf.setFont("helvetica","normal");pdf.setFontSize(9);pdf.setTextColor(120,120,120);pdf.text(label,bx,y);pdf.setFont("helvetica","bold");pdf.setFontSize(11);pdf.setTextColor(30,30,30);pdf.text(String(val),bx+160,y);y+=18};
                tRow("Equipment:",eqLine);
                tRow("Financed Amount:","$"+Number(t.financedAmt||t.amt).toLocaleString());
                tRow("Monthly Payment:","$"+Number(t.pmt).toLocaleString(undefined,{minimumFractionDigits:2}));
                tRow("Term:",t.months+" months");
                tRow("Rate:",t.rate);
                if(ln)tRow("Lender:",ln.name);
                y+=16;

                // Next steps
                pdf.setFont("helvetica","bold");pdf.setFontSize(10);pdf.setTextColor(30,30,30);
                pdf.text("Next Steps:",mx,y);y+=16;
                pdf.setFont("helvetica","normal");pdf.setFontSize(10);pdf.setTextColor(60,60,60);
                ["1. Review the terms above carefully.","2. We will send the financing documents for your signature.","3. Once signed, funding typically completes within 24-48 hours.","4. Contact us with any questions — we're here to help!"].forEach(l=>{pdf.text(l,mx+10,y);y+=15});
                y+=20;

                // Closing
                pdf.text("Thank you for choosing Avanti Equipment Finance. We look forward to helping",mx,y);y+=14;
                pdf.text("you grow your business.",mx,y);y+=30;
                pdf.text("Best regards,",mx,y);y+=22;
                pdf.setFont("helvetica","bold");pdf.setFontSize(11);pdf.setTextColor(79,142,174);
                pdf.text("Brent Christiansen",mx,y);y+=14;
                pdf.setFont("helvetica","normal");pdf.setFontSize(9);pdf.setTextColor(100,100,100);
                pdf.text("Senior Account Manager · Avanti Equipment Finance",mx,y);

                const fn=`Approval_${(d.biz||"Deal").replace(/[^a-zA-Z0-9]/g,"_")}_${new Date().toISOString().slice(0,10)}.pdf`;
                const blob=pdf.output("blob");
                const url=URL.createObjectURL(blob);
                const a=document.createElement("a");a.href=url;a.download=fn;a.style.display="none";
                document.body.appendChild(a);a.click();
                setTimeout(()=>{document.body.removeChild(a);URL.revokeObjectURL(url)},100);
                window.open(url,"_blank");
                notify("Approval letter PDF generated");
              }catch(err){console.error(err);notify("PDF generation failed: "+err.message,"error")}
            }}><I.Doc/> Generate Approval Letter PDF</button>
          </div>
        </Sec>}
        <Sec t={`Documents (${(d.docs||[]).length})`}><div style={{display:"flex",flexDirection:"column",gap:3}}>{REQ_DOCS.map(doc=>{const up=(d.docs||[]).find(x=>x.docId===doc.id);const isReq=d.docReqs?.[doc.id]!==undefined?d.docReqs[doc.id]:doc.req;return(
          <div key={doc.id} style={S.docRow}><div style={{display:"flex",alignItems:"center",gap:7,flex:1}}>{up?<span style={{color:"#4CAF50"}}><I.CheckCirc/></span>:<div style={{width:14,height:14,borderRadius:"50%",border:`1.5px solid ${isReq?"#E89300":"#333"}`}}/>}<span style={{fontSize:11,color:up?"#ccc":"#555"}}>{doc.label}</span>
              <button onClick={()=>toggleDocReq(d.id,doc.id)} style={{fontSize:8,padding:"1px 5px",borderRadius:8,background:isReq?"#E8930018":"transparent",color:isReq?"#E89300":"#333",border:`1px solid ${isReq?"#E8930033":"#CDD1D6"}`,cursor:"pointer",fontFamily:"inherit",fontWeight:600,transition:"all .15s"}}>{isReq?"Req":"Opt"}</button></div>
            {up?<div style={{display:"flex",alignItems:"center",gap:5,fontSize:10,color:"#999"}}><span>{up.name}</span><button style={S.tinyBtn} onClick={()=>rmDoc(d.id,doc.id)}><I.Trash/></button></div>
            :<label style={{cursor:"pointer",display:"flex",alignItems:"center",gap:3,fontSize:10,color:"#4F8EAE",fontWeight:600}}><I.Upload/> Upload<input type="file" style={{display:"none"}} onChange={e=>{const f=e.target?.files?.[0];if(f)addDoc(d.id,{docId:doc.id,name:f.name,at:new Date().toISOString(),by:"admin"});e.target.value=""}}/></label>}
          </div>)})}</div>
          {/* General file upload area */}
          <div style={{marginTop:8,padding:"10px",border:"2px dashed #CDD1D6",borderRadius:6,textAlign:"center"}}
            onDragOver={e=>{e.preventDefault();e.currentTarget.style.borderColor="#4F8EAE";e.currentTarget.style.background="#4F8EAE08"}}
            onDragLeave={e=>{e.preventDefault();e.currentTarget.style.borderColor="#CDD1D6";e.currentTarget.style.background="transparent"}}
            onDrop={e=>{e.preventDefault();e.currentTarget.style.borderColor="#CDD1D6";e.currentTarget.style.background="transparent";Array.from(e.dataTransfer?.files||[]).forEach(f=>addDoc(d.id,{docId:"upload_"+Date.now()+"_"+Math.random().toString(36).slice(2,6),name:f.name,at:new Date().toISOString(),by:"admin"}))}}>
            <div style={{fontSize:10,color:"#999"}}>📂 Drag & drop files here or <label style={{color:"#4F8EAE",cursor:"pointer",fontWeight:600}}>browse<input type="file" multiple style={{display:"none"}} onChange={e=>{Array.from(e.target?.files||[]).forEach(f=>addDoc(d.id,{docId:"upload_"+Date.now()+"_"+Math.random().toString(36).slice(2,6),name:f.name,at:new Date().toISOString(),by:"admin"}));e.target.value=""}}/></label></div>
          </div>
          {d.accountId&&(d.docs||[]).length>0&&<button className="av-ghost" style={{...S.ghost,marginTop:8,fontSize:10,width:"100%",justifyContent:"center",padding:"6px"}} onClick={()=>{
            const acc=accounts.find(a=>a.id===d.accountId);
            if(!acc)return;
            const existing=(acc.vault||[]).map(v=>v.name);
            const newFiles=(d.docs||[]).filter(doc=>!existing.includes(doc.name)).map(doc=>({id:gId(),name:doc.name,at:new Date().toISOString(),by:"admin",fromDeal:d.id}));
            if(newFiles.length){saveAccount({...acc,vault:[...(acc.vault||[]),...newFiles]});notify(`${newFiles.length} doc(s) saved to account vault`)}else{notify("All docs already in vault","info")}
          }}><I.Copy/> Save All Docs to Account Vault</button>}
          {/* Email to lender for credit approval */}
          {(d.docs||[]).length>0&&(()=>{
            const docs=(d.docs||[]);
            const KEY_DOCS=[
              {id:"credit_app",label:"Credit Application",icon:"📋"},
              {id:"sos",label:"Secretary of State / Articles",icon:"🏛️"},
              {id:"bank_statements",label:"Bank Statements",icon:"🏦"},
              {id:"equipment_quote",label:"Equipment Quote / Invoice",icon:"📄"},
              {id:"tax_returns",label:"Tax Returns",icon:"📑"},
              {id:"drivers_license",label:"Driver's License",icon:"🪪"},
              {id:"pnl",label:"Profit & Loss Statement",icon:"📊"},
            ];
            const attached=docs.map(x=>x.docId);
            const ready=KEY_DOCS.filter(k=>attached.includes(k.id));
            const missing=KEY_DOCS.filter(k=>!attached.includes(k.id));
            const extras=docs.filter(x=>!KEY_DOCS.find(k=>k.id===x.docId));
            const allDocs=[...ready.map(k=>{const dc=docs.find(x=>x.docId===k.id);return{name:dc?.name||k.label,label:k.label}}), ...extras.map(x=>({name:x.name,label:x.name}))];
            const eqLine=[d.eqYear,d.eqMake,d.eqModel].filter(Boolean).join(" ")||d.eqDetail||d.eqType||"N/A";
            const appD=d.appData||{};
            const openMailto=(toEmail)=>{
              const docLines=ready.map(k=>{const dc=docs.find(x=>x.docId===k.id);return`  • ${k.label}: ${dc?.name||"attached"}`});
              const extraLines=extras.map(x=>`  • ${x.name}`);
              const docList=[...docLines,...(extraLines.length?["  Additional:",...extraLines]:[])] .join("\n");
              const lienInfo=lndr?[
                lndr.lienholder?`Lienholder: ${lndr.lienholder}`:"",
                lndr.address?`Address: ${lndr.address}`:"",
              ].filter(Boolean).join("\n"):"";
              const bizInfo=[
                `Business: ${d.biz||"N/A"}`,
                appD.dba?`DBA: ${appD.dba}`:"",
                appD.legalStructure?`Entity: ${appD.legalStructure}`:"",
                appD.dateStarted?`Date Started: ${appD.dateStarted}`:"",
                appD.fedId?`EIN: ${appD.fedId}`:"",
                `Contact: ${d.contact||"N/A"}`,
                d.phone?`Phone: ${d.phone}`:"",
                d.email?`Email: ${d.email}`:"",
              ].filter(Boolean).join("\n");
              const ownerInfo=[
                appD.o1Name?`Owner 1: ${appD.o1Name}${appD.o1Title?" — "+appD.o1Title:""}${appD.o1Pct?" ("+appD.o1Pct+"%)":""}`:"",
                appD.o2Name?`Owner 2: ${appD.o2Name}${appD.o2Title?" — "+appD.o2Title:""}${appD.o2Pct?" ("+appD.o2Pct+"%)":""}`:"",
              ].filter(Boolean).join("\n");
              const eqInfo=[
                `Equipment: ${eqLine}`,
                d.eqVin?`VIN/Serial: ${d.eqVin}`:"",
                d.eqMileage?`Mileage/Hours: ${d.eqMileage}`:"",
                `Amount Requested: ${fmt(d.amount)}`,
                d.dealerName?`Dealer: ${d.dealerName}`:"",
              ].filter(Boolean).join("\n");
              const body=[
                "Good morning,",
                "",
                "Please find attached a new credit application for review.",
                "",
                ...(lienInfo?["--- LIENHOLDER / ADDRESS ---",lienInfo,""]:[]),
                "--- DEAL SPECIFICS ---",
                "",
                "Business: " + (d.biz||"N/A"),
                bizInfo.split("\n").slice(1).join("\n"),
                ownerInfo?"":"",
                ownerInfo?"Ownership:":"",
                ownerInfo,
                "",
                "Equipment: " + eqLine,
                eqInfo.split("\n").slice(1).join("\n"),
                "",
                "--- ATTACHED FILES ---",
                docList,
                "",
                "Please let me know if you need any additional information.",
                "",
                "Best regards,",
                AM.name,
                AM.title,
                "Avanti Equipment Finance",
                AM.phone,
                AM.email,
              ].filter(v=>v!==false).join("\n");
              const subject=`Credit Application — ${d.biz||"New Application"} — ${eqLine}`;
              window.open(`mailto:${toEmail||""}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`,"_self");
            };
            return(
            <div style={{marginTop:10,background:"#0A0A0C",border:"1px solid #D8DCE0",borderRadius:8,padding:12}}>
              <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:8}}>
                <span style={{color:"#4F8EAE"}}><I.Send/></span>
                <span style={{fontSize:12,fontWeight:700,color:"#1a1a2e"}}>Submit to Lender for Credit Approval</span>
              </div>
              {/* Doc readiness checklist */}
              <div style={{marginBottom:10}}>
                <div style={{fontSize:9,fontWeight:600,color:"#999",textTransform:"uppercase",letterSpacing:.8,marginBottom:5}}>Document Checklist</div>
                {KEY_DOCS.map(k=>{
                  const have=attached.includes(k.id);
                  const docObj=docs.find(x=>x.docId===k.id);
                  return(
                  <div key={k.id} style={{display:"flex",alignItems:"center",gap:6,padding:"3px 0"}}>
                    {have
                      ?<span style={{color:"#4CAF50",fontSize:13}}>✓</span>
                      :<span style={{color:"#666",fontSize:13}}>○</span>}
                    <span style={{fontSize:11,color:have?"#ccc":"#444",flex:1}}>{k.icon} {k.label}</span>
                    {have&&<span style={{fontSize:9,color:"#999"}}>{docObj?.name}</span>}
                    {!have&&<span style={{fontSize:9,color:"#E89300",fontStyle:"italic"}}>not attached</span>}
                  </div>);
                })}
                {extras.length>0&&<>
                  <div style={{fontSize:9,fontWeight:600,color:"#999",textTransform:"uppercase",letterSpacing:.8,marginTop:6,marginBottom:3}}>Additional Documents</div>
                  {extras.map((x,i)=>(
                    <div key={i} style={{display:"flex",alignItems:"center",gap:6,padding:"2px 0"}}>
                      <span style={{color:"#4CAF50",fontSize:13}}>✓</span>
                      <span style={{fontSize:11,color:"#666"}}>{x.name}</span>
                    </div>
                  ))}
                </>}
              </div>
              <div style={{fontSize:9,color:"#aaa",marginBottom:8,fontStyle:"italic"}}>
                {ready.length} of {KEY_DOCS.length} key documents attached · {allDocs.length} total documents · Outlook will open with a pre-written email — drag your files into the email before sending
              </div>
              {/* Lender quick-send buttons */}
              {lndr&&<button className="av-btn" style={{...S.prim,width:"100%",justifyContent:"center",padding:"10px 14px",fontSize:12,marginBottom:6}} onClick={()=>openMailto(lndr.email||"")}>
                <I.Send/> Email {lndr.name}{lndr.email?" ("+lndr.email+")":""}
              </button>}
              {!lndr&&<button className="av-btn" style={{...S.prim,width:"100%",justifyContent:"center",padding:"10px 14px",fontSize:12,marginBottom:6}} onClick={()=>openMailto("")}>
                <I.Send/> Open Email for Approval
              </button>}
              {/* Send to multiple lenders */}
              {lenders.length>1&&<div style={{display:"flex",gap:4,flexWrap:"wrap",alignItems:"center"}}>
                <span style={{fontSize:9,color:"#999",display:"flex",alignItems:"center"}}>Send to:</span>
                {lenders.filter(x=>!lndr||x.id!==lndr.id).map(x=>(
                  <button key={x.id} className="av-ghost" style={{...S.ghost,padding:"3px 8px",fontSize:9}} onClick={()=>openMailto(x.email||"")}>{x.name}</button>
                ))}
              </div>}
            </div>);
          })()}
        </Sec>
        {["approved","docs_out","docs_signed","funded"].includes(d.stage)&&<Sec t={<span style={{display:"flex",alignItems:"center",gap:6}}>Dealer Required Docs{(()=>{const p=DEALER_REQ_DOCS.filter(item=>!(d.docs||[]).find(x=>x.docId===item.docId)).length;return p>0?<span style={{fontSize:9,padding:"1px 6px",borderRadius:8,background:"#E8930018",color:"#E89300",border:"1px solid #E8930033",fontWeight:700}}>{p} needed</span>:<span style={{fontSize:9,padding:"1px 6px",borderRadius:8,background:"#4CAF5012",color:"#4CAF50",fontWeight:600}}>All received</span>})()}</span>}>
          {DEALER_REQ_DOCS.map(item=>{
            const up=(d.docs||[]).find(x=>x.docId===item.docId);
            return(<div key={item.docId} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 10px",background:up?"#4CAF5008":"#E8930006",border:`1px solid ${up?"#4CAF5018":"#E8930018"}`,borderRadius:5,marginBottom:4}}>
              <div style={{display:"flex",alignItems:"center",gap:7}}>
                {up?<span style={{color:"#4CAF50"}}><I.CheckCirc/></span>:<span style={{fontSize:14}}>{item.icon}</span>}
                <div>
                  <div style={{fontSize:11,color:up?"#666":"#ddd",fontWeight:600,textDecoration:up?"line-through":"none"}}>{item.label}</div>
                  {up&&<div style={{fontSize:9,color:"#4CAF50",marginTop:1}}>{up.name} · by {up.by} · {fDT(up.at)}</div>}
                </div>
              </div>
              <div style={{display:"flex",gap:4}}>
                {!up&&<label className="av-ghost" style={{...S.ghost,padding:"3px 8px",fontSize:9,cursor:"pointer",display:"flex",alignItems:"center",gap:3}}><I.Upload/> Upload<input type="file" style={{display:"none"}} onChange={e=>{const f=e.target?.files?.[0];if(f)addDoc(d.id,{docId:item.docId,name:f.name,at:new Date().toISOString(),by:"admin"});e.target.value=""}}/></label>}
                {up&&<button style={S.tinyBtn} onClick={()=>rmDoc(d.id,item.docId)}><I.Trash/></button>}
              </div>
            </div>);
          })}
        </Sec>}
        {["approved","docs_out","docs_signed","funded"].includes(d.stage)&&<Sec t={<span style={{display:"flex",alignItems:"center",gap:6}}>Customer Required Docs{(()=>{const items=[{docId:"drivers_license"},{docId:"voided_check"}];const p=items.filter(item=>!(d.docs||[]).find(x=>x.docId===item.docId)).length;return p>0?<span style={{fontSize:9,padding:"1px 6px",borderRadius:8,background:"#E8930018",color:"#E89300",border:"1px solid #E8930033",fontWeight:700}}>{p} needed</span>:<span style={{fontSize:9,padding:"1px 6px",borderRadius:8,background:"#4CAF5012",color:"#4CAF50",fontWeight:600}}>All received</span>})()}</span>}>
          {[{docId:"drivers_license",label:"Driver's License (Front & Back)",icon:"🪪"},{docId:"voided_check",label:"Voided Check",icon:"🏦"}].map(item=>{
            const up=(d.docs||[]).find(x=>x.docId===item.docId);
            return(<div key={item.docId} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 10px",background:up?"#4CAF5008":"#E8930006",border:`1px solid ${up?"#4CAF5018":"#E8930018"}`,borderRadius:5,marginBottom:4}}>
              <div style={{display:"flex",alignItems:"center",gap:7}}>
                {up?<span style={{color:"#4CAF50"}}><I.CheckCirc/></span>:<span style={{fontSize:14}}>{item.icon}</span>}
                <div>
                  <div style={{fontSize:11,color:up?"#666":"#ddd",fontWeight:600,textDecoration:up?"line-through":"none"}}>{item.label}</div>
                  {up&&<div style={{fontSize:9,color:"#4CAF50",marginTop:1}}>{up.name} · by {up.by} · {fDT(up.at)}</div>}
                </div>
              </div>
              <div style={{display:"flex",gap:4}}>
                {!up&&<label className="av-ghost" style={{...S.ghost,padding:"3px 8px",fontSize:9,cursor:"pointer",display:"flex",alignItems:"center",gap:3}}><I.Upload/> Upload<input type="file" style={{display:"none"}} onChange={e=>{const f=e.target?.files?.[0];if(f)addDoc(d.id,{docId:item.docId,name:f.name,at:new Date().toISOString(),by:"admin"});e.target.value=""}}/></label>}
                {up&&<button style={S.tinyBtn} onClick={()=>rmDoc(d.id,item.docId)}><I.Trash/></button>}
              </div>
            </div>);
          })}
        </Sec>}
        <ItemsNeededSection deal={d}/>
        <FundingItemsSection deal={d}/>
        <Sec t="Notes">{d.notes&&<div style={S.noteBox}>{d.notes}</div>}{notes.map(n=><div key={n.id} style={S.noteItem}><div style={{fontSize:12,color:"#666"}}>{n.txt}</div><div style={{fontSize:10,color:"#999",marginTop:2}}>{n.who} · {fDT(n.at)}</div></div>)}
          <div style={{display:"flex",gap:6,marginTop:6}}><input style={{...S.inp,flex:1,fontSize:12}} placeholder="Add a note..." value={nn} onChange={e=>setNn(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&nn.trim()){addNote(d.id,nn);setNn("")}}}/><button className="av-btn" style={{...S.prim,padding:"6px 10px",fontSize:11}} onClick={()=>{if(nn.trim()){addNote(d.id,nn);setNn("")}}}><I.Send/></button></div>
        </Sec>
        <ActivityLog deal={d}/>
        <Sec t="History">{(d.history||[]).slice().reverse().map((h,i)=>{const hs=STAGES.find(s=>s.id===h.s);return(<div key={i} style={{padding:"5px 0 5px 12px",borderLeft:`2px solid ${hs?.color||"#333"}`,marginLeft:4}}><div style={{fontSize:11,fontWeight:600,color:hs?.color}}>{hs?.label}</div>{h.n&&<div style={{fontSize:10,color:"#999"}}>{h.n}</div>}<div style={{fontSize:9,color:"#aaa",marginTop:1}}>{fDT(h.d)}</div></div>)})}</Sec>
        <Sec t="Customer Portal">
          {/* Tax financing option toggle */}
          {d.terms&&<div style={{marginBottom:10,padding:10,background:"#FFFFFF",borderRadius:6,border:"1px solid #D8DCE0"}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
              <div><div style={{fontSize:11,fontWeight:600,color:"#666"}}>Allow Customer to Choose Tax Financing</div><div style={{fontSize:9,color:"#999",marginTop:2}}>When enabled, customer portal shows option to finance sales tax or pay separately</div></div>
              <button onClick={()=>{const upd=deals.map(x=>x.id===d.id?{...x,showTaxChoice:!x.showTaxChoice,updatedAt:new Date().toISOString()}:x);setDeals(upd);notify(d.showTaxChoice?"Tax choice hidden from customer":"Tax choice visible to customer")}} style={{width:40,height:22,borderRadius:11,background:d.showTaxChoice?"#4F8EAE":"#CDD1D6",border:"none",cursor:"pointer",position:"relative",transition:"background .2s",flexShrink:0}}><div style={{width:16,height:16,borderRadius:8,background:"#fff",position:"absolute",top:3,left:d.showTaxChoice?21:3,transition:"left .2s"}}/></button>
            </div>
          </div>}
          {/* Auto confirmation email status */}
          <div style={{marginBottom:10}}>
            <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}>
              {d.emailConfSent?<span style={{color:"#4CAF50"}}><I.CheckCirc/></span>:<span style={{color:"#E89300"}}><I.Mail/></span>}
              <span style={{fontSize:11,color:d.emailConfSent?"#888":"#E89300",fontWeight:600}}>Confirmation Email</span>
            </div>
            {d.emailConfSent
              ?<div style={{fontSize:10,color:"#999"}}>Sent to {d.email} · {fDT(d.emailConfSent)}</div>
              :<button className="av-btn" style={{...S.prim,padding:"6px 12px",fontSize:11,background:"#E8930022",border:"1px solid #E8930033",color:"#E89300"}} onClick={()=>{sendConfEmail(d.id);notify("Confirmation email sent to "+d.email)}}>
                <I.Send/> Send Confirmation Email
              </button>
            }
            <div style={{fontSize:9,color:"#aaa",marginTop:3,fontStyle:"italic"}}>"We received your application" — no access code included</div>
          </div>
          {/* Send portal credentials */}
          <div style={{padding:10,background:"#FFFFFF",borderRadius:6,border:"1px solid #D8DCE0"}}>
            <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:6}}>
              {d.emailCodeSent?<span style={{color:"#4CAF50"}}><I.CheckCirc/></span>:<span style={{color:"#4F8EAE"}}><I.Eye/></span>}
              <span style={{fontSize:11,color:d.emailCodeSent?"#888":"#fff",fontWeight:600}}>Portal Login Credentials</span>
            </div>
            <div style={{padding:8,background:"#111",borderRadius:5,border:"1px solid #D8DCE0",marginBottom:8}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                <span style={{fontSize:9,color:"#999",textTransform:"uppercase",letterSpacing:.8}}>Email</span>
                <span style={{fontSize:11,color:"#666",fontFamily:"'JetBrains Mono',monospace"}}>{d.email}</span>
              </div>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <span style={{fontSize:9,color:"#999",textTransform:"uppercase",letterSpacing:.8}}>Password</span>
                <div style={{display:"flex",alignItems:"center",gap:6}}>
                  <span style={{fontSize:11,color:"#4F8EAE",fontFamily:"'JetBrains Mono',monospace",letterSpacing:1}}>{d.custPassword||"—"}</span>
                  <button className="av-ghost" style={{...S.ghost,padding:"3px 8px",fontSize:9}} onClick={()=>{navigator.clipboard.writeText(d.custPassword||"");setCp(true);setTimeout(()=>setCp(false),2000)}}>{cp?"✓":"Copy"}</button>
                </div>
              </div>
            </div>
            {d.emailCodeSent
              ?<div style={{fontSize:10,color:"#999"}}>Credentials sent to {d.email} · {fDT(d.emailCodeSent)}</div>
              :<button className="av-btn" style={{...S.prim,padding:"8px 14px",fontSize:12,width:"100%",justifyContent:"center"}} onClick={()=>{sendCodeEmail(d.id);notify("Login credentials sent to "+d.email)}}>
                <I.Send/> Send Login to {d.contact||"Customer"}
              </button>
            }
            <div style={{fontSize:9,color:"#aaa",marginTop:3,fontStyle:"italic"}}>Sends portal link + login credentials so they can track their status</div>
          </div>
        </Sec>
      </div>
      <div style={S.drFoot}>
        <button className="av-btn" style={{...S.prim,flex:1,justifyContent:"center"}} onClick={()=>onEdit(d)}><I.Edit/> Edit</button>
        {d.stage==="dead"||d.stage==="declined"?<button className="av-btn" style={S.reactBtn} onClick={()=>onMove(d.id,"new_lead","Reactivated")}><I.Refresh/> Reactivate</button>
        :<button className="av-ghost" style={{...S.ghost,color:"#999"}} onClick={()=>{setCloseModal({stage:"dead"});setCloseReason("")}}><I.Archive/> Dead</button>}
        <button className="av-btn" style={S.danger} onClick={()=>{if(confirm("Permanently delete?"))onDel(d.id)}}><I.Trash/></button>
      </div>
      {/* Close-out reason modal */}
      {closeModal&&<div style={{position:"absolute",inset:0,background:"#00000066",display:"flex",alignItems:"center",justifyContent:"center",zIndex:10,borderRadius:12}} onClick={()=>setCloseModal(null)}>
        <div style={{background:"#FFFFFF",border:"1px solid #CDD1D6",borderRadius:10,padding:20,width:340,maxWidth:"90%"}} onClick={e=>e.stopPropagation()}>
          <div style={{fontSize:15,fontWeight:700,color:closeModal.stage==="declined"?"#B0413E":"#666",marginBottom:4}}>
            {closeModal.stage==="declined"?"Decline Deal":"Mark as Dead"}
          </div>
          <div style={{fontSize:11,color:"#999",marginBottom:12}}>
            {closeModal.stage==="declined"?"Why was this deal declined?":"Why is this deal being closed out?"}
          </div>
          <div style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:10}}>
            {(closeModal.stage==="declined"
              ?["Credit too weak","Insufficient time in business","Insufficient revenue","Lender declined — no reason given","Customer withdrew"]
              :["Customer unresponsive","Customer went elsewhere","Deal fell through","Bad contact info","Customer not ready"]
            ).map(r=>(
              <button key={r} onClick={()=>setCloseReason(r)} style={{
                padding:"4px 8px",fontSize:10,borderRadius:5,cursor:"pointer",fontFamily:"inherit",
                border:`1px solid ${closeReason===r?"#B0413E":"#CDD1D6"}`,
                background:closeReason===r?"#B0413E18":"transparent",
                color:closeReason===r?"#B0413E":"#777",transition:"all .15s"
              }}>{r}</button>
            ))}
          </div>
          <textarea style={{...S.inp,resize:"vertical",minHeight:50,fontSize:12,width:"100%",marginBottom:12}} placeholder="Additional details (optional)..." value={closeReason} onChange={e=>setCloseReason(e.target.value)}/>
          <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
            <button className="av-ghost" style={S.ghost} onClick={()=>setCloseModal(null)}>Cancel</button>
            <button className="av-btn" style={{...S.danger,padding:"8px 16px",fontSize:12}} onClick={()=>{
              if(!closeReason.trim()){notify("Please add a reason","error");return}
              onMove(d.id,closeModal.stage,closeReason.trim());
              setCloseModal(null);setCloseReason("");
            }}>{closeModal.stage==="declined"?"Decline":"Mark Dead"}</button>
          </div>
        </div>
      </div>}
    </div></div>);
}

// ── Accounts Management ───────────────────────────────────────

function AccountsMgmt({onSel}) {
  const {accounts,deals,saveAccount} = useContext(Ctx);
  const [selAcct,setSelAcct]=useState(null);
  const [form,setForm]=useState(false);
  const [ed,setEd]=useState(null);
  const [q,setQ]=useState("");

  const filtered=useMemo(()=>{
    if(!q)return accounts;
    const lq=q.toLowerCase();
    return accounts.filter(a=>a.biz?.toLowerCase().includes(lq)||a.contact?.toLowerCase().includes(lq)||a.email?.toLowerCase().includes(lq)||a.industry?.toLowerCase().includes(lq)||(a.phone&&lq.replace(/\D/g,"").length>=3&&a.phone.replace(/\D/g,"").includes(lq.replace(/\D/g,""))));
  },[accounts,q]);

  if(selAcct) return <AccountDetail accId={selAcct} onBack={()=>setSelAcct(null)} onSel={onSel}/>;

  return(<div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
      <div style={{fontSize:15,fontWeight:700,color:"#1a1a2e"}}>Customer Accounts ({accounts.length})</div>
      <button className="av-btn" style={S.prim} onClick={()=>{setEd(null);setForm(true)}}><I.Plus/> New Account</button>
    </div>
    <div style={{marginBottom:12}}><div style={S.srchW}><I.Search/><input type="text" placeholder="Search accounts..." value={q} onChange={e=>setQ(e.target.value)} style={S.srchIn}/>{q&&<button style={S.tinyBtn} onClick={()=>setQ("")}><I.X/></button>}</div></div>
    <div style={{display:"flex",flexDirection:"column",gap:6}}>
      {filtered.map(acc=>{
        const acctDeals=deals.filter(d=>d.accountId===acc.id);
        const active=acctDeals.filter(d=>!["funded","declined","dead"].includes(d.stage)).length;
        const funded=acctDeals.filter(d=>d.stage==="funded");
        const dead=acctDeals.filter(d=>d.stage==="dead"||d.stage==="declined").length;
        const totalFunded=funded.reduce((s,d)=>s+(d.terms?.amt||d.amount||0),0);
        const totalFee=funded.reduce((s,d)=>s+(d.terms?.feeAmt||0),0);
        const lastActivity=[...(acc.activities||[]),...acctDeals.flatMap(d=>d.activities||[])].sort((a,b)=>new Date(b.at)-new Date(a.at))[0];

        return(<div key={acc.id} data-acct-id={acc.id} className="av-card" style={{...S.dlrCard,cursor:"pointer"}} onClick={()=>setSelAcct(acc.id)}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
            <div style={{flex:1}}>
              <div style={{fontSize:14,fontWeight:700,color:"#1a1a2e"}}>{acc.biz}</div>
              <div style={{fontSize:11,color:"#888",marginTop:2}}>{acc.contact}{acc.industry?` · ${acc.industry}`:""}</div>
              <div style={{fontSize:10,color:"#999",marginTop:1,display:"flex",gap:8,flexWrap:"wrap"}}>{acc.email&&<a href={`mailto:${acc.email}`} style={{color:"#6BA3BF",textDecoration:"none",fontSize:10}}>{acc.email}</a>}{acc.phone&&<a href={`tel:${acc.phone.replace(/\D/g,"")}`} style={{color:"#6BA3BF",textDecoration:"none",fontSize:10}}>{acc.phone}</a>}</div>
            </div>
            <button style={S.tinyBtn} onClick={e=>{e.stopPropagation();setEd(acc);setForm(true)}}><I.Edit/></button>
          </div>
          <div style={{display:"flex",gap:12,marginTop:8,fontSize:11}}>
            <span style={{color:"#4F8EAE"}}><strong>{acctDeals.length}</strong> {acctDeals.length===1?"deal":"deals"}</span>
            {active>0&&<span style={{color:"#E89300"}}><strong>{active}</strong> active</span>}
            {funded.length>0&&<span style={{color:"#4CAF50"}}><strong>{fmt(totalFunded)}</strong> funded</span>}
            {totalFee>0&&<span style={{color:"#4CAF50"}}>${totalFee.toLocaleString()} fees</span>}
            {dead>0&&<span style={{color:"#999"}}>{dead} dead</span>}
          </div>
          {lastActivity&&<div style={{marginTop:6,fontSize:10,color:"#999"}}>
            Last activity: {ACT_TYPES.find(t=>t.id===lastActivity.type)?.label||"Other"} — {fDT(lastActivity.at)}
          </div>}
        </div>);
      })}
    </div>
    {form&&<AccountForm acc={ed} onSave={a=>{saveAccount(a);setForm(false);setEd(null)}} onClose={()=>{setForm(false);setEd(null)}}/>}
  </div>);
}

function AccountDetail({accId,onBack,onSel}) {
  const {accounts,deals,lenders,addAcctActivity,saveAccount} = useContext(Ctx);
  const acc=accounts.find(a=>a.id===accId);
  const [actDetail,setActDetail]=useState("");
  const [actType,setActType]=useState(null);
  const [tab,setTab]=useState("overview");
  const [vaultName,setVaultName]=useState("");

  if(!acc)return <div style={{color:"#B0413E"}}>Account not found. <button className="av-ghost" style={S.ghost} onClick={onBack}><I.Back/> Back</button></div>;

  const acctDeals=deals.filter(d=>d.accountId===acc.id).sort((a,b)=>new Date(b.createdAt)-new Date(a.createdAt));
  const active=acctDeals.filter(d=>!["funded","declined","dead"].includes(d.stage));
  const funded=acctDeals.filter(d=>d.stage==="funded");
  const dead=acctDeals.filter(d=>d.stage==="dead"||d.stage==="declined");
  const totalFunded=funded.reduce((s,d)=>s+(d.terms?.amt||d.amount||0),0);
  const totalFee=funded.reduce((s,d)=>s+(d.terms?.feeAmt||0),0);
  const winRate=acctDeals.length>0?Math.round((funded.length/(funded.length+dead.length||1))*100):0;

  // Merge all activities
  const allActivities=[
    ...(acc.activities||[]).map(a=>({...a,source:"Account"})),
    ...acctDeals.flatMap(d=>(d.activities||[]).map(a=>({...a,source:d.eqType?`${d.eqType} — ${fmt(d.amount)}`:(d.biz||"Deal")})))
  ].sort((a,b)=>new Date(b.at)-new Date(a.at));

  // Document vault: account-level files + all deal docs
  const vaultFiles=(acc.vault||[]);
  const dealDocs=acctDeals.flatMap(d=>(d.docs||[]).map(doc=>({...doc,dealId:d.id,dealLabel:d.eqType?`${d.eqType} — ${fmt(d.amount)}`:"Deal"})));
  const allDocs=[...vaultFiles.map(f=>({...f,source:"Account Vault"})),...dealDocs.map(f=>({...f,source:f.dealLabel}))].sort((a,b)=>new Date(b.at)-new Date(a.at));

  const addToVault=(name,fromDeal)=>{
    const file={id:gId(),name,at:new Date().toISOString(),by:"admin",fromDeal:fromDeal||null};
    saveAccount({...acc,vault:[...(acc.vault||[]),file]});
  };
  const rmFromVault=(fileId)=>{
    saveAccount({...acc,vault:(acc.vault||[]).filter(f=>f.id!==fileId)});
  };

  const doLogAcct=(typeId)=>{
    addAcctActivity(acc.id,typeId,ACT_TYPES.find(t=>t.id===typeId)?.who||"other",actDetail.trim());
    setActDetail("");setActType(null);
  };

  const stgColor=(stage)=>STAGES.find(s=>s.id===stage)?.color||"#666";

  return(<div>
    <button className="av-ghost" style={{...S.ghost,marginBottom:12}} onClick={onBack}><I.Back/> All Accounts</button>

    {/* Account Header */}
    <div style={{padding:18,background:"#111113",borderRadius:10,border:"1px solid #D8DCE0",marginBottom:14}}>
      <div style={{fontSize:20,fontWeight:800,color:"#1a1a2e"}}>{acc.biz}</div>
      <div style={{fontSize:12,color:"#999",marginTop:4}}>{acc.contact}{acc.industry?` · ${acc.industry}`:""}{acc.dateStarted?` · Est. ${acc.dateStarted}`:""}</div>
      <div style={{display:"flex",gap:16,marginTop:4,fontSize:11,color:"#999"}}>
        {acc.phone&&<a href={`tel:${acc.phone.replace(/\D/g,"")}`} style={{color:"#6BA3BF",textDecoration:"none",cursor:"pointer",display:"inline-flex",alignItems:"center",gap:3}}><I.Phone/> {acc.phone}</a>}
        {acc.email&&<a href={`mailto:${acc.email}`} style={{color:"#6BA3BF",textDecoration:"none",cursor:"pointer",display:"inline-flex",alignItems:"center",gap:3}}><I.Mail/> {acc.email}</a>}
      </div>
      {acc.address&&<div style={{fontSize:10,color:"#999",marginTop:4}}>{acc.address}</div>}
      <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:8,marginTop:14}}>
        {[
          {l:"Total Deals",v:acctDeals.length,c:"#fff"},
          {l:"Active",v:active.length,c:"#E89300"},
          {l:"Funded",v:funded.length,c:"#4CAF50"},
          {l:"Dead/Declined",v:dead.length,c:"#666"},
          {l:"Win Rate",v:winRate+"%",c:winRate>=50?"#4CAF50":"#E89300"},
        ].map((s,i)=><div key={i} style={{textAlign:"center",padding:"8px 4px",background:"#FFFFFF",borderRadius:6,border:"1px solid #D8DCE0"}}><div style={{fontSize:16,fontWeight:700,color:s.c}}>{s.v}</div><div style={{fontSize:8,fontWeight:600,color:"#999",textTransform:"uppercase",letterSpacing:.5,marginTop:2}}>{s.l}</div></div>)}
      </div>
      {(totalFunded>0||totalFee>0)&&<div style={{display:"flex",gap:14,marginTop:10,fontSize:12}}>
        <span style={{color:"#4CAF50",fontWeight:600}}>Total Funded: {fmt(totalFunded)}</span>
        {totalFee>0&&<span style={{color:"#4CAF50"}}>Total Fees: ${totalFee.toLocaleString()}</span>}
      </div>}
    </div>

    {/* Tabs */}
    <div style={{display:"flex",gap:4,marginBottom:14}}>
      {[{id:"overview",l:"Opportunities"},{id:"vault",l:`Documents (${allDocs.length})`},{id:"activity",l:`Activity (${allActivities.length})`},{id:"notes",l:"Notes"}].map(t=>
        <button key={t.id} className="av-tab" style={{...S.tab,...(tab===t.id?S.tabA:{})}} onClick={()=>setTab(t.id)}>{t.l}</button>
      )}
    </div>

    {/* Opportunities Tab */}
    {tab==="overview"&&<div>
      {active.length>0&&<div style={{marginBottom:16}}>
        <div style={{fontSize:11,fontWeight:700,color:"#E89300",textTransform:"uppercase",letterSpacing:1,marginBottom:8}}>Active ({active.length})</div>
        {active.map(d=><OppRow key={d.id} deal={d} onSel={onSel} stgColor={stgColor}/>)}
      </div>}
      {funded.length>0&&<div style={{marginBottom:16}}>
        <div style={{fontSize:11,fontWeight:700,color:"#4CAF50",textTransform:"uppercase",letterSpacing:1,marginBottom:8}}>Funded ({funded.length})</div>
        {funded.map(d=><OppRow key={d.id} deal={d} onSel={onSel} stgColor={stgColor}/>)}
      </div>}
      {dead.length>0&&<div style={{marginBottom:16}}>
        <div style={{fontSize:11,fontWeight:700,color:"#999",textTransform:"uppercase",letterSpacing:1,marginBottom:8}}>Dead / Declined ({dead.length})</div>
        {dead.map(d=><OppRow key={d.id} deal={d} onSel={onSel} stgColor={stgColor}/>)}
      </div>}
      {acctDeals.length===0&&<div style={{fontSize:12,color:"#999",fontStyle:"italic"}}>No deals yet for this account.</div>}
    </div>}

    {/* Document Vault Tab */}
    {tab==="vault"&&<div>
      {/* Add to vault */}
      <div style={{padding:12,background:"#111113",borderRadius:8,border:"1px solid #D8DCE0",marginBottom:14}}>
        <div style={{fontSize:9,fontWeight:700,color:"#4F8EAE",textTransform:"uppercase",letterSpacing:1,marginBottom:8}}>Add to Document Vault</div>
        <div style={{display:"flex",gap:6}}>
          <input style={{...S.inp,flex:1,fontSize:12}} placeholder="File name (e.g. Torres_DL_2026.pdf)" value={vaultName} onChange={e=>setVaultName(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&vaultName.trim()){addToVault(vaultName.trim());setVaultName("")}}}/>
          <button className="av-btn" style={{...S.prim,padding:"6px 12px",fontSize:11}} onClick={()=>{if(vaultName.trim()){addToVault(vaultName.trim());setVaultName("")}}}><I.Upload/> Add</button>
        </div>
      </div>
      {/* Vault files (account-level) */}
      {vaultFiles.length>0&&<div style={{marginBottom:16}}>
        <div style={{fontSize:10,fontWeight:700,color:"#4CAF50",textTransform:"uppercase",letterSpacing:1,marginBottom:6}}>Account Vault ({vaultFiles.length})</div>
        {vaultFiles.map(f=>(
          <div key={f.id} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 8px",background:"#FFFFFF",borderRadius:5,border:"1px solid #D8DCE0",marginBottom:3}}>
            <span style={{color:"#4CAF50"}}><I.Doc/></span>
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontSize:11,color:"#666",fontWeight:500,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{f.name}</div>
              <div style={{fontSize:9,color:"#aaa"}}>{fDT(f.at)}{f.fromDeal?` · saved from deal`:"  · uploaded directly"}</div>
            </div>
            <button style={{...S.tinyBtn,color:"#999"}} onClick={()=>rmFromVault(f.id)}><I.Trash/></button>
          </div>
        ))}
      </div>}
      {/* Deal documents (pulled from all deals) */}
      {dealDocs.length>0&&<div>
        <div style={{fontSize:10,fontWeight:700,color:"#999",textTransform:"uppercase",letterSpacing:1,marginBottom:6}}>From Deals ({dealDocs.length})</div>
        {acctDeals.filter(d=>(d.docs||[]).length>0).map(d=>(
          <div key={d.id} style={{marginBottom:10}}>
            <div style={{fontSize:10,fontWeight:600,color:"#999",marginBottom:4}}>{d.eqType||"Deal"} — {fmt(d.amount)} · {STAGES.find(s=>s.id===d.stage)?.label}</div>
            {(d.docs||[]).map(doc=>(
              <div key={doc.docId} style={{display:"flex",alignItems:"center",gap:8,padding:"5px 8px",background:"#FFFFFF",borderRadius:5,border:"1px solid #111",marginBottom:2}}>
                <span style={{color:"#4F8EAE"}}><I.Doc/></span>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:11,color:"#999",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{doc.name}</div>
                  <div style={{fontSize:9,color:"#aaa"}}>{fDT(doc.at)} · {doc.by}</div>
                </div>
                <button className="av-ghost" style={{...S.ghost,padding:"3px 8px",fontSize:9}} onClick={()=>{addToVault(`${doc.name}`,d.id)}}><I.Copy/> Save to Vault</button>
              </div>
            ))}
          </div>
        ))}
      </div>}
      {allDocs.length===0&&<div style={{fontSize:12,color:"#999",fontStyle:"italic"}}>No documents yet.</div>}
    </div>}

    {/* Activity Tab */}
    {tab==="activity"&&<div>
      {/* Log new activity */}
      <div style={{padding:12,background:"#111113",borderRadius:8,border:"1px solid #D8DCE0",marginBottom:14}}>
        <div style={{fontSize:9,fontWeight:700,color:"#4F8EAE",textTransform:"uppercase",letterSpacing:1,marginBottom:8}}>Log Activity</div>
        <div style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:8}}>
          {ACT_TYPES.map(t=>(
            <button key={t.id} onClick={()=>setActType(actType===t.id?null:t.id)} style={{
              display:"flex",alignItems:"center",gap:4,padding:"4px 8px",fontSize:10,fontWeight:600,
              borderRadius:6,border:`1px solid ${actType===t.id?t.color:t.color+"33"}`,
              background:actType===t.id?t.color+"18":"transparent",
              color:actType===t.id?t.color:t.color+"99",cursor:"pointer",fontFamily:"inherit",transition:"all .15s"
            }}>{t.icon==="phone"?<I.Phone/>:t.icon==="mail"?<I.Mail/>:<I.Clip/>}{t.label}</button>
          ))}
        </div>
        {actType&&<div style={{display:"flex",gap:6}}>
          <input style={{...S.inp,flex:1,fontSize:12}} placeholder="Details (optional)..." value={actDetail} onChange={e=>setActDetail(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")doLogAcct(actType)}}/>
          <button className="av-btn" style={{...S.prim,padding:"6px 12px",fontSize:11}} onClick={()=>doLogAcct(actType)}>Log</button>
        </div>}
      </div>
      {/* Activity timeline */}
      {allActivities.length===0&&<div style={{fontSize:12,color:"#999",fontStyle:"italic"}}>No activity logged yet.</div>}
      {allActivities.map(a=>{
        const t=ACT_TYPES.find(x=>x.id===a.type);
        return(<div key={a.id} style={{display:"flex",alignItems:"flex-start",gap:10,padding:"8px 0",borderBottom:"1px solid #111"}}>
          <div style={{width:26,height:26,borderRadius:6,background:(t?.color||"#666")+"18",display:"flex",alignItems:"center",justifyContent:"center",color:t?.color||"#666",flexShrink:0,marginTop:1}}>
            {t?.icon==="phone"?<I.Phone/>:t?.icon==="mail"?<I.Mail/>:<I.Clip/>}
          </div>
          <div style={{flex:1,minWidth:0}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <span style={{fontSize:11,fontWeight:600,color:t?.color||"#888"}}>{t?.label||a.type}</span>
              <span style={{fontSize:9,color:"#aaa"}}>{fDT(a.at)}</span>
            </div>
            {a.detail&&<div style={{fontSize:11,color:"#999",marginTop:1}}>{a.detail}</div>}
            <div style={{fontSize:9,color:"#666",marginTop:2,fontStyle:"italic"}}>{a.source}</div>
          </div>
        </div>);
      })}
    </div>}

    {/* Notes Tab */}
    {tab==="notes"&&<div style={{padding:12,background:"#111113",borderRadius:8,border:"1px solid #D8DCE0"}}>
      <div style={{fontSize:12,color:"#666",lineHeight:1.7,whiteSpace:"pre-wrap"}}>{acc.notes||"No notes yet."}</div>
    </div>}
  </div>);
}

function OppRow({deal:d,onSel,stgColor}) {
  const stg=STAGES.find(s=>s.id===d.stage);
  return(<div className="av-card" style={{...S.dlrCard,cursor:"pointer",padding:"10px 14px",marginBottom:4}} onClick={()=>onSel(d)}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
      <div>
        <div style={{fontSize:12,fontWeight:600,color:"#666"}}>{d.eqDetail||d.eqType||"Equipment"}</div>
        <div style={{fontSize:11,color:"#999",marginTop:1}}>{fmt(d.amount)}{d.terms?` · ${fmt(d.terms.pmt)}/mo · ${d.terms.months}mo`:""}</div>
      </div>
      <div style={{textAlign:"right"}}>
        <span style={{fontSize:10,fontWeight:700,color:stg?.color,padding:"2px 8px",background:(stg?.color||"#666")+"15",borderRadius:4}}>{stg?.label}</span>
        <div style={{fontSize:9,color:"#aaa",marginTop:3}}>{fDT(d.createdAt)}</div>
      </div>
    </div>
    {d.dealerName&&<div style={{fontSize:10,color:"#999",marginTop:4}}>via {d.dealerName}</div>}
    {d.terms?.feeAmt>0&&<div style={{fontSize:10,color:"#4CAF50",marginTop:2}}>Fee: ${d.terms.feeAmt.toLocaleString()}</div>}
  </div>);
}

function AccountForm({acc,onSave,onClose}) {
  const [f,sf]=useState({
    id:acc?.id||null,biz:acc?.biz||"",contact:acc?.contact||"",email:acc?.email||"",phone:acc?.phone||"",
    address:acc?.address||"",industry:acc?.industry||"",dateStarted:acc?.dateStarted||"",notes:acc?.notes||"",
    activities:acc?.activities||[],createdAt:acc?.createdAt||null
  });
  const u=(k,v)=>sf(p=>({...p,[k]:v}));
  return(<div style={S.overlay} onClick={onClose}><div style={S.modal} onClick={e=>e.stopPropagation()}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 18px",borderBottom:"1px solid #D8DCE0"}}><span style={{fontSize:15,fontWeight:700,color:"#1a1a2e"}}>{f.id?"Edit Account":"New Account"}</span><button style={S.closeBtn} onClick={onClose}><I.X/></button></div>
    <div style={{padding:"14px 18px",overflowY:"auto",maxHeight:"60vh"}}>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px 12px"}}>
        <FF l="Business Name *" v={f.biz} ch={v=>u("biz",v)}/>
        <FF l="Primary Contact" v={f.contact} ch={v=>u("contact",v)}/>
        <FF l="Email" v={f.email} ch={v=>u("email",v)} t="email"/>
        <FF l="Phone" v={f.phone} ch={v=>u("phone",v)}/>
        <FF l="Industry" v={f.industry} ch={v=>u("industry",v)} ph="e.g. Towing & Recovery"/>
        <FF l="Year Started" v={f.dateStarted} ch={v=>u("dateStarted",v)} ph="e.g. 2018"/>
      </div>
      <div style={{marginTop:8}}><FF l="Address" v={f.address} ch={v=>u("address",v)} ph="Full address"/></div>
      <div style={{marginTop:8}}><label style={S.fLabel}>Notes</label><textarea style={{...S.inp,resize:"vertical",minHeight:60}} value={f.notes} onChange={e=>u("notes",e.target.value)} placeholder="Any background notes on this customer..."/></div>
    </div>
    <div style={{display:"flex",gap:8,justifyContent:"flex-end",padding:"12px 18px",borderTop:"1px solid #D8DCE0"}}><button className="av-ghost" style={S.ghost} onClick={onClose}>Cancel</button><button className="av-btn" style={S.prim} onClick={()=>{if(!f.biz.trim())return;onSave(f)}}>{f.id?"Save":"Create"}</button></div>
  </div></div>);
}

// ── Activity Log ──────────────────────────────────────────────

const ACT_TYPES = [
  {id:"call_cust",label:"Called Customer",icon:"phone",color:"#4CAF50",who:"customer"},
  {id:"email_cust",label:"Emailed Customer",icon:"mail",color:"#4F8EAE",who:"customer"},
  {id:"call_dlr",label:"Called Dealer",icon:"phone",color:"#E89300",who:"dealer"},
  {id:"email_dlr",label:"Emailed Dealer",icon:"mail",color:"#E89300",who:"dealer"},
  {id:"call_lndr",label:"Called Lender",icon:"phone",color:"#9C27B0",who:"lender"},
  {id:"email_lndr",label:"Emailed Lender",icon:"mail",color:"#9C27B0",who:"lender"},
  {id:"other",label:"Other",icon:"clip",color:"#999",who:"other"},
];

function ActivityLog({deal}) {
  const {addActivity} = useContext(Ctx);
  const [detail,setDetail]=useState("");
  const [selType,setSelType]=useState(null);
  const acts=(deal.activities||[]).slice().reverse();

  const doLog=(typeId)=>{
    const t=ACT_TYPES.find(a=>a.id===typeId);
    addActivity(deal.id,typeId,t?.who||"other",detail.trim());
    setDetail("");
    setSelType(null);
  };

  const iconFor=(type)=>{
    const t=ACT_TYPES.find(a=>a.id===type);
    if(!t)return <I.Clip/>;
    if(t.icon==="phone")return <I.Phone/>;
    if(t.icon==="mail")return <I.Mail/>;
    return <I.Clip/>;
  };

  return(
    <Sec t={`Activity Log (${acts.length})`}>
      {/* Quick action buttons */}
      <div style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:8}}>
        {ACT_TYPES.map(t=>(
          <button key={t.id} onClick={()=>setSelType(selType===t.id?null:t.id)} style={{
            display:"flex",alignItems:"center",gap:4,padding:"4px 8px",fontSize:10,fontWeight:600,
            borderRadius:6,border:`1px solid ${selType===t.id?t.color:t.color+"33"}`,
            background:selType===t.id?t.color+"18":"transparent",
            color:selType===t.id?t.color:t.color+"99",cursor:"pointer",fontFamily:"inherit",
            transition:"all .15s"
          }}>
            {t.icon==="phone"?<I.Phone/>:t.icon==="mail"?<I.Mail/>:<I.Clip/>}
            {t.label}
          </button>
        ))}
      </div>
      {/* Detail input + log button */}
      {selType&&<div style={{display:"flex",gap:6,marginBottom:10}}>
        <input style={{...S.inp,flex:1,fontSize:12}} placeholder="Add details (optional)..." value={detail} onChange={e=>setDetail(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")doLog(selType)}}/>
        <button className="av-btn" style={{...S.prim,padding:"6px 12px",fontSize:11,background:ACT_TYPES.find(a=>a.id===selType)?.color+"22",border:`1px solid ${ACT_TYPES.find(a=>a.id===selType)?.color+"44"}`,color:ACT_TYPES.find(a=>a.id===selType)?.color}} onClick={()=>doLog(selType)}>Log</button>
      </div>}
      {/* Activity list */}
      {acts.length===0&&<div style={{fontSize:11,color:"#aaa",fontStyle:"italic"}}>No activity logged yet</div>}
      {acts.map(a=>{
        const t=ACT_TYPES.find(x=>x.id===a.type);
        return(<div key={a.id} style={{display:"flex",alignItems:"flex-start",gap:8,padding:"6px 0",borderBottom:"1px solid #111"}}>
          <div style={{width:24,height:24,borderRadius:6,background:(t?.color||"#666")+"18",display:"flex",alignItems:"center",justifyContent:"center",color:t?.color||"#666",flexShrink:0,marginTop:1}}>
            {iconFor(a.type)}
          </div>
          <div style={{flex:1,minWidth:0}}>
            <div style={{fontSize:11,fontWeight:600,color:t?.color||"#888"}}>{t?.label||a.type}</div>
            {a.detail&&<div style={{fontSize:11,color:"#999",marginTop:1}}>{a.detail}</div>}
            <div style={{fontSize:9,color:"#aaa",marginTop:2}}>{fDT(a.at)}</div>
          </div>
        </div>);
      })}
    </Sec>
  );
}

// ── Dealer & Lender Management ────────────────────────────────

function DlrMgmt() {
  const {dealers,deals,addDlr,updDlr} = useContext(Ctx);
  const [form,setForm]=useState(false);
  const [ed,setEd]=useState(null);
  return(<div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}><div style={{fontSize:15,fontWeight:700,color:"#1a1a2e"}}>Dealer Partners</div><button className="av-btn" style={S.prim} onClick={()=>{setEd(null);setForm(true)}}><I.Plus/> Add Dealer</button></div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(290px,1fr))",gap:10}}>{dealers.map(dl=>{const cnt=deals.filter(d=>d.dealerId===dl.id).length,fun=deals.filter(d=>d.dealerId===dl.id&&d.stage==="funded").reduce((s,d)=>s+(d.amount||0),0);return(
      <div key={dl.id} className="av-card" style={S.dlrCard}><div style={{display:"flex",justifyContent:"space-between"}}><div><div style={{fontSize:14,fontWeight:700,color:"#1a1a2e"}}>{dl.name}</div><div style={{fontSize:11,color:"#888",marginTop:2}}>{dl.contact} · {dl.region}</div></div><button style={S.tinyBtn} onClick={()=>{setEd(dl);setForm(true)}}><I.Edit/></button></div>
        {dl.salesRep&&<div style={{marginTop:6,padding:"6px 8px",background:"#FFFFFF",borderRadius:5,border:"1px solid #D8DCE0"}}>
          <div style={{fontSize:8,fontWeight:700,color:"#4F8EAE",textTransform:"uppercase",letterSpacing:.8,marginBottom:3}}>Sales Rep</div>
          <div style={{fontSize:11,color:"#666",fontWeight:600}}>{dl.salesRep}</div>
          <div style={{display:"flex",gap:8,marginTop:2,flexWrap:"wrap"}}>
            {dl.salesRepPhone&&<a href={`tel:${dl.salesRepPhone.replace(/\D/g,"")}`} style={{fontSize:10,color:"#6BA3BF",textDecoration:"none",cursor:"pointer",display:"inline-flex",alignItems:"center",gap:3}}><I.Phone/> {dl.salesRepPhone}</a>}
            {dl.salesRepEmail&&<a href={`mailto:${dl.salesRepEmail}`} style={{fontSize:10,color:"#6BA3BF",textDecoration:"none",cursor:"pointer",display:"inline-flex",alignItems:"center",gap:3}}><I.Mail/> {dl.salesRepEmail}</a>}
          </div>
        </div>}
        <div style={{display:"flex",gap:4,flexWrap:"wrap",margin:"8px 0"}}>{(dl.specs||[]).map(sp=><span key={sp} style={S.specB}>{sp}</span>)}</div>
        <div style={{display:"flex",gap:14,fontSize:11,color:"#888"}}><span><strong style={{color:"#1a1a2e"}}>{cnt}</strong> deals</span><span><strong style={{color:"#4CAF50"}}>{fmt(fun)}</strong> funded</span>{dl.referralPts>0&&<span style={{color:"#4F8EAE"}}>{dl.referralPts}pts referral</span>}</div>
        <div style={{marginTop:5,display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
          {dl.email&&<a href={`mailto:${dl.email}`} style={{fontFamily:"'JetBrains Mono',monospace",fontSize:10,color:"#6BA3BF",textDecoration:"none",cursor:"pointer",display:"inline-flex",alignItems:"center",gap:3}}><I.Mail/> {dl.email}</a>}
          {dl.phone&&<a href={`tel:${dl.phone.replace(/\D/g,"")}`} style={{fontFamily:"'JetBrains Mono',monospace",fontSize:10,color:"#6BA3BF",textDecoration:"none",cursor:"pointer",display:"inline-flex",alignItems:"center",gap:3}}><I.Phone/> {dl.phone}</a>}
        </div>
      </div>)})}</div>
    {form&&<DlrForm dlr={ed} onSave={d=>{ed?updDlr(d):addDlr(d);setForm(false);setEd(null)}} onClose={()=>{setForm(false);setEd(null)}}/>}
  </div>);
}

function LndrMgmt() {
  const {lenders,setLenders} = useContext(Ctx);
  const [form,setForm]=useState(false);
  const [ed,setEd]=useState(null);
  const [expanded,setExpanded]=useState(null);
  const save=l=>{if(l.id)setLenders(p=>p.map(x=>x.id===l.id?l:x));else setLenders(p=>[...p,{...l,id:"l"+Date.now().toString(36)}]);setForm(false);setEd(null)};
  const addUwDoc=(lId,file)=>{setLenders(p=>p.map(l=>l.id===lId?{...l,uwDocs:[...(l.uwDocs||[]),{id:gId(),name:file.name,at:new Date().toISOString()}]}:l))};
  const rmUwDoc=(lId,docId)=>{setLenders(p=>p.map(l=>l.id===lId?{...l,uwDocs:(l.uwDocs||[]).filter(d=>d.id!==docId)}:l))};
  const updUwNotes=(lId,notes)=>{setLenders(p=>p.map(l=>l.id===lId?{...l,uwNotes:notes}:l))};
  return(<div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}><div style={{fontSize:15,fontWeight:700,color:"#1a1a2e"}}>Lender Configuration</div><button className="av-btn" style={S.prim} onClick={()=>{setEd(null);setForm(true)}}><I.Plus/> Add Lender</button></div>
    <div style={{display:"flex",flexDirection:"column",gap:8}}>{lenders.map(l=>{
      const isExp=expanded===l.id;
      const uwCount=(l.uwDocs||[]).length;
      return(
      <div key={l.id} className="av-card" style={{background:"#0E0E10",border:"1px solid #D8DCE0",borderRadius:8,padding:0,overflow:"hidden"}}>
        <div style={{display:"flex",alignItems:"center",padding:"12px 14px",cursor:"pointer"}} onClick={()=>setExpanded(isExp?null:l.id)}>
          <div style={{flex:1}}>
            <div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:13,fontWeight:600,color:"#1a1a2e"}}>{l.name}</span><span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:10,color:"#4F8EAE",background:"#4F8EAE11",padding:"1px 6px",borderRadius:3}}>{l.code}</span>
              {uwCount>0&&<span style={{fontSize:8,padding:"2px 6px",borderRadius:8,background:"#4F8EAE12",color:"#4F8EAE",fontWeight:600}}>{uwCount} doc{uwCount!==1?"s":""}</span>}
            </div>
            <div style={{fontSize:10,color:"#999",marginTop:3}}>Tax: {l.taxRule==="include"?"Always included":l.taxRule==="exclude"?"Always excluded":"Customer choice"}</div>
            {(l.contact||l.email||l.phone)&&<div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:3,alignItems:"center"}}>
              {l.contact&&<span style={{fontSize:10,color:"#666"}}>{l.contact}</span>}
              {l.phone&&<a href={`tel:${l.phone.replace(/\D/g,"")}`} style={{fontSize:10,color:"#6BA3BF",textDecoration:"none",cursor:"pointer",display:"inline-flex",alignItems:"center",gap:2}}><I.Phone/> {l.phone}</a>}
              {l.email&&<a href={`mailto:${l.email}`} style={{fontSize:10,color:"#6BA3BF",textDecoration:"none",cursor:"pointer",display:"inline-flex",alignItems:"center",gap:2}}><I.Mail/> {l.email}</a>}
            </div>}
            <div style={{fontSize:10,color:"#999",marginTop:2}}>{l.lienholder}</div><div style={{fontSize:9,color:"#aaa"}}>{l.address}</div>
          </div>
          <div style={{display:"flex",gap:6,alignItems:"center"}}>
            <button style={S.tinyBtn} onClick={e=>{e.stopPropagation();setEd(l);setForm(true)}}><I.Edit/></button>
            <span style={{color:"#aaa",fontSize:12,transition:"transform .2s",transform:isExp?"rotate(180deg)":"rotate(0deg)"}}>▼</span>
          </div>
        </div>
        {isExp&&<div style={{borderTop:"1px solid #E2E5E9",padding:"12px 14px"}}>
          {/* Lienholder & Address */}
          <div style={{marginBottom:12,padding:"10px 12px",background:"#111",border:"1px solid #D8DCE0",borderRadius:7}}>
            <div style={{fontSize:10,fontWeight:700,color:"#4F8EAE",textTransform:"uppercase",letterSpacing:.8,marginBottom:6}}>🏢 Lienholder & Address</div>
            {l.lienholder?<div style={{fontSize:12,color:"#1a1a2e",fontWeight:600}}>{l.lienholder}</div>:<div style={{fontSize:11,color:"#aaa",fontStyle:"italic"}}>No lienholder name set</div>}
            {l.address?<div style={{fontSize:11,color:"#999",marginTop:2}}>{l.address}</div>:<div style={{fontSize:10,color:"#aaa",fontStyle:"italic",marginTop:2}}>No address set</div>}
          </div>
          {/* Insurance Requirements */}
          <div style={{marginBottom:12,padding:"10px 12px",background:"#111",border:"1px solid #D8DCE0",borderRadius:7}}>
            <div style={{fontSize:10,fontWeight:700,color:"#4F8EAE",textTransform:"uppercase",letterSpacing:.8,marginBottom:8}}>🛡️ Insurance Requirements</div>
            <div style={{display:"flex",flexWrap:"wrap",gap:"4px 16px"}}>
              {l.insListYMM!==false&&<div style={{fontSize:11,color:"#666",display:"flex",alignItems:"center",gap:4}}><span style={{color:"#4CAF50"}}>✓</span> Year, Make, Model</div>}
              {l.insListVin!==false&&<div style={{fontSize:11,color:"#666",display:"flex",alignItems:"center",gap:4}}><span style={{color:"#4CAF50"}}>✓</span> VIN / Serial #</div>}
              {l.insCompCollision!==false&&<div style={{fontSize:11,color:"#666",display:"flex",alignItems:"center",gap:4}}><span style={{color:"#4CAF50"}}>✓</span> Comp & Collision</div>}
              {l.insACV!==false&&<div style={{fontSize:11,color:"#666",display:"flex",alignItems:"center",gap:4}}><span style={{color:"#4CAF50"}}>✓</span> Actual Cash Value</div>}
            </div>
            <div style={{display:"flex",gap:16,marginTop:6,flexWrap:"wrap"}}>
              {l.insPropertyCov&&<div style={{fontSize:10}}><span style={{color:"#999"}}>Property Coverage:</span> <span style={{color:"#666",fontWeight:600}}>{l.insPropertyCov}</span></div>}
              {l.insMaxDeductible&&<div style={{fontSize:10}}><span style={{color:"#999"}}>Max Deductible:</span> <span style={{color:"#666",fontWeight:600}}>{l.insMaxDeductible}</span></div>}
            </div>
            {!l.insPropertyCov&&!l.insMaxDeductible&&l.insListYMM===false&&l.insListVin===false&&l.insCompCollision===false&&l.insACV===false&&<div style={{fontSize:10,color:"#aaa",fontStyle:"italic"}}>No insurance requirements configured — edit lender to set them</div>}
          </div>
          {/* Insurance Additional Requirements & Notes */}
          <div style={{marginBottom:12,padding:"10px 12px",background:"#111",border:"1px solid #D8DCE0",borderRadius:7}}>
            <div style={{fontSize:10,fontWeight:700,color:"#4F8EAE",textTransform:"uppercase",letterSpacing:.8,marginBottom:6}}>📋 Insurance Additional Requirements & Notes</div>
            <textarea style={{...S.inp,width:"100%",minHeight:60,fontSize:11,lineHeight:1.5,resize:"vertical",fontFamily:"inherit"}} placeholder="Loss payee must match lienholder name exactly, binder requirements, certificate of insurance specifics, additional insured requirements, etc..." value={l.insAddlNotes||""} onChange={e=>{setLenders(p=>p.map(x=>x.id===l.id?{...x,insAddlNotes:e.target.value}:x))}}/>
          </div>
          {/* Underwriting Notes */}
          <div style={{marginBottom:10}}>
            <div style={{fontSize:10,fontWeight:700,color:"#4F8EAE",textTransform:"uppercase",letterSpacing:.8,marginBottom:6}}>Underwriting Criteria & Notes</div>
            <textarea style={{...S.inp,width:"100%",minHeight:80,fontSize:11,lineHeight:1.5,resize:"vertical",fontFamily:"inherit"}} placeholder="Enter underwriting criteria, credit requirements, rate sheets notes, deal preferences, etc..." value={l.uwNotes||""} onChange={e=>updUwNotes(l.id,e.target.value)}/>
          </div>
          {/* Underwriting Documents */}
          <div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
              <div style={{fontSize:10,fontWeight:700,color:"#4F8EAE",textTransform:"uppercase",letterSpacing:.8}}>Underwriting Documents ({uwCount})</div>
            </div>
            {(l.uwDocs||[]).map(doc=>(
              <div key={doc.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"6px 8px",background:"#111",border:"1px solid #D8DCE0",borderRadius:5,marginBottom:4}}>
                <div style={{display:"flex",alignItems:"center",gap:6,flex:1,minWidth:0}}>
                  <span style={{color:"#4CAF50",flexShrink:0}}><I.Doc/></span>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontSize:11,color:"#666",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{doc.name}</div>
                    <div style={{fontSize:9,color:"#999"}}>{fDT(doc.at)}</div>
                  </div>
                </div>
                <button style={S.tinyBtn} onClick={()=>rmUwDoc(l.id,doc.id)}><I.Trash/></button>
              </div>
            ))}
            {/* Upload zone */}
            <div
              onDragOver={e=>{e.preventDefault();e.currentTarget.style.borderColor="#4F8EAE";e.currentTarget.style.background="#4F8EAE08"}}
              onDragLeave={e=>{e.preventDefault();e.currentTarget.style.borderColor="#CDD1D6";e.currentTarget.style.background="transparent"}}
              onDrop={e=>{e.preventDefault();e.currentTarget.style.borderColor="#CDD1D6";e.currentTarget.style.background="transparent";Array.from(e.dataTransfer?.files||[]).forEach(f=>addUwDoc(l.id,f))}}
              style={{padding:"10px",border:"2px dashed #CDD1D6",borderRadius:6,textAlign:"center",marginTop:4,transition:"all .15s"}}>
              <div style={{fontSize:10,color:"#999"}}>📂 Drag & drop rate sheets, guidelines, or criteria here</div>
              <label style={{fontSize:10,color:"#4F8EAE",cursor:"pointer",fontWeight:600,marginTop:3,display:"inline-block"}}>
                or click to browse
                <input type="file" multiple style={{display:"none"}} onChange={e=>{Array.from(e.target?.files||[]).forEach(f=>addUwDoc(l.id,f));e.target.value=""}}/>
              </label>
            </div>
          </div>
        </div>}
      </div>)})}</div>
    {form&&<LndrForm lndr={ed} onSave={save} onClose={()=>{setForm(false);setEd(null)}}/>}
  </div>);
}

function Settings() {
  const {deals,dealers,lenders,accounts,dnotes,reminders,crmUser,crmProfile,notify} = useContext(Ctx);
  const [lastBackup,setLastBackup]=useState(null);
  const [importing,setImporting]=useState(false);
  const [restoreMsg,setRestoreMsg]=useState(null);
  const fileRef=useRef(null);
  // User management state
  const [users,setUsers]=useState([]);
  const [showAddUser,setShowAddUser]=useState(false);
  const [newUser,setNewUser]=useState({email:"",password:"",full_name:"",role:"admin"});
  const [userLoading,setUserLoading]=useState(false);
  // MFA enrollment state
  const [mfaSetup,setMfaSetup]=useState(false);
  const [mfaQR,setMfaQR]=useState(null);
  const [mfaSecret,setMfaSecret]=useState(null);
  const [mfaVerifyCode,setMfaVerifyCode]=useState("");
  const [mfaFactorId,setMfaFactorId]=useState(null);
  const [mfaEnabled,setMfaEnabled]=useState(false);
  const [mfaLoading,setMfaLoading]=useState(false);

  // Load users + check MFA status
  useEffect(()=>{
    (async()=>{
      const res = await DB.adminListUsers();
      if(res.users) setUsers(res.users);
      // Check MFA
      const {supabase} = await import('../../lib/supabase');
      const {data} = await supabase.auth.mfa.listFactors();
      if(data?.totp?.length) setMfaEnabled(true);
    })();
  },[]);

  const handleAddUser=async()=>{
    if(!newUser.email||!newUser.password){notify("Email and password required","error");return}
    setUserLoading(true);
    const res = await DB.adminCreateUser(newUser);
    if(res.error){notify(res.error,"error")}
    else{notify("User created");setShowAddUser(false);setNewUser({email:"",password:"",full_name:"",role:"admin"});
      const r2=await DB.adminListUsers();if(r2.users)setUsers(r2.users);
    }
    setUserLoading(false);
  };

  const handleDeleteUser=async(userId,email)=>{
    if(!window.confirm(`Delete user ${email}? This cannot be undone.`))return;
    const res = await DB.adminDeleteUser(userId);
    if(res.error){notify(res.error,"error")}
    else{notify("User deleted");setUsers(p=>p.filter(u=>u.id!==userId))}
  };

  const startMfaEnroll=async()=>{
    setMfaLoading(true);
    try{
      const {supabase} = await import('../../lib/supabase');
      const {data,error} = await supabase.auth.mfa.enroll({factorType:'totp',friendlyName:'Avanti CRM'});
      if(error){notify(error.message,"error");setMfaLoading(false);return}
      setMfaQR(data.totp.qr_code);
      setMfaSecret(data.totp.secret);
      setMfaFactorId(data.id);
      setMfaSetup(true);
    }catch(e){notify(e.message,"error")}
    setMfaLoading(false);
  };

  const verifyMfaEnroll=async()=>{
    if(!mfaVerifyCode||mfaVerifyCode.length!==6){notify("Enter 6-digit code","error");return}
    setMfaLoading(true);
    try{
      const {supabase} = await import('../../lib/supabase');
      const {data:challenge} = await supabase.auth.mfa.challenge({factorId:mfaFactorId});
      const {error} = await supabase.auth.mfa.verify({factorId:mfaFactorId,challengeId:challenge.id,code:mfaVerifyCode});
      if(error){notify(error.message,"error");setMfaLoading(false);return}
      setMfaEnabled(true);setMfaSetup(false);setMfaQR(null);setMfaSecret(null);setMfaVerifyCode("");
      notify("Two-factor authentication enabled!");
    }catch(e){notify(e.message,"error")}
    setMfaLoading(false);
  };

  const disableMfa=async()=>{
    if(!window.confirm("Disable two-factor authentication?"))return;
    try{
      const {supabase} = await import('../../lib/supabase');
      const {data} = await supabase.auth.mfa.listFactors();
      if(data?.totp?.length){
        await supabase.auth.mfa.unenroll({factorId:data.totp[0].id});
        setMfaEnabled(false);
        notify("Two-factor authentication disabled");
      }
    }catch(e){notify(e.message,"error")}
  };

  const doExport=()=>{
    const backup={version:1,exportedAt:new Date().toISOString(),data:{deals,dealers,lenders,accounts,dnotes,reminders}};
    const blob=new Blob([JSON.stringify(backup,null,2)],{type:"application/json"});
    const url=URL.createObjectURL(blob);
    const a=document.createElement("a");
    a.href=url;a.download=`avanti-backup-${new Date().toISOString().slice(0,10)}.json`;
    document.body.appendChild(a);a.click();document.body.removeChild(a);URL.revokeObjectURL(url);
    setLastBackup(new Date().toLocaleString());
    notify("Backup downloaded","ok");
  };

  const doImport=(e)=>{
    const file=e.target.files?.[0];
    if(!file)return;
    setImporting(true);setRestoreMsg(null);
    const reader=new FileReader();
    reader.onload=(ev)=>{
      try{
        const backup=JSON.parse(ev.target.result);
        if(!backup.data||!backup.data.deals){setRestoreMsg("Invalid backup file");setImporting(false);return}
        const d=backup.data;
        const summary=`${d.deals?.length||0} deals, ${d.accounts?.length||0} accounts, ${d.dealers?.length||0} dealers`;
        if(window.confirm(`Restore backup from ${backup.exportedAt||"unknown date"}?\n\nThis contains: ${summary}\n\nThis will upload all data to Supabase.`)){
          const write=async()=>{
            try{
              if(d.deals) for(const deal of d.deals) await DB.saveDealToDb(deal);
              if(d.dealers) for(const dlr of d.dealers) await DB.saveDealer(dlr);
              if(d.accounts) for(const acc of d.accounts) await DB.saveAccount(acc);
            }catch(e){console.error("Restore error:",e)}
            setRestoreMsg("Backup restored! Refresh the page to see your data.");
            notify("Backup restored — refresh page","ok");
          };
          write();
        }
      }catch(err){setRestoreMsg("Error reading file: "+err.message)}
      setImporting(false);
    };
    reader.readAsText(file);
    e.target.value="";
  };

  const dealStats={total:deals.length,active:deals.filter(d=>!["funded","declined","dead"].includes(d.stage)).length,funded:deals.filter(d=>d.stage==="funded").length,dead:deals.filter(d=>["dead","declined"].includes(d.stage)).length};

  const exportAuditCSV=()=>{
    const rows=[["Deal Code","Business Name","Contact","Email","Phone","Equipment","Amount ($)","Stage","Dealer","Lender","Disposition Reason","Previous Stage","Adverse Action Sent","Adverse Action Method","Date Created","Date Last Updated","Date Closed","Source","Notes","Full History"]];
    deals.forEach(d=>{
      const disp=d.disposition||{};
      const closedEntry=[...(d.history||[])].reverse().find(h=>h.s==="dead"||h.s==="declined");
      const reason=disp.reason||closedEntry?.n||"";
      const prevStage=disp.previousStage||(d.history||[]).slice().reverse().find(h=>h.s!=="dead"&&h.s!=="declined")?.s||"";
      const lndr=lenders.find(l=>l.id===d.lenderId);
      const histStr=(d.history||[]).map(h=>`${STAGES.find(s=>s.id===h.s)?.label||h.s}: ${h.n} (${new Date(h.d).toLocaleDateString()})`).join(" | ");
      rows.push([
        d.code||"",d.biz||"",d.contact||"",d.email||"",d.phone||"",
        [d.eqYear,d.eqMake,d.eqModel].filter(Boolean).join(" ")||d.eqType||d.eqDetail||"",
        d.amount||0,STAGES.find(s=>s.id===d.stage)?.label||d.stage,
        d.dealerName||"Direct",lndr?.name||"",
        reason,STAGES.find(s=>s.id===prevStage)?.label||prevStage||"",
        d.adverseAction?"Yes":"No",d.adverseAction?.method||"",
        d.createdAt?new Date(d.createdAt).toLocaleDateString():"",
        d.updatedAt?new Date(d.updatedAt).toLocaleDateString():"",
        disp.date?new Date(disp.date).toLocaleDateString():closedEntry?.d?new Date(closedEntry.d).toLocaleDateString():"",
        d.source||"",d.notes||"",histStr
      ]);
    });
    const csv=rows.map(r=>r.map(c=>`"${String(c).replace(/"/g,'""')}"`).join(",")).join("\n");
    const blob=new Blob([csv],{type:"text/csv"});
    const url=URL.createObjectURL(blob);
    const a=document.createElement("a");a.href=url;a.download=`Avanti_All_Deals_Audit_${new Date().toISOString().slice(0,10)}.csv`;
    document.body.appendChild(a);a.click();
    setTimeout(()=>{document.body.removeChild(a);URL.revokeObjectURL(url)},100);
    notify("Audit CSV exported","ok");
  };

  return(<div><div style={{fontSize:15,fontWeight:700,color:"#1a1a2e",marginBottom:14}}>Settings & Integration</div>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>

      {/* USER MANAGEMENT CARD */}
      <div style={{...S.setCard,border:"1px solid #4F8EAE44",gridColumn:"1 / -1"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}><I.User/><div style={{...S.setT,color:"#4F8EAE",margin:0}}>User Management</div></div>
          <button className="av-btn" style={{...S.prim,padding:"5px 12px",fontSize:11}} onClick={()=>setShowAddUser(!showAddUser)}>{showAddUser?"Cancel":"+ Add User"}</button>
        </div>
        
        {showAddUser&&<div style={{background:"#F5F6F8",border:"1px solid #E2E5E9",borderRadius:8,padding:14,marginBottom:12}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            <div><label style={S.fLabel}>Full Name</label><input style={S.inp} value={newUser.full_name} onChange={e=>setNewUser(p=>({...p,full_name:e.target.value}))} placeholder="John Smith"/></div>
            <div><label style={S.fLabel}>Role</label><select style={S.fSel} value={newUser.role} onChange={e=>setNewUser(p=>({...p,role:e.target.value}))}>
              <option value="admin">Admin</option><option value="manager">Manager</option><option value="readonly">Read Only</option><option value="dealer">Dealer</option><option value="customer">Customer</option>
            </select></div>
            <div><label style={S.fLabel}>Email *</label><input style={S.inp} type="email" value={newUser.email} onChange={e=>setNewUser(p=>({...p,email:e.target.value}))} placeholder="user@example.com"/></div>
            <div><label style={S.fLabel}>Password *</label><input style={S.inp} type="text" value={newUser.password} onChange={e=>setNewUser(p=>({...p,password:e.target.value}))} placeholder="Secure password"/></div>
          </div>
          <button className="av-btn" style={{...S.prim,marginTop:10,width:"100%",justifyContent:"center",opacity:userLoading?.4:1}} onClick={handleAddUser} disabled={userLoading}>{userLoading?"Creating...":"Create User"}</button>
        </div>}

        <div style={{maxHeight:200,overflowY:"auto"}}>
          {users.map(u=>(
            <div key={u.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 10px",borderBottom:"1px solid #E2E5E9"}}>
              <div>
                <div style={{fontSize:12,color:"#1a1a2e",fontWeight:600}}>{u.full_name||u.email}</div>
                <div style={{fontSize:10,color:"#999"}}>{u.email} · <span style={{color:u.role==="admin"?"#4F8EAE":u.role==="dealer"?"#E89300":"#999",fontWeight:600}}>{u.role}</span></div>
              </div>
              {u.id!==crmUser?.id&&<button onClick={()=>handleDeleteUser(u.id,u.email)} style={{background:"none",border:"none",color:"#B0413E",cursor:"pointer",fontSize:10,fontWeight:600,fontFamily:"inherit"}}>Delete</button>}
              {u.id===crmUser?.id&&<span style={{fontSize:10,color:"#4CAF50",fontWeight:600}}>You</span>}
            </div>
          ))}
          {users.length===0&&<div style={{padding:12,textAlign:"center",color:"#999",fontSize:11}}>Loading users...</div>}
        </div>
      </div>

      {/* MFA / TWO-FACTOR CARD */}
      <div style={{...S.setCard,border:`1px solid ${mfaEnabled?"#4CAF5044":"#E8930044"}`,gridColumn:"1 / -1"}}>
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
          <I.Shield/>
          <div style={{...S.setT,color:mfaEnabled?"#4CAF50":"#E89300",margin:0}}>Two-Factor Authentication {mfaEnabled?"✓ Enabled":"— Not Set Up"}</div>
        </div>
        {!mfaEnabled&&!mfaSetup&&<div>
          <div style={S.setD}>Add an extra layer of security to your account. You'll need an authenticator app like Google Authenticator or Authy.</div>
          <button className="av-btn" style={{...S.prim,marginTop:10,opacity:mfaLoading?.4:1}} onClick={startMfaEnroll} disabled={mfaLoading}>{mfaLoading?"Setting up...":"Enable Two-Factor"}</button>
        </div>}
        {mfaSetup&&<div>
          <div style={S.setD}>Scan this QR code with your authenticator app, then enter the 6-digit code to verify.</div>
          <div style={{display:"flex",gap:16,marginTop:12,alignItems:"flex-start",flexWrap:"wrap"}}>
            {mfaQR&&<img src={mfaQR} alt="MFA QR" style={{width:160,height:160,borderRadius:8,border:"1px solid #E2E5E9"}}/>}
            <div style={{flex:1,minWidth:200}}>
              {mfaSecret&&<div style={{marginBottom:10}}>
                <div style={{fontSize:10,color:"#999",marginBottom:4}}>Manual entry key:</div>
                <div style={{fontFamily:"JetBrains Mono",fontSize:11,color:"#4F8EAE",background:"#F5F6F8",padding:"6px 10px",borderRadius:4,wordBreak:"break-all"}}>{mfaSecret}</div>
              </div>}
              <label style={S.fLabel}>Verification Code</label>
              <input style={{...S.inp,fontSize:18,textAlign:"center",letterSpacing:6,fontFamily:"JetBrains Mono"}} type="text" maxLength={6} placeholder="000000" value={mfaVerifyCode} onChange={e=>setMfaVerifyCode(e.target.value.replace(/\D/g,""))}/>
              <button className="av-btn" style={{...S.prim,marginTop:8,width:"100%",justifyContent:"center",opacity:mfaLoading?.4:1}} onClick={verifyMfaEnroll} disabled={mfaLoading}>{mfaLoading?"Verifying...":"Verify & Enable"}</button>
            </div>
          </div>
        </div>}
        {mfaEnabled&&!mfaSetup&&<div>
          <div style={{...S.setD,color:"#4CAF50"}}>Your account is protected with two-factor authentication.</div>
          <button className="av-ghost" style={{...S.ghost,marginTop:8,fontSize:11,color:"#B0413E",borderColor:"#B0413E33"}} onClick={disableMfa}>Disable 2FA</button>
        </div>}
      </div>

      {/* AUDIT EXPORT CARD */}
      <div style={{...S.setCard,border:"1px solid #4F8EAE44",gridColumn:"1 / -1"}}>
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}><I.Doc/><div style={{...S.setT,color:"#4F8EAE",margin:0}}>Audit & Compliance Export</div></div>
        <div style={S.setD}>Export all deals as CSV with full disposition reasons, stage history, and deal details. Every dead or declined deal includes the documented reason it didn't move forward.</div>
        <div style={{marginTop:12,display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
          <button className="av-btn" onClick={exportAuditCSV} style={{background:"#4F8EAE",color:"#fff",border:"none",borderRadius:8,padding:"10px 20px",fontWeight:700,fontSize:13,cursor:"pointer",display:"flex",alignItems:"center",gap:6}}>
            <I.Download/> Export All Deals CSV
          </button>
          <span style={{fontSize:11,color:"#999"}}>{dealStats.total} deals · {dealStats.active} active · {dealStats.funded} funded · {dealStats.dead} closed</span>
        </div>
      </div>

      {/* BACKUP CARD */}
      <div style={{...S.setCard,border:"1px solid #2A5A3A",gridColumn:"1 / -1"}}>
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}><I.Shield/><div style={{...S.setT,color:"#4CAF50",margin:0}}>Data Backup & Restore</div></div>
        <div style={S.setD}>Your data is stored in Supabase. Export backups for an extra layer of safety.</div>
        <div style={{marginTop:12,display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
          <button className="av-btn" onClick={doExport} style={{background:"#4CAF50",color:"#fff",border:"none",borderRadius:8,padding:"10px 20px",fontWeight:700,fontSize:13,cursor:"pointer",display:"flex",alignItems:"center",gap:6}}>
            <I.Download/> Export Backup
          </button>
          <button className="av-btn" onClick={()=>fileRef.current?.click()} style={{background:"#333",color:"#666",border:"1px solid #444",borderRadius:8,padding:"10px 20px",fontWeight:700,fontSize:13,cursor:"pointer",display:"flex",alignItems:"center",gap:6}}>
            <I.Upload/> Restore from Backup
          </button>
          <input ref={fileRef} type="file" accept=".json" onChange={doImport} style={{display:"none"}}/>
        </div>
        <div style={{marginTop:10,display:"flex",gap:16,fontSize:11,color:"#999"}}>
          <span>Deals: <b style={{color:"#666"}}>{dealStats.total}</b> ({dealStats.active} active, {dealStats.funded} funded)</span>
          <span>Accounts: <b style={{color:"#666"}}>{accounts.length}</b></span>
          <span>Dealers: <b style={{color:"#666"}}>{dealers.length}</b></span>
        </div>
        {lastBackup&&<div style={{marginTop:6,fontSize:11,color:"#4CAF50"}}>Last backup: {lastBackup}</div>}
        {restoreMsg&&<div style={{marginTop:6,fontSize:11,color:restoreMsg.includes("Error")?"#F44336":"#4CAF50",fontWeight:600}}>{restoreMsg}</div>}
      </div>

      <div style={S.setCard}><div style={S.setT}>Jotform Integration</div><div style={S.setD}>Connect your Jotform credit app to auto-create deals.</div><div style={{marginTop:10,fontFamily:"'JetBrains Mono',monospace",fontSize:10,color:"#4F8EAE",background:"#FFFFFF",padding:"8px 12px",borderRadius:5,overflowX:"auto"}}>Webhook: https://app.avantiequipfi.com/api/jotform</div></div>
      <div style={S.setCard}><div style={S.setT}>Notifications</div><div style={S.setD}>Email/SMS triggers when deals move between stages. Coming soon.</div></div>
      <div style={S.setCard}><div style={S.setT}>Account Manager</div><div style={S.setD}>Contact info shown to customers.</div><div style={{marginTop:8,fontSize:12,color:"#666"}}>{AM.name}<br/><span style={{color:"#999"}}>{AM.title}</span><br/><span style={{color:"#4F8EAE"}}>{AM.phone} · {AM.email}</span></div></div>
    </div></div>);
}

// ════════════════════════════════════════════════════════════════
// DEALER PORTAL
// ════════════════════════════════════════════════════════════════

function DealerDash({did,out}) {
  const {deals,dealers,saveDeal} = useContext(Ctx);
  const dlr = dealers.find(d=>d.id===did);
  const dd = useMemo(()=>deals.filter(d=>d.dealerId===did),[deals,did]);
  const [sub,setSub]=useState(false);
  const [sel,setSel]=useState(null);
  const stats = useMemo(()=>({act:dd.filter(d=>!["funded","declined","dead"].includes(d.stage)).length,fun:dd.filter(d=>d.stage==="funded").length,totF:dd.filter(d=>d.stage==="funded").reduce((s,d)=>s+(d.amount||0),0),pen:dd.filter(d=>["app_submitted","in_review"].includes(d.stage)).length}),[dd]);

  return(<div style={S.pWrap}>
    <div style={S.pHead}><div><div style={{fontSize:20,fontWeight:700,color:"#1a1a2e"}}>Welcome, {dlr?.contact}</div><div style={{fontSize:12,color:"#888"}}>{dlr?.name}</div></div><button className="av-btn" style={S.prim} onClick={()=>setSub(true)}><I.Plus/> Submit New Deal</button></div>
    <div style={S.statsG}>{[{l:"Active",v:stats.act,c:"#1a1a2e"},{l:"Pending",v:stats.pen,c:"#6BA3BF"},{l:"Funded",v:stats.fun,c:"#4CAF50"},{l:"Total Funded",v:fmt(stats.totF),c:"#4CAF50"},{l:"Action Needed",v:dd.filter(d=>{const pendFI=(d.fundingItems||[]).some(f=>!f.done);const pendDlr=["approved","docs_out","docs_signed","funded"].includes(d.stage)&&DEALER_REQ_DOCS.some(item=>!(d.docs||[]).find(x=>x.docId===item.docId));return pendFI||pendDlr}).length,c:dd.filter(d=>{const pendFI=(d.fundingItems||[]).some(f=>!f.done);const pendDlr=["approved","docs_out","docs_signed","funded"].includes(d.stage)&&DEALER_REQ_DOCS.some(item=>!(d.docs||[]).find(x=>x.docId===item.docId));return pendFI||pendDlr}).length>0?"#E89300":"#333"}].map((s,i)=><div key={i} style={S.statC}><div style={{...S.statV,color:s.c}}>{s.v}</div><div style={S.statL}>{s.l}</div></div>)}</div>
    <div style={{marginTop:20}}><div style={{fontSize:14,fontWeight:700,color:"#1a1a2e",marginBottom:12}}>Your Referred Deals</div>
      {dd.length===0?<div style={S.empty}><I.Truck/><div style={{marginTop:8,fontSize:13,fontWeight:600,color:"#999"}}>No deals yet</div></div>
      :<div style={{display:"flex",flexDirection:"column",gap:8}}>{_.orderBy(dd.filter(d=>d.stage!=="dead"),["updatedAt"],["desc"]).map(d=>{const stg=STAGES.find(s=>s.id===d.stage);const pendFI=(d.fundingItems||[]).filter(f=>!f.done).length;const pendDlrDocs=["approved","docs_out","docs_signed","funded"].includes(d.stage)?DEALER_REQ_DOCS.filter(item=>!(d.docs||[]).find(x=>x.docId===item.docId)).length:0;const totalPend=pendFI+pendDlrDocs;const hasAccepted=d.accepted;return(
        <div key={d.id} className="av-row" style={{...S.dlrDealRow,...(totalPend>0?{borderLeftColor:"#E89300",borderLeftWidth:3}:{})}} onClick={()=>setSel(d)}>
          <div style={{flex:1}}>
            <div style={{display:"flex",alignItems:"center",gap:6}}><span style={{fontWeight:600,color:"#1a1a2e",fontSize:14}}>{d.biz}</span>
              {totalPend>0&&<span style={{fontSize:8,padding:"2px 6px",borderRadius:8,background:"#E89300",color:"#fff",fontWeight:700,animation:"fadeIn .3s"}}>ACTION NEEDED</span>}
              {hasAccepted&&totalPend===0&&d.stage==="approved"&&<span style={{fontSize:8,padding:"2px 6px",borderRadius:8,background:"#4CAF50",color:"#fff",fontWeight:700}}>ACCEPTED</span>}
            </div>
            <div style={{fontSize:12,color:"#888",marginTop:2}}>{d.contact} · {d.eqType}</div>
            {totalPend>0&&<div style={{fontSize:10,color:"#E89300",marginTop:2}}><I.Alert/> {totalPend} item{totalPend!==1?"s":""} needed</div>}
          </div>
          <div style={{textAlign:"right"}}><div style={{fontWeight:600,color:"#6BA3BF",fontSize:14}}>{fmt(d.amount)}</div><span style={{...S.badge,background:stg?.color+"18",color:stg?.color,borderColor:stg?.color+"33",fontSize:10}}>{stg?.label}</span></div>
        </div>)})}</div>}
    </div>
    {sel&&<DealerDrawer dealId={sel.id} onClose={()=>setSel(null)}/>}
    {sub&&<DealForm deal={{dealerId:did,dealerName:dlr?.name,source:"Dealer Referral",stage:"new_lead"}} onSave={d=>{saveDeal(d);setSub(false)}} onClose={()=>setSub(false)} isDlr/>}
  </div>);
}

function DealerDrawer({dealId,onClose}) {
  const {deals,dealerNotes,addDealerNote,dealers,addDoc} = useContext(Ctx);
  const sel = deals.find(d=>d.id===dealId);
  const [dlrNote,setDlrNote]=useState("");
  if(!sel) return null;
  const stg=STAGES.find(s=>s.id===sel.stage);
  const dlr=dealers.find(d=>d.id===sel.dealerId);
  const myNotes=dealerNotes.filter(n=>n.dealId===sel.id);
  const pastApproved=["approved","docs_out","docs_signed","funded"].includes(sel.stage);
  // Filter activities to only show customer-related communication
  const custActivities=(sel.activities||[]).filter(a=>a.who==="customer"||a.type==="call_cust"||a.type==="email_cust");
  const submitDlrNote=()=>{
    if(!dlrNote.trim())return;
    addDealerNote(sel.id,dlrNote.trim(),dlr?.name||"Dealer",sel.biz);
    setDlrNote("");
  };
  const dlrPending=DEALER_REQ_DOCS.filter(item=>!(sel.docs||[]).find(x=>x.docId===item.docId)).length;
  return(
    <div style={S.overlay} onClick={onClose}><div style={{...S.drawer,maxWidth:460}} onClick={e=>e.stopPropagation()}>
      <div style={S.drHead}><div><div style={{fontSize:17,fontWeight:700,color:"#1a1a2e"}}>{sel.biz}</div><span style={{...S.badge,background:stg?.color+"18",color:stg?.color}}>{stg?.label}</span></div><button style={S.closeBtn} onClick={onClose}><I.X/></button></div>
      <div style={S.drBody}>
        <Sec t="Contact"><div style={S.field}><I.User/> {sel.contact}</div>{sel.phone&&<a href={`tel:${sel.phone.replace(/\D/g,"")}`} style={{...S.field,color:"#6BA3BF",textDecoration:"none",cursor:"pointer"}}><I.Phone/> {sel.phone}</a>}{sel.email&&<a href={`mailto:${sel.email}`} style={{...S.field,color:"#6BA3BF",textDecoration:"none",cursor:"pointer"}}><I.Mail/> {sel.email}</a>}</Sec>
        <Sec t="Equipment"><div style={{fontSize:14,color:"#1a1a2e"}}>{sel.eqType}</div>
          {(sel.eqYear||sel.eqMake||sel.eqModel)&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginTop:6}}><FB l="Year" v={sel.eqYear||"—"}/><FB l="Make" v={sel.eqMake||"—"}/><FB l="Model" v={sel.eqModel||"—"}/></div>}
          {sel.eqVin&&<div style={{marginTop:6}}><FB l="VIN" v={sel.eqVin} vs={{fontFamily:"'JetBrains Mono',monospace",fontSize:11,letterSpacing:.5}}/></div>}
          {sel.eqMileage&&<div style={{marginTop:4}}><FB l="Mileage" v={Number(sel.eqMileage).toLocaleString()+" mi"}/></div>}
          {sel.eqDetail&&<div style={{fontSize:12,color:"#888",marginTop:4}}>{sel.eqDetail}</div>}
          <div style={{fontSize:17,fontWeight:700,color:"#6BA3BF",marginTop:6}}>{fmt(sel.amount)}</div></Sec>
        {sel.terms&&<Sec t="Approval"><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><FB l="Approved Amount" v={fmt(sel.terms.amt)} vs={{color:"#4CAF50"}}/><FB l="Monthly Payment" v={fmt(sel.terms.pmt)}/><FB l="Term" v={`${sel.terms.months}mo`}/></div>{sel.accepted&&<div style={{marginTop:6,fontSize:11,color:"#4CAF50"}}>✓ Customer accepted</div>}</Sec>}
        {pastApproved&&<Sec t={<span style={{display:"flex",alignItems:"center",gap:6}}>Documents Needed{dlrPending>0&&<span style={{fontSize:9,padding:"1px 6px",borderRadius:8,background:"#E8930018",color:"#E89300",border:"1px solid #E8930033",fontWeight:700}}>{dlrPending} needed</span>}{dlrPending===0&&<span style={{fontSize:9,padding:"1px 6px",borderRadius:8,background:"#4CAF5012",color:"#4CAF50",fontWeight:600}}>All received</span>}</span>}>
          <div style={{fontSize:11,color:"#999",marginBottom:8,lineHeight:1.5}}>Please upload the following documents so we can finalize the deal and get financing documents prepared.</div>
          {DEALER_REQ_DOCS.map(item=>{
            const up=(sel.docs||[]).find(x=>x.docId===item.docId);
            return(<div key={item.docId} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 12px",background:up?"#4CAF5008":"#E8930006",border:`1px solid ${up?"#4CAF5018":"#E8930018"}`,borderRadius:6,marginBottom:6}}>
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                {up?<span style={{color:"#4CAF50"}}><I.CheckCirc/></span>:<span style={{fontSize:16}}>{item.icon}</span>}
                <div>
                  <div style={{fontSize:12,color:up?"#666":"#ddd",fontWeight:600,textDecoration:up?"line-through":"none"}}>{item.label}</div>
                  {up&&<div style={{fontSize:10,color:"#4CAF50",marginTop:1}}><I.Doc/> {up.name} · {fDT(up.at)}</div>}
                </div>
              </div>
              {!up&&<label className="av-btn" style={{...S.prim,padding:"6px 14px",fontSize:11,cursor:"pointer",display:"inline-flex",alignItems:"center",gap:4}}><I.Upload/> Upload<input type="file" style={{display:"none"}} onChange={e=>{const f=e.target?.files?.[0];if(f)addDoc(sel.id,{docId:item.docId,name:f.name,at:new Date().toISOString(),by:"dealer"});e.target.value=""}}/></label>}
            </div>);
          })}
        </Sec>}
        <FundingItemsSection deal={sel} role="dealer"/>
        {custActivities.length>0&&<Sec t={`Customer Updates (${custActivities.length})`}>{custActivities.slice().reverse().map((a,i)=><div key={i} style={{padding:"6px 0 6px 12px",borderLeft:"2px solid #4F8EAE",marginLeft:4,marginBottom:4}}><div style={{fontSize:11,color:"#666"}}>{a.detail||({call_cust:"Called customer",email_cust:"Emailed customer"}[a.type]||a.type)}</div><div style={{fontSize:9,color:"#aaa"}}>{fDT(a.at)}</div></div>)}</Sec>}
        <Sec t="Application Status"><div style={{...S.statusCard,borderColor:stg?.color+"33",margin:0}}><div style={{fontSize:11,fontWeight:700,textTransform:"uppercase",letterSpacing:1,color:stg?.color}}>{stg?.label}</div><div style={{fontSize:12,color:"#999",marginTop:4}}>Last updated {fDT(sel.updatedAt)}</div></div></Sec>
        <Sec t="Send a Note to Avanti">
          <div style={{display:"flex",gap:6}}>
            <input value={dlrNote} onChange={e=>setDlrNote(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")submitDlrNote()}} placeholder="Type a message to your account manager..." style={{...S.inp,flex:1,fontSize:12}}/>
            <button className="av-btn" style={{...S.prim,padding:"6px 12px",fontSize:11}} onClick={submitDlrNote}><I.Send/></button>
          </div>
          {myNotes.length>0&&<div style={{marginTop:8}}>
            {myNotes.map(n=><div key={n.id} style={{padding:"6px 10px",background:"#FFFFFF",border:"1px solid #D8DCE0",borderRadius:5,marginBottom:4}}>
              <div style={{fontSize:11,color:"#666",lineHeight:1.4}}>{n.txt}</div>
              <div style={{fontSize:9,color:"#999",marginTop:2}}>{fDT(n.at)}</div>
            </div>)}
          </div>}
        </Sec>
      </div>
    </div></div>
  );
}

// ════════════════════════════════════════════════════════════════
// CUSTOMER PORTAL
// ════════════════════════════════════════════════════════════════

function CustDash({did,out}) {
  const {deals,lenders,acceptDeal,addDoc} = useContext(Ctx);
  const d = deals.find(x=>x.id===did);
  const [tc,setTc]=useState(null);
  if(!d)return(<div style={S.loginWrap}><div style={S.loginCard}><div style={{fontSize:14,color:"#B0413E"}}>Application not found.</div><button className="av-ghost" style={S.ghost} onClick={out}><I.Back/> Try Again</button></div></div>);

  const cs = CUST_STAGES[d.stage];
  const stg = STAGES.find(s=>s.id===d.stage);
  const lndr = lenders.find(l=>l.id===d.lenderId);
  const hideP = d.stage==="declined"||d.stage==="dead";
  const canAcc = d.stage==="approved"&&d.terms&&!d.accepted;
  const pastApproved = ["docs_out","docs_signed","funded"].includes(d.stage);
  const showLH = (d.accepted||pastApproved)&&lndr;

  return(<div style={S.pWrap}><div style={S.custCard}>
    <div style={{textAlign:"center",marginBottom:24}}>
      <div style={{fontSize:12,color:"#999",marginBottom:4}}>Application for</div>
      <div style={{fontSize:22,fontWeight:800,color:"#1a1a2e"}}>{d.biz}</div>
      <div style={{fontSize:13,color:"#888",marginTop:4}}>{d.eqType} · {fmt(d.amount)}</div>
      {(d.eqYear||d.eqMake||d.eqModel)&&<div style={{fontSize:11,color:"#999",marginTop:2}}>{[d.eqYear,d.eqMake,d.eqModel].filter(Boolean).join(" ")}</div>}
      {d.eqVin&&<div style={{fontSize:10,color:"#aaa",fontFamily:"'JetBrains Mono',monospace",marginTop:1}}>VIN: {d.eqVin}</div>}
    </div>
    {!hideP&&<div style={{marginBottom:24}}>
      <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}><span style={{fontSize:11,color:"#999"}}>Progress</span><span style={{fontSize:11,fontWeight:700,color:stg?.color}}>{cs?.progress}%</span></div>
      <div style={S.progTrack}><div style={{...S.progFill,width:`${cs?.progress||0}%`,background:`linear-gradient(90deg,#4F8EAE,${stg?.color})`,animation:"progFill 1s ease-out"}}/></div>
      <div style={{display:"flex",justifyContent:"space-between",marginTop:10}}>{STAGES.filter(s=>!["declined","dead"].includes(s.id)).map(s=>{const on=s.order<=(stg?.order||0);return(<div key={s.id} style={{textAlign:"center",flex:1}}><div style={{width:9,height:9,borderRadius:"50%",background:on?s.color:"#D8DCE0",border:`2px solid ${on?s.color:"#CDD1D6"}`,margin:"0 auto",transition:"all .3s"}}/><div style={{fontSize:7,color:on?"#888":"#333",marginTop:3,textTransform:"uppercase",letterSpacing:.4}}>{s.label}</div></div>)})}</div>
    </div>}
    <div style={{...S.statusCard,borderColor:stg?.color+"33"}}><div style={{fontSize:10,fontWeight:700,textTransform:"uppercase",letterSpacing:1.5,color:stg?.color}}>Current Status</div><div style={{fontSize:18,fontWeight:700,color:"#1a1a2e",margin:"5px 0"}}>{cs?.label}</div><div style={{fontSize:13,color:"#999",lineHeight:1.6}}>{cs?.desc}</div></div>
    {d.terms&&<div style={{marginTop:18}}><div style={S.approvalBox}>
      <div style={{fontSize:10,fontWeight:700,textTransform:"uppercase",letterSpacing:1.5,color:"#4CAF50",marginBottom:10}}>Approval Details</div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}><FB l="Approved Amount" v={fmt(d.terms.amt)} vs={{color:"#4CAF50",fontSize:17}}/><FB l="Monthly Payment" v={fmt(d.terms.pmt)} vs={{fontSize:17}}/><FB l="Term" v={`${d.terms.months} months`}/></div>
      {canAcc&&<div style={{marginTop:14}}>
        {d.showTaxChoice&&<div style={{marginBottom:10}}><div style={{fontSize:11,fontWeight:600,color:"#666",marginBottom:6}}>Would you like to finance your sales tax?</div><div style={{display:"flex",gap:8}}>{[{v:true,l:"Yes, include in financing"},{v:false,l:"No, I'll pay separately"}].map(o=><button key={String(o.v)} onClick={()=>setTc(o.v)} style={{flex:1,padding:"10px",fontSize:12,fontWeight:600,background:tc===o.v?"#4F8EAE22":"#FFFFFF",border:`1px solid ${tc===o.v?"#4F8EAE":"#D8DCE0"}`,color:tc===o.v?"#4F8EAE":"#888",borderRadius:7,cursor:"pointer",fontFamily:"inherit"}}>{o.l}</button>)}</div></div>}
        <button className="av-btn" style={{...S.prim,width:"100%",padding:"12px",fontSize:14,justifyContent:"center",background:"#4CAF50"}} onClick={()=>{if(d.showTaxChoice&&tc===null)return;acceptDeal(d.id,tc)}}><I.Check/> Accept Approval</button>
      </div>}
      {d.accepted&&<div style={{marginTop:10,padding:"8px 12px",background:"#4CAF5012",border:"1px solid #4CAF5033",borderRadius:6,color:"#4CAF50",fontSize:11,fontWeight:600}}>✓ Accepted{d.taxChoice!==null&&<span style={{color:"#999",fontWeight:400}}> · Tax: {d.taxChoice?"Financed":"Separate"}</span>}</div>}
    </div></div>}
    {(d.accepted||pastApproved)&&<div style={{marginTop:14,padding:"14px 18px",background:"#FFFFFF",border:"1px solid #D8DCE0",borderRadius:8}}>
      <div style={{fontSize:10,fontWeight:700,textTransform:"uppercase",letterSpacing:1.5,color:"#E89300",marginBottom:6}}>Documents Needed to Proceed</div>
      <div style={{fontSize:11,color:"#999",marginBottom:10,lineHeight:1.5}}>Please upload the following documents so we can finalize your approval and get your financing documents prepared.</div>
      {[{docId:"drivers_license",label:"Driver's License (Front & Back)",icon:"🪪"},{docId:"voided_check",label:"Voided Check",icon:"🏦"}].map(item=>{
        const up=(d.docs||[]).find(x=>x.docId===item.docId);
        return(<div key={item.docId} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 12px",background:up?"#4CAF5008":"#E8930006",border:`1px solid ${up?"#4CAF5018":"#E8930018"}`,borderRadius:6,marginBottom:6}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            {up?<span style={{color:"#4CAF50"}}><I.CheckCirc/></span>:<span style={{fontSize:16}}>{item.icon}</span>}
            <div>
              <div style={{fontSize:12,color:up?"#666":"#ddd",fontWeight:600,textDecoration:up?"line-through":"none"}}>{item.label}</div>
              {up&&<div style={{fontSize:10,color:"#4CAF50",marginTop:1}}><I.Doc/> {up.name} · uploaded {fDT(up.at)}</div>}
            </div>
          </div>
          {!up&&<label className="av-btn" style={{...S.prim,padding:"6px 14px",fontSize:11,cursor:"pointer",display:"inline-flex",alignItems:"center",gap:4}}><I.Upload/> Upload<input type="file" style={{display:"none"}} onChange={e=>{const f=e.target?.files?.[0];if(f)addDoc(d.id,{docId:item.docId,name:f.name,at:new Date().toISOString(),by:"customer"});e.target.value=""}}/></label>}
        </div>);
      })}
    </div>}
    {showLH&&<div style={{marginTop:14,padding:"14px 18px",background:"#FFFFFF",border:"1px solid #D8DCE0",borderRadius:8}}><div style={{fontSize:10,fontWeight:700,textTransform:"uppercase",letterSpacing:1.5,color:"#999",marginBottom:6}}>Insurance Lienholder Information</div><div style={{fontSize:11,color:"#999",marginBottom:8,lineHeight:1.5}}>Please provide the following lienholder information to your insurance agent. Your insurance policy must list this entity as the lienholder/loss payee.</div><div style={{fontSize:13,fontWeight:600,color:"#1a1a2e"}}>{lndr.lienholder}</div><div style={{fontSize:11,color:"#999",marginTop:2}}>{lndr.address}</div></div>}
    <div style={{marginTop:20}}><div style={{fontSize:10,fontWeight:700,textTransform:"uppercase",letterSpacing:1.5,color:"#999",marginBottom:8}}>Required Documents</div>{REQ_DOCS.filter(doc=>{const isReq=d.docReqs?.[doc.id]!==undefined?d.docReqs[doc.id]:doc.req;return isReq}).map(doc=>{const up=(d.docs||[]).find(x=>x.docId===doc.id);return(<div key={doc.id} style={{...S.docRow,marginBottom:3}}><div style={{display:"flex",alignItems:"center",gap:7,flex:1}}>{up?<span style={{color:"#4CAF50"}}><I.CheckCirc/></span>:<div style={{width:14,height:14,borderRadius:"50%",border:"1.5px solid #444"}}/>}<span style={{fontSize:11,color:up?"#ccc":"#888"}}>{doc.label}</span></div>{up?<span style={{fontSize:10,color:"#4CAF50"}}>✓</span>:<label className="av-ghost" style={{...S.ghost,padding:"3px 8px",fontSize:10,cursor:"pointer",display:"inline-flex",alignItems:"center",gap:3}}><I.Upload/> Upload<input type="file" style={{display:"none"}} onChange={e=>{const f=e.target?.files?.[0];if(f)addDoc(d.id,{docId:doc.id,name:f.name,at:new Date().toISOString(),by:"customer"});e.target.value=""}}/></label>}</div>)})}</div>
    {(d.stips||[]).length>0&&<div style={{marginTop:18}}><ItemsNeededSection deal={d} role="customer"/></div>}
    <div style={{marginTop:20}}><div style={{fontSize:10,fontWeight:700,textTransform:"uppercase",letterSpacing:1.5,color:"#999",marginBottom:8}}>Timeline</div>{(d.history||[]).slice().reverse().map((h,i)=>{const hs=STAGES.find(s=>s.id===h.s),ch=CUST_STAGES[h.s];return(<div key={i} style={{padding:"7px 0 7px 12px",borderLeft:`2px solid ${hs?.color||"#333"}`,marginLeft:6}}><div style={{fontSize:12,fontWeight:600,color:hs?.color}}>{ch?.label||hs?.label}</div><div style={{fontSize:10,color:"#999",marginTop:1}}>{fDT(h.d)}</div></div>)})}</div>
    <div style={S.amCard}><div style={S.amAv}><I.User/></div><div><div style={{fontSize:13,fontWeight:600,color:"#1a1a2e"}}>{AM.name}</div><div style={{fontSize:11,color:"#888"}}>{AM.title}</div><div style={{fontSize:12,color:"#4F8EAE",marginTop:2}}>{AM.phone} · {AM.email}</div></div></div>
  </div></div>);
}

// ════════════════════════════════════════════════════════════════
// CREDIT APPLICATION PAGE
// ════════════════════════════════════════════════════════════════

const APP_STEPS=["Business Info","Ownership","Bank & References","Equipment","Review & Submit"];
const APP_ICONS=["🏢","👤","🏦","🚛","✓"];
const LEGAL_STRUCTS=["LLC","Corporation","Sole Proprietorship","Partnership","S-Corp","Other"];
const EMPTY_APP={bizName:"",dba:"",bizType:"",bizAddress:"",bizCity:"",bizState:"",bizZip:"",bizPhone:"",bizEmail:"",fedId:"",legalStructure:"",employees:"",dateStarted:"",
o1Name:"",o1Title:"",o1Pct:"100",o1Address:"",o1City:"",o1State:"",o1Zip:"",o1Cell:"",o1Ssn:"",o1Dob:"",
o2Name:"",o2Title:"",o2Pct:"",o2Address:"",o2City:"",o2State:"",o2Zip:"",o2Cell:"",o2Ssn:"",o2Dob:"",hasOwner2:false,
bankName:"",bankAcct:"",bankPhone:"",bankContact:"",trade1:"",trade1Ph:"",trade2:"",trade2Ph:"",
vendorName:"",vendorPhone:"",salesPerson:"",
eqType:"",eqDesc:"",eqYear:"",eqMake:"",eqModel:"",eqVin:"",eqMileage:"",newUsed:"New",price:"",termReq:"",
agreeTerms:false,dealerCode:""};

function AF({l,v,ch,type="text",ph,required,err,span,sensitive,mono}){const[show,setShow]=useState(!sensitive);return(<div style={span===2?{gridColumn:"1/-1"}:{}}>
    <label style={{display:"block",fontSize:10,fontWeight:600,color:err?"#B0413E":"#666",marginBottom:3,textTransform:"uppercase",letterSpacing:.5}}>{l}{required&&<span style={{color:"#B0413E",marginLeft:2}}>*</span>}</label>
    <div style={{position:"relative"}}><input type={sensitive&&!show?"password":type} value={v} onChange={e=>ch(e.target.value)} placeholder={ph||""} style={{width:"100%",padding:"9px 12px",fontSize:13,color:"#1a1a2e",background:"#0E0E10",border:`1px solid ${err?"#B0413E44":"#D8DCE0"}`,borderRadius:6,fontFamily:mono?"'JetBrains Mono',monospace":"inherit",outline:"none",transition:"border .15s"}} onFocus={e=>{e.target.style.borderColor="#4F8EAE"}} onBlur={e=>{e.target.style.borderColor=err?"#B0413E44":"#D8DCE0"}}/>
    {sensitive&&<button onClick={()=>setShow(!show)} style={{position:"absolute",right:8,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",color:"#999",cursor:"pointer",fontSize:10,fontFamily:"inherit"}}>{show?"Hide":"Show"}</button>}</div>
    {err&&<div style={{fontSize:10,color:"#B0413E",marginTop:2}}>{err}</div>}
  </div>)}
function ASel({l,v,ch,opts,required,err}){return(<div>
    <label style={{display:"block",fontSize:10,fontWeight:600,color:err?"#B0413E":"#666",marginBottom:3,textTransform:"uppercase",letterSpacing:.5}}>{l}{required&&<span style={{color:"#B0413E",marginLeft:2}}>*</span>}</label>
    <select value={v} onChange={e=>ch(e.target.value)} style={{width:"100%",padding:"9px 12px",fontSize:13,color:v?"#e0e0e8":"#555",background:"#0E0E10",border:`1px solid ${err?"#B0413E44":"#D8DCE0"}`,borderRadius:6,fontFamily:"inherit",cursor:"pointer",outline:"none"}}><option value="">Select...</option>{opts.map(o=><option key={o} value={o}>{o}</option>)}</select>
    {err&&<div style={{fontSize:10,color:"#B0413E",marginTop:2}}>{err}</div>}
  </div>)}
function ASecL({text}){return <div style={{fontSize:10,fontWeight:700,color:"#4F8EAE",textTransform:"uppercase",letterSpacing:.8,marginBottom:8,marginTop:4,paddingBottom:4,borderBottom:"1px solid #E2E5E9"}}>{text}</div>}
const ag2={display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px 12px"};

function SignaturePad({onSign,signature}){
  const canvasRef=useRef(null);
  const [drawing,setDrawing]=useState(false);
  const [hasDrawn,setHasDrawn]=useState(false);
  const lastPos=useRef(null);

  const getPos=(e)=>{
    const canvas=canvasRef.current;
    const rect=canvas.getBoundingClientRect();
    const scaleX=canvas.width/rect.width;
    const scaleY=canvas.height/rect.height;
    if(e.touches){return{x:(e.touches[0].clientX-rect.left)*scaleX,y:(e.touches[0].clientY-rect.top)*scaleY}}
    return{x:(e.clientX-rect.left)*scaleX,y:(e.clientY-rect.top)*scaleY};
  };

  const startDraw=(e)=>{
    e.preventDefault();
    const ctx=canvasRef.current.getContext("2d");
    const pos=getPos(e);
    ctx.beginPath();
    ctx.moveTo(pos.x,pos.y);
    lastPos.current=pos;
    setDrawing(true);
    setHasDrawn(true);
  };

  const draw=(e)=>{
    if(!drawing)return;
    e.preventDefault();
    const ctx=canvasRef.current.getContext("2d");
    const pos=getPos(e);
    ctx.strokeStyle="#e0e0e8";
    ctx.lineWidth=2.5;
    ctx.lineCap="round";
    ctx.lineJoin="round";
    ctx.beginPath();
    ctx.moveTo(lastPos.current.x,lastPos.current.y);
    ctx.lineTo(pos.x,pos.y);
    ctx.stroke();
    lastPos.current=pos;
  };

  const endDraw=(e)=>{
    if(!drawing)return;
    e?.preventDefault();
    setDrawing(false);
    lastPos.current=null;
    if(hasDrawn){
      const dataUrl=canvasRef.current.toDataURL("image/png");
      onSign(dataUrl);
    }
  };

  const clear=()=>{
    const canvas=canvasRef.current;
    const ctx=canvas.getContext("2d");
    ctx.clearRect(0,0,canvas.width,canvas.height);
    setHasDrawn(false);
    onSign(null);
  };

  useEffect(()=>{
    if(signature&&canvasRef.current){
      const img=new Image();
      img.onload=()=>{
        const ctx=canvasRef.current.getContext("2d");
        ctx.clearRect(0,0,canvasRef.current.width,canvasRef.current.height);
        ctx.drawImage(img,0,0);
        setHasDrawn(true);
      };
      img.src=signature;
    }
  },[]);

  return(<div>
    <div style={{fontSize:10,fontWeight:700,color:"#4F8EAE",textTransform:"uppercase",letterSpacing:.8,marginBottom:8}}>Electronic Signature</div>
    <div style={{position:"relative",background:"#FFFFFF",border:"1px solid #D8DCE0",borderRadius:8,overflow:"hidden"}}>
      <canvas ref={canvasRef} width={560} height={160}
        style={{width:"100%",height:120,cursor:"crosshair",touchAction:"none"}}
        onMouseDown={startDraw} onMouseMove={draw} onMouseUp={endDraw} onMouseLeave={endDraw}
        onTouchStart={startDraw} onTouchMove={draw} onTouchEnd={endDraw}
      />
      {!hasDrawn&&<div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",pointerEvents:"none"}}>
        <div style={{color:"#666",fontSize:13}}>Draw your signature here</div>
      </div>}
      <div style={{position:"absolute",bottom:8,left:12,right:12,borderTop:"1px solid #CDD1D6"}}/>
    </div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:6}}>
      <div style={{fontSize:10,color:"#aaa"}}>Sign using your mouse or finger on mobile</div>
      {hasDrawn&&<button onClick={clear} style={{fontSize:10,color:"#B0413E",background:"none",border:"1px solid #B0413E33",borderRadius:5,padding:"3px 10px",cursor:"pointer",fontFamily:"inherit"}}>Clear</button>}
    </div>
  </div>);
}

function ApplyPage(){
  const {saveDeal,dealers,notify}=useContext(Ctx);
  const [step,setStep]=useState(0);
  const [f,sf]=useState(EMPTY_APP);
  const [submitted,setSubmitted]=useState(false);
  const [submitting,setSubmitting]=useState(false);
  const [errors,setErrors]=useState({});
  const [dealCode,setDealCode]=useState("");
  const [signature,setSignature]=useState(null);
  const topRef=useRef(null);
  const u=(k,v)=>{sf(p=>({...p,[k]:v}));setErrors(p=>({...p,[k]:undefined}))};

  const validateStep=()=>{
    const e={};
    if(step===0){if(!f.bizName.trim())e.bizName="Required";if(!f.bizEmail.trim())e.bizEmail="Required";if(!f.bizPhone.trim())e.bizPhone="Required";if(!f.bizAddress.trim())e.bizAddress="Required";if(!f.bizCity.trim())e.bizCity="Required";if(!f.bizState.trim())e.bizState="Required";if(!f.bizZip.trim())e.bizZip="Required";if(!f.legalStructure.trim())e.legalStructure="Required";if(!f.dateStarted.trim())e.dateStarted="Required"}
    if(step===1){if(!f.o1Name.trim())e.o1Name="Required";if(!f.o1Cell.trim())e.o1Cell="Required";if(!f.o1Ssn.trim())e.o1Ssn="Required";if(!f.o1Dob.trim())e.o1Dob="Required";if(!f.o1Address.trim())e.o1Address="Required"}
    if(step===3){if(!f.eqType)e.eqType="Required";if(!f.price.trim())e.price="Required"}
    setErrors(e);return Object.keys(e).length===0;
  };
  const next=()=>{if(validateStep()){setStep(s=>Math.min(s+1,4));topRef.current?.scrollIntoView({behavior:"smooth"})}};
  const back=()=>{setStep(s=>Math.max(s-1,0));topRef.current?.scrollIntoView({behavior:"smooth"})};

  const submit=async()=>{
    const e={};
    if(!f.agreeTerms)e.agreeTerms="You must agree to continue";
    if(!signature)e.signature="Signature is required";
    if(Object.keys(e).length){setErrors(e);return}
    setSubmitting(true);
    await new Promise(r=>setTimeout(r,1500));
    const code=f.bizName.replace(/[^A-Z]/gi,"").slice(0,3).toUpperCase()+"-"+Math.floor(1000+Math.random()*9000);
    const dlr=dealers.find(d=>d.code===f.dealerCode);
    const signedAt=new Date().toISOString();
    saveDeal({biz:f.bizName,contact:f.o1Name,email:f.bizEmail,phone:f.bizPhone,eqType:f.eqType,eqDetail:[f.eqYear,f.eqMake,f.eqModel].filter(Boolean).join(" ")||f.eqDesc,eqYear:f.eqYear,eqMake:f.eqMake,eqModel:f.eqModel,eqVin:f.eqVin,eqMileage:f.eqMileage,amount:parseFloat(f.price.replace(/[^0-9.]/g,""))||0,stage:"new_lead",source:dlr?"Dealer Referral":"Website Application",dealerId:dlr?.id||null,dealerName:dlr?.name||"",code,notes:`Applied online. Structure: ${f.legalStructure||"N/A"}. EIN: ${f.fedId||"N/A"}. Started: ${f.dateStarted||"N/A"}.`,
      appData:{bizName:f.bizName,dba:f.dba,bizType:f.bizType,bizAddress:f.bizAddress,bizCity:f.bizCity,bizState:f.bizState,bizZip:f.bizZip,bizPhone:f.bizPhone,bizEmail:f.bizEmail,fedId:f.fedId,legalStructure:f.legalStructure,employees:f.employees,dateStarted:f.dateStarted,
      o1Name:f.o1Name,o1Title:f.o1Title,o1Pct:f.o1Pct,o1Address:f.o1Address,o1City:f.o1City,o1State:f.o1State,o1Zip:f.o1Zip,o1Cell:f.o1Cell,o1Ssn:f.o1Ssn,o1Dob:f.o1Dob,
      o2Name:f.o2Name,o2Title:f.o2Title,o2Pct:f.o2Pct,o2Address:f.o2Address,o2City:f.o2City,o2State:f.o2State,o2Zip:f.o2Zip,o2Cell:f.o2Cell,o2Ssn:f.o2Ssn,o2Dob:f.o2Dob,hasOwner2:f.hasOwner2,
      bankName:f.bankName,bankAcct:f.bankAcct,bankPhone:f.bankPhone,bankContact:f.bankContact,trade1:f.trade1,trade1Ph:f.trade1Ph,trade2:f.trade2,trade2Ph:f.trade2Ph,
      vendorName:f.vendorName,vendorPhone:f.vendorPhone,salesPerson:f.salesPerson,
      eqDesc:f.eqDesc,newUsed:f.newUsed,price:f.price,termReq:f.termReq,dealerCode:f.dealerCode,
      signature:signature,signedAt:signedAt,signedBy:f.o1Name||f.bizName}
    });
    setDealCode(code);setSubmitted(true);setSubmitting(false);notify("Application submitted!");
  };

  if(submitted)return(
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:40,minHeight:"80vh"}}>
      <div style={{textAlign:"center",maxWidth:480,animation:"fadeIn .5s"}}>
        <div style={{width:64,height:64,borderRadius:"50%",background:"#4CAF5018",border:"2px solid #4CAF50",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 20px"}}><I.Check/></div>
        <h2 style={{fontSize:24,fontWeight:800,color:"#1a1a2e",marginBottom:8}}>Application Submitted!</h2>
        <p style={{fontSize:14,color:"#999",lineHeight:1.6,marginBottom:24}}>Thank you, {f.bizName}. Your credit application has been received and is being reviewed.</p>
        <div style={{background:"#F5F6F8",border:"1px solid #D8DCE0",borderRadius:10,padding:"18px 24px",marginBottom:24}}>
          <div style={{fontSize:10,color:"#999",textTransform:"uppercase",letterSpacing:1,fontWeight:600,marginBottom:6}}>Your Application Code</div>
          <div style={{fontSize:24,fontWeight:800,color:"#4F8EAE",letterSpacing:2,fontFamily:"'JetBrains Mono',monospace"}}>{dealCode}</div>
          <div style={{fontSize:11,color:"#999",marginTop:6}}>Save this code to check your status on the Customer Portal</div>
        </div>
        <div style={{background:"#F5F6F8",border:"1px solid #D8DCE0",borderRadius:10,padding:"16px 20px",textAlign:"left"}}>
          <div style={{fontSize:11,fontWeight:700,color:"#666",marginBottom:8}}>What happens next?</div>
          {["Our team will review your application within 1 business day","Your application will be processed through our underwriting team","You'll receive a call or email with approval details"].map((t,i)=><div key={i} style={{display:"flex",gap:8,padding:"5px 0",fontSize:12,color:"#888"}}><span style={{color:"#4F8EAE",fontWeight:700,fontSize:11}}>{i+1}.</span>{t}</div>)}
        </div>
        <div style={S.amCard}><div style={S.amAv}><I.User/></div><div><div style={{fontSize:13,fontWeight:600,color:"#1a1a2e"}}>{AM.name}</div><div style={{fontSize:11,color:"#888"}}>{AM.title}</div><div style={{fontSize:12,color:"#4F8EAE",marginTop:2}}>{AM.phone} · {AM.email}</div></div></div>
      </div>
    </div>
  );

  return(<div style={{maxWidth:680,margin:"0 auto",padding:"24px 20px 60px"}} ref={topRef}>
    <div style={{textAlign:"center",marginBottom:28}}>
      <h1 style={{fontSize:24,fontWeight:800,color:"#1a1a2e",marginBottom:6}}>Credit Application</h1>
      <p style={{fontSize:13,color:"#999",maxWidth:440,margin:"0 auto",lineHeight:1.5}}>Complete the form below to apply for equipment financing. Your information is encrypted and secure.</p>
    </div>
    {/* Steps */}
    <div style={{display:"flex",gap:2,marginBottom:28}}>{APP_STEPS.map((s,i)=><div key={s} style={{flex:1,cursor:i<step?"pointer":"default"}} onClick={()=>i<step&&setStep(i)}>
      <div style={{height:3,borderRadius:2,transition:"all .4s",background:i<step?"#4F8EAE":i===step?"#4F8EAE":"#E2E5E9"}}/>
      <div style={{display:"flex",alignItems:"center",gap:4,marginTop:6,justifyContent:"center"}}><span style={{fontSize:10,opacity:i<=step?1:.3}}>{APP_ICONS[i]}</span><span style={{fontSize:9,fontWeight:600,color:i<=step?"#999":"#333",letterSpacing:.3,textTransform:"uppercase"}}>{s}</span></div>
    </div>)}</div>
    {/* Form Card */}
    <div style={{background:"#F5F6F8",border:"1px solid #D8DCE0",borderRadius:12,overflow:"hidden"}}>
      <div style={{padding:"24px 22px"}}>
        {step===0&&<><div style={{marginBottom:16}}><h2 style={{fontSize:17,fontWeight:700,color:"#1a1a2e",marginBottom:4}}>Business Information</h2><p style={{fontSize:12,color:"#999"}}>Tell us about your company.</p></div>
        <div style={ag2}><AF l="Legal Business Name" v={f.bizName} ch={v=>u("bizName",v)} required err={errors.bizName} span={2}/><AF l="DBA (if different)" v={f.dba} ch={v=>u("dba",v)}/><AF l="Business Type / Industry" v={f.bizType} ch={v=>u("bizType",v)}/>
        <AF l="Street Address" v={f.bizAddress} ch={v=>u("bizAddress",v)} required err={errors.bizAddress} span={2}/><AF l="City" v={f.bizCity} ch={v=>u("bizCity",v)} required err={errors.bizCity}/>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><AF l="State" v={f.bizState} ch={v=>u("bizState",v)} required err={errors.bizState}/><AF l="ZIP" v={f.bizZip} ch={v=>u("bizZip",v)} required err={errors.bizZip}/></div>
        <AF l="Business Phone" v={f.bizPhone} ch={v=>u("bizPhone",v)} type="tel" required err={errors.bizPhone}/><AF l="Business Email" v={f.bizEmail} ch={v=>u("bizEmail",v)} type="email" required err={errors.bizEmail}/>
        <AF l="Federal Tax ID (EIN)" v={f.fedId} ch={v=>u("fedId",v)} ph="XX-XXXXXXX"/><ASel l="Legal Structure" v={f.legalStructure} ch={v=>u("legalStructure",v)} opts={LEGAL_STRUCTS} required err={errors.legalStructure}/>
        <AF l="# of Employees" v={f.employees} ch={v=>u("employees",v)} type="number"/><AF l="Date Business Started" v={f.dateStarted} ch={v=>u("dateStarted",v)} ph="MM/YYYY" required err={errors.dateStarted}/></div></>}

        {step===1&&<><div style={{marginBottom:16}}><h2 style={{fontSize:17,fontWeight:700,color:"#1a1a2e",marginBottom:4}}>Ownership Information</h2><p style={{fontSize:12,color:"#999"}}>Primary owner/guarantor details required for credit check.</p></div>
        <ASecL text="Owner / Guarantor 1"/>
        <div style={ag2}><AF l="Full Legal Name" v={f.o1Name} ch={v=>u("o1Name",v)} required err={errors.o1Name}/><AF l="Title" v={f.o1Title} ch={v=>u("o1Title",v)} ph="e.g. Owner, CEO"/>
        <AF l="% Ownership" v={f.o1Pct} ch={v=>u("o1Pct",v)} ph="100"/><AF l="Cell Phone" v={f.o1Cell} ch={v=>u("o1Cell",v)} type="tel" required err={errors.o1Cell}/>
        <AF l="Home Address" v={f.o1Address} ch={v=>u("o1Address",v)} required err={errors.o1Address} span={2}/><AF l="City" v={f.o1City} ch={v=>u("o1City",v)}/>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><AF l="State" v={f.o1State} ch={v=>u("o1State",v)}/><AF l="ZIP" v={f.o1Zip} ch={v=>u("o1Zip",v)}/></div>
        <AF l="Social Security #" v={f.o1Ssn} ch={v=>u("o1Ssn",v)} ph="XXX-XX-XXXX" required err={errors.o1Ssn} sensitive/><AF l="Date of Birth" v={f.o1Dob} ch={v=>u("o1Dob",v)} ph="MM/DD/YYYY" required err={errors.o1Dob}/></div>
        <div style={{marginTop:16}}><label style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",fontSize:12,color:"#999"}}><input type="checkbox" checked={f.hasOwner2} onChange={e=>u("hasOwner2",e.target.checked)} style={{width:16,height:16,accentColor:"#4F8EAE"}}/>Add a second owner / guarantor</label></div>
        {f.hasOwner2&&<><ASecL text="Owner / Guarantor 2"/>
        <div style={ag2}><AF l="Full Legal Name" v={f.o2Name} ch={v=>u("o2Name",v)}/><AF l="Title" v={f.o2Title} ch={v=>u("o2Title",v)}/>
        <AF l="% Ownership" v={f.o2Pct} ch={v=>u("o2Pct",v)}/><AF l="Cell Phone" v={f.o2Cell} ch={v=>u("o2Cell",v)} type="tel"/>
        <AF l="Home Address" v={f.o2Address} ch={v=>u("o2Address",v)} span={2}/><AF l="City" v={f.o2City} ch={v=>u("o2City",v)}/>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><AF l="State" v={f.o2State} ch={v=>u("o2State",v)}/><AF l="ZIP" v={f.o2Zip} ch={v=>u("o2Zip",v)}/></div>
        <AF l="Social Security #" v={f.o2Ssn} ch={v=>u("o2Ssn",v)} ph="XXX-XX-XXXX" sensitive/><AF l="Date of Birth" v={f.o2Dob} ch={v=>u("o2Dob",v)} ph="MM/DD/YYYY"/></div></>}</>}

        {step===2&&<><div style={{marginBottom:16}}><h2 style={{fontSize:17,fontWeight:700,color:"#1a1a2e",marginBottom:4}}>Bank & Trade References</h2><p style={{fontSize:12,color:"#999"}}>Your business banking and trade reference information.</p></div>
        <ASecL text="Business Bank"/><div style={ag2}><AF l="Bank Name" v={f.bankName} ch={v=>u("bankName",v)}/><AF l="Account Number" v={f.bankAcct} ch={v=>u("bankAcct",v)} sensitive/><AF l="Bank Phone" v={f.bankPhone} ch={v=>u("bankPhone",v)} type="tel"/><AF l="Contact Name" v={f.bankContact} ch={v=>u("bankContact",v)}/></div>
        <ASecL text="Trade References"/><div style={ag2}><AF l="Trade Reference #1" v={f.trade1} ch={v=>u("trade1",v)} ph="Company name"/><AF l="Phone" v={f.trade1Ph} ch={v=>u("trade1Ph",v)} type="tel"/><AF l="Trade Reference #2" v={f.trade2} ch={v=>u("trade2",v)} ph="Company name"/><AF l="Phone" v={f.trade2Ph} ch={v=>u("trade2Ph",v)} type="tel"/></div></>}

        {step===3&&<><div style={{marginBottom:16}}><h2 style={{fontSize:17,fontWeight:700,color:"#1a1a2e",marginBottom:4}}>Equipment Information</h2><p style={{fontSize:12,color:"#999"}}>Details about the equipment you're looking to finance.</p></div>
        <div style={ag2}><ASel l="Equipment Type" v={f.eqType} ch={v=>u("eqType",v)} opts={EQ_TYPES} required err={errors.eqType}/>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><ASel l="New / Used" v={f.newUsed} ch={v=>u("newUsed",v)} opts={["New","Used"]}/><AF l="Year" v={f.eqYear} ch={v=>u("eqYear",v)} ph="2025"/></div>
        <AF l="Make" v={f.eqMake} ch={v=>u("eqMake",v)} ph="e.g. Kenworth, CAT"/><AF l="Model" v={f.eqModel} ch={v=>u("eqModel",v)} ph="e.g. T370, 320"/>
        <AF l="Equipment Description" v={f.eqDesc} ch={v=>u("eqDesc",v)} span={2} ph="Full description"/><AF l="VIN / Serial #" v={f.eqVin} ch={v=>u("eqVin",v.toUpperCase())} ph="If known" mono/><AF l="Mileage / Hours" v={f.eqMileage} ch={v=>u("eqMileage",v)}/>
        <AF l="Purchase Price" v={f.price} ch={v=>u("price",v)} required err={errors.price} ph="$0"/><AF l="Term Requested" v={f.termReq} ch={v=>u("termReq",v)} ph="e.g. 60 months"/></div>
        <ASecL text="Vendor / Dealer"/><div style={ag2}><AF l="Vendor / Dealer Name" v={f.vendorName} ch={v=>u("vendorName",v)}/><AF l="Vendor Phone" v={f.vendorPhone} ch={v=>u("vendorPhone",v)} type="tel"/><AF l="Sales Person" v={f.salesPerson} ch={v=>u("salesPerson",v)}/></div>
        <div style={{marginTop:12}}><AF l="Dealer Referral Code (optional)" v={f.dealerCode} ch={v=>u("dealerCode",v.toUpperCase())} ph="e.g. NTS2026" mono/></div></>}

        {step===4&&<><div style={{marginBottom:16}}><h2 style={{fontSize:17,fontWeight:700,color:"#1a1a2e",marginBottom:4}}>Review & Submit</h2><p style={{fontSize:12,color:"#999"}}>Please review your information before submitting.</p></div>
        {[{t:"Business",items:[["Legal Name",f.bizName],["DBA",f.dba],["Type",f.bizType],["Address",[f.bizAddress,f.bizCity,f.bizState,f.bizZip].filter(Boolean).join(", ")],["Phone",f.bizPhone],["Email",f.bizEmail],["EIN",f.fedId],["Structure",f.legalStructure]]},
          {t:"Owner 1",items:[["Name",f.o1Name],["Title",f.o1Title],["Ownership",f.o1Pct?f.o1Pct+"%":""],["Cell",f.o1Cell],["SSN",f.o1Ssn?"•••-••-"+f.o1Ssn.slice(-4):""],["DOB",f.o1Dob]]},
          ...(f.hasOwner2&&f.o2Name?[{t:"Owner 2",items:[["Name",f.o2Name],["Title",f.o2Title],["Ownership",f.o2Pct?f.o2Pct+"%":""],["Cell",f.o2Cell],["SSN",f.o2Ssn?"•••-••-"+f.o2Ssn.slice(-4):""],["DOB",f.o2Dob]]}]:[]),
          {t:"Equipment",items:[["Type",f.eqType],["Description",[f.eqYear,f.eqMake,f.eqModel].filter(Boolean).join(" ")||f.eqDesc],["Condition",f.newUsed],["Price",f.price],["Term",f.termReq],["VIN",f.eqVin],["Vendor",f.vendorName]]}
        ].map(sec=><div key={sec.t} style={{marginBottom:14,background:"#FFFFFF",border:"1px solid #E2E5E9",borderRadius:8,overflow:"hidden"}}>
          <div style={{padding:"8px 12px",borderBottom:"1px solid #E2E5E9",fontSize:10,fontWeight:700,color:"#4F8EAE",textTransform:"uppercase",letterSpacing:.8}}>{sec.t}</div>
          <div style={{padding:"8px 12px",display:"grid",gridTemplateColumns:"1fr 1fr",gap:"4px 16px"}}>{sec.items.filter(([,v])=>v).map(([l,v])=><div key={l} style={{display:"flex",gap:6,fontSize:12,padding:"2px 0"}}><span style={{color:"#999",minWidth:80}}>{l}:</span><span style={{color:"#666",fontWeight:500}}>{v}</span></div>)}</div>
        </div>)}
        <div style={{marginTop:12,padding:14,background:"#FFFFFF",border:"1px solid #E2E5E9",borderRadius:8}}>
          <SignaturePad onSign={setSignature} signature={signature}/>
          {errors.signature&&<div style={{fontSize:11,color:"#B0413E",marginTop:4}}>{errors.signature}</div>}
          {signature&&<div style={{marginTop:8,padding:"8px 12px",background:"#4CAF5008",border:"1px solid #4CAF5022",borderRadius:6}}>
            <div style={{fontSize:10,color:"#4CAF50",fontWeight:600}}>✓ Electronically signed by {f.o1Name||f.bizName||"Applicant"}</div>
            <div style={{fontSize:9,color:"#999",marginTop:2}}>{new Date().toLocaleString("en-US",{weekday:"long",year:"numeric",month:"long",day:"numeric",hour:"numeric",minute:"2-digit",timeZoneName:"short"})}</div>
          </div>}
        </div>
        <div style={{marginTop:12,padding:14,background:"#FFFFFF",border:"1px solid #E2E5E9",borderRadius:8}}>
          <div style={{fontSize:10,color:"#999",lineHeight:1.5,marginBottom:12}}>By submitting this application, I certify that the information contained herein is true and accurate. I authorize Avanti Equipment Finance, LLC or its designee(s) to obtain information from any business or consumer reporting agencies or other sources that provide credit reports, account history, and employment information.</div>
          <label style={{display:"flex",alignItems:"flex-start",gap:8,cursor:"pointer"}}><input type="checkbox" checked={f.agreeTerms} onChange={e=>u("agreeTerms",e.target.checked)} style={{width:18,height:18,accentColor:"#4F8EAE",cursor:"pointer",marginTop:1,flexShrink:0}}/><span style={{fontSize:12,color:"#666",fontWeight:500}}>I agree to the terms above and authorize credit inquiries</span></label>
          {errors.agreeTerms&&<div style={{fontSize:11,color:"#B0413E",marginTop:4,marginLeft:26}}>{errors.agreeTerms}</div>}
        </div></>}
      </div>
      {/* Nav */}
      <div style={{padding:"14px 22px",borderTop:"1px solid #E2E5E9",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        {step>0?<button className="av-ghost" style={{...S.ghost,padding:"10px 20px",fontSize:13}} onClick={back}>← Back</button>:<div/>}
        {step<4?<button className="av-btn" style={{...S.prim,padding:"10px 28px",fontSize:13}} onClick={next}>Continue →</button>
        :<button className="av-btn" style={{...S.prim,padding:"12px 32px",fontSize:14,background:submitting?"#3A6F8F":"linear-gradient(135deg,#4F8EAE,#3A6F8F)"}} disabled={submitting} onClick={submit}>{submitting?"Submitting...":"Submit Application ✓"}</button>}
      </div>
    </div>
    <div style={{textAlign:"center",marginTop:16,fontSize:10,color:"#aaa",display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>
      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#444" strokeWidth="2"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0110 0v4"/></svg>
      256-bit SSL Encrypted · Your information is never shared without your consent
    </div>
    {f.dealerCode&&<div style={{textAlign:"center",marginTop:10,fontSize:10,color:"#4F8EAE66"}}>Referred by dealer: {f.dealerCode}</div>}
  </div>);
}

// ════════════════════════════════════════════════════════════════
// LOGIN SCREEN
// ════════════════════════════════════════════════════════════════

function Login({type,onGo}) {
  const [email,setEmail]=useState("");
  const [pw,setPw]=useState("");
  const [err,setErr]=useState("");
  const [showPw,setShowPw]=useState(false);
  const [loading,setLoading]=useState(false);
  const [attempts,setAttempts]=useState(0);
  const [lockUntil,setLockUntil]=useState(0);
  const [lockMsg,setLockMsg]=useState("");
  // MFA state
  const [mfaStep,setMfaStep]=useState(false);
  const [mfaCode,setMfaCode]=useState("");
  const [mfaFactorId,setMfaFactorId]=useState(null);
  const [mfaChallengeId,setMfaChallengeId]=useState(null);
  const isCrm=type==="crm";
  const isDlr=type==="dealer";
  const isCust=type==="customer";
  const title=isCrm?"Admin Login":isDlr?"Dealer Portal Login":"Customer Portal Login";
  const desc=isCrm?"Sign in with your admin credentials to manage deals, dealers, and your pipeline.":isDlr?"Enter your dealer email and password to view referred deals and submit new applications.":"Enter the email and password provided by your account manager to view your financing status.";
  const MAX_ATTEMPTS=5;
  const LOCKOUT_MS=15*60*1000;
  const submit=async()=>{
    const now=Date.now();
    if(lockUntil>now){const mins=Math.ceil((lockUntil-now)/60000);setLockMsg(`Account locked. Try again in ${mins} minute${mins!==1?"s":""}.`);return}
    if(!email.trim()){setErr("Enter your email address");return}
    if(!pw.trim()){setErr("Enter your password");return}
    setLoading(true);setErr("");
    try{
      if(isCrm){
        const result = await onGo(email.trim(),pw.trim());
        if(result?.ok){setLoading(false);return}
        if(result?.mfa){
          // Start MFA challenge
          const {supabase} = await import('../../lib/supabase');
          const {data:factors} = await supabase.auth.mfa.listFactors();
          if(factors?.totp?.length){
            const factor=factors.totp[0];
            setMfaFactorId(factor.id);
            const {data:challenge,error:cErr} = await supabase.auth.mfa.challenge({factorId:factor.id});
            if(cErr){setErr(cErr.message);setLoading(false);return}
            setMfaChallengeId(challenge.id);
            setMfaStep(true);
          }
          setLoading(false);return;
        }
        const next=attempts+1;
        setAttempts(next);
        if(next>=MAX_ATTEMPTS){setLockUntil(now+LOCKOUT_MS);setLockMsg("Too many failed attempts. Account locked for 15 minutes.");setErr("")}
        else{setErr(result?.error||`Invalid email or password. ${MAX_ATTEMPTS-next} attempt${MAX_ATTEMPTS-next!==1?"s":""} remaining.`);setLockMsg("")}
      } else {
        const ok = await onGo(email.trim(),pw.trim());
        if(!ok){
          const next=attempts+1;setAttempts(next);
          if(next>=MAX_ATTEMPTS){setLockUntil(now+LOCKOUT_MS);setLockMsg("Too many failed attempts. Account locked for 15 minutes.");setErr("")}
          else{setErr(`Invalid email or password. ${MAX_ATTEMPTS-next} attempt${MAX_ATTEMPTS-next!==1?"s":""} remaining.`)}
        }
      }
    }catch(e){setErr(e.message||"Login failed")}
    setLoading(false);
  };
  const verifyMfa=async()=>{
    if(!mfaCode||mfaCode.length!==6){setErr("Enter 6-digit code");return}
    setLoading(true);
    try{
      const {supabase} = await import('../../lib/supabase');
      const {data,error} = await supabase.auth.mfa.verify({factorId:mfaFactorId,challengeId:mfaChallengeId,code:mfaCode});
      if(error){setErr(error.message);setLoading(false);return}
      // MFA verified, auth listener will pick up the session
    }catch(e){setErr(e.message||"Verification failed")}
    setLoading(false);
  };
  useEffect(()=>{if(lockUntil<=0)return;const iv=setInterval(()=>{const now=Date.now();if(now>=lockUntil){setLockUntil(0);setLockMsg("");setAttempts(0);clearInterval(iv)}else{const mins=Math.ceil((lockUntil-now)/60000);setLockMsg(`Account locked. Try again in ${mins} minute${mins!==1?"s":""}.`)}},5000);return()=>clearInterval(iv)},[lockUntil]);
  
  if(mfaStep) return(<div style={S.loginWrap}><div style={{...S.loginCard,maxWidth:420}}>
    <div style={{width:48,height:48,borderRadius:"50%",background:"#4F8EAE18",display:"flex",alignItems:"center",justifyContent:"center",color:"#4F8EAE",marginBottom:4}}><I.Shield/></div>
    <h2 style={S.loginTitle}>Two-Factor Authentication</h2>
    <p style={S.loginDesc}>Enter the 6-digit code from your authenticator app.</p>
    <div style={{width:"100%",marginTop:8}}>
      <input style={{...S.inp,width:"100%",fontSize:24,padding:"14px",textAlign:"center",letterSpacing:8,fontFamily:"JetBrains Mono"}} type="text" maxLength={6} placeholder="000000" value={mfaCode} onChange={e=>{setMfaCode(e.target.value.replace(/\D/g,""));setErr("")}} onKeyDown={e=>{if(e.key==="Enter")verifyMfa()}} autoFocus/>
    </div>
    {err&&<div style={S.loginErr}>{err}</div>}
    <button className="av-btn" style={{...S.prim,width:"100%",marginTop:14,padding:"12px 20px",fontSize:14,justifyContent:"center",opacity:loading?.4:1}} onClick={verifyMfa} disabled={loading}>{loading?"Verifying...":"Verify"}</button>
    <button className="av-ghost" style={{...S.ghost,width:"100%",marginTop:8,justifyContent:"center"}} onClick={()=>{setMfaStep(false);setMfaCode("");setErr("")}}>← Back to login</button>
  </div></div>);

  return(<div style={S.loginWrap}><div style={{...S.loginCard,maxWidth:420}}>
    <div style={{width:48,height:48,borderRadius:"50%",background:isCrm?"#4F8EAE18":isDlr?"#4F8EAE18":"#4F8EAE18",display:"flex",alignItems:"center",justifyContent:"center",color:"#4F8EAE",marginBottom:4}}>{isCrm?<I.Shield/>:isDlr?<I.Bldg/>:<I.Eye/>}</div>
    <h2 style={S.loginTitle}>{title}</h2>
    <p style={S.loginDesc}>{desc}</p>
    <div style={{width:"100%",marginTop:4}}>
      <label style={{fontSize:10,fontWeight:600,color:"#999",textTransform:"uppercase",letterSpacing:.8,marginBottom:4,display:"block"}}>Email</label>
      <input style={{...S.inp,width:"100%",fontSize:14,padding:"11px 14px"}} type="email" placeholder={isCrm?"admin@avantiequipfi.com":isDlr?"dealer@example.com":"your@email.com"} value={email} onChange={e=>{setEmail(e.target.value);setErr("")}} onKeyDown={e=>{if(e.key==="Enter")document.getElementById("av-login-pw")?.focus()}} autoComplete="email"/>
    </div>
    <div style={{width:"100%",marginTop:10}}>
      <label style={{fontSize:10,fontWeight:600,color:"#999",textTransform:"uppercase",letterSpacing:.8,marginBottom:4,display:"block"}}>Password</label>
      <div style={{position:"relative"}}>
        <input id="av-login-pw" style={{...S.inp,width:"100%",fontSize:14,padding:"11px 40px 11px 14px"}} type={showPw?"text":"password"} placeholder="••••••••" value={pw} onChange={e=>{setPw(e.target.value);setErr("")}} onKeyDown={e=>{if(e.key==="Enter")submit()}} autoComplete="current-password"/>
        <button style={{position:"absolute",right:8,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",color:"#999",cursor:"pointer",padding:4}} onClick={()=>setShowPw(!showPw)}>{showPw?<I.EyeOff/>:<I.Eye/>}</button>
      </div>
    </div>
    {lockMsg&&<div style={{color:"#F44336",fontSize:12,marginTop:8,padding:"8px 12px",background:"#F4433608",border:"1px solid #F4433622",borderRadius:6,fontWeight:600}}><I.Shield/> {lockMsg}</div>}
    {err&&!lockMsg&&<div style={S.loginErr}>{err}</div>}
    <button className="av-btn" style={{...S.prim,width:"100%",marginTop:14,padding:"12px 20px",fontSize:14,justifyContent:"center",opacity:(lockUntil>Date.now()||loading)?0.4:1,pointerEvents:(lockUntil>Date.now()||loading)?"none":"auto"}} onClick={submit}>{loading?"Signing in...":"Sign In"}</button>
    <p style={{fontSize:10,color:"#666",marginTop:12}}>🔒 Secure login — {MAX_ATTEMPTS} attempts before lockout</p>
    <p style={{fontSize:11,color:"#aaa",marginTop:6}}>Need help? Contact {AM.name} at {AM.phone}</p>
  </div></div>);
}

// ════════════════════════════════════════════════════════════════
// FORM MODALS
// ════════════════════════════════════════════════════════════════

function DealForm({deal,onSave,onClose,isDlr}) {
  const {dealers,lenders,accounts} = useContext(Ctx);
  const [f,sf]=useState({id:deal?.id||null,accountId:deal?.accountId||null,biz:deal?.biz||"",contact:deal?.contact||"",email:deal?.email||"",phone:deal?.phone||"",eqType:deal?.eqType||"",eqDetail:deal?.eqDetail||"",eqYear:deal?.eqYear||"",eqMake:deal?.eqMake||"",eqModel:deal?.eqModel||"",eqVin:deal?.eqVin||"",eqMileage:deal?.eqMileage||"",amount:deal?.amount||"",stage:deal?.stage||"new_lead",source:deal?.source||"",dealerId:deal?.dealerId||null,dealerName:deal?.dealerName||"",notes:deal?.notes||"",code:deal?.code||"",lenderId:deal?.lenderId||null,terms:deal?.terms||null,history:deal?.history||[],createdAt:deal?.createdAt||null,updatedAt:deal?.updatedAt||null,docs:deal?.docs||[],stips:deal?.stips||[],fundingItems:deal?.fundingItems||[],accepted:deal?.accepted??null,taxChoice:deal?.taxChoice??null});
  const u=(k,v)=>sf(p=>({...p,[k]:v}));
  // Approval terms sub-form
  const [showTerms,setShowTerms]=useState(!!f.terms);
  const [ta,setTa]=useState(f.terms?.amt||"");
  const [tp,setTp]=useState(f.terms?.pmt||"");
  const [tm,setTm]=useState(f.terms?.months||"");
  const [tr,setTr]=useState(f.terms?.rate||"");

  return(<div style={S.overlay} onClick={onClose}><div style={S.modal} onClick={e=>e.stopPropagation()}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 18px",borderBottom:"1px solid #D8DCE0"}}><span style={{fontSize:15,fontWeight:700,color:"#1a1a2e"}}>{f.id?"Edit Deal":isDlr?"Submit New Deal":"New Deal"}</span><button style={S.closeBtn} onClick={onClose}><I.X/></button></div>
    <div style={{padding:"14px 18px",overflowY:"auto",maxHeight:"60vh"}}>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px 12px"}}>
        <FF l="Business Name *" v={f.biz} ch={v=>u("biz",v)} ph="e.g. Smith Towing LLC"/>
        {!isDlr&&<div><label style={S.fLabel}>Link to Account</label><select style={S.fSel} value={f.accountId||""} onChange={e=>{const acc=accounts.find(a=>a.id===e.target.value);if(acc){u("accountId",acc.id);u("biz",acc.biz);u("contact",acc.contact);u("email",acc.email);u("phone",acc.phone)}else{u("accountId",null)}}}><option value="">New account (auto-create)</option>{accounts.map(a=><option key={a.id} value={a.id}>{a.biz}</option>)}</select></div>}
        <FF l="Contact Name" v={f.contact} ch={v=>u("contact",v)}/>
        <FF l="Email" v={f.email} ch={v=>u("email",v)} t="email"/>
        <FF l="Phone" v={f.phone} ch={v=>u("phone",v)}/>
        <div><label style={S.fLabel}>Equipment Type</label><select style={S.fSel} value={f.eqType} onChange={e=>u("eqType",e.target.value)}><option value="">Select...</option>{EQ_TYPES.map(t=><option key={t} value={t}>{t}</option>)}</select></div>
        <FF l="Amount" v={f.amount} ch={v=>u("amount",v)} t="number"/>
      </div>
      <div style={{marginTop:8,padding:12,background:"#FFFFFF",borderRadius:8,border:"1px solid #D8DCE0"}}>
        <div style={{fontSize:10,fontWeight:700,color:"#999",textTransform:"uppercase",letterSpacing:.8,marginBottom:8}}>Equipment Details</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"8px 12px"}}>
          <FF l="Year" v={f.eqYear} ch={v=>u("eqYear",v)} ph="2025"/>
          <FF l="Make" v={f.eqMake} ch={v=>u("eqMake",v)} ph="Kenworth"/>
          <FF l="Model" v={f.eqModel} ch={v=>u("eqModel",v)} ph="T370"/>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px 12px",marginTop:8}}>
          <FF l="VIN" v={f.eqVin} ch={v=>u("eqVin",v.toUpperCase())} ph="1NKZL70X5RJ412885"/>
          <FF l="Mileage" v={f.eqMileage} ch={v=>u("eqMileage",v)} ph="0" t="number"/>
        </div>
        <div style={{marginTop:8}}><FF l="Additional Details" v={f.eqDetail} ch={v=>u("eqDetail",v)} ph="Any other equipment notes"/></div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px 12px",marginTop:8}}>
        {!isDlr&&<div><label style={S.fLabel}>Stage</label><select style={S.fSel} value={f.stage} onChange={e=>u("stage",e.target.value)}>{STAGES.map(s=><option key={s.id} value={s.id}>{s.label}</option>)}</select></div>}
        {!isDlr&&<div><label style={S.fLabel}>Source</label><select style={S.fSel} value={f.source} onChange={e=>u("source",e.target.value)}><option value="">Select...</option>{SOURCES.map(s=><option key={s} value={s}>{s}</option>)}</select></div>}
        {!isDlr&&<div><label style={S.fLabel}>Dealer</label><select style={S.fSel} value={f.dealerId||""} onChange={e=>{const dl=dealers.find(d=>d.id===e.target.value);u("dealerId",e.target.value||null);u("dealerName",dl?.name||"")}}><option value="">None</option>{dealers.map(d=><option key={d.id} value={d.id}>{d.name}</option>)}</select></div>}
        {!isDlr&&<div><label style={S.fLabel}>Lender</label><select style={S.fSel} value={f.lenderId||""} onChange={e=>u("lenderId",e.target.value||null)}><option value="">None</option>{lenders.map(l=><option key={l.id} value={l.id}>{l.name} ({l.code})</option>)}</select></div>}
      </div>
      {!isDlr&&<div style={{marginTop:10}}><label style={{...S.fLabel,cursor:"pointer",display:"flex",alignItems:"center",gap:6}}><input type="checkbox" checked={showTerms} onChange={e=>{setShowTerms(e.target.checked);if(!e.target.checked)u("terms",null)}}/> Add Approval Terms</label>
        {showTerms&&<TermsCalc ta={ta} setTa={setTa} tp={tp} setTp={setTp} tm={tm} setTm={setTm} tr={tr} setTr={setTr} terms={f.terms} dealerId={f.dealerId} dealers={dealers}/>}
      </div>}
      <div style={{marginTop:8}}><label style={S.fLabel}>Notes</label><textarea style={{...S.inp,resize:"vertical",minHeight:50}} value={f.notes} onChange={e=>u("notes",e.target.value)}/></div>
    </div>
    <div style={{display:"flex",gap:8,justifyContent:"flex-end",padding:"12px 18px",borderTop:"1px solid #D8DCE0"}}>
      <button className="av-ghost" style={S.ghost} onClick={onClose}>Cancel</button>
      <button className="av-btn" style={S.prim} onClick={()=>{if(!f.biz.trim())return;const terms=showTerms?{amt:parseFloat(ta)||0,pmt:parseFloat(tp)||0,months:parseInt(tm)||0,rate:tr,buyRate:f.terms?.buyRate||null,points:f.terms?.points||null,feeAmt:f.terms?.feeAmt||null,dlrPts:f.terms?.dlrPts||0,dlrFee:f.terms?.dlrFee||0,financedAmt:f.terms?.financedAmt||null}:f.terms;onSave({...f,amount:parseFloat(f.amount)||0,terms})}}>{f.id?"Save":"Create"}</button>
    </div>
  </div></div>);
}

function DlrForm({dlr,onSave,onClose}) {
  const [f,sf]=useState({id:dlr?.id||null,name:dlr?.name||"",contact:dlr?.contact||"",email:dlr?.email||"",phone:dlr?.phone||"",region:dlr?.region||"",specs:dlr?.specs||[],code:dlr?.code||"",password:dlr?.password||gPassword(),active:dlr?.active??true,referralPts:dlr?.referralPts??"",salesRep:dlr?.salesRep||"",salesRepPhone:dlr?.salesRepPhone||"",salesRepEmail:dlr?.salesRepEmail||""});
  const u=(k,v)=>sf(p=>({...p,[k]:v}));
  const togSpec=s=>sf(p=>({...p,specs:p.specs.includes(s)?p.specs.filter(x=>x!==s):[...p.specs,s]}));
  return(<div style={S.overlay} onClick={onClose}><div style={S.modal} onClick={e=>e.stopPropagation()}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 18px",borderBottom:"1px solid #D8DCE0"}}><span style={{fontSize:15,fontWeight:700,color:"#1a1a2e"}}>{f.id?"Edit Dealer":"Add Dealer"}</span><button style={S.closeBtn} onClick={onClose}><I.X/></button></div>
    <div style={{padding:"14px 18px",overflowY:"auto",maxHeight:"60vh"}}>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px 12px"}}><FF l="Company Name *" v={f.name} ch={v=>u("name",v)}/><FF l="Main Contact" v={f.contact} ch={v=>u("contact",v)}/><FF l="Email" v={f.email} ch={v=>u("email",v)}/><FF l="Phone" v={f.phone} ch={v=>u("phone",v)}/><FF l="Region" v={f.region} ch={v=>u("region",v)}/><FF l="Dealer Code" v={f.code} ch={v=>u("code",v)}/><FF l="Password" v={f.password} ch={v=>u("password",v)}/></div>
      <div style={{marginTop:10,padding:10,background:"#FFFFFF",borderRadius:6,border:"1px solid #D8DCE0"}}>
        <div style={{fontSize:9,fontWeight:700,color:"#4F8EAE",textTransform:"uppercase",letterSpacing:1,marginBottom:8}}>Sales Rep Contact</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px 12px"}}>
          <FF l="Sales Rep Name" v={f.salesRep} ch={v=>u("salesRep",v)} ph="e.g. Tommy Gallagher"/>
          <FF l="Sales Rep Phone" v={f.salesRepPhone} ch={v=>u("salesRepPhone",v)} ph="(555) 123-4567"/>
          <div style={{gridColumn:"1/-1"}}><FF l="Sales Rep Email" v={f.salesRepEmail} ch={v=>u("salesRepEmail",v)} ph="rep@dealer.com"/></div>
        </div>
      </div>
      <div style={{marginTop:8,padding:10,background:"#FFFFFF",borderRadius:6,border:"1px solid #D8DCE0"}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px 12px",alignItems:"end"}}>
          <FF l="Referral Points (preset)" v={f.referralPts} ch={v=>u("referralPts",v)} t="number" ph="e.g. 3"/>
          <div style={{fontSize:10,color:"#999",paddingBottom:8}}>Auto-applied when this dealer's deals enter the calculator. Set to 0 or blank for no referral.</div>
        </div>
      </div>
      <div style={{marginTop:8}}><label style={S.fLabel}>Specialties</label><div style={{display:"flex",gap:4,flexWrap:"wrap"}}>{EQ_TYPES.map(t=><button key={t} onClick={()=>togSpec(t)} style={{padding:"3px 8px",fontSize:10,borderRadius:12,border:`1px solid ${f.specs.includes(t)?"#6BA3BF":"#D8DCE0"}`,background:f.specs.includes(t)?"#6BA3BF18":"transparent",color:f.specs.includes(t)?"#6BA3BF":"#666",cursor:"pointer",fontFamily:"inherit"}}>{t}</button>)}</div></div>
    </div>
    <div style={{display:"flex",gap:8,justifyContent:"flex-end",padding:"12px 18px",borderTop:"1px solid #D8DCE0"}}><button className="av-ghost" style={S.ghost} onClick={onClose}>Cancel</button><button className="av-btn" style={S.prim} onClick={()=>{if(!f.name.trim())return;onSave({...f,referralPts:parseFloat(f.referralPts)||0})}}>{f.id?"Save":"Add"}</button></div>
  </div></div>);
}

function LndrForm({lndr,onSave,onClose}) {
  const [f,sf]=useState({id:lndr?.id||null,name:lndr?.name||"",code:lndr?.code||"",contact:lndr?.contact||"",email:lndr?.email||"",phone:lndr?.phone||"",taxRule:lndr?.taxRule||"include",lienholder:lndr?.lienholder||"",address:lndr?.address||"",
    insListYMM:lndr?.insListYMM!==false,insListVin:lndr?.insListVin!==false,insPropertyCov:lndr?.insPropertyCov||"",insMaxDeductible:lndr?.insMaxDeductible||"",insCompCollision:lndr?.insCompCollision!==false,insACV:lndr?.insACV!==false,insAddlNotes:lndr?.insAddlNotes||""
  });
  const u=(k,v)=>sf(p=>({...p,[k]:v}));
  return(<div style={S.overlay} onClick={onClose}><div style={{...S.modal,maxHeight:"90vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 18px",borderBottom:"1px solid #D8DCE0"}}><span style={{fontSize:15,fontWeight:700,color:"#1a1a2e"}}>{f.id?"Edit Lender":"Add Lender"}</span><button style={S.closeBtn} onClick={onClose}><I.X/></button></div>
    <div style={{padding:"14px 18px"}}>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px 12px"}}><FF l="Lender Name *" v={f.name} ch={v=>u("name",v)}/><FF l="Code" v={f.code} ch={v=>u("code",v.toUpperCase())}/>
        <FF l="Contact Name" v={f.contact} ch={v=>u("contact",v)} ph="Rep / underwriter name"/>
        <FF l="Email" v={f.email} ch={v=>u("email",v)} ph="submissions@lender.com"/>
        <FF l="Phone" v={f.phone} ch={v=>u("phone",v)} ph="(555) 123-4567"/>
        <div><label style={S.fLabel}>Sales Tax Rule</label><select style={S.fSel} value={f.taxRule} onChange={e=>u("taxRule",e.target.value)}><option value="include">Always Included</option><option value="exclude">Always Excluded</option><option value="customer_choice">Customer Choice</option></select></div>
      </div>
      <div style={{marginTop:8}}><FF l="Lienholder Name" v={f.lienholder} ch={v=>u("lienholder",v)}/></div>
      <div style={{marginTop:8}}><FF l="Lienholder Address" v={f.address} ch={v=>u("address",v)}/></div>
      {/* Insurance Requirements */}
      <div style={{marginTop:14,padding:"12px 14px",background:"#0A0A0C",border:"1px solid #D8DCE0",borderRadius:8}}>
        <div style={{fontSize:11,fontWeight:700,color:"#4F8EAE",textTransform:"uppercase",letterSpacing:.8,marginBottom:10}}>🛡️ Insurance Requirements</div>
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          <div style={{display:"flex",gap:16,flexWrap:"wrap"}}>
            <label style={{display:"flex",alignItems:"center",gap:6,fontSize:11,color:"#666",cursor:"pointer"}}><input type="checkbox" checked={f.insListYMM} onChange={e=>u("insListYMM",e.target.checked)}/> List Year, Make, Model</label>
            <label style={{display:"flex",alignItems:"center",gap:6,fontSize:11,color:"#666",cursor:"pointer"}}><input type="checkbox" checked={f.insListVin} onChange={e=>u("insListVin",e.target.checked)}/> List VIN / Serial #</label>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px 12px"}}>
            <FF l="Property Coverage Amount" v={f.insPropertyCov} ch={v=>u("insPropertyCov",v)} ph="e.g. $185,000 or Full replacement"/>
            <FF l="Maximum Deductible" v={f.insMaxDeductible} ch={v=>u("insMaxDeductible",v)} ph="e.g. $2,500"/>
          </div>
          <div style={{display:"flex",gap:16,flexWrap:"wrap"}}>
            <label style={{display:"flex",alignItems:"center",gap:6,fontSize:11,color:"#666",cursor:"pointer"}}><input type="checkbox" checked={f.insCompCollision} onChange={e=>u("insCompCollision",e.target.checked)}/> Comprehensive & Collision Required</label>
            <label style={{display:"flex",alignItems:"center",gap:6,fontSize:11,color:"#666",cursor:"pointer"}}><input type="checkbox" checked={f.insACV} onChange={e=>u("insACV",e.target.checked)}/> Actual Cash Value (ACV)</label>
          </div>
          <FF l="Additional Insurance Notes" v={f.insAddlNotes} ch={v=>u("insAddlNotes",v)} ph="e.g. Loss payee must match lienholder exactly"/>
        </div>
      </div>
    </div>
    <div style={{display:"flex",gap:8,justifyContent:"flex-end",padding:"12px 18px",borderTop:"1px solid #D8DCE0"}}><button className="av-ghost" style={S.ghost} onClick={onClose}>Cancel</button><button className="av-btn" style={S.prim} onClick={()=>{if(!f.name.trim())return;onSave(f)}}>{f.id?"Save":"Add"}</button></div>
  </div></div>);
}

// ════════════════════════════════════════════════════════════════
// ── TERMS CALCULATOR (Admin Only) ─────────────────────────

function TermsCalc({ta,setTa,tp,setTp,tm,setTm,tr,setTr,terms,dealerId,dealers}) {
  const [buyRate,setBuyRate]=useState(terms?.buyRate||"");
  const [points,setPoints]=useState(terms?.points||"");
  const [dlrPtsOn,setDlrPtsOn]=useState(terms?.dlrPts!=null&&terms.dlrPts>0);
  const dealer=dealers?.find(d=>d.id===dealerId);
  const presetDlrPts=dealer?.referralPts||0;
  const [dlrPts,setDlrPts]=useState(terms?.dlrPts!=null?terms.dlrPts:(presetDlrPts||""));

  // Auto-populate dealer pts when dealer changes
  useEffect(()=>{
    if(dealer&&presetDlrPts>0){
      setDlrPts(presetDlrPts);
      setDlrPtsOn(true);
    } else if(!dealer){
      setDlrPts("");
      setDlrPtsOn(false);
    }
  },[dealerId]);

  const amt=parseFloat(ta)||0;
  const months=parseInt(tm)||0;
  const br=parseFloat(buyRate)||0;
  const pts=parseFloat(points)||0;
  const dp=dlrPtsOn?parseFloat(dlrPts)||0:0;
  const myFee=amt*(pts/100);
  const dlrFee=amt*(dp/100);
  const totalPtsAmt=myFee+dlrFee;
  const financedAmt=amt+totalPtsAmt;

  const calcPmt=(principal,annualRate,termMo)=>{
    if(!principal||!termMo)return 0;
    if(!annualRate)return principal/termMo;
    const r=annualRate/100/12;
    return principal*(r*Math.pow(1+r,termMo))/(Math.pow(1+r,termMo)-1);
  };

  const custPmt=calcPmt(financedAmt,br,months);
  const basePmt=calcPmt(amt,br,months);

  useEffect(()=>{
    if(amt&&months&&br){
      setTp(custPmt.toFixed(2));
      setTr(br.toFixed(2)+"%");
      if(terms){
        terms.buyRate=br;
        terms.points=pts;
        terms.feeAmt=myFee;
        terms.dlrPts=dlrPtsOn?dp:0;
        terms.dlrFee=dlrFee;
        terms.financedAmt=financedAmt;
      }
    }
  },[buyRate,points,ta,tm,dlrPts,dlrPtsOn]);

  const tgStyle=(on)=>({position:"relative",width:36,height:20,borderRadius:10,background:on?"#4F8EAE":"#D8DCE0",cursor:"pointer",border:"none",transition:"background .2s",flexShrink:0});
  const knobStyle=(on)=>({position:"absolute",top:2,left:on?18:2,width:16,height:16,borderRadius:8,background:on?"#fff":"#555",transition:"left .2s"});

  return(
    <div style={{marginTop:8,padding:12,background:"#FFFFFF",borderRadius:8,border:"1px solid #D8DCE0"}}>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px 12px"}}>
        <FF l="Approved Amount" v={ta} ch={v=>setTa(v)} t="number"/>
        <FF l="Term (months)" v={tm} ch={v=>setTm(v)} t="number"/>
        <FF l="Buy Rate %" v={buyRate} ch={v=>setBuyRate(v)} t="number" ph="e.g. 5.2"/>
      </div>

      {/* ── Dealer Referral Points ── */}
      <div style={{marginTop:10,padding:"10px 12px",background:"#F5F6F8",borderRadius:6,border:`1px solid ${dlrPtsOn?"#4F8EAE22":"#D8DCE0"}`}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:dlrPtsOn?8:0}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <div style={{fontSize:9,fontWeight:700,color:"#4F8EAE",textTransform:"uppercase",letterSpacing:1}}>Dealer Referral Points</div>
            {dealer&&presetDlrPts>0&&<span style={{fontSize:9,color:"#999",fontStyle:"italic"}}>Preset: {presetDlrPts}pts for {dealer.name}</span>}
          </div>
          <button onClick={()=>setDlrPtsOn(!dlrPtsOn)} style={tgStyle(dlrPtsOn)}><div style={knobStyle(dlrPtsOn)}/></button>
        </div>
        {dlrPtsOn&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px 10px"}}>
          <div>
            <div style={{fontSize:10,fontWeight:600,color:"#999",marginBottom:3,textTransform:"uppercase",letterSpacing:.5}}>Dealer Points</div>
            <input type="number" value={dlrPts} onChange={e=>setDlrPts(e.target.value)} placeholder="e.g. 3" style={{width:"100%",padding:"7px 10px",fontSize:13,color:"#1a1a2e",background:"#FFFFFF",border:"1px solid #D8DCE0",borderRadius:5,fontFamily:"inherit"}}/>
          </div>
          <div>
            <div style={{fontSize:10,fontWeight:600,color:"#999",marginBottom:3,textTransform:"uppercase",letterSpacing:.5}}>Dealer Fee</div>
            <div style={{padding:"7px 10px",background:"#FFFFFF",border:"1px solid #D8DCE0",borderRadius:5,fontSize:14,fontWeight:700,color:dlrFee?"#4F8EAE":"#333"}}>{dlrFee?"$"+dlrFee.toLocaleString(undefined,{maximumFractionDigits:0}):"—"}</div>
          </div>
        </div>}
      </div>

      {/* ── Your Points ── */}
      <div style={{marginTop:8,padding:"10px 12px",background:"#F5F6F8",borderRadius:6,border:"1px solid #D8DCE0"}}>
        <div style={{fontSize:9,fontWeight:700,color:"#E89300",textTransform:"uppercase",letterSpacing:1,marginBottom:8}}>Your Points — Admin Only</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px 10px"}}>
          <div>
            <div style={{fontSize:10,fontWeight:600,color:"#999",marginBottom:3,textTransform:"uppercase",letterSpacing:.5}}>Your Points</div>
            <input type="number" value={points} onChange={e=>setPoints(e.target.value)} placeholder="e.g. 10" style={{width:"100%",padding:"7px 10px",fontSize:13,color:"#1a1a2e",background:"#FFFFFF",border:"1px solid #D8DCE0",borderRadius:5,fontFamily:"inherit"}}/>
          </div>
          <div>
            <div style={{fontSize:10,fontWeight:600,color:"#999",marginBottom:3,textTransform:"uppercase",letterSpacing:.5}}>Your Fee</div>
            <div style={{padding:"7px 10px",background:"#FFFFFF",border:"1px solid #D8DCE0",borderRadius:5,fontSize:14,fontWeight:700,color:myFee?"#4CAF50":"#333"}}>{myFee?"$"+myFee.toLocaleString(undefined,{maximumFractionDigits:0}):"—"}</div>
          </div>
        </div>

        {amt>0&&months>0&&br>0&&(pts>0||dp>0)&&<>
          <div style={{marginTop:10,display:"grid",gridTemplateColumns:dp>0?"1fr 1fr 1fr 1fr":"1fr 1fr 1fr",gap:8}}>
            <div style={{padding:"8px 10px",background:"#FFFFFF",borderRadius:6,border:"1px solid #D8DCE0"}}>
              <div style={{fontSize:8,fontWeight:600,color:"#999",textTransform:"uppercase",letterSpacing:.8}}>Actual Financed</div>
              <div style={{fontSize:14,fontWeight:700,color:"#E89300",marginTop:2}}>${financedAmt.toLocaleString(undefined,{maximumFractionDigits:0})}</div>
            </div>
            <div style={{padding:"8px 10px",background:"#FFFFFF",borderRadius:6,border:"1px solid #D8DCE0"}}>
              <div style={{fontSize:8,fontWeight:600,color:"#999",textTransform:"uppercase",letterSpacing:.8}}>Customer Payment</div>
              <div style={{fontSize:14,fontWeight:700,color:"#4F8EAE",marginTop:2}}>${custPmt.toFixed(2)}</div>
            </div>
            {dp>0&&<div style={{padding:"8px 10px",background:"#4F8EAE08",borderRadius:6,border:"1px solid #4F8EAE22"}}>
              <div style={{fontSize:8,fontWeight:600,color:"#4F8EAE",textTransform:"uppercase",letterSpacing:.8}}>Dealer Fee</div>
              <div style={{fontSize:14,fontWeight:700,color:"#4F8EAE",marginTop:2}}>${dlrFee.toLocaleString(undefined,{maximumFractionDigits:0})}</div>
            </div>}
            <div style={{padding:"8px 10px",background:"#4CAF5008",borderRadius:6,border:"1px solid #4CAF5022"}}>
              <div style={{fontSize:8,fontWeight:600,color:"#4CAF50",textTransform:"uppercase",letterSpacing:.8}}>Your Fee</div>
              <div style={{fontSize:14,fontWeight:700,color:"#4CAF50",marginTop:2}}>${myFee.toLocaleString(undefined,{maximumFractionDigits:0})}</div>
            </div>
          </div>
          <div style={{marginTop:6,fontSize:10,color:"#aaa"}}>
            Customer sees: ${amt.toLocaleString()} approved at ${custPmt.toFixed(2)}/mo{dp>0&&` · Total points: ${pts+dp} (${pts} yours + ${dp} dealer)`} · Base pmt: ${basePmt.toFixed(2)}/mo
          </div>
        </>}
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px 12px",marginTop:8}}>
        <FF l="Customer Payment (auto)" v={tp} ch={v=>setTp(v)} t="number"/>
        <FF l="Rate Shown (auto)" v={tr} ch={v=>setTr(v)} ph="auto-calculated"/>
      </div>
    </div>
  );
}

// ── GENERATE CREDIT APP ──────────────────────────────────

function GenerateCreditApp({deal}) {
  const [showForm, setShowForm] = useState(false);
  const [template, setTemplate] = useState("avanti");
  const [appData, setAppData] = useState(null);

  // Pre-fill from deal data — use appData if customer applied online
  const a = deal.appData||{};
  const prefill = {
    bizName: a.bizName||deal.biz||"", dba: a.dba||"", bizType: a.bizType||deal.eqType||"",
    bizAddress: a.bizAddress||"", bizCityStateZip: a.bizCity?[a.bizCity,a.bizState,a.bizZip].filter(Boolean).join(", "):"",
    bizPhone: a.bizPhone||deal.phone||"", bizFax: "", bizEmail: a.bizEmail||deal.email||"",
    fedId: a.fedId||"", legalStructure: a.legalStructure||"", employees: a.employees||"",
    dateStarted: a.dateStarted||"", dateOwner: "",
    o1Name: a.o1Name||deal.contact||"", o1Pct: a.o1Pct?(a.o1Pct+"%"):"100%", o1Title: a.o1Title||"",
    o1Address: a.o1Address||"", o1CityStateZip: a.o1City?[a.o1City,a.o1State,a.o1Zip].filter(Boolean).join(", "):"",
    o1HomePhone: "", o1Cell: a.o1Cell||deal.phone||"", o1Ssn: a.o1Ssn||"", o1Dob: a.o1Dob||"",
    o2Name: a.o2Name||"", o2Pct: a.o2Pct?(a.o2Pct+"%"):"", o2Title: a.o2Title||"",
    o2Address: a.o2Address||"", o2CityStateZip: a.o2City?[a.o2City,a.o2State,a.o2Zip].filter(Boolean).join(", "):"",
    o2HomePhone: "", o2Cell: a.o2Cell||"", o2Ssn: a.o2Ssn||"", o2Dob: a.o2Dob||"",
    bankName: a.bankName||"", bankAcct: a.bankAcct||"", bankPhone: a.bankPhone||"", bankContact: a.bankContact||"",
    trade1: a.trade1||"", trade1Ph: a.trade1Ph||"", trade2: a.trade2||"", trade2Ph: a.trade2Ph||"",
    vendorName: a.vendorName||deal.dealerName||"", vendorPhone: a.vendorPhone||"", salesPerson: a.salesPerson||"",
    eqDesc: [deal.eqYear,deal.eqMake,deal.eqModel].filter(Boolean).join(" ")||deal.eqDetail||a.eqDesc||"",
    newUsed: a.newUsed||(deal.eqMileage==="0"||!deal.eqMileage?"New":"Used"),
    price: deal.amount?`$${Number(deal.amount).toLocaleString()}`:(a.price||""),
    termReq: deal.terms?.months?`${deal.terms.months} months`:(a.termReq||""),
  };

  return(
    <Sec t={<span style={{display:"flex",alignItems:"center",gap:6}}>Credit Application <span style={{fontSize:9,padding:"1px 6px",borderRadius:8,background:"#4F8EAE18",color:"#4F8EAE",border:"1px solid #4F8EAE33",fontWeight:600}}>PDF</span></span>}>
      <div style={{display:"flex",gap:6,alignItems:"center"}}>
        <select style={{...S.fSel,flex:1,fontSize:11}} value={template} onChange={e=>setTemplate(e.target.value)}>
          <option value="avanti">Avanti Credit Application</option>
          <option value="fincap">FinCap Business Application</option>
        </select>
        <button className="av-btn" style={{...S.prim,padding:"7px 14px",fontSize:11,whiteSpace:"nowrap"}} onClick={()=>setShowForm(true)}>
          <I.Doc/> Generate PDF
        </button>
      </div>
      {appData&&<div style={{marginTop:6,padding:"6px 10px",background:"#4CAF5008",border:"1px solid #4CAF5018",borderRadius:5,fontSize:11,color:"#4CAF50",display:"flex",alignItems:"center",gap:6}}>
        <I.CheckCirc/> Application generated · {template==="avanti"?"Avanti":"FinCap"} template
      </div>}
      {showForm&&<CreditAppForm prefill={prefill} template={template} onClose={()=>setShowForm(false)} onGenerate={(data)=>{setAppData(data);setShowForm(false)}}/>}
    </Sec>
  );
}

function CreditAppForm({prefill,template,onClose,onGenerate}) {
  const [f,sf]=useState(prefill);
  const u=(k,v)=>sf(p=>({...p,[k]:v}));

  const [genLoading,setGenLoading]=useState(false);
  const genPdf=async()=>{setGenLoading(true);try{await generate()}finally{setGenLoading(false)}};

  const generate = async () => {
    // Real browser-side PDF generation using jsPDF
    try {
      const jsPDF = await new Promise((resolve, reject) => {
        if (window.jspdf) return resolve(window.jspdf.jsPDF);
        const s = document.createElement("script");
        s.src = "https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.2/jspdf.umd.min.js";
        s.onload = () => window.jspdf ? resolve(window.jspdf.jsPDF) : reject(new Error("jsPDF not found"));
        s.onerror = () => reject(new Error("Failed to load jsPDF"));
        document.head.appendChild(s);
      });
      const pdf = new jsPDF({unit:"pt",format:"letter"});
      const pw=612,ph=792;
      const mx=40,my=40;
      let y=my;
      const clr=(r,g,b)=>pdf.setTextColor(r,g,b);
      const gray=()=>clr(100,100,100);
      const dark=()=>clr(30,30,30);
      const blue=()=>clr(79,142,174);
      const line=(y1)=>{pdf.setDrawColor(200,200,200);pdf.setLineWidth(0.5);pdf.line(mx,y1,pw-mx,y1)};
      const addPage=()=>{pdf.addPage();y=my};
      const checkPage=(need)=>{if(y+need>ph-my)addPage()};
      const isAvanti=template==="avanti";

      // Header
      blue();pdf.setFontSize(20);pdf.setFont("helvetica","bold");
      pdf.text(isAvanti?"AVANTI EQUIPMENT FINANCE":"FINCAP BUSINESS APPLICATION",mx,y);
      y+=16;pdf.setFontSize(10);gray();pdf.setFont("helvetica","normal");
      pdf.text("Credit Application",mx,y);
      y+=10;pdf.setFontSize(8);pdf.text(`Generated: ${new Date().toLocaleString()}`,mx,y);
      y+=6;line(y);y+=16;

      // Section helper
      const section=(title)=>{
        checkPage(30);
        blue();pdf.setFontSize(11);pdf.setFont("helvetica","bold");
        pdf.text(title.toUpperCase(),mx,y);y+=4;
        pdf.setDrawColor(79,142,174);pdf.setLineWidth(1.5);pdf.line(mx,y,mx+120,y);
        y+=14;
      };

      // Field row helper
      const field=(label,value,x,width)=>{
        if(!value)return;
        gray();pdf.setFontSize(7);pdf.setFont("helvetica","normal");
        pdf.text(label,x,y);
        dark();pdf.setFontSize(10);pdf.setFont("helvetica","bold");
        pdf.text(String(value),x,y+11);
      };

      // Two-column field pair
      const row2=(l1,v1,l2,v2)=>{
        checkPage(28);
        field(l1,v1,mx);
        if(l2&&v2)field(l2,v2,mx+260);
        y+=26;
      };

      // Three-column field
      const row3=(l1,v1,l2,v2,l3,v3)=>{
        checkPage(28);
        field(l1,v1,mx);
        if(l2&&v2)field(l2,v2,mx+180);
        if(l3&&v3)field(l3,v3,mx+360);
        y+=26;
      };

      // ─── BUSINESS INFORMATION ───
      section("Business Information");
      row2("Legal Business Name",f.bizName,"DBA",f.dba);
      row2("Business Type",f.bizType,"Federal Tax ID (EIN)",f.fedId);
      row2("Physical Address",f.bizAddress,"City, State, ZIP",f.bizCityStateZip);
      row2("Phone",f.bizPhone,"Email",f.bizEmail);
      row3("Legal Structure",f.legalStructure,"# Employees",f.employees,"Date Started",f.dateStarted);

      // ─── OWNER / GUARANTOR 1 ───
      section("Owner / Guarantor 1");
      row2("Full Name",f.o1Name,"Title",f.o1Title);
      row2("Ownership %",f.o1Pct,"Cell Phone",f.o1Cell);
      row2("Home Address",f.o1Address,"City, State, ZIP",f.o1CityStateZip);
      row2("SSN",f.o1Ssn?"•••-••-"+String(f.o1Ssn).slice(-4):"","Date of Birth",f.o1Dob);

      // ─── OWNER 2 (if present) ───
      if(f.o2Name){
        section("Owner / Guarantor 2");
        row2("Full Name",f.o2Name,"Title",f.o2Title);
        row2("Ownership %",f.o2Pct,"Cell Phone",f.o2Cell);
        row2("Home Address",f.o2Address,"City, State, ZIP",f.o2CityStateZip);
        row2("SSN",f.o2Ssn?"•••-••-"+String(f.o2Ssn).slice(-4):"","Date of Birth",f.o2Dob);
      }

      // ─── BANKING ───
      section("Banking Information");
      row2("Bank Name",f.bankName,"Account #",f.bankAcct);
      row2("Bank Phone",f.bankPhone,"Bank Contact",f.bankContact);

      // ─── TRADE REFERENCES ───
      if(f.trade1||f.trade2){
        section("Trade References");
        row2("Trade Reference #1",f.trade1,"Phone",f.trade1Ph);
        if(f.trade2)row2("Trade Reference #2",f.trade2,"Phone",f.trade2Ph);
      }

      // ─── EQUIPMENT ───
      section("Equipment / Vendor Information");
      row2("Equipment Description",f.eqDesc,"New or Used",f.newUsed);
      row2("Purchase Price",f.price,"Term Requested",f.termReq);
      row2("Vendor / Dealer Name",f.vendorName,"Vendor Phone",f.vendorPhone);
      if(f.salesPerson){checkPage(28);field("Sales Person",f.salesPerson,mx);y+=26}

      // ─── AUTHORIZATION ───
      checkPage(80);
      y+=10;line(y);y+=16;
      gray();pdf.setFontSize(7);pdf.setFont("helvetica","normal");
      pdf.text("AUTHORIZATION: By submitting this application, the undersigned authorizes Avanti Equipment Finance and its lender",mx,y);y+=10;
      pdf.text("partners to obtain credit reports and verify all information provided. This is not a commitment to lend.",mx,y);y+=20;

      // Signature lines
      pdf.setDrawColor(150,150,150);pdf.setLineWidth(0.5);
      pdf.line(mx,y,mx+220,y);pdf.line(mx+280,y,mx+220+260,y);
      y+=12;gray();pdf.setFontSize(8);
      pdf.text("Applicant Signature",mx,y);
      pdf.text("Date",mx+280,y);
      y+=20;
      pdf.line(mx,y,mx+220,y);pdf.line(mx+280,y,mx+220+260,y);
      y+=12;
      pdf.text("Co-Applicant Signature (if applicable)",mx,y);
      pdf.text("Date",mx+280,y);

      // Footer
      pdf.setFontSize(7);gray();
      pdf.text("Avanti Equipment Finance · (603) 459-9731 · Brent@AvantiEquipFi.com",mx,ph-25);
      pdf.text(`Page 1 of ${pdf.getNumberOfPages()}`,pw-mx-60,ph-25);

      // Multi-page footer
      const totalPages=pdf.getNumberOfPages();
      for(let i=2;i<=totalPages;i++){
        pdf.setPage(i);pdf.setFontSize(7);gray();
        pdf.text("Avanti Equipment Finance · (603) 459-9731 · Brent@AvantiEquipFi.com",mx,ph-25);
        pdf.text(`Page ${i} of ${totalPages}`,pw-mx-60,ph-25);
      }

      // Download
      const filename=`CreditApp_${(f.bizName||"Application").replace(/[^a-zA-Z0-9]/g,"_")}_${new Date().toISOString().slice(0,10)}.pdf`;
      const blob=pdf.output("blob");
      const url=URL.createObjectURL(blob);
      const a=document.createElement("a");a.href=url;a.download=filename;a.style.display="none";
      document.body.appendChild(a);a.click();
      setTimeout(()=>{document.body.removeChild(a);URL.revokeObjectURL(url)},100);
      // Also open in new tab as fallback
      window.open(url,"_blank");
      onGenerate(f);
    } catch(err) {
      console.error("PDF generation error:",err);
      alert("PDF generation failed: "+err.message+"\n\nMake sure you have an internet connection for the PDF library to load.");
    }
  };

  const secStyle = {marginTop:12,padding:"10px 12px",background:"#FFFFFF",border:"1px solid #E2E5E9",borderRadius:7};
  const secTitle = {fontSize:10,fontWeight:700,color:"#4F8EAE",textTransform:"uppercase",letterSpacing:.8,marginBottom:8};
  const grid2 = {display:"grid",gridTemplateColumns:"1fr 1fr",gap:"6px 10px"};
  const grid3 = {display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"6px 10px"};

  return(
    <div style={S.overlay} onClick={onClose}><div style={{...S.modal,maxWidth:620}} onClick={e=>e.stopPropagation()}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 18px",borderBottom:"1px solid #D8DCE0"}}>
        <span style={{fontSize:15,fontWeight:700,color:"#1a1a2e"}}>Credit Application — {template==="avanti"?"Avanti":"FinCap"}</span>
        <button style={S.closeBtn} onClick={onClose}><I.X/></button>
      </div>
      <div style={{padding:"14px 18px",overflowY:"auto",maxHeight:"65vh"}}>

        <div style={secStyle}><div style={secTitle}>Business Information</div>
          <div style={grid2}>
            <FF l="Legal Business Name *" v={f.bizName} ch={v=>u("bizName",v)}/>
            <FF l="DBA" v={f.dba} ch={v=>u("dba",v)}/>
            <FF l="Business Type" v={f.bizType} ch={v=>u("bizType",v)}/>
            <FF l="Federal Tax ID (EIN)" v={f.fedId} ch={v=>u("fedId",v)} ph="XX-XXXXXXX"/>
          </div>
          <div style={{...grid2,marginTop:6}}>
            <FF l="Physical Address" v={f.bizAddress} ch={v=>u("bizAddress",v)}/>
            <FF l="City, State, ZIP" v={f.bizCityStateZip} ch={v=>u("bizCityStateZip",v)}/>
            <FF l="Phone" v={f.bizPhone} ch={v=>u("bizPhone",v)}/>
            <FF l="Email" v={f.bizEmail} ch={v=>u("bizEmail",v)}/>
          </div>
          <div style={{...grid3,marginTop:6}}>
            <FF l="Legal Structure" v={f.legalStructure} ch={v=>u("legalStructure",v)} ph="LLC, Corp, etc."/>
            <FF l="# Employees" v={f.employees} ch={v=>u("employees",v)}/>
            <FF l="Date Started" v={f.dateStarted} ch={v=>u("dateStarted",v)} ph="MM/YYYY"/>
          </div>
        </div>

        <div style={secStyle}><div style={secTitle}>Owner / Guarantor 1</div>
          <div style={grid2}>
            <FF l="Full Name *" v={f.o1Name} ch={v=>u("o1Name",v)}/>
            <FF l="% Ownership" v={f.o1Pct} ch={v=>u("o1Pct",v)}/>
            <FF l="Home Address" v={f.o1Address} ch={v=>u("o1Address",v)}/>
            <FF l="City, State, ZIP" v={f.o1CityStateZip} ch={v=>u("o1CityStateZip",v)}/>
            <FF l="Cell Phone" v={f.o1Cell} ch={v=>u("o1Cell",v)}/>
            <FF l="Home Phone" v={f.o1HomePhone} ch={v=>u("o1HomePhone",v)}/>
            <FF l="Social Security #" v={f.o1Ssn} ch={v=>u("o1Ssn",v)} ph="XXX-XX-XXXX"/>
            <FF l="Date of Birth" v={f.o1Dob} ch={v=>u("o1Dob",v)} ph="MM/DD/YYYY"/>
          </div>
        </div>

        <div style={secStyle}><div style={secTitle}>Owner / Guarantor 2 <span style={{fontWeight:400,color:"#999"}}>(if applicable)</span></div>
          <div style={grid2}>
            <FF l="Full Name" v={f.o2Name} ch={v=>u("o2Name",v)}/>
            <FF l="% Ownership" v={f.o2Pct} ch={v=>u("o2Pct",v)}/>
            <FF l="Home Address" v={f.o2Address} ch={v=>u("o2Address",v)}/>
            <FF l="City, State, ZIP" v={f.o2CityStateZip} ch={v=>u("o2CityStateZip",v)}/>
            <FF l="Cell Phone" v={f.o2Cell} ch={v=>u("o2Cell",v)}/>
            <FF l="Home Phone" v={f.o2HomePhone} ch={v=>u("o2HomePhone",v)}/>
            <FF l="Social Security #" v={f.o2Ssn} ch={v=>u("o2Ssn",v)}/>
            <FF l="Date of Birth" v={f.o2Dob} ch={v=>u("o2Dob",v)}/>
          </div>
        </div>

        <div style={secStyle}><div style={secTitle}>Bank & Trade References</div>
          <div style={grid2}>
            <FF l="Business Bank Name" v={f.bankName} ch={v=>u("bankName",v)}/>
            <FF l="Account #" v={f.bankAcct} ch={v=>u("bankAcct",v)}/>
            <FF l="Bank Phone" v={f.bankPhone} ch={v=>u("bankPhone",v)}/>
            <FF l="Bank Contact" v={f.bankContact} ch={v=>u("bankContact",v)}/>
          </div>
          <div style={{...grid2,marginTop:6}}>
            <FF l="Trade Reference #1" v={f.trade1} ch={v=>u("trade1",v)}/>
            <FF l="Phone" v={f.trade1Ph} ch={v=>u("trade1Ph",v)}/>
            <FF l="Trade Reference #2" v={f.trade2} ch={v=>u("trade2",v)}/>
            <FF l="Phone" v={f.trade2Ph} ch={v=>u("trade2Ph",v)}/>
          </div>
        </div>

        <div style={secStyle}><div style={secTitle}>Equipment / Vendor</div>
          <div style={grid2}>
            <FF l="Vendor/Dealer Name" v={f.vendorName} ch={v=>u("vendorName",v)}/>
            <FF l="Vendor Phone" v={f.vendorPhone} ch={v=>u("vendorPhone",v)}/>
            <FF l="Sales Person" v={f.salesPerson} ch={v=>u("salesPerson",v)}/>
            <FF l="Equipment Description" v={f.eqDesc} ch={v=>u("eqDesc",v)}/>
            <FF l="New or Used" v={f.newUsed} ch={v=>u("newUsed",v)}/>
            <FF l="Price" v={f.price} ch={v=>u("price",v)}/>
            <FF l="Term Requested" v={f.termReq} ch={v=>u("termReq",v)} ph="e.g. 60 months"/>
          </div>
        </div>

      </div>
      <div style={{display:"flex",gap:8,justifyContent:"flex-end",padding:"12px 18px",borderTop:"1px solid #D8DCE0"}}>
        <button className="av-ghost" style={S.ghost} onClick={onClose}>Cancel</button>
        <button className="av-btn" style={{...S.prim,padding:"10px 20px",opacity:genLoading?0.5:1,pointerEvents:genLoading?"none":"auto"}} onClick={genPdf}><I.Doc/> {genLoading?"Generating...":(`Generate ${template==="avanti"?"Avanti":"FinCap"} PDF`)}</button>
      </div>
    </div></div>
  );
}

// SHARED COMPONENTS
// ════════════════════════════════════════════════════════════════

function Sec({t,children}){return(<div style={{marginBottom:14}}><div style={{fontSize:10,fontWeight:700,textTransform:"uppercase",letterSpacing:1.3,color:"#999",marginBottom:7}}>{t}</div><div style={{display:"flex",flexDirection:"column",gap:4}}>{children}</div></div>)}

function ItemsNeededSection({deal,role="admin"}) {
  const {addStip,removeStip,toggleStip,resolveStip} = useContext(Ctx);
  const [newItem,setNewItem]=useState("");
  const [showPresets,setShowPresets]=useState(false);
  const stips = (deal.stips||[]).filter(s=> role==="admin" || s.visibility==="all" || s.visibility===role);
  const pending = stips.filter(s=>!s.done).length;
  const done = stips.filter(s=>s.done).length;
  const isAdmin = role==="admin";

  const handleAdd = (text) => {
    if(!text.trim()) return;
    addStip(deal.id,text.trim(),"all");
    setNewItem("");
    setShowPresets(false);
  };

  const handleUpload = (stipId, stipText) => {
    const fileName = `${deal.biz?.replace(/\s/g,"_")}_${stipText.replace(/[\s()]/g,"_")}.pdf`;
    resolveStip(deal.id, stipId, fileName, role);
  };

  return(
    <Sec t={<span style={{display:"flex",alignItems:"center",gap:6}}>Items Needed {pending>0&&<span style={{fontSize:9,padding:"1px 6px",borderRadius:8,background:"#E8930018",color:"#E89300",border:"1px solid #E8930033",fontWeight:700}}>{pending} pending</span>}{done>0&&<span style={{fontSize:9,padding:"1px 6px",borderRadius:8,background:"#4CAF5012",color:"#4CAF50",fontWeight:600}}>{done} resolved</span>}</span>}>
      {stips.length===0&&<div style={{fontSize:11,color:"#aaa",padding:"8px 0"}}>{isAdmin?"No items requested. Add lender stipulations below.":"No items currently needed."}</div>}
      {stips.map(s=>(
        <div key={s.id} style={{display:"flex",alignItems:"flex-start",gap:8,padding:"7px 9px",background:s.done?"#4CAF5006":"#E8930008",border:`1px solid ${s.done?"#4CAF5018":"#E8930018"}`,borderRadius:5,transition:"all .15s"}}>
          {isAdmin&&<button onClick={()=>toggleStip(deal.id,s.id)} style={{width:18,height:18,borderRadius:4,border:`1.5px solid ${s.done?"#4CAF50":"#E89300"}`,background:s.done?"#4CAF5022":"transparent",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0,padding:0,marginTop:1}}>
            {s.done&&<I.Check/>}
          </button>}
          {!isAdmin&&<div style={{width:18,height:18,borderRadius:4,border:`1.5px solid ${s.done?"#4CAF50":"#E89300"}`,background:s.done?"#4CAF5022":"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,marginTop:1}}>
            {s.done&&<span style={{color:"#4CAF50"}}><I.Check/></span>}
          </div>}
          <div style={{flex:1,minWidth:0}}>
            <span style={{fontSize:12,color:s.done?"#666":"#ddd",textDecoration:s.done?"line-through":"none"}}>{s.text}</span>
            {s.file&&<div style={{fontSize:10,color:"#4CAF50",marginTop:2,display:"flex",alignItems:"center",gap:4}}><I.Doc/> {s.file} <span style={{color:"#999"}}>· by {s.resolvedBy||"—"}</span></div>}
          </div>
          <div style={{display:"flex",gap:3,flexShrink:0}}>
            {!s.done&&<label className="av-ghost" style={{...S.ghost,padding:"3px 7px",fontSize:9,gap:3,cursor:"pointer",display:"inline-flex",alignItems:"center"}}><I.Upload/> Attach<input type="file" style={{display:"none"}} onChange={e=>{const file=e.target?.files?.[0];if(file)resolveStip(deal.id,s.id,file.name,role);e.target.value=""}}/></label>}
            {isAdmin&&<button style={S.tinyBtn} onClick={()=>removeStip(deal.id,s.id)}><I.X/></button>}
          </div>
        </div>
      ))}
      {isAdmin&&<div style={{marginTop:6}}>
        <div style={{display:"flex",gap:6}}>
          <input style={{...S.inp,flex:1,fontSize:12}} placeholder="Add item needed..." value={newItem} onChange={e=>setNewItem(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")handleAdd(newItem)}}/>
          <button className="av-btn" style={{...S.prim,padding:"6px 10px",fontSize:11}} onClick={()=>handleAdd(newItem)}><I.Plus/></button>
          <button className="av-ghost" style={{...S.ghost,padding:"6px 8px",fontSize:10}} onClick={()=>setShowPresets(!showPresets)} title="Common items"><I.Clip/></button>
        </div>
        {showPresets&&<div style={{marginTop:6,padding:8,background:"#FFFFFF",border:"1px solid #E2E5E9",borderRadius:6,maxHeight:180,overflowY:"auto"}}>
          <div style={{fontSize:9,fontWeight:600,color:"#999",textTransform:"uppercase",letterSpacing:.8,marginBottom:6}}>Common Stipulations</div>
          <div style={{display:"flex",flexDirection:"column",gap:2}}>
            {COMMON_STIPS.filter(cs=>!stips.find(s=>s.text===cs)).map(cs=>(
              <button key={cs} onClick={()=>handleAdd(cs)} style={{textAlign:"left",padding:"5px 8px",fontSize:11,color:"#999",background:"transparent",border:"1px solid transparent",borderRadius:4,cursor:"pointer",fontFamily:"inherit",transition:"all .1s"}}
                onMouseOver={e=>{e.target.style.background="#FFFFFF";e.target.style.borderColor="#D8DCE0"}}
                onMouseOut={e=>{e.target.style.background="transparent";e.target.style.borderColor="transparent"}}>
                + {cs}
              </button>
            ))}
          </div>
        </div>}
      </div>}
    </Sec>
  );
}

function FundingItemsSection({deal,role="admin"}) {
  const {addFundingItem,resolveFundingItem,removeFundingItem} = useContext(Ctx);
  const [newItem,setNewItem]=useState("");
  const items = deal.fundingItems||[];
  const pending = items.filter(f=>!f.done).length;
  const isAdmin = role==="admin";
  const isDealer = role==="dealer";

  const COMMON_FUNDING = ["Final Invoice","Title Copy (Front)","Title Copy (Back)","Bill of Sale","Odometer Statement","Insurance Binder","Inspection Report","Equipment Photos"];

  const handleUpload = (itemId, text) => {
    const fileName = `${deal.biz?.replace(/\s/g,"_")}_${text.replace(/[\s()]/g,"_")}.pdf`;
    resolveFundingItem(deal.id, itemId, fileName);
  };

  // Dealers only see this section if there are funding items listed
  if(isDealer && items.length===0) return null;
  // Non-admin, non-dealer only see if deal accepted
  if(!isDealer && !isAdmin && !deal.accepted) return null;

  return(
    <Sec t={<span style={{display:"flex",alignItems:"center",gap:6}}>
      {isDealer?"Items Needed for Funding":"Dealer Funding Items"}
      {pending>0&&<span style={{fontSize:9,padding:"1px 6px",borderRadius:8,background:"#E8930018",color:"#E89300",border:"1px solid #E8930033",fontWeight:700}}>{pending} needed</span>}
      {pending===0&&items.length>0&&<span style={{fontSize:9,padding:"1px 6px",borderRadius:8,background:"#4CAF5012",color:"#4CAF50",fontWeight:600}}>All received</span>}
    </span>}>
      {isDealer&&<div style={{padding:"10px 12px",background:"#4F8EAE08",border:"1px solid #4F8EAE22",borderRadius:6,marginBottom:8}}>
        <div style={{fontSize:12,fontWeight:600,color:"#4F8EAE",marginBottom:3}}>Funding Documents Required</div>
        <div style={{fontSize:11,color:"#999",lineHeight:1.5}}>Please attach the items listed below. Items marked optional can be submitted later if not available now.</div>
      </div>}
      {items.length===0&&isAdmin&&<div style={{fontSize:11,color:"#aaa",padding:"4px 0"}}>No funding items yet. Add items the dealer needs to provide.</div>}
      {items.map(f=>(
        <div key={f.id} style={{display:"flex",alignItems:"flex-start",gap:8,padding:"7px 9px",background:f.done?"#4CAF5006":"#E8930008",border:`1px solid ${f.done?"#4CAF5018":"#E8930018"}`,borderRadius:5}}>
          <div style={{width:18,height:18,borderRadius:4,border:`1.5px solid ${f.done?"#4CAF50":"#E89300"}`,background:f.done?"#4CAF5022":"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,marginTop:1,...(isAdmin?{cursor:"pointer"}:{})}}>
            {f.done&&<span style={{color:"#4CAF50"}}><I.Check/></span>}
          </div>
          <div style={{flex:1,minWidth:0}}>
            <span style={{fontSize:12,color:f.done?"#666":"#ddd",textDecoration:f.done?"line-through":"none"}}>{f.text}</span>
            {f.file&&<div style={{fontSize:10,color:"#4CAF50",marginTop:2,display:"flex",alignItems:"center",gap:4}}><I.Doc/> {f.file}</div>}
            {!f.done&&isDealer&&<div style={{fontSize:9,color:"#999",marginTop:1}}>Tap Attach to upload this document</div>}
          </div>
          <div style={{display:"flex",gap:3,flexShrink:0}}>
            {!f.done&&(isDealer||isAdmin)&&<label className="av-ghost" style={{...S.ghost,padding:"3px 7px",fontSize:9,gap:3,cursor:"pointer",display:"inline-flex",alignItems:"center"}}><I.Upload/> Attach<input type="file" style={{display:"none"}} onChange={e=>{const file=e.target?.files?.[0];if(file)resolveFundingItem(deal.id,f.id,file.name);e.target.value=""}}/></label>}
            {isAdmin&&<button style={S.tinyBtn} onClick={()=>removeFundingItem(deal.id,f.id)}><I.X/></button>}
          </div>
        </div>
      ))}
      {isAdmin&&<div style={{marginTop:6,display:"flex",gap:6}}>
        <input style={{...S.inp,flex:1,fontSize:12}} placeholder="Add funding item..." value={newItem} onChange={e=>setNewItem(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&newItem.trim()){addFundingItem(deal.id,newItem.trim());setNewItem("")}}}/>
        <button className="av-btn" style={{...S.prim,padding:"6px 10px",fontSize:11}} onClick={()=>{if(newItem.trim()){addFundingItem(deal.id,newItem.trim());setNewItem("")}}}><I.Plus/></button>
      </div>}
      {isAdmin&&items.length===0&&<div style={{display:"flex",gap:4,flexWrap:"wrap",marginTop:4}}>
        {COMMON_FUNDING.map(cf=><button key={cf} className="av-ghost" style={{...S.ghost,padding:"3px 8px",fontSize:9}} onClick={()=>addFundingItem(deal.id,cf)}>+ {cf}</button>)}
      </div>}
    </Sec>
  );
}
function FB({l,v,sub,vs}){return(<div><div style={{fontSize:9,color:"#999",textTransform:"uppercase",letterSpacing:.8,marginBottom:2}}>{l}</div><div style={{fontSize:13,color:"#ddd",fontWeight:500,...vs}}>{v}</div>{sub&&<div style={{fontSize:11,color:"#999",marginTop:1}}>{sub}</div>}</div>)}
function FF({l,v,ch,ph,t}){return(<div><label style={S.fLabel}>{l}</label><input type={t||"text"} style={S.inp} value={v} onChange={e=>ch(e.target.value)} placeholder={ph}/></div>)}

// ════════════════════════════════════════════════════════════════
// STYLES
// ════════════════════════════════════════════════════════════════

const S = {
  app:{minHeight:"100vh",background:"#FFFFFF",color:"#1a1a2e",fontFamily:"'Outfit',-apple-system,sans-serif"},
  // Nav
  nav:{background:"#FFFFFF",borderBottom:"1px solid #E2E5E9",padding:"0 20px",position:"sticky",top:0,zIndex:50,boxShadow:"0 1px 3px rgba(0,0,0,.05)"},
  navIn:{display:"flex",alignItems:"center",gap:16,maxWidth:1440,margin:"0 auto",flexWrap:"wrap"},
  logo:{display:"flex",alignItems:"center",gap:8,padding:"10px 0"},
  logoIc:{color:"#4F8EAE",display:"flex"},
  logoTxt:{fontFamily:"'Outfit',sans-serif",fontSize:20,fontWeight:900,letterSpacing:3,color:"#1a1a2e"},
  logoSub:{fontSize:7,letterSpacing:2.5,color:"#5A6872",textTransform:"uppercase",fontWeight:600,marginTop:-2},
  navTabs:{display:"flex",gap:1,flex:1},
  navTab:{display:"flex",alignItems:"center",gap:6,padding:"14px 14px",background:"none",border:"none",borderBottom:"2px solid transparent",color:"#999",fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"inherit",transition:"all .15s",whiteSpace:"nowrap"},
  navTabA:{color:"#1a1a2e",borderBottomColor:"#4F8EAE"},
  navR:{display:"flex",alignItems:"center",gap:8},
  navUser:{fontSize:12,color:"#999",display:"flex",alignItems:"center",gap:5},
  // Main
  main:{padding:0},
  wrap:{maxWidth:1440,margin:"0 auto",padding:"16px 20px 40px"},
  pWrap:{maxWidth:900,margin:"0 auto",padding:"20px 20px 40px"},
  // Header
  hdr:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14,flexWrap:"wrap",gap:10},
  srchW:{display:"flex",alignItems:"center",gap:8,background:"#FFFFFF",border:"1px solid #D8DCE0",borderRadius:8,padding:"8px 12px",color:"#999"},
  srchIn:{background:"none",border:"none",color:"#1a1a2e",fontSize:13,outline:"none",width:240,fontFamily:"inherit"},
  // Stats
  statsG:{display:"flex",gap:1,flexWrap:"wrap",marginBottom:14},
  statC:{flex:1,minWidth:120,background:"#F5F6F8",padding:"12px 16px",borderRadius:0},
  statV:{fontSize:19,fontWeight:700,fontVariantNumeric:"tabular-nums"},
  statL:{fontSize:9,color:"#999",textTransform:"uppercase",letterSpacing:.8,marginTop:2},
  // Tabs
  tabs:{display:"flex",gap:2,borderBottom:"1px solid #E2E5E9"},
  tab:{display:"flex",alignItems:"center",gap:5,padding:"9px 14px",background:"none",border:"none",borderBottom:"2px solid transparent",color:"#999",fontSize:11,fontWeight:600,cursor:"pointer",fontFamily:"inherit"},
  tabA:{color:"#1a1a2e",borderBottomColor:"#4F8EAE"},
  // Kanban
  kanban:{display:"flex",gap:5,overflowX:"auto",paddingBottom:16},
  kCol:{flex:"1 1 0",minWidth:130,minHeight:360,background:"#F5F6F8",borderRadius:9,border:"1px solid #E2E5E9",transition:"all .15s"},
  kHead:{padding:"10px 10px 7px",borderBottom:"1px solid #E2E5E9"},
  kTitle:{display:"flex",alignItems:"center",gap:5,fontSize:10,fontWeight:600,textTransform:"uppercase",letterSpacing:.7,color:"#aaa"},
  kCnt:{fontSize:9,color:"#999",background:"#E2E5E9",borderRadius:8,padding:"1px 5px",fontWeight:600},
  kTot:{fontSize:10,color:"#999",marginTop:2,fontVariantNumeric:"tabular-nums"},
  kCards:{padding:5,display:"flex",flexDirection:"column",gap:4},
  kCard:{background:"#FFFFFF",borderRadius:6,padding:"9px 10px",cursor:"grab",borderLeft:"3px solid #4F8EAE",userSelect:"none",transition:"all .1s",border:"1px solid #E2E5E9"},
  kName:{fontSize:11,fontWeight:600,color:"#1a1a2e",marginBottom:4,lineHeight:1.3},
  kMeta:{display:"flex",alignItems:"center",gap:4,fontSize:9,color:"#888",marginBottom:1},
  kBot:{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:5},
  kAmt:{fontSize:11,fontWeight:700,color:"#6BA3BF",fontVariantNumeric:"tabular-nums"},
  kTime:{fontSize:8,color:"#aaa"},
  kDlr:{fontSize:8,color:"#4F8EAE66",marginTop:2,fontStyle:"italic"},
  kStip:{fontSize:9,color:"#E89300",marginTop:3,display:"flex",alignItems:"center",gap:3,fontWeight:600},
  kEmpty:{padding:14,textAlign:"center",color:"#666",fontSize:10},
  dot:{width:7,height:7,borderRadius:"50%",flexShrink:0},
  // Table
  table:{width:"100%",borderCollapse:"collapse"},
  th:{textAlign:"left",padding:"8px 10px",fontSize:9,fontWeight:600,textTransform:"uppercase",letterSpacing:.8,color:"#999",borderBottom:"1px solid #E2E5E9",whiteSpace:"nowrap"},
  tr:{cursor:"pointer",borderBottom:"1px solid #F5F6F8",transition:"all .15s"},
  td:{padding:"9px 10px",fontSize:12,verticalAlign:"top",color:"#aaa"},
  badge:{display:"inline-block",padding:"2px 7px",borderRadius:14,fontSize:10,fontWeight:600,border:"1px solid",whiteSpace:"nowrap"},
  // Buttons
  prim:{display:"flex",alignItems:"center",gap:5,background:"#4F8EAE",color:"#fff",border:"none",borderRadius:7,padding:"8px 16px",fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"inherit",transition:"all .15s"},
  ghost:{display:"flex",alignItems:"center",gap:5,padding:"7px 14px",background:"none",border:"1px solid #CDD1D6",color:"#999",borderRadius:7,fontSize:12,cursor:"pointer",fontFamily:"inherit",transition:"all .15s"},
  danger:{padding:"7px 12px",background:"#B0413E18",color:"#B0413E",border:"1px solid #B0413E33",borderRadius:7,fontSize:12,cursor:"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",gap:4},
  tinyBtn:{background:"none",border:"none",color:"#999",cursor:"pointer",padding:3},
  closeBtn:{background:"none",border:"none",color:"#999",cursor:"pointer",padding:4},
  reactBtn:{display:"flex",alignItems:"center",gap:5,padding:"7px 14px",background:"#4F8EAE12",color:"#4F8EAE",border:"1px solid #4F8EAE33",borderRadius:7,fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"inherit",whiteSpace:"nowrap"},
  // Overlay / Drawer / Modal
  overlay:{position:"fixed",inset:0,background:"rgba(0,0,0,.3)",zIndex:100,display:"flex",justifyContent:"flex-end"},
  drawer:{width:440,maxWidth:"92vw",height:"100vh",background:"#FFFFFF",borderLeft:"1px solid #E2E5E9",display:"flex",flexDirection:"column",animation:"slideIn .25s ease-out",boxShadow:"-4px 0 20px rgba(0,0,0,.08)"},
  drHead:{display:"flex",justifyContent:"space-between",alignItems:"flex-start",padding:"16px 18px",borderBottom:"1px solid #E2E5E9"},
  drBody:{flex:1,padding:"16px 18px",overflowY:"auto"},
  drFoot:{display:"flex",gap:8,padding:"12px 18px",borderTop:"1px solid #D8DCE0"},
  modal:{position:"fixed",top:"50%",left:"50%",transform:"translate(-50%,-50%)",width:520,maxWidth:"94vw",maxHeight:"90vh",background:"#FFFFFF",borderRadius:10,border:"1px solid #E2E5E9",zIndex:200,display:"flex",flexDirection:"column",overflow:"hidden",animation:"scaleIn .2s ease-out",boxShadow:"0 8px 32px rgba(0,0,0,.12)"},
  // Form
  fLabel:{fontSize:10,fontWeight:600,color:"#999",textTransform:"uppercase",letterSpacing:.6,marginBottom:3,display:"block"},
  inp:{width:"100%",background:"#FFFFFF",border:"1px solid #CDD1D6",borderRadius:6,padding:"8px 10px",color:"#1a1a2e",fontSize:13,fontFamily:"inherit",outline:"none",boxSizing:"border-box"},
  fSel:{width:"100%",background:"#FFFFFF",border:"1px solid #CDD1D6",borderRadius:6,padding:"8px 10px",color:"#1a1a2e",fontSize:13,fontFamily:"inherit",outline:"none"},
  // Doc row
  docRow:{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"5px 8px",background:"#FFFFFF",borderRadius:5,border:"1px solid #E2E5E9"},
  reqB:{fontSize:8,padding:"1px 5px",borderRadius:8,background:"#3A6F8F18",color:"#3A6F8F",border:"1px solid #3A6F8F33"},
  // Misc
  field:{display:"flex",alignItems:"center",gap:6,fontSize:12,color:"#666"},
  codeChip:{display:"flex",alignItems:"center",gap:5,padding:"3px 8px",background:"#4F8EAE09",border:"1px solid #4F8EAE22",borderRadius:4,cursor:"pointer",fontSize:11,color:"#4F8EAE",fontFamily:"inherit"},
  bigCode:{fontFamily:"'JetBrains Mono',monospace",fontSize:17,fontWeight:700,color:"#4F8EAE",background:"#4F8EAE09",padding:"7px 14px",borderRadius:6,letterSpacing:2},
  noteBox:{fontSize:12,color:"#999",lineHeight:1.5,background:"#FFFFFF",padding:"8px 12px",borderRadius:6},
  noteItem:{padding:"6px 10px",background:"#FFFFFF",borderRadius:5,border:"1px solid #E2E5E9"},
  approvalBox:{padding:"14px 18px",background:"#FFFFFF",border:"1px solid #D8DCE0",borderRadius:9},
  statusCard:{background:"#FFFFFF",border:"1px solid",borderRadius:10,padding:"18px 22px",marginBottom:18},
  progTrack:{height:6,background:"#D8DCE0",borderRadius:3,overflow:"hidden"},
  progFill:{height:"100%",borderRadius:3,transition:"width .5s ease"},
  amCard:{marginTop:24,background:"#FFFFFF",borderRadius:10,padding:"14px 18px",display:"flex",alignItems:"center",gap:12},
  amAv:{width:36,height:36,borderRadius:"50%",background:"#4F8EAE18",display:"flex",alignItems:"center",justifyContent:"center",color:"#4F8EAE"},
  // Portal
  pHead:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14,flexWrap:"wrap",gap:10},
  custCard:{background:"#F5F6F8",border:"1px solid #E2E5E9",borderRadius:12,padding:"28px 24px",maxWidth:600,margin:"0 auto"},
  dlrCard:{background:"#F5F6F8",border:"1px solid #E2E5E9",borderRadius:9,padding:"14px 18px",transition:"all .15s"},
  dlrDealRow:{display:"flex",justifyContent:"space-between",alignItems:"center",background:"#F5F6F8",border:"1px solid #E2E5E9",borderRadius:7,padding:"12px 16px",cursor:"pointer",transition:"all .15s"},
  specB:{fontSize:9,padding:"2px 7px",borderRadius:10,background:"#6BA3BF12",color:"#6BA3BF",border:"1px solid #6BA3BF25"},
  deadRow:{display:"flex",alignItems:"center",gap:12,background:"#F5F6F8",border:"1px solid #E2E5E9",borderRadius:7,padding:"12px 16px",borderLeft:"3px solid #555",marginBottom:6,transition:"all .15s"},
  deadWas:{fontSize:9,color:"#999",background:"#D8DCE0",padding:"1px 5px",borderRadius:3},
  lndrRow:{display:"flex",alignItems:"center",gap:12,background:"#F5F6F8",border:"1px solid #E2E5E9",borderRadius:7,padding:"14px 18px",transition:"all .15s"},
  setCard:{background:"#F5F6F8",border:"1px solid #E2E5E9",borderRadius:9,padding:"18px 22px"},
  setT:{fontSize:14,fontWeight:700,color:"#1a1a2e",marginBottom:4},
  setD:{fontSize:12,color:"#888",lineHeight:1.5},
  // Login
  loginWrap:{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"70vh",padding:20},
  loginCard:{background:"#F5F6F8",border:"1px solid #E2E5E9",borderRadius:12,padding:"32px 28px",maxWidth:380,width:"100%",textAlign:"center"},
  loginTitle:{fontSize:20,fontWeight:700,color:"#1a1a2e",margin:"10px 0 4px"},
  loginDesc:{fontSize:12,color:"#888",marginBottom:18,lineHeight:1.5},
  loginErr:{color:"#B0413E",fontSize:12,marginTop:8},
  empty:{padding:50,textAlign:"center",color:"#aaa",background:"#F5F6F8",borderRadius:10},
  toast:{position:"fixed",bottom:24,left:"50%",transform:"translateX(-50%)",padding:"10px 20px",borderRadius:8,color:"#fff",fontSize:13,fontWeight:600,zIndex:999,display:"flex",alignItems:"center",gap:8,boxShadow:"0 4px 20px rgba(0,0,0,.15)"},
};
