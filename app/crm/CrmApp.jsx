...
css.textContent = `
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  @media(max-width:768px){
    ...
    .av-nav-tabs.open{display:flex !important;flex-direction:column;position:absolute;top:100%;left:0;right:0;background:#080808;border-bottom:1px solid #1A1A1C;z-index:60;padding:8px 0}
...
const STAGES = [
  ...
  { id:"dead", label:"Dead", color:"#555", order:9 },
];
...
const ADMIN_CREDS = { email:"brent@avantiequipfi.com", password:"Topgun123456789;" };
...
export default function App() {
  ...
  // Persistence
  useEffect(() => { (async()=>{try{const r=await window.storage.get("av5_deals");if(r?.value){const p=JSON.parse(r.value);if(p.length)setDeals(p)}}catch(e){}try{const r=await window.storage.get("av5_dlrs");if(r?.value){const p=JSON.parse(r.value);if(p.length)setDealers(p)}}catch(e){}try{const r=await window.storage.get("av5_lndr");if(r?.value){const p=JSON.parse(r.value);if(p.length)setLenders(p)}}catch(e){}try{const r=await window.storage.get("av5_acct");if(r?.value){const p=JSON.parse(r.value);if(p.length)setAccounts(p)}}catch(e){}try{const r=await window.storage.get("av5_notes");if(r?.value)setDnotes(JSON.parse(r.value))}catch(e){}try{const r=await window.storage.get("av5_dlrNotes");if(r?.value)setDealerNotes(JSON.parse(r.value))}catch(e){}try{const r=await window.storage.get("av5_crmAuth");if(r?.value){const ts=parseInt(r.value);if(ts&&(Date.now()-ts)<1209600000){setCrmAuth(true)}else{await window.storage.set("av5_crmAuth","")}}}catch(e){}})()}, []);
  useEffect(()=>{(async()=>{try{await window.storage.set("av5_deals",JSON.stringify(deals))}catch(e){}})()},[deals]);
  useEffect(()=>{(async()=>{try{await window.storage.set("av5_dlrs",JSON.stringify(dealers))}catch(e){}})()},[dealers]);
  useEffect(()=>{(async()=>{try{await window.storage.set("av5_lndr",JSON.stringify(lenders))}catch(e){}})()},[lenders]);
  useEffect(()=>{(async()=>{try{await window.storage.set("av5_acct",JSON.stringify(accounts))}catch(e){}})()},[accounts]);
  useEffect(()=>{(async()=>{try{await window.storage.set("av5_notes",JSON.stringify(dnotes))}catch(e){}})()},[dnotes]);
  useEffect(()=>{(async()=>{try{await window.storage.set("av5_dlrNotes",JSON.stringify(dealerNotes))}catch(e){}})()},[dealerNotes]);
  ...
  const loginCrm = useCallback((email,pw)=>{if(email.toLowerCase()===ADMIN_CREDS.email.toLowerCase()&&pw===ADMIN_CREDS.password){setCrmAuth(true);(async()=>{try{await window.storage.set("av5_crmAuth",String(Date.now()))}catch(e){}})();return true}return false},[]);
  const logoutCrm = useCallback(()=>{setCrmAuth(false);(async()=>{try{await window.storage.set("av5_crmAuth","")}catch(e){}})()},[]);
  const loginDlr = useCallback((email,code)=>{const d=dealers.find(x=>x.code?.toLowerCase()===code.toLowerCase()&&x.email?.toLowerCase()===email.toLowerCase());if(d){setDlrAuth(d.id);return true}return false},[dealers]);
  const loginCust = useCallback((email,code)=>{const d=deals.find(x=>x.code?.toLowerCase()===code.toLowerCase()&&x.email?.toLowerCase()===email.toLowerCase());if(d){setCustAuth(d.id);return true}return false},[deals]);

  const ctx = useMemo(()=>({deals,dealers,lenders,accounts,dnotes,dealerNotes,saveDeal,moveDeal,delDeal,addDoc,rmDoc,toggleDocReq,addStip,removeStip,toggleStip,resolveStip,addFundingItem,resolveFundingItem,removeFundingItem,acceptDeal,sendConfEmail,sendCodeEmail,addNote,addDealerNote,dismissDealerNote,dismissAllDealerNotes,addActivity,addDlr,updDlr,saveAccount,addAcctActivity,notify,setLenders,loginCrm,logoutCrm}),[deals,dealers,lenders,accounts,dnotes,dealerNotes,saveDeal,moveDeal,delDeal,addDoc,rmDoc,toggleDocReq,addStip,removeStip,toggleStip,resolveStip,addFundingItem,resolveFundingItem,removeFundingItem,acceptDeal,sendConfEmail,sendCodeEmail,addNote,addDealerNote,dismissDealerNote,dismissAllDealerNotes,addActivity,addDlr,updDlr,saveAccount,addAcctActivity,notify,loginCrm,logoutCrm]);
      ...
      <div style={S.app}>
        <nav style={{...S.nav,position:"relative"}}><div style={S.navIn}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <button className="av-hamburger" onClick={()=>setMobileMenu(!mobileMenu)} style={{background:"none",border:"none",color:"#fff",cursor:"pointer",padding:6,display:"none"}}><I.Menu/></button>
          ...
          <div style={S.navR}>
            {portal==="crm"&&crmAuth&&<><span style={S.navUser}><I.User/> Brent C.</span><button className="av-ghost" style={{...S.ghost,marginLeft:6}} onClick={()=>{logoutCrm();setPortal("apply")}}><I.Logout/> Log Out</button></>}
            {portal==="dealer"&&dlrAuth&&<button className="av-ghost" style={S.ghost} onClick={()=>setDlrAuth(null)}><I.Logout/> Log Out</button>}
            {portal==="customer"&&custAuth&&<button className="av-ghost" style={S.ghost} onClick={()=>setCustAuth(null)}><I.Logout/> Log Out</button>}
          </div>
        </div></nav>
...
function CRM() {
  const {deals,moveDeal,delDeal,saveDeal,dealerNotes,notify} = useContext(Ctx);
  ...
  return(
    <div style={S.wrap} onDragOver={e=>{e.preventDefault();if(e.dataTransfer.types.includes("Files"))setPdfDrag(true)}} onDragLeave={e=>{if(!e.currentTarget.contains(e.relatedTarget))setPdfDrag(false)}} onDrop={handlePdfDrop}>
      ...
      <div className="av-stats-row" style={S.statsG}>{[{l:"Active",v:stats.act,c:"#fff"},{l:"Pipeline",v:fmt(stats.pipe),c:"#6BA3BF"},{l:"Funded",v:fmt(stats.fun),c:"#4CAF50"},{l:"Won",v:stats.won,c:"#2E7D32"},{l:"Total",v:stats.tot,c:"#fff"},{l:"Dead",v:stats.dead,c:"#666"}].map((s,i)=><div key={i} style={S.statC}><div style={{...S.statV,color:s.c}}>{s.v}</div><div style={S.statL}>{s.l}</div></div>)}</div>
      <div className="av-crm-tabs" style={S.tabs}>{[{id:"dashboard",ic:<I.Star/>,l:"Dashboard"},{id:"pipeline",ic:<I.Pipe/>,l:unreadDlr>0?`Pipeline (${unreadDlr} 🔔)`:"Pipeline"},{id:"list",ic:<I.List/>,l:"All Deals"},{id:"won",ic:<I.CheckCirc/>,l:`Won (${stats.won})`},{id:"accounts",ic:<I.User/>,l:"Accounts"},{id:"dead",ic:<I.Archive/>,l:`Dead (${stats.dead})`},{id:"dealers",ic:<I.Bldg/>,l:"Dealers"},{id:"lenders",ic:<I.Dollar/>,l:"Lenders"},{id:"settings",ic:<I.Gear/>,l:"Settings"}].map(t=><button key={t.id} className="av-tab" style={{...S.tab,...(view===t.id?S.tabA:{})}} onClick={()=>setView(t.id)}>{t.ic} {t.l}</button>)}</div>
      <div style={{marginTop:14,animation:"fadeIn .3s"}}>
        {view==="dashboard"&&<Dashboard/>}
        {view==="pipeline"&&<><Pipeline deals={filt} onSel={setSel} dragId={dragId} setDragId={setDragId} dragO={dragO} setDragO={setDragO} moveDeal={moveDeal}/><DealerNotifFeed onSel={setSel}/></>}
...
function Dashboard() {
  ...
  const now=new Date();
  const activeDeals=deals.filter(d=>!["funded","declined","dead"].includes(d.stage));
  ...
  const stageCounts=STAGES.filter(s=>!["declined","dead"].includes(s.id)).map(s=>({...s,count:deals.filter(d=>d.stage===s.id).length,total:deals.filter(d=>d.stage===s.id).reduce((sum,d)=>sum+(d.amount||0),0)}));

  // Recent activity (last 10 across all deals)
  const allActivities=deals.flatMap(d=>(d.activities||[]).map(a=>({...a,dealBiz:d.biz,dealId:d.id}))).sort((a,b)=>new Date(b.at)-new Date(a.at)).slice(0,10);
  ...
  const dlrStats=dealers.map(dl=>({...dl,dealCount:deals.filter(d=>d.dealerId===dl.id).length,funded:deals.filter(d=>d.dealerId===dl.id&&d.stage==="funded").length,volume:deals.filter(d=>d.dealerId===dl.id&&d.stage==="funded").reduce((s,d)=>s+(d.amount||0),0)})).filter(dl=>dl.dealCount>0).sort((a,b)=>b.dealCount-a.dealCount);

  const DB=({l,v,c,sub})=><div style={{background:"#161618",borderRadius:8,padding:"14px 16px",border:"1px solid #1E1E20",flex:1,minWidth:140}}>
    <div style={{fontSize:22,fontWeight:800,color:c||"#fff",fontVariantNumeric:"tabular-nums"}}>{v}</div>
    <div style={{fontSize:10,color:"#666",textTransform:"uppercase",letterSpacing:.8,marginTop:3}}>{l}</div>
  ...
  return(<div>
    ...
    <div className="av-kpi-row" style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:16}}>
      <DB l="Active Deals" v={activeDeals.length} c="#fff"/>
      <DB l="Pipeline Value" v={fmt(totalPipeline)} c="#6BA3BF"/>
      ...
      <DB l="Avg Deal Size" v={fmt(avgDealSize)} c="#6BA3BF"/>
    </div>

    <div className="av-dash-grid" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
      {/* Deal Funnel */}
      <div style={{background:"#121214",border:"1px solid #1A1A1C",borderRadius:9,padding:"16px 18px"}}>
        <div style={{fontSize:12,fontWeight:700,color:"#fff",marginBottom:12}}>Deal Pipeline</div>
        {stageCounts.map(s=>{const pct=deals.length?Math.max(s.count/Math.max(...stageCounts.map(x=>x.count))*100,4):0;return(
          <div key={s.id} style={{display:"flex",alignItems:"center",gap:10,marginBottom:6}}>
            <div style={{width:80,fontSize:10,color:"#888",textAlign:"right",flexShrink:0}}>{s.label}</div>
            <div style={{flex:1,height:22,background:"#1A1A1C",borderRadius:4,overflow:"hidden",position:"relative"}}>
              <div style={{height:"100%",width:pct+"%",background:s.color,borderRadius:4,transition:"width .5s",opacity:.8}}/>
              <span style={{position:"absolute",left:6,top:3,fontSize:10,fontWeight:700,color:"#fff"}}>{s.count}</span>
            </div>
            <div style={{width:80,fontSize:10,color:"#555",textAlign:"right"}}>{fmt(s.total)}</div>
          </div>
        )})}
      </div>
      ...
      {/* Alerts & Aging */}
      <div style={{background:"#121214",border:"1px solid #1A1A1C",borderRadius:9,padding:"16px 18px"}}>
        <div style={{fontSize:12,fontWeight:700,color:"#fff",marginBottom:12}}>Attention Needed</div>
        {pendingStips>0&&<div style={{display:"flex",alignItems:"center",gap:8,padding:"8px 10px",background:"#E8930008",border:"1px solid #E8930018",borderRadius:6,marginBottom:6}}>
          ...
          <div><div style={{fontSize:12,color:"#E89300",fontWeight:600}}>{pendingStips} pending stip{pendingStips!==1?"s":""}</div><div style={{fontSize:10,color:"#666"}}>Items needed from customers</div></div>
        </div>}
        {unreadDlrMsgs>0&&<div style={{display:"flex",alignItems:"center",gap:8,padding:"8px 10px",background:"#E8930008",border:"1px solid #E8930018",borderRadius:6,marginBottom:6}}>
          <span style={{color:"#E89300",fontSize:16}}>🔔</span>
          <div><div style={{fontSize:12,color:"#E89300",fontWeight:600}}>{unreadDlrMsgs} unread dealer message{unreadDlrMsgs!==1?"s":""}</div><div style={{fontSize:10,color:"#666"}}>Check Pipeline tab for details</div></div>
        </div>}
        {pendingFI>0&&<div style={{display:"flex",alignItems:"center",gap:8,padding:"8px 10px",background:"#4F8EAE08",border:"1px solid #4F8EAE18",borderRadius:6,marginBottom:6}}>
          ...
          <div><div style={{fontSize:12,color:"#4F8EAE",fontWeight:600}}>{pendingFI} funding item{pendingFI!==1?"s":""} outstanding</div><div style={{fontSize:10,color:"#666"}}>Waiting on dealers</div></div>
        </div>}
        {agingDeals.length>0&&<div style={{marginTop:6}}>
          <div style={{fontSize:10,fontWeight:600,color:"#666",textTransform:"uppercase",letterSpacing:.8,marginBottom:6}}>Aging Deals</div>
          {agingDeals.map(d=>{const stg=STAGES.find(s=>s.id===d.stage);return(
            <div key={d.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 8px",borderLeft:`2px solid ${stg?.color||"#555"}`,marginBottom:4,background:"#161618",borderRadius:"0 5px 5px 0"}}>
              <div><div style={{fontSize:11,color:"#ccc",fontWeight:600}}>{d.biz}</div><div style={{fontSize:9,color:"#555"}}>{stg?.label}</div></div>
              <div style={{fontSize:10,color:d.daysInStage>7?"#B0413E":"#E89300",fontWeight:700}}>{d.daysInStage}d</div>
            </div>
          )})}
        </div>}
        {pendingStips===0&&pendingFI===0&&unreadDlrMsgs===0&&agingDeals.length===0&&<div style={{padding:20,textAlign:"center",color:"#333",fontSize:11}}>All clear — no items need attention</div>}
      </div>

      {/* Recent Activity */}
      <div style={{background:"#121214",border:"1px solid #1A1A1C",borderRadius:9,padding:"16px 18px"}}>
        <div style={{fontSize:12,fontWeight:700,color:"#fff",marginBottom:12}}>Recent Activity</div>
        {allActivities.length?allActivities.map(a=>(
          <div key={a.id} style={{display:"flex",gap:8,padding:"5px 0",borderBottom:"1px solid #1A1A1C"}}>
            <span style={{fontSize:13,flexShrink:0}}>{actIcons[a.type]||"📎"}</span>
            <div style={{flex:1}}><div style={{fontSize:11,color:"#ccc"}}>{a.detail||a.type.replace(/_/g," ")}</div><div style={{fontSize:9,color:"#555"}}>{a.dealBiz} · {fDT(a.at)}</div></div>
          </div>
        )):<div style={{padding:20,textAlign:"center",color:"#333",fontSize:11}}>No recent activity</div>}
      </div>

      {/* Top Dealers */}
      <div style={{background:"#121214",border:"1px solid #1A1A1C",borderRadius:9,padding:"16px 18px"}}>
        <div style={{fontSize:12,fontWeight:700,color:"#fff",marginBottom:12}}>Dealer Performance</div>
        {dlrStats.length?dlrStats.map(dl=>(
          <div key={dl.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 10px",borderBottom:"1px solid #1A1A1C"}}>
            <div><div style={{fontSize:12,color:"#ccc",fontWeight:600}}>{dl.name}</div><div style={{fontSize:10,color:"#555"}}>{dl.contact}</div></div>
        ...
        )):<div style={{padding:20,textAlign:"center",color:"#333",fontSize:11}}>No dealer activity yet</div>}
      </div>
    </div>

    {/* Fee Breakdown */}
    {totalFees>0&&<div style={{marginTop:14,background:"#121214",border:"1px solid #1A1A1C",borderRadius:9,padding:"16px 18px"}}>
      <div style={{fontSize:12,fontWeight:700,color:"#fff",marginBottom:12}}>Fee Income Summary</div>
      <div style={{display:"flex",gap:20}}>
        <div><div style={{fontSize:20,fontWeight:800,color:"#E89300"}}>{fmt(yourFees)}</div><div style={{fontSize:10,color:"#666",textTransform:"uppercase",letterSpacing:.8,marginTop:2}}>Your Points Income</div></div>
        {dlrFees>0&&<div><div style={{fontSize:20,fontWeight:800,color:"#4F8EAE"}}>{fmt(dlrFees)}</div><div style={{fontSize:10,color:"#666",textTransform:"uppercase",letterSpacing:.8,marginTop:2}}>Dealer Referral Fees</div></div>}
        <div><div style={{fontSize:20,fontWeight:800,color:"#4CAF50"}}>{fmt(totalFees)}</div><div style={{fontSize:10,color:"#666",textTransform:"uppercase",letterSpacing:.8,marginTop:2}}>Total Fees on Funded</div></div>
      </div>
    </div>}
  </div>);
}
...
function DealerNotifFeed({onSel}) {
  const {dealerNotes,dismissDealerNote,dismissAllDealerNotes,deals}=useContext(Ctx);
  if(!dealerNotes.length)return null;
  const unread=dealerNotes.filter(n=>!n.read).length;
  return(<div style={{marginTop:16,background:"#121214",border:"1px solid #1A1A1C",borderRadius:9,padding:"14px 18px"}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
      <div style={{display:"flex",alignItems:"center",gap:8}}>
        <span style={{fontSize:13,fontWeight:700,color:"#fff"}}>Dealer Messages</span>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:4,maxHeight:300,overflowY:"auto"}}>
      {dealerNotes.slice(0,20).map(n=>{
        const d=deals.find(x=>x.id===n.dealId);
        return(<div key={n.id} style={{display:"flex",gap:10,padding:"8px 10px",background:n.read?"transparent":"#E8930006",border:`1px solid ${n.read?"#1A1A1C":"#E8930022"}`,borderRadius:6,transition:"all .2s ease",cursor:"pointer"}} onClick={()=>{if(d)onSel(d);dismissDealerNote(n.id)}}>
          <div style={{width:32,height:32,borderRadius:"50%",background:n.read?"#1A1A1C":"#E8930012",border:`1px solid ${n.read?"#222":"#E8930033"}`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,color:n.read?"#555":"#E89300",fontSize:12}}><I.Bldg/></div>
          <div style={{flex:1,minWidth:0}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <span style={{fontSize:11,fontWeight:700,color:n.read?"#666":"#E89300"}}>{n.dealer}</span>
              <span style={{fontSize:9,color:"#444"}}>{relT(n.at)}</span>
            </div>
            <div style={{fontSize:12,color:n.read?"#777":"#ccc",lineHeight:1.4,marginTop:2}}>{n.txt}</div>
            <div style={{fontSize:10,color:"#555",marginTop:2}}>Re: {n.biz}</div>
          </div>
          {!n.read&&<div style={{width:8,height:8,borderRadius:"50%",background:"#E89300",flexShrink:0,marginTop:4}}/>}
        </div>);
      })}
    </div>
  </div>);
...
function KCard({d,stg,dragId,setDragId,setDragO,onTouchStart,touchDragging,onSel}){
  const {dnotes,addNote}=useContext(Ctx);
  ...
  const leavePopup=()=>{leaveTimer.current=setTimeout(()=>setHov(false),250)};
  const submitNote=(e)=>{
    e.stopPropagation();
  ...
  return(<div ref={cardRef} style={{position:"relative"}}
    ...
    {hov&&<div style={{position:"absolute",...(flipLeft?{right:"100%",marginRight:6}:{left:"100%",marginLeft:6}),top:0,width:260,background:"#1A1A1C",border:"1px solid #2A2A2E",borderRadius:8,padding:0,zIndex:80,boxShadow:"0 8px 24px rgba(0,0,0,.5)",animation:"fadeIn .15s"}} onClick={e=>e.stopPropagation()} onMouseEnter={enterPopup} onMouseLeave={leavePopup}>
      {(d.contact||d.phone||d.email)&&<div style={{padding:"8px 12px",borderBottom:"1px solid #222"}}>
        {d.contact&&<div style={{fontSize:12,fontWeight:600,color:"#fff",marginBottom:3}}>{d.contact}</div>}
        {d.phone&&<div style={{display:"flex",alignItems:"center",gap:5,fontSize:11,color:"#6BA3BF",marginBottom:2,cursor:"pointer"}} onClick={e=>{e.stopPropagation();navigator.clipboard.writeText(d.phone)}}><I.Phone/> {d.phone}</div>}
        {d.email&&<div style={{display:"flex",alignItems:"center",gap:5,fontSize:11,color:"#6BA3BF",cursor:"pointer"}} onClick={e=>{e.stopPropagation();navigator.clipboard.writeText(d.email)}}><I.Mail/> {d.email}</div>}
      </div>}
      <div style={{padding:"8px 12px",borderBottom:"1px solid #222",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <span style={{fontSize:10,fontWeight:700,color:"#4F8EAE",textTransform:"uppercase",letterSpacing:.8}}>Notes</span>
        <span style={{fontSize:9,color:"#444"}}>{d.biz}</span>
      </div>
      <div style={{maxHeight:160,overflowY:"auto",padding:"6px 12px"}}>
        {notes.length===0&&<div style={{fontSize:10,color:"#444",padding:"8px 0",textAlign:"center",fontStyle:"italic"}}>No notes yet</div>}
        {notes.map(n=><div key={n.id} style={{padding:"5px 0",borderBottom:"1px solid #1E1E20"}}>
          <div style={{fontSize:11,color:"#ccc",lineHeight:1.4}}>{n.txt}</div>
          <div style={{fontSize:8,color:"#555",marginTop:2}}>{n.who} · {fDT(n.at)}</div>
        </div>)}
      </div>
      <div style={{padding:"6px 10px",borderTop:"1px solid #222",display:"flex",gap:4}}>
        <input value={quickNote} onChange={e=>setQuickNote(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")submitNote(e)}} onClick={e=>e.stopPropagation()} placeholder="Quick note..." style={{flex:1,background:"#111",border:"1px solid #2A2A2E",borderRadius:5,padding:"5px 8px",fontSize:11,color:"#e0e0e8",fontFamily:"inherit",outline:"none"}}/>
...
function WonV({deals,onSel}) {
  ...
  if(!sorted.length)return(<div style={S.empty}><I.CheckCirc/><div style={{marginTop:8,fontSize:13,fontWeight:600,color:"#555"}}>No funded deals yet</div><div style={{fontSize:11,color:"#444",marginTop:4}}>Deals that are funded will appear here as won.</div></div>);
  return(<div>
    <div style={{display:"flex",gap:16,marginBottom:14,padding:"12px 16px",background:"#121214",borderRadius:8,border:"1px solid #1A1A1C"}}>
      <div><div style={{fontSize:18,fontWeight:800,color:"#4CAF50"}}>{sorted.length}</div><div style={{fontSize:9,color:"#555",textTransform:"uppercase",letterSpacing:.8}}>Deals Won</div></div>
      <div><div style={{fontSize:18,fontWeight:800,color:"#6BA3BF"}}>{fmt(totalVol)}</div><div style={{fontSize:9,color:"#555",textTransform:"uppercase",letterSpacing:.8}}>Total Volume</div></div>
      <div><div style={{fontSize:18,fontWeight:800,color:"#E89300"}}>{fmt(totalFees)}</div><div style={{fontSize:9,color:"#555",textTransform:"uppercase",letterSpacing:.8}}>Your Fees</div></div>
    </div>
    {sorted.map(d=>{const fundedEntry=(d.history||[]).slice().reverse().find(h=>h.s==="funded");return(
      <div key={d.id} className="av-row" style={{display:"flex",alignItems:"center",gap:12,background:"#121214",border:"1px solid #1A1A1C",borderRadius:7,padding:"12px 16px",borderLeft:"3px solid #2E7D32",marginBottom:6,cursor:"pointer",transition:"all .15s"}} onClick={()=>onSel(d)}>
        <div style={{flex:1}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <span style={{fontSize:13,fontWeight:600,color:"#fff"}}>{d.biz}</span>
          ...
          <div style={{fontSize:12,color:"#777",marginTop:3}}>{d.contact} · {d.eqType} · {fmt(d.amount)}</div>
          <div style={{display:"flex",gap:12,marginTop:4}}>
            ...
            {fundedEntry&&<span style={{fontSize:10,color:"#555"}}>Funded {relT(fundedEntry.d)}</span>}
          </div>
        </div>
      </div>
    )})}
    ...
    <div style={S.overlay} onClick={onClose}><div style={S.drawer} onClick={e=>e.stopPropagation()}>
      ...
      <div style={S.drBody}>
        ...
        {d.appData?.signature&&<Sec t="Electronic Signature">
          <div style={{background:"#0A0A0A",border:"1px solid #1A1A1C",borderRadius:7,padding:"12px 14px"}}>
            <div style={{background:"#161618",borderRadius:6,padding:4,marginBottom:8}}><img src={d.appData.signature} alt="Signature" style={{width:"100%",height:60,objectFit:"contain"}}/></div>
            <div style={{fontSize:11,color:"#4CAF50",fontWeight:600}}>✓ Electronically signed by {d.appData.signedBy||d.contact}</div>
            <div style={{fontSize:10,color:"#555",marginTop:2}}>{d.appData.signedAt?new Date(d.appData.signedAt).toLocaleString("en-US",{weekday:"long",year:"numeric",month:"long",day:"numeric",hour:"numeric",minute:"2-digit",timeZoneName:"short"}):"—"}</div>
            <div style={{fontSize:9,color:"#333",marginTop:4,fontStyle:"italic"}}>Valid under ESIGN Act & UETA</div>
          </div>
        </Sec>}
            ...
            {d.approvalLetter?<div>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                <div style={{display:"flex",alignItems:"center",gap:8}}>
                  ...
                  <div><div style={{fontSize:12,color:"#ccc",fontWeight:600}}>{d.approvalLetter.name}</div>
                  <div style={{fontSize:10,color:"#555",marginTop:1}}>Uploaded {fDT(d.approvalLetter.at)}</div></div>
                </div>
              ...
              <div style={{fontSize:11,color:"#666",marginBottom:8}}>Attach the lender approval letter for this deal.</div>
    ...
    reader.onload=(ev)=>{
      try{
        ...
        if(window.confirm(`Restore backup from ${backup.exportedAt||"unknown date"}?\n\nThis contains: ${summary}\n\nThis will REPLACE all current data.`)){
          ...
          const write=async()=>{
            try{
              if(d.deals)await window.storage.set("av5_deals",JSON.stringify(d.deals));
              if(d.dealers)await window.storage.set("av5_dlrs",JSON.stringify(d.dealers));
              if(d.lenders)await window.storage.set("av5_lndr",JSON.stringify(d.lenders));
              if(d.accounts)await window.storage.set("av5_acct",JSON.stringify(d.accounts));
              if(d.dnotes)await window.storage.set("av5_notes",JSON.stringify(d.dnotes));
            }catch(e){}
...
function DealerDash({did,out}) {
  ...
  return(<div style={S.pWrap}>
    ...
    <div style={{marginTop:20}}><div style={{fontSize:14,fontWeight:700,color:"#fff",marginBottom:12}}>Your Referred Deals</div>
      ...
      :<div style={{display:"flex",flexDirection:"column",gap:8}}>{_.orderBy(dd.filter(d=>d.stage!=="dead"),["updatedAt"],["desc"]).map(d=>{const stg=STAGES.find(s=>s.id===d.stage);const pendFI=(d.fundingItems||[]).filter(f=>!f.done).length;const hasAccepted=d.accepted;return(
...
function DealerDrawer({dealId,onClose}) {
  const {deals,dealerNotes,addDealerNote,dealers} = useContext(Ctx);
      ...
      <div style={S.drBody}>
        ...
        <Sec t="Send a Note to Avanti">
          ...
          {myNotes.length>0&&<div style={{marginTop:8}}>
            {myNotes.map(n=><div key={n.id} style={{padding:"6px 10px",background:"#161618",border:"1px solid #1E1E20",borderRadius:5,marginBottom:4}}>
              <div style={{fontSize:11,color:"#ccc",lineHeight:1.4}}>{n.txt}</div>
              <div style={{fontSize:9,color:"#555",marginTop:2}}>{fDT(n.at)}</div>
            </div>)}
          </div>}
        </Sec>
      </div>
...
function SignaturePad({onSign,signature}){
  ...
  return(<div>
    ...
    <div style={{position:"relative",background:"#0A0A0A",border:"1px solid #1E1E20",borderRadius:8,overflow:"hidden"}}>
      ...
      {!hasDrawn&&<div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",pointerEvents:"none"}}>
        <div style={{color:"#333",fontSize:13}}>Draw your signature here</div>
      </div>}
      <div style={{position:"absolute",bottom:8,left:12,right:12,borderTop:"1px solid #2A2A2E"}}/>
    </div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:6}}>
      <div style={{fontSize:10,color:"#444"}}>Sign using your mouse or finger on mobile</div>
      ...
      <div style={{padding:"24px 22px"}}>
        ...
        <div style={{marginTop:12,padding:14,background:"#0A0A0A",border:"1px solid #1A1A1C",borderRadius:8}}>
          ...
          {signature&&<div style={{marginTop:8,padding:"8px 12px",background:"#4CAF5008",border:"1px solid #4CAF5022",borderRadius:6}}>
            ...
            <div style={{fontSize:9,color:"#555",marginTop:2}}>{new Date().toLocaleString("en-US",{weekday:"long",year:"numeric",month:"long",day:"numeric",hour:"numeric",minute:"2-digit",timeZoneName:"short"})}</div>
          </div>}
        </div>
        <div style={{marginTop:12,padding:14,background:"#0A0A0A",border:"1px solid #1A1A1C",borderRadius:8}}>
...
function Login({type,onGo}) {
  ...
  const title=isCrm?"Admin Login":isDlr?"Dealer Portal":"Customer Portal";
  const desc=isCrm?"Sign in to manage deals, dealers, and your pipeline.":isDlr?"Enter your email and dealer access code to view referred deals and submit new applications.":"Enter your email and access code provided by your account manager to view your financing status.";
  const submit=()=>{
    if(!email.trim()){setErr("Enter your email");return}
    if(!code.trim()){setErr(isCrm?"Enter your password":"Enter your access code");return}
    if(!onGo(email.trim(),code.trim()))setErr(isCrm?"Invalid email or password":isDlr?"Invalid email or dealer code":"No application found for this email and code.");
  };
  return(<div style={S.loginWrap}><div style={{...S.loginCard,maxWidth:420}}>
    <div style={{width:48,height:48,borderRadius:"50%",background:"#4F8EAE18",display:"flex",alignItems:"center",justifyContent:"center",color:"#4F8EAE",marginBottom:4}}>{isCrm?<I.Shield/>:isDlr?<I.Bldg/>:<I.Eye/>}</div>
    <h2 style={S.loginTitle}>{title}</h2>
    <p style={S.loginDesc}>{desc}</p>
    <div style={{width:"100%",marginTop:4}}>
      <label style={{fontSize:10,fontWeight:600,color:"#666",textTransform:"uppercase",letterSpacing:.8,marginBottom:4,display:"block"}}>Email</label>
      <input style={{...S.inp,width:"100%",fontSize:14,padding:"11px 14px"}} type="email" placeholder={isCrm?"admin@avantiequipfi.com":isDlr?"dealer@example.com":"your@email.com"} value={email} onChange={e=>{setEmail(e.target.value);setErr("")}} onKeyDown={e=>{if(e.key==="Enter")document.getElementById("av-login-code")?.focus()}}/>
    </div>
    <div style={{width:"100%",marginTop:10}}>
      <label style={{fontSize:10,fontWeight:600,color:"#666",textTransform:"uppercase",letterSpacing:.8,marginBottom:4,display:"block"}}>{isCrm?"Password":"Access Code"}</label>
      <div style={{position:"relative"}}>
        <input id="av-login-code" style={{...S.inp,width:"100%",fontSize:isCrm?14:18,fontFamily:isCrm?"inherit":"'JetBrains Mono',monospace",letterSpacing:isCrm?0:2,padding:"11px 40px 11px 14px"}} type={showPw?"text":"password"} placeholder={isCrm?"••••••••":isDlr?"DEALER CODE":"ABC-1234"} value={code} onChange={e=>{setCode(isCrm?e.target.value:e.target.value.toUpperCase());setErr("")}} onKeyDown={e=>{if(e.key==="Enter")submit()}}/>
        <button style={{position:"absolute",right:8,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",color:"#555",cursor:"pointer",padding:4}} onClick={()=>setShowPw(!showPw)}>{showPw?<I.EyeOff/>:<I.Eye/>}</button>
      </div>
    </div>
    {err&&<div style={S.loginErr}>{err}</div>}
    <button className="av-btn" style={{...S.prim,width:"100%",marginTop:14,padding:"12px 20px",fontSize:14,justifyContent:"center"}} onClick={submit}>
      {isCrm?"Sign In":isDlr?"Sign In":"View Status"}
    </button>
  ...
  return(
    <Sec t={<span style={{display:"flex",alignItems:"center",gap:6}}>
      ...
      {isDealer&&<div style={{padding:"10px 12px",background:"#4F8EAE08",border:"1px solid #4F8EAE22",borderRadius:6,marginBottom:8}}>
        ...
        <div style={{fontSize:11,color:"#888",lineHeight:1.5}}>Please attach the items listed below. Items marked optional can be submitted later if not available now.</div>
      </div>}
      ...
      {items.map(f=>(
        <div key={f.id} style={{display:"flex",alignItems:"flex-start",gap:8,padding:"7px 9px",background:f.done?"#4CAF5006":"#E8930008",border:`1px solid ${f.done?"#4CAF5018":"#E8930018"}`,borderRadius:5}}>
          ...
          <div style={{flex:1,minWidth:0}}>
            ...
            {!f.done&&isDealer&&<div style={{fontSize:9,color:"#555",marginTop:1}}>Tap Attach to upload this document</div>}
          </div>
          <div style={{display:"flex",gap:3,flexShrink:0}}>
...
const S = {
  ...
  kCol:{flex:"1 1 0",minWidth:130,minHeight:360,background:"#121214",borderRadius:9,border:"1px solid #1A1A1C",transition:"all .15s"},
